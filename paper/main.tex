...
\begin{abstract}
Grammar-constrained decoding ensures that large language model (LLM) outputs conform to specified syntactic structures, enabling reliable generation of code, JSON, and other structured formats. However, existing approaches either incur substantial per-token overhead or require expensive runtime computations that scale with grammar complexity. We present \sep{}, a precomputation-based approach that compiles context-free grammars into deterministic weighted automata (DWA), enabling \emph{near-constant-time} per-token constraint computation independent of grammar size. Our method introduces three key innovations: (1) a two-stage compilation pipeline that separates grammar analysis from vocabulary indexing, (2) weighted automata that encode token validity as bitvector operations, and (3) state equivalence analysis that reduces the effective vocabulary from 50,257 tokens to just 395 (99.2\% reduction). On JavaScript grammar constraints with a GPT-2 vocabulary, \sep{} achieves per-token latency of \textbf{17 microseconds}---an 83,000$\times$ improvement over naive simulation---while maintaining exact constraint satisfaction. We provide an open-source Rust implementation with Python bindings suitable for production deployment.
\end{abstract}
...
We evaluate \sep{} on grammar-constrained generation tasks, measuring compilation time, runtime latency, and memory usage. All experiments use real benchmark data from our implementation.

\subsection{Experimental Setup}

\paragraph{Grammar.} We evaluate primarily on a comprehensive JavaScript grammar (306 production rules) covering expressions, statements, functions, classes, modules, and ES6+ features. This grammar represents a realistic, complex target for structured code generation.

\paragraph{Vocabulary.} We use the GPT-2 tokenizer with 50,257 tokens, representative of modern LLM vocabularies.

\paragraph{Baselines.} We compare against:
\begin{itemize}
\item \textbf{Brute Force}: Per-token simulation that iterates through all vocabulary tokens, testing each for validity. This represents the naive approach.
\item \textbf{Brute Force (Fast)}: An optimized brute force variant using early termination and caching.
\end{itemize}

\paragraph{Hardware.} Experiments run on an Apple M1 Max with 32GB RAM. We report median latency over multiple trials.
...
\begin{tabular}{lrr}
\toprule
\textbf{Method} & \textbf{Per-Token Latency} & \textbf{Speedup} \\
\midrule
Brute Force & 1,434.8 ms & 1$\times$ \\
Brute Force (Fast) & 32.4 ms & 44$\times$ \\
\sep{} (Pure Python) & 3.5 ms & 406$\times$ \\
\sep{} (Rust + Python FFI) & 1.6 ms & 880$\times$ \\
\sep{} (Native Rust) & \textbf{0.017 ms} & \textbf{83,810$\times$} \\
\bottomrule
\end{tabular}
\caption{Per-token mask computation latency on JavaScript grammar with GPT-2 vocabulary (50,257 tokens). Native Rust achieves 17 microseconds per token---nearly five orders of magnitude faster than naive brute force.}
\label{tab:latency}
\end{table}

Table~\ref{tab:latency} presents our main latency results. The native Rust implementation achieves \textbf{17 microseconds} per \texttt{get\_mask} call, representing an 83,810$\times$ speedup over brute force simulation. Even with Python FFI overhead, the Rust model achieves 880$\times$ speedup.

The dramatic speedup stems from precomputation: brute force must simulate $|\vocab| = 50{,}257$ token effects per step, while \sep{} performs a single DWA state lookup plus bitvector operations.

\subsubsection{Vocabulary Reduction via State Equivalence}
...
\midrule
Original vocabulary size & 50,257 tokens \\
Reduced vocabulary size & 395 tokens \\
Reduction ratio & 99.2\% \\
\midrule
DWA states & 8,266 \\
Weight pool size & 1,139 \\
Match patterns & 187 \\
\bottomrule
\end{tabular}
\caption{State equivalence analysis reduces the effective vocabulary from 50,257 to just 395 tokens for JavaScript grammar---a 99.2\% reduction. This enables compact bitvector representations.}
\label{tab:equivalence}
\end{table}

Table~\ref{tab:equivalence} shows the impact of state equivalence analysis. For JavaScript grammar, the vast majority of GPT-2 tokens exhibit identical constraint behavior (e.g., most tokens that spell valid identifiers behave identically). Our analysis identifies these equivalence classes, reducing the effective vocabulary from 50,257 to just 395 tokens---a \textbf{99.2\% reduction}.

This reduction has cascading benefits: smaller bitvectors, faster set operations, and more cache-efficient data structures. The 1,139 unique weight vectors in the pool represent all distinct token validity patterns across all DWA states.

\subsubsection{Load and Initialization Time}
...
\midrule
Load compiled constraint from disk & 74--93 ms \\
Create initial state & 0.30--0.34 ms \\
\midrule
Constraint file size (compressed) & 389 KB \\
\bottomrule
\end{tabular}
\caption{Initialization costs. Loading the precompiled constraint is a one-time cost amortized over generation.}
\label{tab:init}
\end{table}

Table~\ref{tab:init} shows initialization costs. Loading the compiled constraint takes under 100ms---negligible compared to LLM model loading (typically seconds to minutes). The compressed constraint file is only 389KB, easily cacheable and distributable.

\subsubsection{Compilation Time}

Grammar compilation is a one-time offline process. For the JavaScript grammar (306 rules), compilation takes approximately 20--60 seconds depending on optimization level. This cost is amortized over arbitrarily many generation sessions, as the compiled constraint can be serialized and reloaded.

\subsubsection{End-to-End Impact}

To contextualize these results, consider a typical LLM generation scenario:
\begin{itemize}
\item LLM forward pass: 10--50 ms per token (GPU-dependent)
\item \sep{} constraint overhead: 0.017 ms per token
\item Constraint overhead ratio: \textbf{0.03--0.17\%}
\end{itemize}

Grammar-constrained decoding with \sep{} adds negligible overhead to LLM inference, making it practical for real-time applications.

\subsection{Comparison with Related Systems}

While direct comparison with Outlines and XGrammar requires careful benchmarking on identical grammars and hardware, our results suggest \sep{} achieves competitive or superior performance:

\begin{itemize}
\item \textbf{Outlines}~\citep{willard2023outlines} reports millisecond-scale latency for regex constraints. Our 17$\mu$s latency for full CFG constraints suggests substantial improvement.
\item \textbf{XGrammar}~\citep{dong2024xgrammar} reports 50$\mu$s-scale latency. Our results are in the same order of magnitude, though grammar and hardware differences complicate direct comparison.
\end{itemize}
...
We presented \sep{}, a precomputation-based approach to grammar-constrained decoding that achieves near-constant-time per-token constraint computation. By compiling grammars into deterministic weighted automata with bitvector weights, we eliminate the vocabulary-dependent overhead that limits existing approaches. On JavaScript grammar with a 50,257-token vocabulary, our implementation achieves \textbf{17 microseconds} per-token latency---an 83,000$\times$ speedup over brute force and competitive with or faster than existing systems.

The key insight is that while valid token sets vary with parsing context, the \emph{structure} of validity computation is fixed once grammar and vocabulary are known. This enables aggressive precomputation that shifts complexity from runtime to compile time. Our state equivalence analysis further reduces the effective vocabulary from 50,257 to just 395 tokens (99.2\% reduction), making grammar-constrained generation practical for latency-sensitive applications.

Our open-source Rust implementation with Python bindings provides a production-ready solution for structured LLM output generation, with applications in code synthesis, data extraction, and API integration.
