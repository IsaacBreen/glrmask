...
impl RuntimeArtifact {
    ...
    pub fn from_bytes(bytes: Vec<u8>) -> Result<Self> {
        ...
        if bytes.len() < HEADER || bytes[..8] != ARTIFACT_MAGIC {
            return Err(GlrMaskError::Serialization(
                "invalid glrmask runtime artifact magic".to_owned(),
            ));
        }
        let version = u16::from_le_bytes([bytes[8], bytes[9]]);
        if version != ARTIFACT_VERSION {
            return Err(GlrMaskError::Serialization(format!(
                "unsupported glrmask runtime artifact version {version}"
            )));
        }
        ...
        if bytes.len() != HEADER.saturating_add(length) {
            return Err(GlrMaskError::Serialization(
                "invalid glrmask runtime artifact payload length".to_owned(),
            ));
        }
        Ok(Self { bytes })
    }
    ...
    pub fn into_constraint(self) -> Result<Constraint> {
        Constraint::load_runtime_payload_v3(&self.bytes[18..])
    }
}
...
impl Session {
    ...
    pub fn mask_words(&self) -> Vec<u32> {
        self.state.mask()
    }
