...
fn json_schema_kubernetes_container_ports_prefix_has_schema_shaped_two_stack_split() {
    // Minimized from Kubernetes kb_996: the split depends on the ordered
    // Container/EphemeralContainer key prefixes through `ports`, both
    // `required: ["name"]` entries, and one later `workingDir` key on
    // Container. Metadata, outer optional fields, and nested value schemas
    ...
    const K8S_ORDERED_PORTS_SCHEMA_FRAGMENT: &str = r####"
{
    ...
    const K8S_ORDERED_PORTS_PREFIX: &[u8] = br####"{"spec": {"template": {"spec": {"containers": [{"image": "x", "name": "x", "ports"####;
