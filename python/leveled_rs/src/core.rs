...
//! - merge, peek, reduce_acc, visit_accs
//!
...
impl<T: Clone + Eq + Hash, A: Merge + Clone + Eq + Hash> LeveledGSS<T, A> {
    ...
    pub fn to_graph_string<FmtT, FmtA>(
        &self,
        upper_only: bool,
        fmt_t: &mut FmtT,
        fmt_a: &mut FmtA,
    ...
    where
        ...
        FmtA: FnMut(&A) -> String,
    {
        let mut printed_nodes: HashSet<usize> = HashSet::new();
        let mut output_lines: Vec<String> = Vec::new();

        fn get_node_info_lower<T: Clone + Eq + Hash>(node: &Arc<Lower<T>>) -> String {
            let terminal_info = if node.empty { " [TERMINAL]" } else { "" };
            format!(
                "Lower @ {:p} (MaxDepth: {}){}",
                Arc::as_ptr(node),
                node.max_depth,
        ...
        fn get_node_info_upper<T, A, FmtA>(node: &Arc<Upper<T, A>>, fmt_a: &mut FmtA) -> String
        where
            T: Clone + Eq + Hash,
            A: Merge + Clone + Eq + Hash,
            FmtA: FnMut(&A) -> String,
        {
            match &**node {
                Upper::Branch(b) => {
                    let terminal_info = if let Some(e) = &b.empty {
                        format!(" [TERMINAL empty: {}]", fmt_a(e))
                    } else {
                        "".to_string()
                    };
                    format!(
                        "UpperBranch @ {:p} (MaxDepth: {}){}",
                        Arc::as_ptr(b),
                        b.max_depth,
                        terminal_info
                    )
                ...
                Upper::Interface(i) => {
                    let mut terminal_info = "".to_string();
                    if let Some(e) = &i.empty {
                        terminal_info = format!(" [TERMINAL empty: {}]", fmt_a(e));
                    }
                    if i.children.is_empty() && i.empty.is_none() {
                        terminal_info = format!(" [IMPLICIT TERMINAL acc: {}]", fmt_a(&i.acc));
                    }
                    format!(
                        "Interface @ {:p} (MaxDepth: {}) | acc: {}{}",
                        Arc::as_ptr(i),
                        i.max_depth,
                        fmt_a(&i.acc),
                        terminal_info
        ...
        fn format_recursive_lower<T, FmtT>(
            node: &Arc<Lower<T>>,
            current_prefix: &str,
            ...
            FmtT: FnMut(&T) -> String,
        {
            ...
            let mut children_to_print = vec![];
            for (v, kids_at_depths) in node.children.iter() {
                for (depth, child) in kids_at_depths.iter() {
                    let label = format!("Edge {} (d={})", fmt_t(v), depth);
                    children_to_print.push((label, child.clone()));
                }
            }

            for (i, (label, child)) in children_to_print.iter().enumerate() {
                let is_last = i == children_to_print.len() - 1;
                let prefix_char = if is_last { "└── " } else { "├── " };

                let child_id_ptr = Arc::as_ptr(child);
                if printed_nodes.contains(&(child_id_ptr as usize)) {
                    let line = format!(
                        "{}{}{} -> Ref to Node @ {:p}",
                        current_prefix, prefix_char, label, child_id_ptr
                    );
                    ...
                    let line = format!(
                        ...
                        prefix_char,
                        label,
                        get_node_info_lower(child)
                    );
                    output_lines.push(line);

                    let child_prefix =
                        format!("{}{}", current_prefix, if is_last { "    " } else { "│   " });
                    format_recursive_lower(child, &child_prefix, printed_nodes, output_lines, fmt_t);
                }
            }
        }

        fn format_recursive_upper<T, A, FmtT, FmtA>(
            node: &Arc<Upper<T, A>>,
            current_prefix: &str,
            printed_nodes: &mut HashSet<usize>,
            output_lines: &mut Vec<String>,
            ...
            FmtA: FnMut(&A) -> String,
        {
            ...
            match &**node {
                Upper::Branch(b) => {
                    let mut children_to_print = vec![];
                    for (v, kids_at_depths) in b.children.iter() {
                        for (depth, child) in kids_at_depths.iter() {
                            let label = format!("Edge {} (d={})", fmt_t(v), depth);
                            children_to_print.push((label, child.clone()));
                        }
                    ...
                    for (i, (label, child)) in children_to_print.iter().enumerate() {
                        ...
                        if printed_nodes.contains(&(child_id_ptr as usize)) {
                            ...
                            let line = format!(
                                ...
                                prefix_char,
                                label,
                                get_node_info_upper(child, fmt_a)
                            );
                            output_lines.push(line);
                ...
                Upper::Interface(i) => {
                    if upper_only && !i.children.is_empty() {
                        let num_lower_edges: usize =
                            i.children.values().map(|kids| kids.len()).sum();
                        let line = format!(
                            "{}└── [{} lower edges omitted]",
                            current_prefix, num_lower_edges
                        );
                        output_lines.push(line);
                        return;
                    }
                    ...
                    let mut children_to_print = vec![];
                    for (v, kids_at_depths) in i.children.iter() {
                        for (depth, child) in kids_at_depths.iter() {
                            let label = format!("Edge {} (d={})", fmt_t(v), depth);
                            children_to_print.push((label, child.clone()));
                        }
        ...
        let root = &self.inner;
        let root_id_ptr = Arc::as_ptr(root);
        if printed_nodes.contains(&(root_id_ptr as usize)) {
            output_lines.push(format!("--- Root -> Ref to Node @ {:p} ---", root_id_ptr));
        } else {
            output_lines.push(format!("--- Root {} ---", get_node_info_upper(root, fmt_a)));
            format_recursive_upper(
                root,
                "",
                &mut printed_nodes,
