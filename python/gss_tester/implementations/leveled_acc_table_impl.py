...
class LeveledAccTableGSS(GSS[T, Acc], Generic[T, Acc]):
    ...
    def filter_by_length(self, min_len: Optional[int] = None, max_len: Optional[int] = None) -> "LeveledAccTableGSS[T, Acc]":
        ...
        def _filter_upper(node: Upper[T], depth: int) -> Optional[Upper[T]]:
            key = ('U', id(node), depth)
            if key in memo:
                return memo[key]
            ...
            new_children: Dict[T, Dict[int, Upper[T]]] = {}
            if depth < _max:
                for v, kids in node.children.items():
                    new_kids_for_v: Dict[int, Upper[T]] = {}
                    for d, child in kids.items():
