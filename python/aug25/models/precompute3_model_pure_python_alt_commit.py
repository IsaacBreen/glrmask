...
class Model(GraphProvider):
    ...
    def from_json_string(s: str) -> 'Model':
        ...
        original_to_internal_map = {int(k): v for k, v in data['original_to_internal_id_bimap']}
...
class Precompute0Model(Model):
    """
    A model that uses the precomputed0 trie for commit, similar to the Rust `commit(token_id)` logic.
    """
    ...
    def from_json_string(s: str) -> 'Precompute0Model':
        ...
        for uid, node in arena0_dict.items():
            children = node.get("children") or []
            new_children = []
