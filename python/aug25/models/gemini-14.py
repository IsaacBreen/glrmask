...
class Model(GraphProvider):
    ...
    This version includes a highly optimized get_mask implementation.
    """
    ...
    def from_json_string(s: str) -> 'Model':
        ...
        model.id_to_token = {v: bytes(k) for k, v in data['original_llm_vocab']['llm_token_map']}
    ...
    def _build_gss_node_map(self) -> Dict[int, object]:
        node_map = {}
        q = collections.deque()
        visited_nodes = set()

        for gss in self.state.values():
            for _, acc_map in gss.heads.items():
                for _, nodes in acc_map.items():
                    for node in nodes:
        ...
        while q:
            ...
            for parent_set in node.parents.values():
                for parent_node in parent_set:
                    if id(parent_node) not in visited_nodes:
                        q.append(parent_node)
                        visited_nodes.add(id(parent_node))
        return node_map

    def _gss_pop(self, g_node_id: int, pop: int, gss_node_map: Dict[int, object], pop_cache: Dict[Tuple[int, int], List[int]]) -> List[int]:
        if (g_node_id, pop) in pop_cache:
            return pop_cache[(g_node_id, pop)]
        if pop == 0:
            return [g_node_id]

        # Pop == 1 is the base case for the recursion
        if pop == 1:
            node = gss_node_map[g_node_id]
            # A GSS node's parents are at the previous level (closer to stack bottom)
            # LeveledGSSNode.parents: Dict[state_id, Set[LeveledGSSNode]]
            parent_ids = [id(p) for parent_set in node.parents.values() for p in parent_set]
            pop_cache[(g_node_id, 1)] = parent_ids
            return parent_ids

        # Recursive step with memoization
        res = set()
        # Pop `pop-1` times first, then one more level
        prev_pop_ids = self._gss_pop(g_node_id, pop - 1, gss_node_map, pop_cache)
        for pid in prev_pop_ids:
            res.update(self._gss_pop(pid, 1, gss_node_map, pop_cache))

        result = list(res)
        pop_cache[(g_node_id, pop)] = result
        return result

    def get_mask(self) -> RangeSet:
        """
        Compute the final LLM token mask using a dataflow analysis on the
        product of the precomputed trie and the GSS graphs. This avoids
        costly GSS operations in the main traversal loop.
        """
        q = collections.deque()
        masks: Dict[Tuple[int, int], RangeSet] = {}  # (trie_node_id, gss_node_id) -> RangeSet

        # Helper for seeding
        pmc = self.possible_matches_cache or {}
        ...
        for sid, gss in self.state.items():
            ...
            gss_initialized = gss.apply(initialize_acc, apply_memo)

            # gss.heads is Dict[state_id, Dict[acc, Set[node]]]
            for _, acc_map in gss_initialized.heads.items():
                for acc, nodes in acc_map.items():
                    initial_mask = acc.llm_mask
                    if initial_mask.is_empty():
                        continue
                    for node in nodes:
                        product_node = (r, id(node))
                        current_mask = masks.get(product_node)
                        if current_mask is None:
                            masks[product_node] = initial_mask
                            q.append(product_node)
                        elif not initial_mask.issubset(current_mask):
                             new_mask = current_mask.union(initial_mask)
                             masks[product_node] = new_mask
                             if product_node not in q:
                                 q.append(product_node)

        # Phase 2: Dataflow Propagation
        final_mask = RangeSet.empty()
        pop_cache: Dict[Tuple[int, int], List[int]] = {}
        gss_node_map = self._build_gss_node_map()

        while q:
            t_node, g_node_id = q.popleft()
            current_mask = masks[(t_node, g_node_id)]

            if self.is_end(t_node):
                final_mask = final_mask.union(current_mask)

            edges = self.arena.get(t_node, {}).get("children") or []
            for (pop, llm_bv), dests in edges:
                ancestor_ids = self._gss_pop(g_node_id, pop, gss_node_map, pop_cache)

                if not ancestor_ids:
                    continue

                new_mask_part = current_mask.intersection(llm_bv)
                if new_mask_part.is_empty():
                    continue

                for ancestor_id in ancestor_ids:
                    ancestor_node = gss_node_map.get(ancestor_id)
                    if ancestor_node is None: continue

                    for dest_idx, state_bv in dests:
                        if state_bv.contains(ancestor_node.state_id):
                            dest_product_node = (dest_idx, ancestor_id)
                            old_mask = masks.get(dest_product_node)

                            if old_mask is None:
                                masks[dest_product_node] = new_mask_part
                                q.append(dest_product_node)
                            elif not new_mask_part.issubset(old_mask):
                                updated_mask = old_mask.union(new_mask_part)
                                masks[dest_product_node] = updated_mask
                                if dest_product_node not in q:
                                    q.append(dest_product_node)
