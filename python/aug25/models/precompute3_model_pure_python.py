...
class Model(GraphProvider):
    ...
    def optimize_alive_prune_group(
        ...
        # This is a faster, monolithic implementation of:
        #   nodeopts = self._to_nodeopt_graph()
        #   alive = self._compute_alive_states(nodeopts)
        #   self._from_nodeopt_graph(nodeopts, alive)
        # It avoids the slow intermediate NodeOpt representation that explodes
        # edges by token.

        # 1. Compute alive states directly on the arena graph.
        # This is equivalent to _compute_alive_states, but on arena data.
        start_state = self.parser_table.start_state_id
        alive: Dict[int, Set[int]] = collections.defaultdict(set)
        ...
        # The alive state computation is a graph traversal, so a simple
        # progress bar is tricky. We'll just show progress on the second pass.
        while q:
            node_id = q.popleft()
            S = alive[node_id]
            if not S:
            ...
            for (pop, _llm_bv), dests in children:
                ...
                for dest_id, state_bv in dests:
                    ...
                    if not state_bv.is_empty():
                        ...
                        current_pre = pre.intersection(states)
                        if not current_pre:
                            continue

                    dest_alive = alive[dest_id]
                    new_states = current_pre - dest_alive
                    if new_states:
                        dest_alive.update(new_states)
                        q.append(dest_id)

        # 2. Prune and rebuild arena using alive states.
        # This is equivalent to _from_nodeopt_graph, but without NodeOpt.
        for node_id_str, node in tqdm(self.arena.items(), desc="Pruning and repacking graph"):
            node_id = int(node_id_str)
            node_alive = alive.get(node_id, set())

            # Group new children by signature to merge token sets
            groups: Dict[Tuple[int, Tuple[Tuple[int, Optional[Tuple[int, ...]]], ...]], LLMTokenSet] = collections.defaultdict(LLMTokenSet.empty)
            pre_cache: Dict[int, Set[int]] = {}
            ...
            for (pop, llm_bv), dests in children:
                ...
                new_dests: List[Tuple[int, Optional[Tuple[int, ...]]]] = []
                for dest_id, state_bv in dests:
                    dest_id = int(dest_id)
                    if state_bv.is_empty():
                        new_dests.append((dest_id, None))
                    else:
                        states = set(state_bv.to_indices())
                        ...
                        if trimmed == pre:
                            ...
                            new_dests.append((dest_id, tuple(sorted(trimmed))))

                if new_dests:
                    signature = (pop, tuple(sorted(new_dests)))
                    groups[signature] = groups[signature].union(llm_bv)

            # 3. Rebuild children in arena format
            new_children: List[Tuple[Tuple[int, LLMTokenSet], List[Tuple[int, StateIDSet]]]] = []
            llm_union: LLMTokenSet = LLMTokenSet.empty()
            for (pop, dests_sig), llm_bv in groups.items():
                ...
                dests_list: List[Tuple[int, StateIDSet]] = []
                for dest_id, states_tuple in dests_sig:
                    if states_tuple is None:
                        state_bv = StateIDSet.empty()
                    else:
                        state_bv = StateIDSet.from_indices(list(states_tuple))
                    dests_list.append((dest_id, state_bv))
