...
class Model(GraphProvider):
    ...
    def from_json_string(s: str) -> 'Model':
        ...
        # After LLM token reordering, optimize state masks and collapse edges
    ...
    # ===========================
    # Graph optimization (Alt 6 + edge reduction Alt 2)
    # ===========================
    def _rs_equals(self, a: RangeSet, b: RangeSet) -> bool:
        # Robust equality via mutual subset checks
        return a.difference(b).is_empty() and b.difference(a).is_empty()

    def _universe_lr_states_set(self) -> Set[int]:
        return set(self._compute_universe_lr_states())

    def _universe_lr_states_rangeset(self) -> StateIDSet:
        return RangeSet.from_indices(sorted(self._compute_universe_lr_states()))

    def _normalize_children_for_node(self, node: ArenaNode) -> None:
        """
        Merge child edges with identical (pop, dests) signature by uniting their llm_bv.
        """
        children = node.children
        if not children:
            node.llm_bv_union = RangeSet.empty()
    ...
    def _normalize_all_children(self) -> None:
        for node in self.arena.values():
            self._normalize_children_for_node(node)

    def _compute_terminal_states_by_node(self) -> Dict[NodeID, Set[int]]:
        """
        Compute, for each node, the set of LR states that are 'terminal' (can eventually
        reach an end node) ignoring LLM tokens. Uses a fixpoint over the product graph.
        """
        arena = self.arena
        if not arena:
            return {}

        universe: Set[int] = self._universe_lr_states_set()

        # Initialize terminal sets
        terminal_by_node: Dict[NodeID, Set[int]] = {}
        for nid, node in arena.items():
            if node.clean_end:
                terminal_by_node[int(nid)] = set(universe)
            else:
                terminal_by_node[int(nid)] = set()

        # Build edges ignoring LLM tokens: (u) -> [(pop, v, mask_set), ...]
        edges_by_node: Dict[NodeID, List[Tuple[int, NodeID, Set[int]]]] = {}
        for u, node in arena.items():
            u = int(u)
            lst: List[Tuple[int, NodeID, Set[int]]] = []
            for (pop, _llm_bv), dests in node.children:
                for dest_id, state_bv in dests:
                    mask_set = set(int(i) for i in state_bv.to_indices())
                    lst.append((int(pop), int(dest_id), mask_set))
            edges_by_node[u] = lst
        ...
        while changed:
            changed = False
            for u, lst in edges_by_node.items():
                if not lst:
                    continue
                base = terminal_by_node[u]
                accum = set(base)
                for pop, v, mask_set in lst:
                    tv = terminal_by_node.get(int(v), set())
                    if not tv:
                        continue
                    intersect = mask_set.intersection(tv)
                    if not intersect:
                        continue
                    pre = self._reverse_expand_n(intersect, int(pop))
                    if pre:
                        before = len(accum)
                        accum.update(pre)
                        if len(accum) > before:
                            # mark that u grew
                            pass
                if len(accum) > len(base):
                    terminal_by_node[u] = accum
                    changed = True
        return terminal_by_node

    def _optimize_state_masks_and_edges(self) -> None:
        """
        1) Expand state masks: For each edge to dest D, union in all NonTerminal(D) states.
           This pushes masks towards the full universe where safe (Alt 6).
        2) Collapse chains A->B->C conservatively (Replacement 1) only when:
           - B is not an end node
           - B has exactly one unique destination C across all children
           - All incoming edges to B have full (universe) masks to B
           - The collapse strictly reduces edge count: (incoming_count == 1) or (outgoing_group_count == 1)
           LLM tokens are preserved by intersecting A->B.llm_bv and B->C.llm_bv for the new A->C edge.
        """
        print("Optimizing state masks and collapsing edges...", end='', flush=True)

        if not self.arena:
            print(" done (empty arena).")
            return

        # Universe and terminal sets
        universe_rs: StateIDSet = self._universe_lr_states_rangeset()
        universe_set: Set[int] = set(universe_rs.to_indices())
        terminal_by_node: Dict[NodeID, Set[int]] = self._compute_terminal_states_by_node()
        terminal_rs_by_node: Dict[NodeID, StateIDSet] = {
            nid: RangeSet.from_indices(sorted(list(s)))
            for nid, s in terminal_by_node.items()
        }

        # Step 1: Expand state masks by adding NonTerminal(D) for destination D
        for nid, node in list(self.arena.items()):
            ...
            for (pop, llm_bv), dests in node.children:
                ...
                for dest_id, state_bv in dests:
                    term_rs = terminal_rs_by_node.get(int(dest_id), universe_rs)
                    nonterm_rs = universe_rs.difference(term_rs)
                    expanded_bv = state_bv.union(nonterm_rs)
                    new_dests.append((int(dest_id), expanded_bv))
                new_children.append(((int(pop), llm_bv), new_dests))
            node.children = new_children

        # Normalize after mask expansions
        self._normalize_all_children()
        self._recompute_llm_bv_unions()

        # Step 2: Bottom-up collapses with Replacement 1 where beneficial
        roots_set: Set[int] = set(int(v) for v in self.roots_map.values())

        def collect_incoming_edges(target_nid: int) -> List[Tuple[int, int, int]]:
            """
            Return list of (A_node_id, child_idx, dest_idx) such that A.children[child_idx]
            has a dest entry at dest_idx pointing to target_nid. Computed on the fly.
            """
            incoming: List[Tuple[int, int, int]] = []
            for aid, anode in self.arena.items():
                aid_int = int(aid)
                for ci, child in enumerate(anode.children):
                    _sig, dests = child
                    for di, (dest_id, _bv) in enumerate(dests):
                        if int(dest_id) == int(target_nid):
                            incoming.append((aid_int, int(ci), int(di)))
            return incoming

        def rs_is_universe(rs: RangeSet) -> bool:
            return self._rs_equals(rs, universe_rs)
        ...
        while changed_any:
            ...
            for b in list(node_order):
                ...
                if bnode.clean_end:
                    continue
                # Determine unique destination across all children
                dest_nodes: Set[int] = set()
                for (_sig, dests) in bnode.children:
                    for (did, _bv) in dests:
                        dest_nodes.add(int(did))
                if not dest_nodes:
                    # Orphan node (no outgoing edges). If not root and no incoming, remove it.
                    incoming = collect_incoming_edges(b)
                    if not incoming and (b not in roots_set):
                        # Remove b
                        del self.arena[b]
                ...
                if len(dest_nodes) != 1:
                    continue
                c = next(iter(dest_nodes))
                if c == b:
                    # self-loop; skip
                    continue

                # Collect B->C groups (must be only destination for each child)
                bc_groups: List[Tuple[int, LLMTokenSet, StateIDSet]] = []
                valid_bc = True
                for (pop_m, llm_bv_bc), dests in bnode.children:
                    # All dests must be exactly to c
                    if len(dests) != 1 or int(dests[0][0]) != int(c):
                        valid_bc = False
                        break
                    y_bv = dests[0][1]
                    bc_groups.append((int(pop_m), llm_bv_bc, y_bv))
                if not valid_bc:
                    continue
                k_bc = len(bc_groups)
                incoming = collect_incoming_edges(b)
                ...
                if in_count == 0:
                    # If B has no incoming edges and is not a root, we can safely remove it and its outgoing edges.
                    if b not in roots_set:
                        bnode.children = []
                        del self.arena[b]
                        if b in self.max_depth:
                            del self.max_depth[b]
                ...
                # Check all incoming A->B masks are universe
                all_in_masks_universe = True
                for (aid, ci, di) in incoming:
                    anode = self.arena.get(aid)
                    if anode is None:
                        all_in_masks_universe = False
                        break
                    _sig, dests = anode.children[ci]
                    _did, x_bv = dests[di]
                    if not rs_is_universe(x_bv):
                        all_in_masks_universe = False
                        break
                if not all_in_masks_universe:
                    continue

                # Ensure net edge reduction: in_count == 1 or k_bc == 1
                if not (in_count == 1 or k_bc == 1):
                    continue

                # Perform collapse: for each incoming A->B, create A->C edges based on llm_bv intersections
                nodes_to_normalize: Set[int] = set()
                for (aid, ci, di) in incoming:
                    anode = self.arena.get(aid)
                    if anode is None:
                        continue
                    (pop_n, llm_bv_ab), dests = anode.children[ci]
                    # For each B->C group, intersect tokens and add A->C edge
                    for (pop_m, llm_bv_bc, y_bv) in bc_groups:
                        inter_llm = llm_bv_ab.intersection(llm_bv_bc)
                        if inter_llm.is_empty():
                            continue
                        new_pop = int(pop_n) + int(pop_m)
                        # Append new edge ((new_pop, inter_llm), [(c, y_bv)])
                        anode.children.append(((new_pop, inter_llm), [(int(c), y_bv)]))
                        nodes_to_normalize.add(aid)

                # Remove all incoming A->B dest entries
                for (aid, ci, di) in incoming:
                    if aid not in self.arena:
                        continue
                    anode = self.arena[aid]
                    if ci >= len(anode.children):
                        continue
                    (pop_n, llm_bv_ab), dests = anode.children[ci]
                    # Filter out the B entry
                    new_dests = [(did, bv) for (did, bv) in dests if int(did) != int(b)]
                    if not new_dests:
                        # Remove the entire child edge group if no destinations left
                        # Guard against index shifts: rebuild children without index 'ci'
                        new_children = []
                        for idx, item in enumerate(anode.children):
                            if idx != ci:
                                new_children.append(item)
                        anode.children = new_children
                    else:
                        anode.children[ci] = ((pop_n, llm_bv_ab), new_dests)
                    nodes_to_normalize.add(aid)

                # Remove B->C edges and possibly node B (if not a root)
                if b in self.arena:
                    self.arena[b].children = []
                # If B is not a root and there are no remaining incoming edges, remove B
                incoming_after = collect_incoming_edges(b)
                if (b not in roots_set) and (not incoming_after):
                    if b in self.arena:
                        del self.arena[b]
                    if b in self.max_depth:
                        del self.max_depth[b]

                # Normalize changed nodes and recompute unions
                for aid in nodes_to_normalize:
                    if aid in self.arena:
                        self._normalize_children_for_node(self.arena[aid])
                self._recompute_llm_bv_unions()
                # Recompute depths since graph changed
                self.max_depth = self._recompute_max_depth_from_arena()
                changed_any = True

        # Final normalization and metrics recompute
        self._normalize_all_children()
        self._recompute_llm_bv_unions()
        self.max_depth = self._recompute_max_depth_from_arena()
