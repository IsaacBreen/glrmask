...
except Exception:  # pragma: no cover (fallback if tqdm unavailable)
    def tqdm(x, **kwargs):
        return x
...
class Model(GraphProvider):
    ...
    # ==============================================================
    # Graph optimization (Alt 6 masks + Alt 2 edge reduction)
    # ==============================================================
    def _state_universe_range_set(self) -> RangeSet:
        """
        Construct the universe of LR state IDs as a RangeSet by collecting:
        - parser_table states (rows)
        - all shift/goto targets
        - every state present in arena edge state masks
        """
        states: Set[int] = set()
        # Rows (state IDs)
        states.update(int(sid) for sid in self.parser_table.table.keys())
        # Shift/goto targets
        for row in self.parser_table.table.values():
            # Shift actions
            for action in row.actions.values():
                if isinstance(action, int):
        ...
        for node in self.arena.values():
            for (_pop, _llm_bv), dests in node.children:
                for _dest, state_bv in dests:
                    for s in state_bv.to_indices():
                        states.add(int(s))
        if not states:
            return RangeSet.empty()
        return RangeSet.from_indices(sorted(states))
    def _build_parents_map(self) -> Dict[int, Set[int]]:
        """
        Build a mapping from a “below” state to the set of “above” states that can
        have it directly beneath them (the inverse of ``reverse_state_map``).
        """
        parents: Dict[int, Set[int]] = collections.defaultdict(set)
        for parent, belows in self.reverse_state_map.items():
            p = int(parent)
            for b in belows:
                parents[int(b)].add(p)
        return dict(parents)
    def _up1_on_stateset(self, sset: RangeSet, parents_map: Dict[int, Set[int]]) -> RangeSet:
        """
        One‑step pre‑image under the reverse‑state expansion:
        Up₁(B) = ⋃_{b∈B} parents_map[b]
        """
        if sset.is_empty():
            return sset
        acc: Set[int] = set()
        for b in sset.to_indices():
            acc.update(parents_map.get(int(b), ()))
        if not acc:
            return RangeSet.empty()
        return RangeSet.from_indices(sorted(acc))
    def optimize_state_masks_and_edges(self) -> None:
        """
        Two‑phase optimization:
        1️⃣  Mask transformation (Alt 6): replace edge masks with the full LR
            state universe when the destination node has no terminal states for
            the current token.
        2️⃣  Edge reduction (Alt 2): collapse A → B → C patterns when the
            middle node B can be eliminated for the token in question.
        The procedure works token‑by‑token, rebuilding the arena children at the
        end to keep the representation compact.
        """
        print("Optimizing graph (Alt 6 masks & Alt 2 edges)…", flush=True)
        # 1️⃣ Build LR‑state universe and parents map
        U: RangeSet = self._state_universe_range_set()
        if U.is_empty():
            print(" - No LR states found; skipping optimization.", flush=True)
            return
        parents_map: Dict[int, Set[int]] = self._build_parents_map()
        # Helper to compare RangeSets for equality
        def rs_eq(a: RangeSet, b: RangeSet) -> bool:
            return a.difference(b).is_empty() and b.difference(a).is_empty()
        # 2️⃣ Determine which LLM tokens actually appear in the graph
        tokens_in_graph: Set[int] = set()
        for node in self.arena.values():
            if node.llm_bv_union and not node.llm_bv_union.is_empty():
                for t in node.llm_bv_union.to_indices():
                    tokens_in_graph.add(int(t))
        if not tokens_in_graph:
            print(" - No LLM tokens in graph; nothing to optimise.", flush=True)
            return
        # -----------------------------------------------------------------
        # Build per‑token edge representation:
        # edges_by_token[token][src][(pop, dest)] = RangeSet mask
        # -----------------------------------------------------------------
        edges_by_token: Dict[int, Dict[NodeID, Dict[Tuple[int, NodeID], RangeSet]]] = {}
        max_pop_seen = 0
        for src_id, node in self.arena.items():
            for (pop, llm_bv), dests in node.children:
                pi = int(pop)
                max_pop_seen = max(max_pop_seen, pi)
                if llm_bv.is_empty():
                    continue
                toks = [int(x) for x in llm_bv.to_indices()]
                if not toks:
                    continue
                for (dest_id, state_bv) in dests:
                    did = int(dest_id)
                    for tok in toks:
                        tmap = edges_by_token.setdefault(tok, {}).setdefault(int(src_id), {})
                        key = (pi, did)
                        if key in tmap:
                            tmap[key] = tmap[key].union(state_bv)
                        else:
                            tmap[key] = state_bv
        # -----------------------------------------------------------------
        # Build incoming‑edge look‑ups for the backward fix‑point
        # -----------------------------------------------------------------
        incoming_by_token: Dict[int, Dict[NodeID, List[Tuple[NodeID, int, RangeSet]]]] = {}
        for tok, out_map in edges_by_token.items():
            inc: Dict[NodeID, List[Tuple[NodeID, int, RangeSet]]] = collections.defaultdict(list)
            for src, pd_map in out_map.items():
                for (pop, dest), mask in pd_map.items():
                    inc[int(dest)].append((int(src), int(pop), mask))
            incoming_by_token[int(tok)] = dict(inc)
        # -----------------------------------------------------------------
        # LRU‑cached Upᵏ implementation
        # -----------------------------------------------------------------
        @lru_cache(maxsize=50000)
        def _upk_cached(k: int, ranges_key: Tuple[Tuple[int, int], ...]) -> Tuple[Tuple[int, int], ...]:
            rs = RangeSet.from_ranges(list(ranges_key))
            acc = rs
            if k <= 0:
                return tuple(acc.to_ranges())
            for _ in range(k):
                acc = self._up1_on_stateset(acc, parents_map)
                if acc.is_empty():
                    break
            return tuple(acc.to_ranges())
        def up_k(rs: RangeSet, k: int) -> RangeSet:
            if k <= 0:
                return rs
            key = tuple((int(a), int(b)) for (a, b) in rs.to_ranges())
            out_ranges = _upk_cached(int(k), key)
            if not out_ranges:
                return RangeSet.empty()
            return RangeSet.from_ranges(list(out_ranges))
        # -----------------------------------------------------------------
        # Compute terminal sets per token via backward fix‑point
        # -----------------------------------------------------------------
        all_nodes: List[NodeID] = list(self.arena.keys())
        node_is_end: Dict[NodeID, bool] = {
        ...
        total_repl1 = 0
        total_repl2 = 0
        for tok in tqdm(working_tokens, desc="Per‑token optimisation", leave=False):
            out_map = edges_by_token[tok]
            inc_map = incoming_by_token.get(tok, {})
            # Initialise terminal sets: end nodes → U, others → ∅
            T: Dict[NodeID, RangeSet] = {}
            q = collections.deque()
            for nid in all_nodes:
                if node_is_end.get(int(nid), False):
                    T[int(nid)] = U
                    q.append(int(nid))
                else:
            ...
            while q:
                dest = q.popleft()
                dest_T = T.get(dest, RangeSet.empty())
                if dest_T.is_empty():
                    continue
                for (src, pop, mask) in inc_map.get(dest, []):
                    inter = dest_T.intersection(mask)
                    if inter.is_empty():
                        continue
                    contrib = up_k(inter, pop)
                    if contrib.is_empty():
                        continue
                    old = T.get(src, RangeSet.empty())
                    new = old.union(contrib)
                    if not new.difference(old).is_empty():
                        T[src] = new
                        q.append(int(src))
            # -------------------------
            # Alt 6 – mask expansion
            # -------------------------
            full_masks_this_tok = 0
            for src, pd_map in out_map.items():
                for (pop, dest), mask in list(pd_map.items()):
                    if T.get(int(dest), RangeSet.empty()).is_empty():
                        if not rs_eq(mask, U):
                            pd_map[(pop, dest)] = U
                            full_masks_this_tok += 1
            total_full_masks += full_masks_this_tok
            # -------------------------
            # Alt 2 – edge reduction
            # -------------------------
            outgoing_count: Dict[NodeID, int] = collections.defaultdict(int)
            incoming_count: Dict[NodeID, int] = collections.defaultdict(int)
            for s, pd_map in out_map.items():
                outgoing_count[int(s)] = len(pd_map)
                for (_pop, d) in pd_map.keys():
                    incoming_count[int(d)] += 1
            candidates_B: List[NodeID] = [
                int(nid) for nid in all_nodes
                if incoming_count.get(int(nid), 0) == 1
                and outgoing_count.get(int(nid), 0) == 1
                and not node_is_end.get(int(nid), False)
            ]
            def get_unique_out_edge(node_b: int) -> Optional[Tuple[int, RangeSet, NodeID]]:
                pd = out_map.get(node_b, {})
                if len(pd) != 1:
                    return None
                (pop_m, dest_c), mask_y = next(iter(pd.items()))
                return (int(pop_m), mask_y, int(dest_c))
            def get_unique_in_edge(node_b: int) -> Optional[Tuple[NodeID, int, RangeSet]]:
                incs = []
                for s, pd in out_map.items():
                    for (pop_n, dest), mask_x in pd.items():
                        if dest == node_b:
                            incs.append((int(s), int(pop_n), mask_x))
                if len(incs) != 1:
                    return None
                return incs[0]
            repl1_this_tok = 0
            ...
            for b in candidates_B:
                out_edge = get_unique_out_edge(b)
                in_edge = get_unique_in_edge(b)
                if not out_edge or not in_edge:
                    continue
                pop_m, mask_y, dest_c = out_edge
                a_src, pop_n, mask_x = in_edge
                # Replacement 1: incoming mask is full universe
                if rs_eq(mask_x, U):
                    out_map[a_src].pop((pop_n, b), None)
                    out_map[b].pop((pop_m, dest_c), None)
                    merged_key = (pop_n + pop_m, dest_c)
                    out_map.setdefault(a_src, {})[merged_key] = out_map.setdefault(a_src, {}).get(merged_key, RangeSet.empty()).union(mask_y)
                    outgoing_count[b] = incoming_count[b] = 0
                    repl1_this_tok += 1
                    continue
                # Replacement 2: trailing pop is zero
                if pop_m == 0:
                    new_mask = mask_x.intersection(mask_y)
                    out_map[a_src].pop((pop_n, b), None)
                    out_map[b].pop((pop_m, dest_c), None)
                    merged_key = (pop_n, dest_c)
                    out_map.setdefault(a_src, {})[merged_key] = out_map.setdefault(a_src, {}).get(merged_key, RangeSet.empty()).union(new_mask)
                    outgoing_count[b] = incoming_count[b] = 0
                    repl2_this_tok += 1
                    continue
        ...
        # -----------------------------------------------------------------
        print(" - Rebuilding children with token grouping…", flush=True)
        new_children_by_node: Dict[NodeID, List[Tuple[Tuple[int, LLMTokenSet], List[Tuple[int, StateIDSet]]]]] = {
            int(nid): [] for nid in all_nodes
        }
        for nid in tqdm(all_nodes, desc="Rebuild pass", leave=False):
            # token → (pop, dest → mask) mapping for this node
            token_sig: Dict[int, Dict[int, Dict[int, RangeSet]]] = {}
            for tok in working_tokens:
                pd = edges_by_token.get(tok, {}).get(int(nid), {})
                if not pd:
                    continue
                token_sig.setdefault(tok, {})
                for (pop, dest), mask in pd.items():
                    token_sig[tok].setdefault(int(pop), {})[int(dest)] = mask
            # Group tokens that share the exact same (pop → dest → mask) map
            groups: Dict[Tuple[int, Tuple[Tuple[int, Tuple[Tuple[int, int], ...]], ...]], Set[int]] = collections.defaultdict(set)
            for tok, pop_map in token_sig.items():
                for pop, dest_map in pop_map.items():
                    dest_sig = tuple(sorted(
                        (dest_id, tuple((a, b) for (a, b) in mask.to_ranges()))
                        for dest_id, mask in dest_map.items()
                    ))
                    key = (int(pop), dest_sig)
                    groups[key].add(int(tok))
            # Convert each group into a child entry
            children_list: List[Tuple[Tuple[int, LLMTokenSet], List[Tuple[int, StateIDSet]]]] = []
            for (pop, dest_sig), toks in groups.items():
                llm_bv = RangeSet.from_indices(sorted(toks))
                dests_list: List[Tuple[int, StateIDSet]] = [
                    (int(dest_id), RangeSet.from_ranges(list(ranges)))
                    for dest_id, ranges in dest_sig
                ]
            ...
            children_list.sort(
                key=lambda itm: (
                    int(itm[0][0]),
                    _rs_key(itm[0][1]),
                    tuple(sorted(int(d[0]) for d in itm[1])),
                )
            )
            new_children_by_node[int(nid)] = children_list
        # Install rebuilt children and recompute auxiliary data
        for nid, node in self.arena.items():
            node.children = new_children_by_node.get(int(nid), [])
