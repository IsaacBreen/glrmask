...
class Model(GraphProvider):
    ...
    def _unconditionalize_guaranteed_transitions(
        ...
        # Strategy (safe local transformation):
        # For each node, and for each pop value present on its outgoing edges:
        # - Compute P_pop = union of all state masks across ALL edges with that pop
        #   (ignoring edges that are already unconditional for that pop).
        # - For each specific edge (pop, llm_bv) with dests:
        #     Let U_edge = union of its dest masks (ignoring unconditional).
        #     If:
        #       a) All dests map to the same dest_id (or there is exactly one dest), and
        #       b) U_edge == P_pop, and
        #       c) There is no already-unconditional edge for this pop at this node,
        #     then we can replace this edge's dest list with a single unconditional entry
        #     (state_bv = empty) to avoid per-state filtering at runtime.
        #
        # This is safe because for this token group, after popping 'pop' frames, the next
        # destination does not depend on which specific parser state is on top of the stack
        # (for any parser state that can actually appear at that node for that pop).

        def rs_eq(a: RangeSetABC, b: RangeSetABC) -> bool:
            # Compare by ranges to be robust across RangeSet implementations.
            try:
                return tuple((int(x), int(y)) for x, y in a.to_ranges()) == tuple((int(x), int(y)) for x, y in b.to_ranges())
            except Exception:
                # Fallback equality check via symmetric difference emptiness, if available.
                try:
        ...
        for node in self.arena.values():
            ...
            # 1) Build per-pop unions of state masks, and record whether any edge is already unconditional for that pop.
            union_by_pop: Dict[int, RangeSetABC] = {}
            has_uncond_by_pop: Set[int] = set()

            for (pop, _llm_bv), dests in children:
                p = int(pop)
                # Initialize union with empty
                accum = union_by_pop.get(p, PyRangeSet.empty())
                for _dest_idx, state_bv in dests:
                    if state_bv.is_empty():
                        # Presence of any unconditional edge for this pop means we cannot
                        # reliably reconstruct the full "universe" for this pop from masks.
                        has_uncond_by_pop.add(p)
                    else:
                        accum = accum.union(state_bv) if not accum.is_empty() else state_bv
                union_by_pop[p] = accum

            # 2) Attempt unconditionalization per edge, using the per-pop unions computed above.
            new_children: List[Tuple[Tuple[int, LLMTokenSet], List[Tuple[int, StateIDSet]]]] = []
            for (pop, llm_bv), dests in children:
                p = int(pop)
                # Skip pops where an unconditional edge already exists (universe unknown).
                if p in has_uncond_by_pop:
                    new_children.append(((pop, llm_bv), dests))
                    continue

                # Compute this edge's union of masks and check if all dests refer to the same destination.
                edge_union: RangeSetABC = PyRangeSet.empty()
                first_dest_id: Optional[int] = None
                single_dest_only = True
                all_masks_empty = True
                for dest_idx, state_bv in dests:
                    if first_dest_id is None:
                        first_dest_id = int(dest_idx)
                    elif first_dest_id != int(dest_idx):
                        single_dest_only = False
                    if not state_bv.is_empty():
                        all_masks_empty = False
                        edge_union = edge_union.union(state_bv) if not edge_union.is_empty() else state_bv

                # If it's already unconditional, keep as-is.
                if all_masks_empty:
                    new_children.append(((pop, llm_bv), dests))
                    continue

                # Compare with the per-pop union and decide whether to unconditionalize.
                pop_universe = union_by_pop.get(p, PyRangeSet.empty())
                if not pop_universe.is_empty() and single_dest_only and first_dest_id is not None and rs_eq(edge_union, pop_universe):
                    # Safe to make unconditional: replace masks with a single unconditional entry.
                    new_children.append(((pop, llm_bv), [(int(first_dest_id), PyRangeSet.empty())]))
                    changed_edges += 1
                else:
                    new_children.append(((pop, llm_bv), dests))
        ...
        print(f" done. {changed_edges} edges unconditionalized.", flush=True)
