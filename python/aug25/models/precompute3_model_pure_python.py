...
class Model(GraphProvider):
    ...
    def _to_nodeopt(self) -> NodeOptGraph:
        ...
        q = collections.deque()
        old_root_to_new_root: Dict[int, int] = {}

        for state_id, old_root_id in self.roots_map.items():
            if old_root_id in old_root_to_new_root:
                new_roots_map[state_id] = old_root_to_new_root[old_root_id]
                continue

            new_root_id = fresh_id()
            old_root_to_new_root[old_root_id] = new_root_id
            new_roots_map[state_id] = new_root_id

            nodes[new_root_id] = NodeOpt(is_end=self.is_end(old_root_id))
            q.append((old_root_id, new_root_id))

        all_states: Set[int] = set(self.parser_table.table.keys())
        def is_all_states(state_bv: StateIDSet) -> bool:
            return not all_states or all_states.issubset(state_bv.to_indices())

        processed_new_ids = set()
        while q:
            old_src_id, new_src_id = q.popleft()
            if new_src_id in processed_new_ids:
                continue
            processed_new_ids.add(new_src_id)

            new_src_nodeopt = nodes[new_src_id]
            old_src_arena_node = self.arena.get(old_src_id, {})

            children = old_src_arena_node.get("children") or []
            for (pop, llm_bv), dests in children:
                pop = int(pop)
                tokens = [int(t) for t in llm_bv.to_indices()]

                for dest_idx, state_bv in dests:
                    old_dest_id = int(dest_idx)
                    unconditional = state_bv.is_empty() or is_all_states(state_bv)

                    if unconditional:
                        new_dest_id = fresh_id()
                        nodes[new_dest_id] = NodeOpt(is_end=self.is_end(old_dest_id))
                        q.append((old_dest_id, new_dest_id))

                        edge = PopEdge(pop) if pop > 0 else UnconditionalEdge()
                        for tok in tokens:
                            new_src_nodeopt.children.setdefault(tok, {})[new_dest_id] = edge
                    else:
                        states_set = set(int(s) for s in state_bv.to_indices())
                        if pop == 0:
                            new_dest_id = fresh_id()
                            nodes[new_dest_id] = NodeOpt(is_end=self.is_end(old_dest_id))
                            q.append((old_dest_id, new_dest_id))

                            edge = StateEdge(states_set)
                            for tok in tokens:
                                new_src_nodeopt.children.setdefault(tok, {})[new_dest_id] = edge
                        else:
                            for tok in tokens:
                                mid_id = fresh_id()
                                nodes[mid_id] = NodeOpt(is_end=False)
                                new_dest_id = fresh_id()
                                nodes[new_dest_id] = NodeOpt(is_end=self.is_end(old_dest_id))
                                q.append((old_dest_id, new_dest_id))

                                new_src_nodeopt.children.setdefault(tok, {})[mid_id] = PopEdge(pop)
                                nodes[mid_id].children.setdefault(tok, {})[new_dest_id] = StateEdge(states_set)

        return NodeOptGraph(nodes=nodes, roots_map=new_roots_map)

    def _from_nodeopt(self, graph: NodeOptGraph) -> None:
        new_arena: Dict[NodeID, ArenaNode] = {}

        for src_id, node_opt in graph.nodes.items():
            groups: Dict[Tuple[int, int], List[Tuple[int, StateIDSet]]] = collections.defaultdict(list)
            tokens_present: Set[int] = set()

            for tok, dest_map in node_opt.children.items():
                tokens_present.add(tok)
                for dest, edge in dest_map.items():
                    if isinstance(edge, PopEdge):
                        groups[(edge.n, tok)].append((dest, PyRangeSet.empty()))
                    elif isinstance(edge, StateEdge):
                        state_bv = PyRangeSet.from_indices(sorted(list(edge.states)))
                        groups[(0, tok)].append((dest, state_bv))
                    elif isinstance(edge, UnconditionalEdge):
                        groups[(0, tok)].append((dest, PyRangeSet.empty()))

            children_list: List[Tuple[Tuple[int, LLMTokenSet], List[Tuple[NodeID, StateIDSet]]]] = []
            for (pop, token), dests_list in sorted(groups.items()):
                llm_bv = PyRangeSet.from_indices([token])
                children_list.append(((pop, llm_bv), sorted(dests_list)))

            llm_union = PyRangeSet.from_indices(sorted(list(tokens_present)))
            node_dict: ArenaNode = {
