        ...
        while depth_heap:
            ...
            while todo[depth]:
                ...
                for (pop, llm_bv), dests in edges:
                    ...
                    peeked = popped.peek()
                    peeked_sorted = sorted(peeked)
                    num_peeked = len(peeked_sorted)

                    # OPTIMIZATION: Share acc_memo across all destinations for this edge group
                    # to improve hit rate.
                    ...
                    for dest_idx, state_bv in dests:
                        ...
                        stats['sid_candidates_total'] += num_peeked

                        # OPTIMIZATION: Instead of many `contains` calls, do a single
                        # pass over sorted sids and ranges. This avoids FFI overhead.
                        t_intersect_start = time.time()
                        values_to_keep = []
                        ranges = state_bv.intervals
                        num_ranges = len(ranges)
                        
                        range_idx = 0
                        peek_idx = 0

                        while range_idx < num_ranges and peek_idx < num_peeked:
                            start, end = ranges[range_idx]
                            sid = peeked_sorted[peek_idx]
                            if sid >= end:
                                range_idx += 1
                            elif sid < start:
                                peek_idx += 1
                            else:  # start <= sid < end
                                values_to_keep.append(sid)
                                peek_idx += 1
                    ...
                    # Update memo stats once per edge group
