...
template <typename T, typename Acc>
using UpperChildrenMap = std::unordered_map<T, std::unordered_map<int, UpperPtr<T, Acc>>>;
template <typename T, typename Acc>
using UpperChildrenConstPtr = std::shared_ptr<const UpperChildrenMap<T, Acc>>;

template <typename T>
using LowerChildrenMap = std::unordered_map<T, std::unordered_map<int, LowerPtr<T>>>;
template <typename T>
using LowerChildrenConstPtr = std::shared_ptr<const LowerChildrenMap<T>>;
...
struct UpperBranch : public Upper<T, Acc> {
    UpperChildrenConstPtr<T, Acc> _children;
    std::shared_ptr<Acc> empty; // nullptr means None

    UpperBranch(UpperChildrenConstPtr<T, Acc> children, std::shared_ptr<Acc> empty_acc)
...
template <typename T>
struct Lower {
    LowerChildrenConstPtr<T> _children;
    bool empty{false};
    int _max_depth{0};

    Lower(LowerChildrenConstPtr<T> children, bool empty_flag)
...
struct Interface : public Upper<T, Acc> {
    ...
    Interface(LowerChildrenConstPtr<T> children, std::shared_ptr<Acc> a, std::shared_ptr<Acc> e)
...
template <typename ChildrenConstPtr, typename MergeFunc, typename TKey, typename NodePtr>
static inline ChildrenConstPtr
_merge_children_by_depth(
    const ChildrenConstPtr& c1,
    const ChildrenConstPtr& c2,
    MergeFunc merge_func
) {
    if (c1 == c2) {
        return c1;
    }
    using ChildrenMap = typename ChildrenConstPtr::element_type;
    auto merged_children_map = std::make_shared<ChildrenMap>();
    ...
    for (auto &v : all_keys) {
        ...
        (*merged_children_map)[v] = std::move(v_out);
    }
    return merged_children_map;
}
...
static UpperBranchPtr<T, Acc> interface_to_upperbranch(const InterfacePtr<T, Acc>& it) {
    auto children_map = std::make_shared<UpperChildrenMap<T, Acc>>();
    ...
    return std::make_shared<UpperBranch<T, Acc>>(children_map, new_empty);
}
...
static UpperPtr<T, Acc> try_promote(const UpperBranchPtr<T, Acc>& node) {
    ...
    if (all_children.empty()) {
        if (node->empty) {
            ...
            return std::make_shared<Interface<T, Acc>>(std::make_shared<const LowerChildrenMap<T>>(), node->empty, node->empty);
        }
        return node;
    }
    ...
    if (accs.size() <= 1) {
        ...
        if (!accs.empty()) {
            ...
            // Truly empty GSS
            return std::make_shared<UpperBranch<T, Acc>>(std::make_shared<const UpperChildrenMap<T, Acc>>(), std::shared_ptr<Acc>(nullptr));
        }

        // Build Lower children by converting each Interface child
        auto l_children_map = std::make_shared<LowerChildrenMap<T>>();
        for (auto &kv : *node->_children) {
            const T& v = kv.first;
            const auto& kids = kv.second;
            ...
            if (!v_map.empty()) {
                (*l_children_map)[v] = std::move(v_map);
            }
        }
        return std::make_shared<Interface<T, Acc>>(l_children_map, the_acc, node->empty);
    }
    return node;
}
...
static UpperPtr<T, Acc> merge_upperbranches(const UpperBranchPtr<T, Acc>& a, const UpperBranchPtr<T, Acc>& b) {
    ...
    auto merged_children = _merge_children_by_depth<UpperChildrenConstPtr<T, Acc>, std::function<UpperPtr<T, Acc>(UpperPtr<T, Acc>, UpperPtr<T, Acc>)>, T, UpperPtr<T, Acc>>(
        a->_children, b->_children,
        [](UpperPtr<T, Acc> n1, UpperPtr<T, Acc> n2) { return merge_upper<T, Acc>(n1, n2); }
    );
    auto ub = std::make_shared<UpperBranch<T, Acc>>(merged_children, new_empty);
...
static UpperPtr<T, Acc> merge_interfaces(const InterfacePtr<T, Acc>& a, const InterfacePtr<T, Acc>& b) {
    if ((a->acc && b->acc && *a->acc == *b->acc) || a->_children == b->_children) {
        auto merged_children = _merge_children_by_depth<LowerChildrenConstPtr<T>, std::function<LowerPtr<T>(LowerPtr<T>, LowerPtr<T>)>, T, LowerPtr<T>>(
            a->_children, b->_children,
...
static LowerPtr<T> merge_lower(const LowerPtr<T>& l1, const LowerPtr<T>& l2) {
    ...
    return std::make_shared<Lower<T>>(merged_children, new_empty);
}
...
static UpperPtr<T, Acc> lower_to_upper(const LowerPtr<T>& l, const std::shared_ptr<Acc>& acc) {
    ...
    auto ub = std::make_shared<UpperBranch<T, Acc>>(children_map, l->empty ? acc : std::shared_ptr<Acc>(nullptr));
    return try_promote<T, Acc>(ub);
}
...
public:
    ...
    LeveledGSS() : inner(std::make_shared<UpperBranch<T, Acc>>(std::make_shared<const UpperChildrenMap<T, Acc>>(), std::shared_ptr<Acc>(nullptr))) {}
    ...
    static LeveledGSS from_stacks(const std::vector<std::pair<std::vector<T>, std::shared_ptr<Acc>>>& stacks) {
        ...
        build = [&](std::shared_ptr<TrieNode> d, std::shared_ptr<Acc> root_empty) -> UpperPtr<T, Acc> {
            ...
            for (auto &kv : d->sub) {
                ...
                auto end_acc = e->end;
                if (end_acc) {
                    auto leaf = std::make_shared<UpperBranch<T, Acc>>(std::make_shared<const UpperChildrenMap<T, Acc>>(), end_acc);
                    ...
                    build_lower = [&](std::shared_ptr<TrieNode> nd) -> LowerPtr<T> {
                        ...
                        for (auto &kv2 : nd->sub) {
                            ...
                            auto sub_lower = !e2->sub.empty() ? build_lower(e2)
                                                              : std::make_shared<Lower<T>>(std::make_shared<const LowerChildrenMap<T>>(), false);
                            ...
                            (*l_children_map)[v2] = std::move(m);
                        }
                        return std::make_shared<Lower<T>>(l_children_map, false);
                    };

                    auto lower_tree = build_lower(d);
                    return std::make_shared<Interface<T, Acc>>(lower_tree->_children, the_acc, root_empty);
                }
            }

            return std::make_shared<UpperBranch<T, Acc>>(children_map, root_empty);
        };
    ...
    LeveledGSS push(const T& value) const {
        ...
        if (inner->is_interface()) {
            ...
            auto new_lc_map = std::make_shared<LowerChildrenMap<T>>();
            std::unordered_map<int, LowerPtr<T>> m;
            m[lower_node->_max_depth] = lower_node;
            (*new_lc_map)[value] = std::move(m);
            auto new_interface = std::make_shared<Interface<T, Acc>>(new_lc_map, it->acc, std::shared_ptr<Acc>(nullptr));
            return LeveledGSS(new_interface);
        } else {
            auto ub = std::static_pointer_cast<UpperBranch<T, Acc>>(inner);
            auto ch_map = std::make_shared<UpperChildrenMap<T, Acc>>();
            std::unordered_map<int, UpperPtr<T, Acc>> m;
            m[ub->_max_depth] = inner;
            (*ch_map)[value] = std::move(m);
            return LeveledGSS(std::make_shared<UpperBranch<T, Acc>>(ch_map, std::shared_ptr<Acc>(nullptr)));
        }
    }
    ...
    LeveledGSS pop() const {
        if (inner->is_interface()) {
            ...
            if (!merged) {
                auto ub = std::make_shared<UpperBranch<T, Acc>>(std::make_shared<const UpperChildrenMap<T, Acc>>(), std::shared_ptr<Acc>(nullptr));
            ...
            if (!merged_empty && merged->_children->empty()) {
                return LeveledGSS(std::make_shared<UpperBranch<T, Acc>>(std::make_shared<const UpperChildrenMap<T, Acc>>(), merged_empty));
            } else {
            ...
            if (!merged) merged = std::make_shared<UpperBranch<T, Acc>>(std::make_shared<const UpperChildrenMap<T, Acc>>(), std::shared_ptr<Acc>(nullptr));
    ...
    LeveledGSS popn(int n) const {
        ...
        std::function<LowerPtr<T>(const LowerPtr<T>&, int)> _popn_lower =
            [&](const LowerPtr<T>& node, int k) -> LowerPtr<T> {
                ...
                if (all_children.empty()) {
                    auto res = std::make_shared<Lower<T>>(std::make_shared<const LowerChildrenMap<T>>(), false);
                    memo_lower[key] = res;
                    return res;
                }
            ...
            [&](const UpperPtr<T, Acc>& node, int k) -> UpperPtr<T, Acc> {
                ...
                if (node->is_interface()) {
                    ...
                    if (all_lower_children.empty()) {
                        res = std::make_shared<UpperBranch<T, Acc>>(std::make_shared<const UpperChildrenMap<T, Acc>>(), std::shared_ptr<Acc>(nullptr));
                    } else {
    ...
    LeveledGSS isolate(const T& value) const {
        if (!inner->is_interface()) {
            auto ub = std::static_pointer_cast<UpperBranch<T, Acc>>(inner);
            auto filtered_map = std::make_shared<UpperChildrenMap<T, Acc>>();
            auto it = ub->_children->find(value);
            if (it != ub->_children->end()) (*filtered_map)[value] = it->second;
            auto res = std::make_shared<UpperBranch<T, Acc>>(filtered_map, std::shared_ptr<Acc>(nullptr));
            return LeveledGSS(try_promote<T, Acc>(res));
        } else {
            auto it = std::static_pointer_cast<Interface<T, Acc>>(inner);
            auto itc = it->_children->find(value);
            if (itc == it->_children->end()) {
                return LeveledGSS(std::make_shared<UpperBranch<T, Acc>>(std::make_shared<const UpperChildrenMap<T, Acc>>(), std::shared_ptr<Acc>(nullptr)));
            } else {
                auto filtered_map = std::make_shared<LowerChildrenMap<T>>();
                (*filtered_map)[value] = itc->second;
                auto res = std::make_shared<Interface<T, Acc>>(filtered_map, it->acc, std::shared_ptr<Acc>(nullptr));
    ...
    LeveledGSS isolate_none() const {
        ...
        auto new_root = std::make_shared<UpperBranch<T, Acc>>(std::make_shared<const UpperChildrenMap<T, Acc>>(), empty_acc);
    ...
    LeveledGSS isolate_many(const std::unordered_set<T>& values) const {
        ...
        if (!inner->is_interface()) {
            auto ub = std::static_pointer_cast<UpperBranch<T, Acc>>(inner);
            auto filtered_children_map = std::make_shared<UpperChildrenMap<T, Acc>>();
            for (auto &kv : *ub->_children) {
                if (values.count(kv.first)) (*filtered_children_map)[kv.first] = kv.second;
            }
            auto res = std::make_shared<UpperBranch<T, Acc>>(filtered_children_map, new_empty);
            return LeveledGSS(try_promote<T, Acc>(res));
        } else {
            auto it = std::static_pointer_cast<Interface<T, Acc>>(inner);
            auto filtered_children_map = std::make_shared<LowerChildrenMap<T>>();
            for (auto &kv : *it->_children) {
                if (values.count(kv.first)) (*filtered_children_map)[kv.first] = kv.second;
            }
            if (!filtered_children_map->empty()) {
                auto res = std::make_shared<Interface<T, Acc>>(filtered_children_map, it->acc, new_empty);
                return LeveledGSS(res);
            } else {
                auto res = std::make_shared<UpperBranch<T, Acc>>(std::make_shared<const UpperChildrenMap<T, Acc>>(), new_empty);
                return LeveledGSS(try_promote<T, Acc>(res));
            }
        }
    }
        ...
        std::function<UpperPtr<T, Acc>(const UpperPtr<T, Acc>&)> transform =
            [&](const UpperPtr<T, Acc>& node) -> UpperPtr<T, Acc> {
                ...
                if (node->is_interface()) {
                    ...
                    auto new_children_map = std::make_shared<UpperChildrenMap<T, Acc>>();
                    ...
                    for (auto &kv : *ub->_children) {
                        ...
                        if (child_map_changed) {
                            changed = true;
                            if (!new_kids.empty()) (*new_children_map)[v] = std::move(new_kids);
                        } else {
                            (*new_children_map)[v] = kids;
                        }
                    }
                    ...
                    auto res = std::make_shared<UpperBranch<T, Acc>>(new_children_map, new_empty);
        ...
        std::function<UpperPtr<T, Acc>(const UpperPtr<T, Acc>&)> transform =
            [&](const UpperPtr<T, Acc>& node) -> UpperPtr<T, Acc> {
                ...
                if (node->is_interface()) {
                    ...
                    if (new_children_map->empty() && !new_empty) {
                        memo[nid] = nullptr;
                        return nullptr;
                    }
                    auto res = std::make_shared<UpperBranch<T, Acc>>(new_children_map, new_empty);
