...
public:
    ...
    LeveledGSS apply(
        ...
        std::unordered_map<Acc*, std::shared_ptr<Acc>>* acc_memo_ptr = nullptr
    ) const {
        std::unordered_map<std::uintptr_t, UpperPtr<T, Acc>> memo;

        std::unordered_map<Acc*, std::shared_ptr<Acc>> local_acc_memo;
        ...
        auto apply_func = [&](const std::shared_ptr<Acc>& a) -> std::shared_ptr<Acc> {
            ...
            auto it = acc_memo.find(a.get());
            if (it != acc_memo.end()) return it->second;
            auto r = func(a);
            acc_memo.emplace(a.get(), r);
            return r;
        };
    ...
    LeveledGSS apply_and_prune(
        ...
        std::unordered_map<Acc*, std::shared_ptr<Acc>>* acc_cache_ptr = nullptr
    ) const {
        // mutator returns nullptr to prune stacks carrying acc; otherwise updated acc
        std::unordered_map<std::uintptr_t, UpperPtr<T, Acc>> memo;

        std::unordered_map<Acc*, std::shared_ptr<Acc>> local_acc_cache;
        std::unordered_map<Acc*, std::shared_ptr<Acc>>& acc_cache =
            acc_cache_ptr ? *acc_cache_ptr : local_acc_cache;

        auto mutate_acc = [&](const std::shared_ptr<Acc>& a) -> std::shared_ptr<Acc> {
            if (!a) return std::shared_ptr<Acc>(nullptr);
            auto it = acc_cache.find(a.get());
            if (it != acc_cache.end()) return it->second;
            auto r = mutator(a);
            acc_cache.emplace(a.get(), r);
            return r;
        };
