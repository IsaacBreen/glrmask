...
class PyAcc:
    ...
    def __hash__(self):
        ...
        # Note: Uses len(terminals_union) to be consistent with original __hash__
...
class Model(GraphProvider):
    """
    Precomputed trie model (third-generation), optimized for performance.

    Key optimizations in get_mask:
    - Remove all stats collection to avoid overhead.
    - Accumulate outbound child GSS per node as lists and merge once when the node
      is actually processed. This dramatically reduces the number of intermediate merges.
    - Avoid repeated popped.peek() calls per-dest by computing it once per (pop, llm_bv) block.
    - Memoize per-edge accumulator transformations keyed by the current llm_mask (RangeSet),
      not the PyAcc instance, to maximize reuse across structurally different but equal masks.
    - Micro-optimizations in loops: local variable bindings, early continues, minimal allocations.
    """
    ...
    def commit(self, token_id: int):
        # Kept largely identical to the baseline; commit time is small compared to get_mask.
    ...
    def get_mask(self) -> RangeSet:
        """
        Optimized LLM token mask computation.

        High-level algorithm:
        - Initialize each accumulator's allowed llm_mask once (consuming terminals_union).
        - Traverse the trie in non-decreasing order of depth.
        - For each edge (pop, llm_bv) and each destination block, pop stacks once,
          peek heads once, filter heads with state_bv, isolate_many once, and intersect
          per-acc llm_mask with llm_bv using a memo keyed by the current RangeSet mask.
        - Accumulate GSS contributions per-destination as lists and merge only when the
          destination node is actually processed next time.
        """
        ...
        max_state: int = self.tokenizer_max_state if self.tokenizer_max_state is not None else -1

        all_ones: Optional[RangeSet] = self.all_internal_llm_tokens_bitset
        final_mask: RangeSet = RangeSet.empty()

        # values_bag holds node -> list of GSS; we only merge when popping the node to process.
        values_bag: Dict[int, List[GSS]] = {}
        todo: Dict[int, Set[int]] = {}
        depth_heap: List[int] = []

        hp, hpop = heapq.heappush, heapq.heappop

        # Seed: Initialize llm_mask per-acc, consume terminals_union, enqueue roots.
        def initialize_acc(acc: PyAcc) -> PyAcc:
            ...
            # Fast path: if no disallowed terminals, everyone is allowed
            if not disallowed_map:
                allowed_mask = all_ones if all_ones is not None else RangeSet.empty()
                return PyAcc(
                    terminals_union={},  # consume
                    llm_mask=allowed_mask,
                )

            # Union mapped LLM masks for each disallowed terminal in each tokenizer state
            # Note: We intentionally iterate over terminal indices using to_indices(),
            # since these sets are typically small.
            for tsid, disallowed_terminals in disallowed_map.items():
                if tsid > max_state:
                    continue
                terminals_to_llm = pmc.get(tsid)
                if not terminals_to_llm:
                    continue
        ...
        for sid, gss in state_map.items():
            ...
            lst = values_bag.get(r)
            if lst is None:
                values_bag[r] = [gss_initialized]
            else:
        ...
        # Global memo for accumulator intersection on a given edge mask.
        # Keyed by (llm_mask RangeSet, llm_bv RangeSet) -> Optional[PyAcc]
        # We key by the acc.llm_mask value (RangeSet), not the PyAcc instance, in order
        # to reuse across structurally distinct but equal accumulators.
        global_acc_intersect_cache: Dict[Tuple[RangeSet, RangeSet], Optional[PyAcc]] = {}
        ...
        while depth_heap:
            ...
            while bucket:
                ...
                gss_list = values_bag.pop(node, None)
                if not gss_list:
                    # This node was pruned; skip
                    continue

                gss_node: GSS = gss_list[0] if len(gss_list) == 1 else GSS.merge_many(gss_list)

                # End-node handling: union allowed LLM tokens
                if is_end(node):
                ...
                for (pop, llm_bv), dests in edges:
                    ...
                    # Compute peeked once for this popped; reused across all dests under this (pop, llm_bv) block.
                    peeked = popped.peek()
                    if not peeked:
                        continue
                    # Localize for speed
                    peeked_list = list(peeked)

                    # Per-edge memo keyed by the acc.llm_mask RangeSet (not PyAcc) to cut
                    # down repeated intersection work across equivalent masks in this edge.
                    # We still keep a global cache keyed by (mask, llm_bv) for reuse across edges.
                    per_edge_acc_memo: Dict[RangeSet, Optional[PyAcc]] = {}

                    def intersect_and_prune(acc: PyAcc) -> Optional[PyAcc]:
                        # Fast path: check per-edge memo
                        acc_mask = acc.llm_mask
                        cached = per_edge_acc_memo.get(acc_mask)
                        if cached is not None or (acc_mask in per_edge_acc_memo):
                            return cached
                        # Check global cache
                        global_key = (acc_mask, llm_bv)
                        cached_global = global_acc_intersect_cache.get(global_key)
                        if cached_global is not None or (global_key in global_acc_intersect_cache):
                            per_edge_acc_memo[acc_mask] = cached_global
                            return cached_global

                        new_mask = acc_mask.intersection(llm_bv)
                        ...
                        per_edge_acc_memo[acc_mask] = result
                        global_acc_intersect_cache[global_key] = result
                        return result

                    # Process destination partitions
                    for dest_idx, state_bv in dests:
                        contains = state_bv.contains  # localize
                        # Filter heads using contains; peeked_list is typically small (tens),
                        # so this Python loop is faster than RangeSet intersections for large #dests.
                        values_to_keep = [sid for sid in peeked_list if contains(sid)]
                        ...
                        d: int = int(dest_idx)
                        lst = values_bag.get(d)
                        if lst is None:
                            values_bag[d] = [child_gss]
                        else:
                            lst.append(child_gss)
                        enqueue(max_depth[d], d)

            # Remove bucket for this depth to allow GC
            todo.pop(depth, None)
