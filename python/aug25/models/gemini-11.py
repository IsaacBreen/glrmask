...
class Model(GraphProvider):
    ...
    def get_mask(self) -> RangeSet:
        ...
        This version is optimized to reduce GSS merge operations and improve traversal efficiency.
        """
        ...
        # values will store lists of GSSs to be merged lazily.
        values: Dict[int, List[GSS]] = collections.defaultdict(list)
        ...
        def initialize_acc(acc: PyAcc) -> PyAcc:
            ...
            for tsid, disallowed_terminals in disallowed_map.items():
                ...
                # Iterate over intervals to avoid creating a large list of indices
                for start, end in disallowed_terminals.intervals:
                    for terminal_id in range(start, end + 1):
        ...
        while depth_heap:
            ...
            while todo[depth]:
                ...
                gss_node = GSS.merge_many(gss_list) if len(gss_list) > 1 else gss_list[0]
                ...
                for (pop, llm_bv), dests in edges:
                    popped: GSS = gss_node.popn(pop) if pop > 0 else gss_node
                    if popped.is_empty():
                        continue

                    peeked_states = popped.peek()
                    ...
                    for dest_idx, state_bv in dests:
                        ...
                        d: int = int(dest_idx)
                        values[d].append(child_gss)
                        
                        dest_depth = max_depth[d]
                        bucket = todo.get(dest_depth)
                        if bucket is None:
                            todo[dest_depth] = {d}
