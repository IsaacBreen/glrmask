# This is a simplified version of precompute3_model_pure_python_opt4_5.py,
# with statistics, debugging, and oracle-related code removed for clarity.
...
def _optimize_intermediate_arena(intermediate_arena: Dict[NodeID, IntermediateArenaNode], max_depth: Dict[NodeID, int]):
    for node in intermediate_arena.values():
...
class Model(GraphProvider):
    ...
    def _shrink_edges_by_descendant(self) -> None:
        """Restrict edge masks by descendant closures to prune impossible paths."""
    ...
    def _build_fast_children(self) -> None:
        """Build pop -> state_id -> dest -> (llm_bv, llm_bv_not, llm_bv_thru_desc) for each node."""
        all_ones = self.all_internal_llm_tokens_bitset
        for node in self.arena.values():
            if not node.children:
                node.fast_children = {}
            ...
            for edge in node.children:
                ...
                for dest in edge.dests:
                    for sid in dest.state_bv.iter_indices():
                        ...
                        state_map[int(dest.dest_idx)] = edge.llm_bv if prev is None else prev.union(edge.llm_bv)
        ...
        if a_node.fast_children:
            ...
            for pop_val in pops_in_node:
                ...
                if pop_val in pop_cache:
                    ...
                    if pop_val == 0:
                        popped, popped_acc, peeked, peek_rs = gss_node, types.SimpleNamespace(llm_mask=gss_mask), peek0, peek0_rs
                        pop_cache[pop_val] = (popped, popped_acc, peeked, peek_rs)
                    else:
                        popped = None
                        if global_pop_cache is not None and (id(gss_node), pop_val) in global_pop_cache:
                            popped, popped_acc, peeked, peek_rs = global_pop_cache[(id(gss_node), pop_val)]
                            pop_cache[pop_val] = (popped, popped_acc, peeked, peek_rs)
                        if popped is None:
                ...
                for sid in peeked:
                    ...
                    if not per_state_map: continue
                    for dest_id, triple in per_state_map.items():
                        llm_bv, llm_bv_not, llm_bv_thru = triple
                        if popped_acc.llm_mask.isdisjoint(llm_bv_thru): continue
                ...
                dest_keys = sorted(list(groups.keys()), key=lambda k: (k[0], k[1]))
                ...
                for (dest_id, llm_bv_id) in dest_keys:
                    ...
                    if new_mask is not None: active_remaining_mask = new_mask
                    if active_remaining_mask.is_empty(): return
            ...
            # Fallback: original edge-scanning block
            for edge_i, edge in enumerate(a_node.children):
                if edge.pop in skip_pops: continue
                if edge.llm_bv.isdisjoint(active_remaining_mask): continue
                ...
                if edge.pop in pop_cache:
                    ...
                    if edge.pop == 0:
                        ...
                        popped = None
                        if global_pop_cache is not None and (id(gss_node), edge.pop) in global_pop_cache:
                            popped, popped_acc, peeked, peek_rs = global_pop_cache[(id(gss_node), edge.pop)]
                            pop_cache[edge.pop] = (popped, popped_acc, peeked, peek_rs)
                        if popped is None:
                ...
                for dest_j in dest_keys:
                    if dests_proc >= max_dests:
                        ...
                        if active_remaining_mask.is_empty(): return
                        dests_proc = 0
                    if active_remaining_mask.is_empty(): break
    ...
    def get_mask(self) -> RangeSetOut:
        ...
        while work_heap:
            ...
            try:
                yielded = next(gen) if is_new_gen else gen.send(remaining_mask)
