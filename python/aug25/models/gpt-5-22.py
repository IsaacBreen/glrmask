"""
Ultra-fast get_mask implementation for the precompute3 model.

Key optimizations:
- Avoid repeated GSS merges during traversal: accumulate child GSS into per-node lists
  and merge once when the node is actually processed. This reduces merge overhead
  drastically in dense graphs.
- Move expensive popped.peek() calls out of inner loops; compute once per edge.
- Replace per-state RangeSet.contains calls with a fast Python-side intersection:
  sort peeked states once and use bisect slicing over the (disjoint, sorted) intervals
  from state_bv to build values_to_keep. This avoids crossing the FFI boundary for
  thousands of contains() checks.
- Keep the overall algorithm the same as the original precompute3 model, but strip
  all Stats instrumentation and reduce Python overhead.

This class is a drop-in replacement of the Python precompute3 model for get_mask,
while keeping commit() and parser-table-based token processing identical.
"""
...
from python.aug25.range_set.ffi_range_set import FFIRangeSet as RangeSet
...
# Precompiled edge data for faster traversal
@dataclass(frozen=True)
class EdgeDest:
    dest_idx: int
    # Disjoint, sorted intervals representing the allowed parser states for this edge
...
class CompiledEdge:
    pop: int
    llm_bv: RangeSet
    dests: Tuple[EdgeDest, ...]


class Model:
    """
    Precomputed trie model with an aggressively optimized get_mask implementation.
    """
    ...
    def __init__(self, roots_map: List[Tuple[int, int]], arena: Dict[int, dict]):
        ...
        self.ignore_terminal_id: Optional[Union[int, None]] = None
        ...
        # Optimized children per node: node_id -> Tuple[CompiledEdge, ...]
        ...
        for uid, node in self.arena.items():
            ...
            if not children:
                node["children"] = []
                self.compiled_children[uid_int] = tuple()
                continue

            # Build both:
            # - normalized node["children"] in RS form (for compatibility if needed)
            # - compiled_children with precomputed intervals for state filters.
            new_children = []
            ...
            for edge_key, dest_map in children:
                ...
                for dest_idx, state_bv_json in dest_map:
                    ...
                    # Precompute disjoint, sorted intervals for quick Python-side filtering
                    intervals = tuple(state_bv.intervals)
                    compiled_dests.append(EdgeDest(dest_idx=int(dest_idx), intervals=intervals))

                new_children.append(((int(pop), llm_bv), new_dest_map))
                compiled_edges.append(CompiledEdge(pop=int(pop), llm_bv=llm_bv, dests=tuple(compiled_dests)))

            node["children"] = new_children
            self.compiled_children[uid_int] = tuple(compiled_edges)
    ...
    def iter_edges(self, node: int, token: int):
        ...
        This function isn't used by the optimized get_mask but is kept for compatibility.
        """
    ...
    def commit(self, token_id: int):
        ...
        for tokenizer_sid in list(self.state.keys()):
    ...
    def _select_states_by_intervals(self, sorted_states: List[int], intervals: Tuple[Tuple[int, int], ...]) -> List[int]:
        """
        Given a sorted list of states and a tuple of disjoint, sorted intervals,
        return the subset of states that fall within any interval.
        """
    ...
    def get_mask(self) -> RangeSet:
        ...
        Optimizations:
          - Batched GSS merges (only once per node).
          - Single popped.peek() per edge; fast Python-side filtering for dest state_bv.
          - Stateless traversal with no Stats overhead.
        """
        ...
        # Instead of keeping a single merged GSS per node (and merging repeatedly),
        # we accumulate child GSS into lists and only merge when the node is processed.
        pending: Dict[int, List[GSS]] = collections.defaultdict(list)
        ...
        max_state: int = self.tokenizer_max_state or 0
        ...
        for sid, gss in state_map.items():
            ...
            pending[r].append(gss_initialized)
            d: int = max_depth[r]
        ...
        while depth_heap:
            ...
            nodes = todo.get(depth)
            if not nodes:
                # Nothing at this depth (might have been cleared), continue
                todo.pop(depth, None)
                continue

            while nodes:
                node: int = nodes.pop()
                gss_list = pending.pop(node, None)
                if not gss_list:
                    # No payload to process
                    continue

                # Merge all incoming child GSS once per node
                ...
                edges = self.compiled_children.get(node, ())
                if not edges:
                    continue

                for edge in edges:
                    popped: GSS = gss_node.popn(edge.pop)
                    if popped.is_empty():
                        continue

                    # Compute peeked states once per edge
                    peeked = popped.peek()
                    if not peeked:
                        continue
                    # Sorting small lists is cheap and enables fast interval slicing
                    sorted_peeked = sorted(peeked)

                    # Memoize acc -> Optional[acc'] per edge (same llm_bv), to avoid repeated intersections
                    ...
                    for dest in edge.dests:
                        values_to_keep = self._select_states_by_intervals(sorted_peeked, dest.intervals)
                        ...
                        # Apply edge LLM mask by intersecting per-acc llm_mask with edge.llm_bv
                        child_gss = child_gss.apply_and_prune(intersect_and_prune)
                        if child_gss.is_empty():
                            continue

                        d_idx: int = dest.dest_idx
                        pending[d_idx].append(child_gss)
                        enqueue(self.max_depth[d_idx], d_idx)

            # Clear bucket for this depth
            todo.pop(depth, None)
