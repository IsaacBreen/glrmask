...
@dataclass
class ArenaNode:
    # children[pop][state_id][dest_idx] = (llm_bv, llm_bv_not)
    children: Dict[int, Dict[int, Dict[int, Tuple[LLMTokenSet, Optional[LLMTokenSet]]]]] = field(default_factory=dict)
...
def _convert_arena(loaded_arena: Dict[NodeID, LoadedArenaNode], max_depth: Dict[NodeID, int]) -> Dict[NodeID, ArenaNode]:
    """Convert loaded arena to pop -> state_id -> dest_id -> (llm_bv, llm_bv_not) map.
    If multiple loaded edges yield the same (pop, state, dest), union their llm_bv."""
    arena: Dict[NodeID, ArenaNode] = {}
    for uid, loaded_node in tqdm(loaded_arena.items(), desc="Converting arena"):
        node_children: DefaultDict[int, DefaultDict[int, Dict[int, Tuple[LLMTokenSet, Optional[LLMTokenSet]]]]] = collections.defaultdict(lambda: collections.defaultdict(dict))
        ...
        for ledge in loaded_node.children:
            ...
            for d in ledge.dests:
                ...
                # Unpack states and fill mapping
                for sid in d.state_bv.iter_indices():
                    m = node_children[p][int(sid)]
                    existing = m.get(dest_idx)
                    if existing is None:
                        m[dest_idx] = (llm, None)
                    else:
                        # Union with any existing mask for the same (pop, state, dest)
                        m[dest_idx] = (existing[0].union(llm), None)
        arena[uid] = ArenaNode(children=dict(node_children), llm_bv_union=llm_bv_union, clean_end=loaded_node.clean_end)
    return arena
...
class Model(GraphProvider):
        ...
        for node in self.arena.values():
            ...
            for pop, state_map in node.children.items():
                for sid, dest_map in state_map.items():
                    for dest_idx, (llm, neg) in dest_map.items():
                        if neg is None:
                            dest_map[dest_idx] = (llm, all_ones.difference(llm))

    def _compute_bucket_unions(self) -> None:
        """Per node, per-pop unions for early bucket pruning."""
        for node in self.arena.values():
            pop_state_union: Dict[int, StateIDSet] = {}
            pop_llm_union: Dict[int, LLMTokenSet] = {}
            for pop, state_map in node.children.items():
                pop_state_union[pop] = RangeSetStates.from_indices(list(state_map.keys())) if state_map else RangeSetStates.empty()
                u = RangeSet.empty()
                for dest_map in state_map.values():
                    for (llm, _) in dest_map.values():
                        u |= llm
                pop_llm_union[pop] = u
            node.pop_to_state_union = pop_state_union
            node.pop_to_llm_union = pop_llm_union
    ...
    def _compute_descendant_llm_closure(self) -> None:
        ...
        while changed:
            changed = False
            for nid, node in self.arena.items():
                if node.clean_end or not node.children:
                    continue
                ...
                for pop, state_map in node.children.items():
                    for sid, dest_map in state_map.items():
                        ...
                        if not child_union.is_empty():
                            for (llm, _) in dest_map.values():
                                # tokens that can flow to some end via this transition
                                thru = llm.intersection(child_union)
    ...
    def optimize_traversal(self) -> None:
        # With state-indexed children maps, no additional per-edge indexing is needed.
        return
    ...
    def _process_internal_node_gen(
        ...
        for pop, state_map in a_node.children.items():
            ...
            # Bucket-level LLM and state pruning
            pop_llm_union = a_node.pop_to_llm_union.get(pop, RangeSet.empty())
            if pop_llm_union.isdisjoint(active_remaining_mask):
                stats.inc('get_mask.main_loop.bucket_pruned_llm')
                skip_pops.add(pop)
                continue

            # Prepare peek set and prune by states present in this bucket
            if peek0_rs is None:
                peek0_rs = RangeSetStates.from_indices(gss_node.peek())
            pop_states_union = a_node.pop_to_state_union.get(pop, RangeSetStates.empty())
            if pop_states_union.isdisjoint(peek0_rs):
                stats.inc('get_mask.main_loop.bucket_pruned_state')
                skip_pops.add(pop)
                continue

            # Pop once per bucket (cached)
            if pop in pop_cache:
            ...
            # Now traverse only relevant (state, dest) pairs
            # Group heads by (dest_idx, llm_bv_id) to amortize apply calls.
            transitions: Dict[Tuple[int, int], List[int]] = {}
            bv_by_id: Dict[int, Tuple[LLMTokenSet, Optional[LLMTokenSet]]] = {}
            # Pre-skip using gss_mask against bucket llm union
            if pop_llm_union.isdisjoint(gss_mask):
                stats.inc('get_mask.main_loop.edge.pre_gss_disjoint_skips')
                continue

            for sid in peeked:
                dmap = state_map.get(int(sid))
                if not dmap:
                    continue
                for dest_idx, (llm, llm_not) in dmap.items():
                    # Early skip by remaining and GSS mask
                    if llm.isdisjoint(active_remaining_mask):
                        continue
                    if llm.isdisjoint(gss_mask):
                        continue
                    key = (int(dest_idx), id(llm))
                    lst = transitions.get(key)
                    if lst is None:
                        transitions[key] = [int(sid)]
                        bv_by_id[id(llm)] = (llm, llm_not)
                    else:
                        lst.append(int(sid))

            # Process grouped transitions
            # Order by destination scores if provided
            dest_keys = list(transitions.keys())
            if dest_scores:
                dest_keys.sort(key=lambda t: (-int(dest_scores.get(int(t[0]), 0)), t[0]))
            else:
                dest_keys.sort()

            for (dest_idx, llm_id) in dest_keys:
                if dests_proc >= max_dests:
                    priority = (-self.max_depth.get(node_id, 0), pop, 0)
                    yield Suspend(priority, depth)
                    dests_proc = 0
                heads = transitions[(dest_idx, llm_id)]
                llm, llm_not = bv_by_id[llm_id]
                # Stats: count a traversed "edge" as a grouped (dest, llm) for this pop
                stats.inc('get_mask.traversal.edges_traversed')
                stats.inc(f'get_mask.traversal.edge_pop_val.{pop}')
                if oracle_counters is not None and local_ctr is not None:
                    local_ctr["edges"] += 1

                # Apply-and-prune caching across groups sharing the same llm
                source_after_apply = popped
                if not (llm_not and popped_acc.llm_mask.isdisjoint(llm_not)):
                    if popped_acc.llm_mask.isdisjoint(llm):
                        continue
                    key_apply = (id(popped), llm_id)
                ...
                if len(heads) == len(peeked):
                    ...
                    key_isolate = (id(source_after_apply), tuple(heads))
                    if isolate_cache is not None and key_isolate in isolate_cache:
                        ...
                        _t0 = _perf_now()
                        child_gss = source_after_apply.isolate_many(heads)
                        _t1 = _perf_now()
            ...
            for u, node in self.arena.items():
                ...
                for pop, state_map in node.children.items():
                    for sid, dest_map in state_map.items():
                        ...
                        if not child_union.is_empty():
                            for (llm, _) in dest_map.values():
                                thru = child_union.intersection(llm)
                                if not thru.is_empty():
                                    accum |= thru
                if accum.is_empty():
                    continue
    ...
    def _oracle_node_cost(self, node_id: int, analysis_node_costs: Optional[Dict[int, Dict[str, int]]] = None) -> float:
        ...
        if not node:
            return 1.0
        # Cheap fallback: number of (pop,state,dest) triples + 1
        total = 0
        for pop, state_map in node.children.items():
            for sid, dest_map in state_map.items():
                total += len(dest_map)
        return 1.0 + 0.5 * float(total)
    ...
    def _oracle_prepare_guidance(self, selected_end_nodes: List[int]) -> Tuple[Dict[int, int], Dict[int, List[int]]]:
        ...
        edge_order_map: Dict[int, List[int]] = {}
        # Return scores; rely on dest_scores during traversal. No explicit edge ordering list.
