# Deep dive: runtime/commit/token_lookup.rs

This file documents `35_commit_token_lookup_deep_dive.md`'s target role after Chunk 11.  The focus is
original-token to byte-string lookup.  It is intentionally more detailed than ordinary inline comments
because these notes are meant to guide the later all-at-once implementation
and compile-repair pass.

## What belongs here

- Code whose primary responsibility is original-token to byte-string lookup.
