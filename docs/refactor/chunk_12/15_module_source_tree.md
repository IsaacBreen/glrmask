# Commit source tree after Chunk 12

```text
src/runtime/commit/acceptance.rs
src/runtime/commit/api.rs
src/runtime/commit/fast_path.rs
src/runtime/commit/general.rs
src/runtime/commit/initial_scan.rs
src/runtime/commit/mask_assert.rs
src/runtime/commit/mod.rs
src/runtime/commit/options.rs
src/runtime/commit/parser_advance.rs
src/runtime/commit/profile.rs
src/runtime/commit/profiled.rs
src/runtime/commit/pruning.rs
src/runtime/commit/queue.rs
src/runtime/commit/single_top.rs
src/runtime/commit/template_advance.rs
src/runtime/commit/terminal_advance.rs
src/runtime/commit/token_lookup.rs
src/runtime/commit/tokenizer_scan.rs
src/runtime/commit/types.rs
```

Each file corresponds to one part of the Commit relation. The top-level `mod.rs` intentionally contains no transition implementation.
