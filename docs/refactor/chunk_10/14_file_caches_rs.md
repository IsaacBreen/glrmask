...
The file is still large, but its subject is now narrow: rebuild derived runtime caches from semantic artifact fields.  This is an improvement over mixing cache rebuild, public API, and mask materialization in `constraint.rs`.
