# `compiled.rs` deep dive

## Summary

The file contains the current public storage object and the compile handoff struct.  Its central decision is to keep `Constraint` stable while creating a boundary that would allow a later `Arc<CompiledArtifact>` owner split.
