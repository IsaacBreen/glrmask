...
impl Pool {
    ...
    fn intern(&mut self, r: Vec<Run>) -> u32 {
    ...
    fn emit(out: &mut Vec<Run>, lo: u32, hi: u32, b: u32) {
        if b == 0 || lo > hi {
            return;
        }
        ...
        while i < self.values[a as usize].len() && j < self.values[b as usize].len() {
            ...
            if lo <= hi {
                let bid = if x.b == y.b {
                    ...
                    let mut z = [0; WORDS];
                    for k in 0..WORDS {
                        z[k] = self.bits[x.b as usize][k] & self.bits[y.b as usize][k]
                    }
                    self.intern_bits(z)
                };
                Self::emit(&mut out, lo, hi, bid)
    ...
    fn join(&mut self, a: u32, b: u32) -> u32 {
        ...
        for band in endpoints.windows(2) {
            ...
            let bid = if x == y || y == 0 {
                ...
                let mut z = [0; WORDS];
                for k in 0..WORDS {
                    z[k] = self.bits[x as usize][k] | self.bits[y as usize][k]
                }
                self.intern_bits(z)
            };
...
fn closure(states: &[State], p: &mut Pool, seeds: Vec<(u32, u32)>) -> Vec<(u32, u32)> {
    ...
    let mut pairs = seen.into_iter().collect::<Vec<_>>();
    pairs.sort_unstable();
    pairs
}
#[cfg(test)]
...
fn run(nwa: &NWA, minimize: bool) -> Option<(DWA, Profile)> {
    ...
    let mut prof = Profile {
        import_ms: t.elapsed().as_secs_f64() * 1000.,
        ..Default::default()
    };
    let t = Instant::now();
    let start = closure(
        &states,
        &mut p,
        nwa.start_states().iter().map(|&s| (s, 1)).collect(),
    );
    let mut subsets = vec![start.clone()];
    let mut ids = FxHashMap::default();
    ids.insert(start, 0u32);
    let mut out = Vec::<(u32, Vec<(i32, u32, u32)>)>::new();
    let mut cursor = 0;
    ...
    prof.runs = p.values.iter().map(Vec::len).sum();
...
type Graph = Vec<(u32, Vec<(i32, u32, u32)>)>;
#[derive(Default)]
struct Regions {
    values: Vec<(u32, Vec<(i32, u32)>)>,
    ids: FxHashMap<Vec<(i32, u32)>, u32>,
    merges: FxHashMap<(u32, u32), Option<u32>>,
}
impl Regions {
    fn intern(&mut self, profile: Vec<(i32, u32)>, p: &mut Pool) -> u32 {
    ...
    fn merge(&mut self, a: u32, b: u32, p: &mut Pool) -> Option<u32> {
        ...
        while i < self.values[a as usize].1.len() || j < self.values[b as usize].1.len() {
            ...
            let bits = if ab == bb || bb == 0 {
                ...
                let mut z = [0; WORDS];
                for k in 0..WORDS {
                    z[k] = p.bits[ab as usize][k] | p.bits[bb as usize][k]
                }
                p.intern_bits(z)
            };
            out.push((l, bits));
...
fn minimize_sparse_graph(mut graph: Graph, p: &mut Pool) -> Option<(Graph, u32)> {
    ...
    for (h, candidates) in levels.iter().enumerate() {
        ...
        if h == 0 {
            classes.push(candidates.clone())
        } else if small {
            classes = candidates.iter().map(|&s| vec![s]).collect()
        } else {
            let mut hashes = FxHashMap::<u64, Vec<usize>>::default();
            for &s in candidates {
            ...
            for bucket in hashes.values() {
                let mut subs = Vec::<Vec<usize>>::new();
        ...
        for members in classes {
            ...
            let profile = if aligned {
                ...
                let mut byrow = FxHashMap::<u64, Vec<(i32, u32)>>::default();
            ...
            'groups: for (i, g) in groups.iter().enumerate() {
                if !targets
                    .iter()
                    .all(|(l, d)| g.targets.get(l).is_none_or(|old| old == d))
                {
                    continue;
                }
                merged.clear();
            ...
            if let Some(i) = selected {
                ...
                g.targets.extend(targets);
                g.behavior.extend(merged);
            } else {
                if slots.is_some_and(|n| n.saturating_mul(groups.len() + 1) > 2_000_000) {
                    return None;
                }
                groups.push(Group {
                    members,
        ...
        for g in groups {
            ...
            let mut fs = Vec::new();
            let mut weights = BTreeMap::<i32, Vec<Run>>::new();
            for (row, reg) in rows {
            ...
            let es = weights
                .into_iter()
                .map(|(l, rs)| (l, g.targets[&l], p.intern(rs)))
                .collect();
            output.push((fw, es));
        }
        if profile {
...
mod tests {
    ...
    fn region_overlay_matches_exact_bitwise_partial_functions() {
        ...
        for case in 0..512 {
            ...
            let a = side();
            let b = side();
            let ai = r.intern(a.clone(), &mut p);
            let bi = r.intern(b.clone(), &mut p);
    ...
    fn failure_budgets_do_not_publish_partial_output() {
        ...
        let id = p.intern(vec![Run { lo: 0, hi: 0, b: 0 }]);
        assert_eq!(id, 0);
        assert!(p.failed);
