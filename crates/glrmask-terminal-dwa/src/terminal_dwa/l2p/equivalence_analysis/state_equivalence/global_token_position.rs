...
pub struct GlobalTokenPositionEquivalenceProfile {
    pub tail_start_position: usize,
    pub position_byte_counts: Vec<usize>,
    pub position_active_state_counts: Vec<usize>,
    pub position_class_counts: Vec<usize>,
    pub tail_byte_count: usize,
    pub tail_active_state_count: usize,
...
#[derive(Debug, Clone)]
pub struct GlobalTokenBoundaryStatePartition {
    map: ManyToOneIdMap,
}
...
#[derive(Debug, Clone)]
pub struct GlobalTokenPositionStatePartition {
    map: ManyToOneIdMap,
}
...
fn nfa_partial_position_partition_and_advance(
    view: &mut NfaTokenPositionView<'_>,
    position: usize,
    active_states: &[bool],
    bytes: &[u8],
...
/// inside the token.
pub fn compute_global_token_boundary_state_partition(
    tokenizer: &Tokenizer,
    vocab: &Vocab,
...
fn compute_global_token_position_map_with_tail_start_and_flat_trans(
    ...
    for (index, bytes) in byte_sets.position_bytes.iter().enumerate() {
        ...
        if let Some(view) = nfa_view.as_mut() {
            let (partition, next_active_states) = nfa_partial_position_partition_and_advance(
                view,
                index,
                &active_states,
                bytes,
            ...
            partition_ms.push(partition_started_at.map_or(0.0, |started_at| {
                started_at.elapsed().as_secs_f64() * 1000.0
            }));
            advance_ms.push(0.0);
        } else {
            ...
            advance_ms.push(advance_started_at.map_or(0.0, |started_at| {
                started_at.elapsed().as_secs_f64() * 1000.0
            }));
        }
    }
...
pub fn compute_global_token_position_state_partition(
    tokenizer: &Tokenizer,
    vocab: &Vocab,
...
pub fn compute_global_token_position_state_views(
    tokenizer: &Tokenizer,
    vocab: &Vocab,
    flat_trans: Option<&[u32]>,
) -> Option<(
...
pub fn compute_global_token_position_state_quotient(
    tokenizer: &Tokenizer,
    vocab: &Vocab,
...
mod tests {
    ...
    #[test]
    fn fused_epsilon_position_observation_matches_separate_partition_and_advance() {
        let tokenizer = arbitrary_epsilon_l1_test_tokenizer();
        ...
        for (position, bytes) in [b"ab".as_slice(), b"a".as_slice()].into_iter().enumerate() {
            let separate_partition =
                nfa_partial_position_partition(&mut separate_view, position, &active, bytes);
            let separate_next = nfa_advance_active_states(&mut separate_view, &active, bytes);
