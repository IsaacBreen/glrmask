...
#[derive(Clone)]
pub struct NodesByTokenizerState {
    pub entries: FxHashMap<TokenizerState, Vec<NwaState>>,
}
...
pub struct TerminalNwaBuilder<'tok, 'pm, 'nwa> {
    ...
    deferred_uncolored_future_leaf_buffer: FxHashMap<(u32, TokenizerState), LeafTokenIds>,
...
impl<'tok, 'pm, 'nwa> TerminalNwaBuilder<'tok, 'pm, 'nwa> {
    pub fn new(
        ...
        Self {
            ...
            deferred_uncolored_future_leaf_buffer: FxHashMap::default(),
            defer_uncolored_future_leaves: std::env::var("GLRMASK_L2P_DEFER_UNCOLORED_FUTURE_LEAVES")
                .map(|value| {
        ...
        if !self.use_terminal_coloring {
            if self.defer_uncolored_future_leaves {
                ...
                for &source in sources {
                    self.deferred_uncolored_future_leaf_buffer
                        .entry((source, tokenizer_state))
                        .or_default()
                        .push(internal_token_id);
                }
                return;
    ...
    pub fn flush_transition_buffer(&mut self) {
        ...
        if !deferred_uncolored.is_empty() {
            let mut terminal_cache = FxHashMap::<TokenizerState, Vec<TerminalID>>::default();
            for &(_, tokenizer_state) in deferred_uncolored.keys() {
                terminal_cache
                    .entry(tokenizer_state)
                    .or_insert_with(|| self.possible_future_terminals_for_state(tokenizer_state));
            }

            let mut deferred_groups = 0usize;
            let mut deferred_token_ids = 0usize;
            let mut expanded_labels = 0usize;
            for ((source, tokenizer_state), token_ids) in deferred_uncolored {
                if token_ids.is_empty() {
                    continue;
                }
                deferred_groups += 1;
                deferred_token_ids += token_ids.len();
            ...
            if std::env::var_os("GLRMASK_PROFILE_NWA_DEFERRED_FUTURE").is_some() {
                eprintln!(
                    "[glrmask/profile][nwa_deferred_uncolored_future] groups={} token_ids={} terminal_states={} expanded_labels={}",
                    deferred_groups,
                    deferred_token_ids,
                    terminal_cache.len(),
                    expanded_labels,
...
pub fn internal_vocab_entries(vocab: &Vocab, id_map: &InternalIdMap) -> Vec<(u32, Vec<u8>)> {
    id_map
        .vocab_tokens
...
pub enum TransportScannerStateMap {
...
pub struct DirectMacroQuotientParts<'a> {
    pub macro_class_for_source_class: &'a [u32],
    pub representative_for_macro_class: &'a [TokenizerState],
    pub source_macro_for_target_deviations: &'a [(u32, u32)],
}

impl TransportScannerStateMap {
    #[inline]
    pub fn direct_macro_quotient_parts(&self) -> Option<DirectMacroQuotientParts<'_>> {
        match self {
            Self::MacroQuotient {
    ...
    #[inline]
    pub fn scanner_state(&self, original_state: TokenizerState) -> TokenizerState {
        match self {
    ...
    pub fn materialized(&self) -> Arc<[TokenizerState]> {
        match self {
    ...
    #[inline]
    pub fn innermost_source_domain_key(&self) -> usize {
    ...
    #[inline]
    pub fn innermost_source_class_count(&self) -> usize {
        match self.innermost_source_map() {
            Self::Explicit(states) => states.len(),
            Self::Quotient {
                representative_for_class,
    ...
    #[inline]
    pub fn innermost_source_class(&self, original_state: TokenizerState) -> usize {
        match self.innermost_source_map() {
    ...
    #[inline]
    pub fn innermost_class_for_original(&self) -> Option<&[u32]> {
        match self.innermost_source_map() {
    ...
    #[inline]
    pub fn innermost_source_representative(&self, class: usize) -> TokenizerState {
        match self.innermost_source_map() {
    ...
    pub fn append_atomic_transforms<'a>(
        &'a self,
    ...
    #[inline]
    pub fn quotient_deviations(&self) -> Option<&[(u32, u32)]> {
        match self {
            Self::Quotient {
                source_class_for_target_deviations,
                ..
    ...
    #[inline]
    pub fn direct_quotient_default_key(&self) -> Option<(usize, usize)> {
        match self {
            Self::Quotient {
                class_for_original,
                representative_for_class,
    ...
    pub fn make_explicit_mut(&mut self) -> &mut [TokenizerState] {
...
#[derive(Clone, Debug)]
pub struct TerminalNwaTransportMode {
    pub scanner_state_for_original: TransportScannerStateMap,
    member_reconstruction: Option<(TerminalID, TerminalID)>,
}

impl TerminalNwaTransportMode {
    pub fn ordinary(state_count: usize) -> Self {
        Self {
    ...
    #[inline]
    pub fn member_reconstruction(&self) -> Option<(TerminalID, TerminalID)> {
...
pub fn build_transport_nwa_via_trie_walk<'a>(
    tokenizer: &'a Tokenizer,
    ignore_terminal: Option<TerminalID>,
    nwa: &mut NWA,
    start_state: u32,
