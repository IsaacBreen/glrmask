...
fn build_finite_projected_vocab_only(
    ...
    let (prep_wall_ms, prep_cpu_ms) = prep_timer.elapsed();
    let aliases = finite_vocab.aliases.as_slice();
    let tokens = finite_vocab.tokens.as_slice();
    let trie = &finite_vocab.trie;

    let scan_timer = CpuTimer::start();
    let mut scanner = Scanner::new(input);
    let mut starts = BTreeMap::<u32, Vec<u32>>::new();
    if let Some(state_map) = input.initial_state_map {
        for (class, &representative) in state_map.representative_original_ids.iter().enumerate() {
    ...
    let (scan_wall_ms, scan_cpu_ms) = scan_timer.elapsed();
    let _ = (cache_hits, pair_visits, uniform_subtrees, uniform_tokens);

    let compact_timer = CpuTimer::start();
    ...
    let (compact_wall_ms, compact_cpu_ms) = compact_timer.elapsed();
...
fn build_binary_vocab_only(input: BuildInput<'_>) -> Option<L1VocabEquivResult> {
    ...
    let total_timer = CpuTimer::start();
    let setup_timer = CpuTimer::start();
    ...
    let (bytes, input_byte_representative) = quotient_input_bytes(input, &vocab_bytes);
    let (prep_wall_ms, prep_cpu_ms) = setup_timer.elapsed();
...
#[cfg(test)]
mod vocab_equiv_exactness_tests {
    use super::*;
    ...
    use crate::terminal_dwa::types::TerminalColoring;
    use crate::Vocab;

    fn make_grammar() -> AnalyzedGrammar {
    ...
    #[test]
    fn test_l1_projected_vocab_equiv_matches_full_production() {
        ...
        let flat = Arc::from(crate::terminal_dwa::l1::build_flat_transition_table(&tokenizer));
        ...
        let full_prod = build(input).expect("production build should succeed");
        // Run vocab-only build
        let vocab_only = build_projected_vocab_equivalence(input).expect("vocab-only build should succeed");

        let prod_map = &full_prod.id_map.vocab_tokens;
        let fast_map = &vocab_only.vocab_map;

        // Verify equivalence relation exactness:
        // For any pair of tokens (t1, t2), prod_map says they are in same class <=> fast_map says same class
        let num_tokens = vocab.len();
        for t1 in 0..num_tokens as u32 {
            for t2 in 0..num_tokens as u32 {
                let prod_eq = prod_map.original_to_internal[t1 as usize] == prod_map.original_to_internal[t2 as usize];
                let fast_eq = fast_map.original_to_internal[t1 as usize] == fast_map.original_to_internal[t2 as usize];
                assert_eq!(
                    prod_eq, fast_eq,
                    "Equivalence mismatch between production and vocab-only for tokens {} and {}",
                    t1, t2
                );
            }
        }

        println!("vocab_only: num_internal_ids={}, original_to_internal={:?}", vocab_only.vocab_map.num_internal_ids(), vocab_only.vocab_map.original_to_internal);
println!("prod_map: num_internal_ids={}, original_to_internal={:?}", prod_map.num_internal_ids(), prod_map.original_to_internal);
assert_eq!(vocab_only.vocab_map.num_internal_ids(), prod_map.num_internal_ids());
    }
}
