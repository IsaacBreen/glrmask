...
# Set `SCHEMA_TEST_DISABLE_SUFFIX_PRUNE=0` to explicitly opt in via ENABLE_SUFFIX_PRUNE=1.
...
test-js: ffi ## Compile the JavaScript grammar (verifies it compiles)
	...
	$(if $(filter 1,$(SCHEMA_TEST_DISABLE_SUFFIX_PRUNE)),DISABLE_SUFFIX_PRUNE=1,ENABLE_SUFFIX_PRUNE=1) \
