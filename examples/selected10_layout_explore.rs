...
fn prepare(cache_dir: &Path, cfa_root: &Path, vocab: &Vocab, rebuild: bool) {
    ...
    for (index, (short_name, file)) in SELECTED10.iter().enumerate() {
        ...
        if rebuild || !path.exists() {
            ...
            let constraint = Constraint::from_json_schema(&source, vocab)
    ...
    if rebuild || !dispatch_path.exists() {
        ...
        let dispatch = if layout == Layout::Current {
            ...
            let parent = Constraint::from_glrm_grammar(&dispatcher_literal_names_parent_source(), vocab).unwrap();
            ...
            for index in 0..SELECTED10.len() {
                let parent = Constraint::from_glrm_grammar(&wrapped_schema_parent_source(index, layout), vocab).unwrap();
                wrapped.push(parent.compose_compiled_subgrammars(&[("TOOL_ARGS_SLOT", &schemas[index])], vocab).unwrap());
            }
            let dispatcher_parent = Constraint::from_glrm_grammar(&dispatcher_whole_child_parent_source(), vocab).unwrap();
            let bindings = (0..SELECTED10.len())
    ...
    if rebuild || !core_path.exists() {
        let started = Instant::now();
        let core = Constraint::from_glrm_grammar(&js_core_source(cfa_root, layout), vocab).unwrap();
