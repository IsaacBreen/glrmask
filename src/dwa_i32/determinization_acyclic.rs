...
fn build_destination_for_label(
    ...
    if edge_weight.is_empty() || dest_map.is_empty() {
        return None;
    }
...
pub(crate) fn determinize_acyclic_with_progress(
    ...
    timeit!("acyclic_det::materialize_weighted_closures", {
        ...
        for &cid in &dwa_topo {
            if weighted_closures[cid].is_empty() {
                continue;
            }

            dest_cache[cid].clear();
            let closure_snapshot = weighted_closures[cid].clone();
            for &(lbl, dest_id) in &transition_cache[cid] {
                ...
                dest_cache[cid].push((lbl, dest_id, w_edge));
            }
        }
    });
