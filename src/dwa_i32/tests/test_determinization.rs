...
fn test_determinize_simple_divergence() {
    ...
    nwa.add_transition(s0, 'a' as Label, s1, full_weight()).unwrap();
    nwa.add_transition(s1, 'c' as Label, s2, full_weight()).unwrap();
    ...
    nwa.add_transition(s3, 'b' as Label, s4, full_weight()).unwrap();
    nwa.add_transition(s4, 'c' as Label, s5, full_weight()).unwrap();
    nwa.states[s5].final_weight = Some(Weight::from_item(1));

    let start = nwa.states.add_state();
    nwa.add_epsilon(start, s0, full_weight());
    nwa.add_epsilon(start, s3, full_weight());
...
fn test_determinize_hypercube_catastrophe() {
    ...
    for i in 0..N {
        ...
        for j in 0..N { if i != j { nwa.add_transition(s, alphabet[j], s, full_weight()).unwrap(); } }
    }
    let start = nwa.states.add_state();
    for &s_comp in &component_starts { nwa.add_epsilon(start, s_comp, full_weight()); }
    nwa.body.start_states = vec![start];
    let dwa = nwa.determinize();
...
fn test_epsilon_explosion_minimal() {
    ...
    for i in 0..N {
        ...
        nwa_labeled.add_transition(start_labeled, i as Label, intermediate, full_weight()).unwrap();
        // Pattern transition: intermediate --char--> final
        nwa_labeled.add_transition(intermediate, char_label, final_state, full_weight()).unwrap();
        // Final weight
    ...
    for i in 0..N {
        ...
        nwa_epsilon.add_epsilon(start_eps, intermediate, full_weight());
        // Pattern transition: intermediate --char--> final
        nwa_epsilon.add_transition(intermediate, char_label, final_state, full_weight()).unwrap();
        // Final weight
...
fn test_epsilon_explosion_diverging_patterns() {
    ...
    for i in 0..N {
        ...
        nwa_labeled.add_transition(start_labeled, i as Label, q_i, full_weight()).unwrap();
        nwa_labeled.add_transition(q_i, shared_char, q_i_a, full_weight()).unwrap();
        nwa_labeled.add_transition(q_i_a, (i + 100) as Label, f_i, full_weight()).unwrap();  // Unique second char
    ...
    for i in 0..N {
        ...
        nwa_epsilon.add_epsilon(start_eps, q_i, full_weight());
        nwa_epsilon.add_transition(q_i, shared_char, q_i_a, full_weight()).unwrap();
        nwa_epsilon.add_transition(q_i_a, (i + 100) as Label, f_i, full_weight()).unwrap();
...
fn test_epsilon_explosion_overlapping_alphabet() {
    ...
    for i in 0..N {
        ...
        nwa_labeled.add_transition(start_labeled, i as Label, prev, full_weight()).unwrap();
        
        for _ in 0..i {
            let next = nwa_labeled.states.add_state();
            nwa_labeled.add_transition(prev, char_a, next, full_weight()).unwrap();
            prev = next;
        }
        
        let final_state = nwa_labeled.states.add_state();
        nwa_labeled.add_transition(prev, char_a, final_state, full_weight()).unwrap();
        nwa_labeled.states[final_state].final_weight = Some(Weight::from_item(i));
    }
    ...
    for i in 0..N {
        ...
        nwa_epsilon.add_epsilon(start_eps, prev, full_weight());
        
        for _ in 0..i {
            let next = nwa_epsilon.states.add_state();
            nwa_epsilon.add_transition(prev, char_a, next, full_weight()).unwrap();
            prev = next;
        }
        
        let final_state = nwa_epsilon.states.add_state();
        nwa_epsilon.add_transition(prev, char_a, final_state, full_weight()).unwrap();
        nwa_epsilon.states[final_state].final_weight = Some(Weight::from_item(i));
    }
...
fn test_epsilon_explosion_shared_second_hop() {
    ...
    nwa_labeled.states[shared_state].final_weight = Some(full_weight());
    
    for i in 0..N {
        let first_hop = nwa_labeled.states.add_state();
        // Labeled transition: start --i--> first_hop
        nwa_labeled.add_transition(start_labeled, i as Label, first_hop, full_weight()).unwrap();
    ...
    nwa_epsilon.states[shared_state_eps].final_weight = Some(full_weight());
    
    for i in 0..N {
        let first_hop = nwa_epsilon.states.add_state();
        // EPSILON transition: start --eps--> first_hop
        nwa_epsilon.add_epsilon(start_eps, first_hop, full_weight());
        // All first_hops go to shared_state on 'x', but with DIFFERENT weights
...
fn test_epsilon_explosion_paths_through_shared() {
    ...
    let final_state = nwa_labeled.states.add_state();
    nwa_labeled.states[final_state].final_weight = Some(full_weight());
    ...
    for i in 0..N {
        ...
        nwa_labeled.add_transition(start_labeled, i as Label, fh, full_weight()).unwrap();
        first_hops.push(fh);
    }
    ...
    for i in 0..N-1 {
        ...
        nwa_labeled.add_transition(sh, 'f' as Label, final_state, full_weight()).unwrap();
    }
    ...
    nwa_epsilon.states[final_state_eps].final_weight = Some(full_weight());
    
    let mut first_hops_eps = vec![];
    for _ in 0..N {
        let fh = nwa_epsilon.states.add_state();
        nwa_epsilon.add_epsilon(start_eps, fh, full_weight());
    ...
    for i in 0..N-1 {
        ...
        nwa_epsilon.add_transition(sh, 'f' as Label, final_state_eps, full_weight()).unwrap();
    }
...
fn test_epsilon_explosion_many_sources_with_continuation() {
    ...
    nwa_labeled.add_transition(shared_target, 'X' as Label, after_shared, full_weight()).unwrap();
    nwa_labeled.states[after_shared].final_weight = Some(full_weight());
    ...
    nwa_epsilon.add_transition(shared_target_eps, 'X' as Label, after_shared_eps, full_weight()).unwrap();
    nwa_epsilon.states[after_shared_eps].final_weight = Some(full_weight());
...
fn test_epsilon_explosion_different_alphabets() {
    ...
    for i in 0..N {
        ...
        nwa_labeled.add_transition(start_labeled, i as Label, root, full_weight()).unwrap();
    ...
    for i in 0..N {
        ...
        nwa_epsilon.add_epsilon(start_eps, root, full_weight());
...
fn test_epsilon_explosion_json_like() {
    ...
    for root_idx in 0..NUM_ROOTS {
        let root = nwa_labeled.states.add_state();
        nwa_labeled.add_transition(start_labeled, root_idx as Label, root, full_weight()).unwrap();
