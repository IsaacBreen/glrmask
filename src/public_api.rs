...
impl<'a> ConstraintSpec<'a> {
    ...
    pub fn compile_dynamic(&self) -> Result<DynamicConstraint> {
        ...
        let children = prepare_compiled_children(
            children,
            self.vocab,
            SegmentedBoundaryBackend::DynamicTerminalTrie,
        )?;
    ...
    children
        ...
        .map(|(name, child)| {
            ...
            match boundary_backend {
                ...
                SegmentedBoundaryBackend::DynamicTerminalTrie => constraint
    ...
    T: IntoGrammarBinding<'a>,
{
    ...
    let mode = match boundary_backend {
        ...
        SegmentedBoundaryBackend::DynamicTerminalTrie => ChildCompileMode::Dynamic,
    };
...
mod tests {
    ...
    fn assert_dynamic_boundary(constraint: &RuntimeConstraint) {
        ...
        assert!(overlay.segmented_boundary_terminal_trie.is_some());
    }
    ...
    fn authoritative_ab_keeps_component_parser_dwas_unchanged_across_scoped_ignores() {
        ...
        for dynamic_boundary in [false, true] {
            let bound = if dynamic_boundary {
                ...
                parent.clone().bind_grammar("child", child.clone()).unwrap()
            };
            let overlay = bound
                .static_dynamic_overlay
                .as_ref()
            ...
            for constraint in [&bound, &loaded] {
                ...
                let mut expected = monolithic.start();
                assert_eq!(actual.mask(), expected.mask(), "dynamic_boundary={dynamic_boundary}");
                for token in [1, 0, 2, 3, 1, 4] {
                    ...
                    assert_eq!(
                        actual.mask(),
                        expected.mask(),
                        "after token {token}, dynamic_boundary={dynamic_boundary}",
                    );
                }
                ...
                for token in [5, 6, 7, 8] {
                    ...
                    assert_eq!(
                        ...
                        "fused token {token}, dynamic_boundary={dynamic_boundary}",
                    );
            ...
            for constraint in [&bound, &loaded] {
                ...
                for token in [0, 1] {
                    assert_eq!(
                        actual.mask(),
                        expected.mask(),
                        "before token {token}, dynamic_boundary={dynamic_boundary}",
                    );
                    actual.commit_token(token).unwrap();
                    expected.commit_token(token).unwrap();
                }
                ...
                assert_eq!(
                    actual_mask,
                    expected_mask,
                    "after child completion, dynamic_boundary={dynamic_boundary}",
                );
                assert_ne!(
                    actual_mask[0] & (1 << 4),
                    0,
                    "fused parent token !? must be allowed (dynamic_boundary={dynamic_boundary})",
                );
