...
/// Ultrafast, DAG-oriented exact merge:
/// - Builds a compact child->parents CSR with unique parents per child.
/// - Processes nodes bottom-up via Kahn: a node is ready once all its unique children are assigned representatives.
/// - Uses an order-independent fingerprint combining:
///     end-flag, (pop, LLM mask), child-representative id, and SIDs bitset (hash),
///   to map nodes into tiny candidate buckets; then confirms equality with a precise structural check.
/// - Rewrites only representative nodes' outgoing edges to point to representatives and coalesces duplicates by SID-OR.
/// - Updates roots, GC, and recomputes max depths.
pub fn merge_nodes_trie3_ultrafast(
    roots: &mut BTreeMap<TokenizerStateID, PrecomputeNode3Index>,
    trie3_god: &Trie3GodWrapper,
){
    crate::debug!(2, "Merging identical subtrees in precomputed trie 3 (ultrafast DAG pass)...");
    ...
    // Dense indexing
    let mut dense_of: HashMap<Trie2Index, usize> = HashMap::with_capacity(n.saturating_mul(2));
    for (i, node_idx) in all_nodes.iter().enumerate() {
        dense_of.insert(*node_idx, i);
    }

    // Phase A: Build unique-child counts per parent and unique-parents count per child.
    let mut unique_child_count: Vec<u32> = vec![0; n];        // number of unique children for each parent
    let mut parent_count_for_child: Vec<u32> = vec![0; n];    // number of unique parents for each child

    #[cfg(not(rustrover))]
    let it = tqdm!(all_nodes.iter(), desc = "Trie3 UltraMerge (Count)", total = all_nodes.len(), disable = !PROGRESS_BAR_ENABLED, leave = true);
    #[cfg(rustrover)]
    let it = all_nodes.iter();
    for (u_dense, node_idx) in it.enumerate() {
        ...
        for (_ek, dm) in g.children() {
            for (dst, _sids) in dm {
                if let Some(&v_dense) = dense_of.get(dst) {
                    if seen_children.insert(v_dense) {
                        unique_child_count[u_dense] = unique_child_count[u_dense].saturating_add(1);
                        parent_count_for_child[v_dense] = parent_count_for_child[v_dense].saturating_add(1);
                    }
                }
            }
        }
    ...
    // CSR for child -> parents adjacency
    let mut offsets: Vec<usize> = Vec::with_capacity(n + 1);
    offsets.push(0);
    for i in 0..n {
        offsets.push(offsets[i].saturating_add(parent_count_for_child[i] as usize));
    }
    let mut parents: Vec<u32> = vec![0; offsets[n]]; // parent indices in [0, n), stored as u32 for memory
    let mut cursor = offsets.clone();

    #[cfg(not(rustrover))]
    let it = tqdm!(all_nodes.iter(), desc = "Trie3 UltraMerge (CSR Fill)", total = all_nodes.len(), disable = !PROGRESS_BAR_ENABLED, leave = true);
    #[cfg(rustrover)]
    let it = all_nodes.iter();
    for (u_dense, node_idx) in it.enumerate() {
        let g = node_idx.read(trie3_god).expect("read");
        ...
        for (_ek, dm) in g.children() {
            for (dst, _sids) in dm {
                if let Some(&v_dense) = dense_of.get(dst) {
                    if seen_children.insert(v_dense) {
                        let pos = cursor[v_dense];
                        parents[pos] = u_dense as u32;
                        cursor[v_dense] = pos + 1;
                    }
                }
            }
        }
    ...
    // Helpers for hashing and equality
    #[inline]
    ...
    // Compute an order-independent fingerprint for a node whose children already have representatives.
    // The fingerprint includes: end flag, (pop, LLM mask contents), child representative id, and SIDs bitset contents.
    #[inline]
    fn node_fingerprint(
        u_dense: usize,
        all_nodes: &[PrecomputeNode3Index],
        trie3_god: &Trie3GodWrapper,
        dense_of: &HashMap<Trie2Index, usize>,
        rep_of: &[usize],
    ) -> u64 {
        let g = all_nodes[u_dense].read(trie3_god).expect("read");
        let mut acc: u64 = if g.value.end { 0xD6E8FEB86659FD93 } else { 0xA0761D6478BD642F };
        let mut edge_count: u64 = 0;

        for ((pop, llm_bv), dest_map) in g.children() {
            // Hash key (pop, llm_bv)
            let mut key_hasher = std::collections::hash_map::DefaultHasher::new();
            pop.hash(&mut key_hasher);
            llm_bv.hash(&mut key_hasher);
            let key_hash = key_hasher.finish();

            for (dst, sids) in dest_map {
                let v_dense = *dense_of.get(dst).expect("dense child");
                let rep_dense = rep_of[v_dense];

                let mut h = std::collections::hash_map::DefaultHasher::new();
                key_hash.hash(&mut h);
                rep_dense.hash(&mut h);
                sids.hash(&mut h); // relies on StateIDBV: Hash
                let comp = h.finish();
        ...
        acc ^= mix64(edge_count);
        acc
    }

    // Structural equality using child representatives:
    // Build canonical maps for both nodes:
    //   (pop, ptr(LLM mask), child_rep) -> union(StateIDBV)
    // and compare.
    #[inline]
    fn nodes_structurally_equal(
        ...
        let a = all_nodes[a_dense].read(trie3_god).expect("read");
        let b = all_nodes[b_dense].read(trie3_god).expect("read");
        if a.value.end != b.value.end {
            return false;
        }

        type Key = (usize, *const RangeSetBlaze<usize>, usize);
        let mut map_a: BTreeMap<Key, StateIDBV> = BTreeMap::new();
        for ((pop, llm_bv), dm) in a.children() {
            let llm_ptr = Arc::as_ptr(&llm_bv.inner);
            for (dst, sids) in dm {
                let v_dense = *dense_of.get(dst).expect("dense child a");
                let rep = rep_of[v_dense];
                let key = (*pop, llm_ptr, rep);
                map_a.entry(key)
                    .and_modify(|e| *e |= sids)
                    .or_insert_with(|| sids.clone());
            }
        }
        let mut map_b: BTreeMap<Key, StateIDBV> = BTreeMap::new();
        for ((pop, llm_bv), dm) in b.children() {
            let llm_ptr = Arc::as_ptr(&llm_bv.inner);
            for (dst, sids) in dm {
                let v_dense = *dense_of.get(dst).expect("dense child b");
                let rep = rep_of[v_dense];
                let key = (*pop, llm_ptr, rep);
                map_b.entry(key)
                    .and_modify(|e| *e |= sids)
    ...
    // Phase B: Kahn bottom-up: assign representatives using fingerprint + precise structural check
    let mut pending_unique_children: Vec<u32> = unique_child_count.clone();
    let mut queue: VecDeque<usize> = VecDeque::new();
    for i in 0..n {
        if pending_unique_children[i] == 0 {
            queue.push_back(i);
        }
    }
    let mut rep_of: Vec<usize> = vec![usize::MAX; n];
    ...
    let pb = {
        ...
        p.set_style(
            ProgressStyle::default_bar()
                .template("{spinner:.green} [Trie3 UltraMerge] [{elapsed_precise}] [{wide_bar:.cyan/blue}] {pos}/{len} ({eta})")
                .unwrap(),
        );
        if !PROGRESS_BAR_ENABLED {
    ...
    while let Some(u) = queue.pop_front() {
        // Children have reps assigned at this point.
        let fp = node_fingerprint(u, &all_nodes, trie3_god, &dense_of, &rep_of);

        let rep_chosen = if let Some(cands) = buckets.get_mut(&fp) {
            // Try to match any existing candidate with a precise structural equality.
            let mut found = None;
            for &cand in cands.iter() {
                if nodes_structurally_equal(u, cand, &all_nodes, trie3_god, &dense_of, &rep_of) {
                    found = Some(cand);
                    break;
                }
            }
            ...
            buckets.insert(fp, vec![u]);
            u
        };
        rep_of[u] = rep_chosen;

        // Ready to bubble to parents of u
        let start = offsets[u];
        let end = offsets[u + 1];
        for i in start..end {
            let p = parents[i] as usize;
            let c = pending_unique_children[p].saturating_sub(1);
            pending_unique_children[p] = c;
            if c == 0 {
                queue.push_back(p);
            }
        }
    ...
    if processed != n {
        // Not a DAG or some nodes didn't become ready. Be safe: do not rewrite anything.
        crate::debug!(1, "Ultrafast merge aborted (processed {}/{}). Graph may contain cycles; keeping original structure.", processed, n);
        return;
    }

    // Phase C: Rebuild children for representatives so that edges point to representatives and duplicates are coalesced.
    let mut num_reps = 0usize;
    #[cfg(not(rustrover))]
    let it = tqdm!(0..n, desc = "Trie3 UltraMerge (Rewrite Reps)", total = n, disable = !PROGRESS_BAR_ENABLED, leave = true);
    #[cfg(rustrover)]
    let it = 0..n;
    for i in it {
        if rep_of[i] != i {
        ...
        let mut w = all_nodes[i].write(trie3_god).expect("write");
        let old_children = std::mem::take(w.children_mut());
        let mut new_children: BTreeMap<(usize, LLMTokenBV), OrderedHashMap<PrecomputeNode3Index, StateIDBV>> = BTreeMap::new();
        ...
        for ((pop, llm_bv), dm) in old_children {
            union_bv |= &llm_bv;
            let entry = new_children.entry((pop, llm_bv.clone())).or_insert_with(OrderedHashMap::new);
            for (dst, sids) in dm {
                let v_dense = *dense_of.get(&dst).expect("dense dst");
                let rep_dense = rep_of[v_dense];
                let rep_idx = all_nodes[rep_dense];
                entry.entry(rep_idx)
                    .and_modify(|e| *e |= &sids)
                    .or_insert(sids);
            }
    ...
    // Update roots to point to representatives
    for root_idx in roots.values_mut() {
        let d = *dense_of.get(root_idx).expect("root dense");
        let r = rep_of[d];
        *root_idx = all_nodes[r];
    }
    ...
    crate::debug!(2, "Ultrafast DAG merge complete: {} -> {} nodes ({} merged).", n, num_reps, n.saturating_sub(num_reps));
}
