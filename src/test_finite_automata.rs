...
mod reproduction_tests {
    ...
    #[test]
    fn test_derivative_dfa_exponential_blowup() {
        use crate::choice;
        use crate::seq;
        ...
        for n in 6..=20 {
            println!("\n=== Testing Derivative DFA N={} identical lines ===", n);
            ...
            // Build DFA via derivatives
            let start = std::time::Instant::now();
            let dfa = expr_groups.build_dfa_via_derivatives();
            let elapsed = start.elapsed();
            let dfa_states = dfa.states.len();
            println!("  Derivative DFA states: {} in {:.2?}", dfa_states, elapsed);
            
            results.push((n, dfa_states, elapsed.as_millis()));
        }

        // Verify DFA doesn't explode exponentially
        // With derivatives, we expect polynomial (ideally linear or quadratic) growth
        let mut dfa_deltas = Vec::new();
        for i in 0..results.len() - 1 {
            let delta = results[i+1].1 as isize - results[i].1 as isize;
            dfa_deltas.push(delta);
        ...
        println!("\nDerivative DFA deltas: {:?}", dfa_deltas);
        println!("Results: {:?}", results);

        // Check that growth is not exponential (2^n)
        // The ratio of consecutive state counts should be bounded
        for i in 1..results.len() {
            let ratio = results[i].1 as f64 / results[i-1].1 as f64;
            println!("  N={} -> N={}: ratio = {:.2}", results[i-1].0, results[i].0, ratio);
            assert!(
                ratio < 1.5, // Should not be doubling with each increment
                "Derivative DFA shows exponential growth! Ratio {} at N={}", ratio, results[i].0
            );
        }
    }
