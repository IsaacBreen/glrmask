...
/// Precomputed DFA with flat transition tables for fast token execution.
...
/// Scratch space for position-0 DFA execution across multiple initial states.
struct Pos0Scratch {
...
/// Scratch space for suffix DAG construction and hashing.
...
impl Pos0Scratch {
    fn new(num_states: usize, num_groups: usize) -> Self {
        Pos0Scratch {
            ...
            match_gen: vec![0u32; num_states.saturating_mul(num_groups)],
            cur_gen: 1,
...
fn compute_pos0(
    ...
    // Reset target tracking from previous call
    ...
    // Process initial finalizers at each start state
    ...
    // Walk each byte of the token (hot path)
                    ...
                    if next_state != NONE_STATE {
                        ...
                        *current_states.get_unchecked_mut(i) = ns;
                        for f in pre.finalizers.get_unchecked(ns) {
                            if f.gid < num_groups {
                                let ix = base + f.gid;
                                let slot_gen = match_gen.get_unchecked_mut(ix);
                                let was_none = *slot_gen != cur_gen;
                                if !f.non_greedy || was_none {
                                    *slot_gen = cur_gen;
                                    *match_positions.get_unchecked_mut(ix) = position;
                                }
                                if was_none {
...
/// Run DFA on a suffix (from start_state on the remaining bytes after base_pos).
fn execute_suffix(
    pre: &PrecomputedDfa,
    slice: &[u8],
    base_pos: usize,
    ...
    // Initial finalizers (all slots start at NONE_POS so first write always succeeds)
    ...
    for (idx, &byte) in slice.iter().enumerate() {
        ...
        if pre.future_modes[current] == FutureMode::Terminate {
            done = true;
        }
    }
...
/// Build suffix DAG via BFS from match target positions and compute hashes bottom-up.
fn compute_suffix_hashes(
    ...
    while cursor < scratch.queue.len() {
        ...
        cursor += 1;
        let (end_state, edges) =
            execute_suffix(pre, &slice[pos..], pos, &mut scratch.match_positions);
...
/// Compute a signature for a token given a chunk of initial states.
fn compute_chunk_signature(
    ...
    for i in 0..chunk_states.len() {
        let completion = pos0.end_states[i]
            .map(|id| pre.completion_hash[id])
            .unwrap_or(pre.none_completion_hash);
...
/// Find vocab equivalence classes with optional follow-set parameters.
///
/// `suffix_group_mask`, `ever_allowed_by_group`, and `group_to_class` are accepted
/// for API compatibility but unused internally.
pub fn find_vocab_equivalence_classes_with_follow<S: AsRef<[u8]> + Sync>(
    ...
    for batch_start in (0..num_states).step_by(batch_size) {
        ...
        // Compute signatures for all active tokens in parallel
        let active_sigs: Vec<(usize, u64)> = active_indices
            ...
            .map_init(
                ...
                |state, &token_idx| {
                    let (pos0, suffix, cache) = state;
                    let token = strings[token_idx].as_ref();
                    if cache.len() <= token.len() {
                        cache.resize(token.len() + 1, None);
                    }
                    cache.iter_mut().for_each(|x| *x = None);
                    let sig =
                        compute_chunk_signature(&pre, token, batch, pos0, suffix, cache);
                    (token_idx, sig)
                },
            )
            .collect();

        // Refine partition by (old_class, new_signature)
