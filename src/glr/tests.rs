...
use crate::glr::grammar::{nt, prod, regex_name, t, Production, Symbol, Terminal};
...
fn create_simple_parser() -> GLRParser {
    ...
    generate_glr_parser(&productions, &HashSet::new(), HashSet::new())
}
...
fn test_repetition_no_eof_2() {
    ...
    let parser_with_b = generate_glr_parser(&productions_with_b, &HashSet::new(), HashSet::new());
...
fn create_expression_parser() -> GLRParser {
    ...
    //
    // This test uses a standard expression grammar with the chain E -> T -> F. It verifies
    // that the optimized parser (with unit production elimination now being the default)
    // still correctly accepts and rejects inputs.
    let productions = vec![
...
fn test_standard_expression_grammar_parse() {
    ...
    // println!("Grammar before minimization: {}", display_productions(&productions));
    // println!("Minimized grammar: {}", display_productions(&minimize_grammar(&productions, 0).0));
...
fn test_parser_stats_conflicts() {
    ...
    let rr_parser = generate_glr_parser(&rr_productions, &HashSet::new(), HashSet::new());
    ...
    let sr_parser = generate_glr_parser(&sr_productions, &HashSet::new(), HashSet::new());
...
fn test_unit_production_elimination() {
    ...
    let parser_elim = generate_glr_parser(&productions, &HashSet::new(), HashSet::new());
