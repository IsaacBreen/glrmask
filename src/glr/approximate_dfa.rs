...
fn build_nfa(parser: &GLRParser, num_states: usize, underneath_map: &[Bitset]) -> ApproximateParserNFA {
    ...
    // Fixpoint propagation: for each NFA edge S --T--> G added by a reduce action,
    // if G also has a transition on T (reduce→goto→shift or reduce→goto→reduce→...),
    // add those targets to S's transition for T.
    //
    // This is needed because LR parsing does not consume the lookahead terminal during
    // a reduce: the same terminal T is re-examined from the goto state G after reducing.
    // If G shifts T → H, then H is the actual parser state after T is consumed, not G.
    //
    // Without this fixpoint, the approx DFA would incorrectly believe the parser is in
    // the intermediate goto state (e.g. {16,17,28}) instead of the final shift state
    // (e.g. {18,20}) after processing a terminal like comma following a reduce.
    //
    // We only propagate through reduce-goto edges (not shift edges), because after a
    // shift the terminal is consumed and the process starts fresh.
    //
    // reduce_goto_edges[S][T] = set of goto states G reached by reduce actions from S on T.
    let mut reduce_goto_edges: Vec<BTreeMap<TerminalID, Bitset>> =
        vec![BTreeMap::new(); num_states];
    for (&state_id, row) in parser.table.iter() {
        for term_idx in 0..num_terminals {
            let terminal_id = TerminalID(term_idx);
            let action = row.get_shifts_and_reduces_for_terminal(&terminal_id);
            match action {
                Some(Stage7ShiftsAndReducesLookaheadValue::Reduce { nonterminal_id, len, .. }) => {
                    let below_states = compute_states_below(
                        state_id, len, num_states, underneath_map, &mut below_cache,
                    );
                    for below_state in below_states.iter() {
                        if let Some(&goto_state) = goto_map[below_state].get(&nonterminal_id) {
                            let entry = reduce_goto_edges[state_id.0]
                                .entry(terminal_id)
                                .or_insert_with(|| Bitset::new(num_states));
                            entry.insert(goto_state.0);
                        }
                    }
                }
                Some(Stage7ShiftsAndReducesLookaheadValue::Split { reduces, .. }) => {
                    for (len, nts) in reduces {
                        for (nt_id, _) in nts {
                            ...
                            for below_state in below_states.iter() {
                                if let Some(&goto_state) = goto_map[below_state].get(&nt_id) {
                                    let entry = reduce_goto_edges[state_id.0]
                                        .entry(terminal_id)
                                        .or_insert_with(|| Bitset::new(num_states));
                                    entry.insert(goto_state.0);
    ...
    // Fixpoint: for each reduce-goto edge S --T--> G, add transitions[G][T] to transitions[S][T].
    // This captures reduce→goto→shift chains (and reduce→goto→reduce→...→shift chains).
    let mut changed = true;
    while changed {
        changed = false;
        for s in 0..num_states {
            let terminal_keys: Vec<TerminalID> = reduce_goto_edges[s].keys().copied().collect();
            for terminal_id in terminal_keys {
                let goto_states: Vec<usize> = reduce_goto_edges[s][&terminal_id].iter().collect();
                for g in goto_states {
                    if let Some(g_targets) = transitions[g].get(&terminal_id) {
                        let new_targets: Vec<usize> = g_targets.iter().collect();
                        let s_entry = transitions[s]
                            .entry(terminal_id)
                            .or_insert_with(|| Bitset::new(num_states));
                        for h in new_targets {
                            if !s_entry.contains(h) {
                                s_entry.insert(h);
                                changed = true;
                            }
                        }
                    }
                    // Also propagate through further reduce-goto edges from g
                    if let Some(g_reduce_gotos) = reduce_goto_edges[g].get(&terminal_id) {
                        let new_targets: Vec<usize> = g_reduce_gotos.iter().collect();
                        for h in new_targets {
                            let rge_s = reduce_goto_edges[s]
                                .entry(terminal_id)
                                .or_insert_with(|| Bitset::new(num_states));
                            if !rge_s.contains(h) {
                                rge_s.insert(h);
