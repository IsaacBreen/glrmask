    ...
    for prod in productions {
        ...
        let non_nullable_symbols: Vec<&Symbol> = rhs
            ...
            .filter(|symbol| match symbol {
                ...
                Symbol::NonTerminal(nt) => nonterminal_nullability.get(nt) != Some(&Nullability::Nullable),
            })
            .collect();
                ...
                if nt == lhs { // Found potential left recursion: A ::= ... A ...
                    ...
                    if !prefix.is_empty() { // Only check if there's a prefix
                        let prefix_is_nullable = prefix.iter().all(|sym| match sym {
                            Symbol::NonTerminal(prefix_nt) => nonterminal_nullability.get(prefix_nt) == Some(&Nullability::Nullable),
...
pub fn compute_terminal_follow_sets(productions: &[Production]) -> BTreeMap<Terminal, BTreeSet<Terminal>> {
    ...
    let nonterminal_nullability = compute_nonterminal_nullability(productions);
    let nonterminal_follow_sets = compute_follow_sets_for_nonterminals(productions, &first_sets, &nonterminal_nullability);
                ...
                for next_symbol in &rhs[i + 1..] {
                    match next_symbol {
                        ...
                        Symbol::NonTerminal(next_nt) => {
                            ...
                            if nonterminal_nullability.get(next_nt) != Some(&Nullability::Nullable) {
                                all_following_are_nullable = false;
                                break;
                            }
                        }
