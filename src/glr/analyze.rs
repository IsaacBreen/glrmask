...
/// Inlines all nullable productions.
///
/// This function iteratively finds nullable non-terminals and rewrites rules that use them,
/// creating variants of those rules where the nullable non-terminal is omitted.
/// This process is repeated until no more nullable non-terminals (and thus no epsilon-productions) exist.
/// For example, a rule `A -> X N Y` where `N` is nullable will result in two rules:
/// `A -> X N Y` and `A -> X Y`. After all transformations, the rule `N -> ε` is removed.
pub fn inline_nullable_productions(productions: &[Production]) -> Vec<Production> {
    let mut current_prods = productions.to_vec();
    loop {
        let nullable = compute_nullable_nonterminals(&current_prods);
        if nullable.is_empty() {
            break;
        }

        let mut next_prods_set = BTreeSet::new();

        for p in &current_prods {
            // Epsilon productions are the source of nullability; they are removed by not being re-added.
            if p.rhs.is_empty() {
                continue;
            }

            let nullable_indices: Vec<usize> = p.rhs.iter().enumerate()
            ...
            if nullable_indices.is_empty() {
                next_prods_set.insert(p.clone());
                continue;
            }

            let num_nullable = nullable_indices.len();
            // For a rule with `k` nullable symbols, generate `2^k` variants by considering all subsets of those symbols to omit.
            for i in 0..(1 << num_nullable) {
                let mut to_omit = BTreeSet::new();
                for (k, &nullable_idx) in nullable_indices.iter().enumerate() {
                    if (i >> k) & 1 == 1 {
                        to_omit.insert(nullable_idx);
                    }
                }

                let new_rhs: Vec<Symbol> = p.rhs.iter().enumerate()
                    .filter(|(idx, _)| !to_omit.contains(idx))
                    .map(|(_, symbol)| symbol.clone())
                    .collect();

                // The expansion might result in an empty RHS. We don't add it back,
                // effectively removing the now-inlined epsilon production.
                if !new_rhs.is_empty() {
                    next_prods_set.insert(Production {
                        lhs: p.lhs.clone(),
                        rhs: new_rhs,
                    });
                }
        ...
        let next_prods_vec: Vec<_> = next_prods_set.into_iter().collect();
        
        // Check for a fixed point.
        let current_prods_set: BTreeSet<_> = current_prods.iter().cloned().collect();
        if current_prods_set == next_prods_vec.iter().cloned().collect() {
            current_prods = next_prods_vec;
            break;
        }
...
/// Inlines all unit productions (rules of the form `A -> B`).
///
/// This is done by first computing the transitive closure of the unit-production relation.
/// Then, for every rule `A -> alpha` where `alpha` is not a single non-terminal, a new rule
/// `B -> alpha` is added for every `B` that can derive `A` through a chain of unit productions.
/// This process eliminates the need for unit-reduction steps during parsing.
/// It correctly handles cycles (e.g., `A -> B`, `B -> A`).
pub fn inline_unit_productions(productions: &[Production]) -> Vec<Production> {
    let mut unit_production_map = BTreeMap::<NonTerminal, BTreeSet<NonTerminal>>::new();
    let mut non_unit_productions = Vec::new();
    ...
    for p in productions {
        ...
        if p.rhs.len() == 1 {
            if let Symbol::NonTerminal(nt) = &p.rhs[0] {
                unit_production_map.entry(p.lhs.clone()).or_default().insert(nt.clone());
                continue;
            }
        }
    ...
    if unit_production_map.is_empty() {
        return productions.to_vec();
    }

    // For each non-terminal `A`, compute the set of non-terminals `B` such that `A ->* B`
    // via one or more unit productions, plus `A` itself (reflexive transitive closure).
    let mut tc = BTreeMap::new();
    for nt in &all_nonterminals {
        let mut derives = BTreeSet::new();
        let mut worklist = VecDeque::from([nt.clone()]);
        let mut visited_for_this_start_nt = BTreeSet::new();

        while let Some(current_nt) = worklist.pop_front() {
            if !visited_for_this_start_nt.insert(current_nt.clone()) {
                continue;
            }
            derives.insert(current_nt.clone());
            if let Some(direct_derives) = unit_production_map.get(&current_nt) {
                for derived_nt in direct_derives {
                    worklist.push_back(derived_nt.clone());
                }
            }
        }
        tc.insert(nt.clone(), derives);
    }

    let mut final_productions = BTreeSet::new();
    for (a, bs) in &tc { // For each non-terminal `a`
        for b in bs { // For each non-terminal `b` that `a` can derive via unit productions
            for p in &non_unit_productions {
                if p.lhs == *b { // If `b` has a non-unit production `b -> alpha`
                    // Add a new production `a -> alpha`
    ...
    final_productions.into_iter().collect()
}
