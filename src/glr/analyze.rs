...
use crate::glr::grammar::{NonTerminal, Production, Symbol, Terminal, compute_first_sets};
...
/// Generates a unique non-terminal name by appending primes (').
fn generate_unique_nt_name(base: &str, all_nts: &mut BTreeSet<NonTerminal>) -> NonTerminal {
    let mut new_name_str = format!("{}'", base);
    while all_nts.contains(&NonTerminal(new_name_str.clone())) {
        new_name_str.push('\'');
    }
    let new_nt = NonTerminal(new_name_str);
    all_nts.insert(new_nt.clone());
    new_nt
}

/// Finds the first non-nullable symbol in a sequence.
/// Returns the nullable prefix, the first non-nullable symbol, and the rest of the sequence.
fn find_first_non_nullable<'a>(
    rhs: &'a [Symbol],
    nullable_set: &BTreeSet<NonTerminal>,
) -> (&'a [Symbol], Option<&'a Symbol>, &'a [Symbol]) {
    for (i, symbol) in rhs.iter().enumerate() {
        ...
        if !is_nullable {
            return (&rhs[0..i], Some(&rhs[i]), &rhs[i + 1..]);
        }
    }
    (rhs, None, &[]) // All symbols are nullable
}


/// Resolves both direct and indirect left-recursion from a set of productions.
/// This includes "hidden" left-recursion where a production starts with a sequence of
...
pub fn resolve_left_recursion(productions: &[Production]) -> Vec<Production> {
    ...
    let ordered_nts: Vec<_> = all_nts.iter().cloned().collect();
    let mut new_prods = productions.to_vec();

    for i in 0..ordered_nts.len() {
        let ni = &ordered_nts[i];

        // Substitution loop to make indirect recursion direct
        for j in 0..i {
            let nj = &ordered_nts[j];
            let nullable_set = compute_nullable_nonterminals(&new_prods);

            let mut prods_after_subst = Vec::new();
            let prods_for_nj: Vec<_> = new_prods.iter().filter(|p| p.lhs == *nj).cloned().collect();

            for p in &new_prods {
                if p.lhs == *ni {
                    let (prefix, first_non_nullable, suffix) = find_first_non_nullable(&p.rhs, &nullable_set);
                    if let Some(Symbol::NonTerminal(nt)) = first_non_nullable {
                        if nt == nj {
                            // Substitute: Ni -> P Nj γ becomes Ni -> P δ γ
                            for nj_prod in &prods_for_nj {
                                let mut new_rhs = prefix.to_vec();
                                new_rhs.extend(nj_prod.rhs.clone());
                                new_rhs.extend_from_slice(suffix);
                                prods_after_subst.push(Production { lhs: ni.clone(), rhs: new_rhs });
                            }
                        } else {
                            prods_after_subst.push(p.clone());
                        }
                    } else {
                        prods_after_subst.push(p.clone());
                    }
            ...
            new_prods = prods_after_subst;
        }

        // Eliminate direct (and hidden direct) left recursion for Ni
        let nullable_set = compute_nullable_nonterminals(&new_prods);
        let mut recursive = Vec::new();
        let mut non_recursive = Vec::new();

        for p in new_prods.iter().filter(|p| p.lhs == *ni) {
            let (_prefix, first_non_nullable, _suffix) = find_first_non_nullable(&p.rhs, &nullable_set);
            if let Some(Symbol::NonTerminal(nt)) = first_non_nullable {
                if nt == ni {
                    recursive.push(p.clone());
                } else {
                    non_recursive.push(p.clone());
                }
            } else {
                non_recursive.push(p.clone());
            }
        ...
        if !recursive.is_empty() {
            let ni_prime = generate_unique_nt_name(&ni.0, &mut all_nts);

            let mut updated_prods_for_ni = Vec::new();
            for p in &non_recursive {
                let mut new_rhs = p.rhs.clone();
                new_rhs.push(Symbol::NonTerminal(ni_prime.clone()));
                updated_prods_for_ni.push(Production { lhs: ni.clone(), rhs: new_rhs });
            }

            let mut prods_for_ni_prime = Vec::new();
            for p in &recursive {
                let (prefix, _, suffix) = find_first_non_nullable(&p.rhs, &nullable_set);
                let mut alpha = prefix.to_vec();
                alpha.extend_from_slice(suffix);
                alpha.push(Symbol::NonTerminal(ni_prime.clone()));
                prods_for_ni_prime.push(Production { lhs: ni_prime.clone(), rhs: alpha });
            }
            prods_for_ni_prime.push(Production { lhs: ni_prime.clone(), rhs: vec![] });

            new_prods.retain(|p| p.lhs != *ni);
            new_prods.extend(updated_prods_for_ni);
            new_prods.extend(prods_for_ni_prime);
        }
    }
    new_prods
}
