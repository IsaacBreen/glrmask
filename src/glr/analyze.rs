...
/// Represents a suggestion for creating a synthetic terminal to optimize parsing.
#[derive(Debug, Clone)]
pub struct SyntheticTerminalSuggestion {
    /// The set of terminals that frequently co-occur and are candidates for grouping.
    pub terminals: BTreeSet<Terminal>,
    /// An estimated "gain" from implementing this suggestion. Higher is better.
    /// The gain is calculated as `(number of terminals in set - 1) * number of non-terminals affected`.
    pub gain: f64,
    /// The set of non-terminals whose `FOLLOW` sets contain the suggested terminal set.
    pub non_terminals: BTreeSet<NonTerminal>,
}

/// Analyzes a grammar to find the best candidate set of terminals to group into a synthetic terminal.
///
/// This function computes the `FOLLOW` sets for all non-terminals and looks for pairs of terminals
/// that co-occur most frequently across these sets. Such pairs are good candidates for optimization
/// because they increase lookahead complexity during parsing.
///
/// # Arguments
/// * `productions` - The list of productions in the grammar.
///
/// # Returns
/// An `Option<SyntheticTerminalSuggestion>` containing the best suggestion, or `None` if no
/// suitable pair (co-occurring in at least one `FOLLOW` set) is found.
pub fn analyze_for_synthetic_terminal(productions: &[Production]) -> Option<SyntheticTerminalSuggestion> {
    ...
    let mut follow_terminal_sets: BTreeMap<BTreeSet<Terminal>, Vec<NonTerminal>> = BTreeMap::new();
    for (nt, follows) in follow_sets {
        let terminal_set: BTreeSet<Terminal> = follows.into_iter().flatten().collect();
        if terminal_set.len() > 1 {
            follow_terminal_sets.entry(terminal_set).or_default().push(nt);
        }
    }

    let mut pair_counts: BTreeMap<(Terminal, Terminal), usize> = BTreeMap::new();
    for (terminal_set, nts) in &follow_terminal_sets {
        let terminals: Vec<_> = terminal_set.iter().cloned().collect();
        for i in 0..terminals.len() {
            for j in i + 1..terminals.len() {
                let t1 = terminals[i].clone();
                let t2 = terminals[j].clone();
                let pair = if t1 < t2 { (t1, t2) } else { (t2, t1) };
                *pair_counts.entry(pair).or_insert(0) += nts.len();
            }
        }
    }

    if let Some(((t1, t2), _count)) = pair_counts.into_iter().max_by_key(|(_, count)| *count) {
        let terminals = BTreeSet::from([t1, t2]);
        let mut affected_nts = BTreeSet::new();
        let mut gain = 0.0;
        for (terminal_set, nts) in &follow_terminal_sets {
            if terminal_set.is_superset(&terminals) {
                affected_nts.extend(nts.iter().cloned());
                gain += (terminals.len() - 1) as f64 * nts.len() as f64;
            }
        }
        Some(SyntheticTerminalSuggestion {
            terminals,
            gain,
            non_terminals: affected_nts,
        })
    } else {
        None
    }
