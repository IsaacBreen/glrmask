...
///
/// Intuition:
/// - If x is in this set for terminal t, it means that whenever t appears in the
///   grammar, x can immediately follow it in every possible context.
/// - This lets downstream consumers "shortcut" t-edges when they are followed by
///   x (e.g. replace t by None and let a subsequent pass shortcut the edge).
pub fn compute_always_allowed_terminal_follows(
    ...
    // We keep an Option to accumulate intersections across occurrences.
    ...
    for production in productions {
        ...
        for (i, symbol) in rhs.iter().enumerate() {
            if let Symbol::Terminal(t) = symbol {
                ...
                // Intersect with the running set for t.
                always_map
                    .entry(t.clone())
                    .and_modify(|maybe_set| {
                        match maybe_set {
                            Some(existing) => {
                                let inter: BTreeSet<_> = existing.intersection(&occ_follows).cloned().collect();
                                *existing = inter;
                            }
    ...
    // Convert Option<_> to concrete sets (skip terminals with no occurrences).
