...
pub fn generate_glr_parser_with_maps(productions: &[Production], terminal_map: BiBTreeMap<Terminal, TerminalID>, mut non_terminal_map: BiBTreeMap<NonTerminal, NonTerminalID>, actions: BTreeMap<NonTerminal, crate::glr::parser::ActionFn>, ignore_terminal_id: Option<TerminalID>) -> crate::glr::parser::GLRParser {
    ...
    let hallucinate_masks = compute_hallucinate_masks(&stage_8_table);
    ...
    crate::glr::parser::GLRParser::new(final_table, productions, terminal_map, non_terminal_map, item_set_map, start_state_id, everything_state_id, actions, ignore_terminal_id, hallucinate_masks)
}
...
pub struct HallucinateMasks {
    pub shift_sources: BTreeMap<TerminalID, TerminalBV>, // Terminal -> bitset of source state IDs
    pub reduce_sources: BTreeMap<TerminalID, BTreeMap<usize, BTreeMap<NonTerminalID, TerminalBV>>>, // Terminal -> len -> nt -> bitset
    pub goto_sources: BTreeMap<NonTerminalID, TerminalBV>, // NonTerminal -> bitset of source state IDs having goto(nt)
}

pub fn compute_hallucinate_masks(table: &Table) -> HallucinateMasks {
    let mut masks = HallucinateMasks::default();

    for (src_state_id, row) in table {
        // Shifts (and Splits that include a shift)
        for (tid, action) in &row.shifts_and_reduces_full {
            let has_shift = match action {
                Stage7ShiftsAndReducesLookaheadValue::Shift(_) => true,
                Stage7ShiftsAndReducesLookaheadValue::Split { shift, .. } => shift.is_some(),
                _ => false,
            };
            if has_shift {
                masks.shift_sources.entry(*tid).or_default().insert(src_state_id.0);
            }
            // Reduces (including those inside Splits)
            match action {
                Stage7ShiftsAndReducesLookaheadValue::Reduce { nonterminal_id, len, .. } => {
                    masks.reduce_sources
                        .entry(*tid).or_default()
                        .entry(*len).or_default()
                        .entry(*nonterminal_id).or_default()
                        .insert(src_state_id.0);
                }
                Stage7ShiftsAndReducesLookaheadValue::Split { reduces, .. } => {
                    for (len, nts) in reduces {
                        for (nt, _pids) in nts {
