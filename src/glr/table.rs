...
pub fn eliminate_unit_productions(
    table: &mut BTreeMap<StateID, Stage7Row>,
    item_set_map: &mut bimap::BiBTreeMap<BTreeSet<crate::glr::items::Item>, StateID>,
    productions: &[Production],
    non_terminal_map: &BiBTreeMap<NonTerminal, NonTerminalID>,
) -> usize {
    crate::debug!(2, "Eliminating unit productions...");
    let initial_state_count = table.len();

    // 1. Identify unit productions, leaves, and nodes.
    let mut unit_productions = BTreeSet::new();
    let mut lhs_nodes = BTreeSet::new();
    let mut rhs_symbols = BTreeSet::new();

    for (prod_id, prod) in productions.iter().enumerate() {
        if prod.rhs.len() == 1 {
            if let Symbol::NonTerminal(nt_rhs) = &prod.rhs[0] {
                unit_productions.insert(prod_id);
                lhs_nodes.insert(prod.lhs.clone());
                rhs_symbols.insert(Symbol::NonTerminal(nt_rhs.clone()));
            }
        }
    }

    let leaves: BTreeSet<NonTerminal> = rhs_symbols
        .iter()
        .filter_map(|s| match s {
            Symbol::NonTerminal(nt) if !lhs_nodes.contains(nt) => Some(nt.clone()),
            _ => None,
        })
        .collect();

    if leaves.is_empty() {
        crate::debug!(2, "No leaf non-terminals found, no unit productions to eliminate.");
        return 0;
    }
    crate::debug!(3, "Leaf non-terminals: {:?}", leaves.iter().map(|l| &l.0).collect::<Vec<_>>());

    // 2. Build the derivation map (e.g., E => {T, F, id}).
    let mut derives_map: BTreeMap<NonTerminal, BTreeSet<NonTerminal>> = BTreeMap::new();
    for nt in non_terminal_map.left_values() {
        derives_map.entry(nt.clone()).or_default().insert(nt.clone());
    }
    let mut changed = true;
    while changed {
        changed = false;
        for prod_id in &unit_productions {
            let prod = &productions[*prod_id];
            if let Symbol::NonTerminal(rhs_nt) = &prod.rhs[0] {
                let derived_by_rhs = derives_map.get(rhs_nt).cloned().unwrap_or_default();
                let lhs_derives = derives_map.entry(prod.lhs.clone()).or_default();
                let old_len = lhs_derives.len();
                lhs_derives.extend(derived_by_rhs);
    ...
    let mut combination_cache: BTreeMap<BTreeSet<StateID>, StateID> = BTreeMap::new();
    let mut next_state_id = StateID(table.keys().map(|s| s.0).max().unwrap_or(0) + 1);

    let mut modified = true;
    while modified {
        modified = false;
        let state_ids: Vec<_> = table.keys().cloned().collect();

        for state_id in state_ids {
            // We need to re-fetch the row in each iteration as the table is modified.
            if let Some(row) = table.get(&state_id).cloned() {
                for leaf in &leaves {
                    let leaf_nt_id = *non_terminal_map.get_by_left(leaf).unwrap();
                    if let Some(goto_action) = row.gotos.get(&leaf_nt_id) {
                        if let Some(target_state_id) = goto_action.state_id {
                            let target_row = &table[&target_state_id];
                            // Check if the target state has a unit reduction.
                            let has_unit_reduce = target_row.shifts_and_reduces_full.values().any(|action| {
                                if let Stage7ShiftsAndReducesLookaheadValue::Reduce { production_ids, .. } = action {
                                    production_ids.iter().any(|pid| unit_productions.contains(&pid.0))
                                } else { false }
                            });

                            if has_unit_reduce {
                                // This state is a candidate for merging.
                                let nts_deriving_leaf: BTreeSet<_> = derives_map.iter()
                                    .filter(|(_, derives)| derives.contains(leaf))
                                    .map(|(nt, _)| *non_terminal_map.get_by_left(nt).unwrap())
                                    .collect();

                                let states_to_merge: BTreeSet<_> = row.gotos.iter()
                                    .filter(|(nt_id, _)| nts_deriving_leaf.contains(nt_id))
                                    .filter_map(|(_, goto)| goto.state_id)
                                    .collect();

                                if states_to_merge.len() > 1 {
                                    let new_state_id = *combination_cache.entry(states_to_merge.clone()).or_insert_with(|| {
                                        let id = next_state_id;
                                        next_state_id.0 += 1;
                                        let mut new_row = Stage7Row::default();
                                        let mut new_item_set = BTreeSet::new();
                                        for s_id in &states_to_merge {
                                            let row_to_merge = &table[s_id];
                                            new_row.shifts_and_reduces_full.extend(row_to_merge.shifts_and_reduces_full.clone());
                                            new_row.gotos.extend(row_to_merge.gotos.clone());
                                            new_item_set.extend(item_set_map.get_by_right(s_id).unwrap().clone());
                                        }
                                        table.insert(id, new_row);
                                        item_set_map.insert(new_item_set, id);
                                        id
                                    });

                                    let current_row = table.get_mut(&state_id).unwrap();
                                    for nt_id in &nts_deriving_leaf {
                                        if let Some(goto) = current_row.gotos.get_mut(nt_id) {
                                            goto.state_id = Some(new_state_id);
                                        }
                                    }
                                    modified = true;
                                }
    ...
    // 4. Remove transitions on node non-terminals.
    let node_nt_ids: BTreeSet<_> = lhs_nodes.iter().map(|nt| *non_terminal_map.get_by_left(nt).unwrap()).collect();
    for row in table.values_mut() {
        row.gotos.retain(|nt_id, _| !node_nt_ids.contains(nt_id));
    }

    // 5. Remove unreachable states.
    // (This is often done by the caller or a subsequent pass, but is part of the full algorithm)

    // let removed_count = initial_state_count - table.len();
    let removed_count = initial_state_count.saturating_sub(table.len());
    crate::debug!(2, "Unit production elimination complete. Removed {} states.", removed_count);
    removed_count
}
...
pub fn generate_glr_parser_with_maps(productions: &[Production], start_production_id: usize, terminal_map: BiBTreeMap<Terminal, TerminalID>, mut non_terminal_map: BiBTreeMap<NonTerminal, NonTerminalID>, actions: BTreeMap<NonTerminal, ActionFn>, ignore_terminal_id: Option<TerminalID>, enable_unit_production_elimination: bool) -> GLRParser {
    ...
    if enable_unit_production_elimination {
        crate::debug!(2, "Eliminating unit productions");
        eliminate_unit_productions(&mut stage_7_table, &mut item_set_map, &productions, &non_terminal_map);
    }
