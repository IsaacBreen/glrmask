        ...
        let (phase1_shifts_and_reduces, phase3_default_reduce) = if let Some((nonterminal_id, len)) = promoted_reduce_key {
            ...
            let phase1 = phase2_shifts_and_reduces.iter().filter_map(|(&tid, action)| {
                match action {
                    ...
                    Stage7ShiftsAndReducesLookaheadValue::Split { shift, reduces } => {
                        let mut remaining_reduces = reduces.clone();
                        use std::collections::btree_map::Entry;
                        match remaining_reduces.entry(len) {
                            Entry::Occupied(mut entry) => {
                                ...
                                if remaining_reduces.is_empty() {
                                    shift.map(|shift_id| (tid, Stage7ShiftsAndReducesLookaheadValue::Shift(shift_id)))
                                } else if shift.is_none() && remaining_reduces.len() == 1 && remaining_reduces.values().next().unwrap().len() == 1 {
                                    // Only a simple reduce is left, simplify the Split to a Reduce.
                                    let (rem_len, nts) = remaining_reduces.into_iter().next().unwrap();
                                    let (rem_nt, pids) = nts.into_iter().next().unwrap();
                                    Some((tid, Stage7ShiftsAndReducesLookaheadValue::Reduce { nonterminal_id: rem_nt, len: rem_len, production_ids: pids }))
                                } else {
                                    // A conflict (S/R or R/R) remains.
                                    Some((tid, Stage7ShiftsAndReducesLookaheadValue::Split { shift: *shift, reduces: remaining_reduces }))
                                }
                            },
                            Entry::Vacant(_) => Some((tid, action.clone())), // Promoted reduce was not in this Split action.
                        }
                    },
