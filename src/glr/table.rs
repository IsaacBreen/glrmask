...
use crate::glr::automaton::{compute_closure, compute_first_sets_for_nonterminals, compute_follow_sets_for_nonterminals, compute_goto, compute_nonterminal_nullability, split_on_dot, Nullability};
...
fn stage_1(productions: &[Production]) -> Stage1Result {
    ...
    let follow_sets = compute_follow_sets_for_nonterminals(productions, &first_sets, &nonterminal_nullability);
    ...
    while let Some(item_set) = worklist.pop_front() {
        ...
        let closure = compute_closure(&item_set, productions, &first_sets, &nonterminal_nullability, &follow_sets);
