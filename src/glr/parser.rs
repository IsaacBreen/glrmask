...
use crate::datastructures::gss::{
    disallow_terminals_and_prune_arc, find_longest_path, format_acc, gather_gss_stats, print_gss_forest, Acc, GSSNode, GSSPopper,
    GSSPopperItem, GSSPrintConfig, GSSStats, GSSPeek, LLMTokenBV, PrecomputeNode2, PrecomputedNodeContents,
};
...
use crate::glr::table::{
    stage_9, DefaultReduce, Goto, NonTerminalID, ProductionID, Reduce, Row, ShiftsAndReducesFull, ShiftsAndReducesWithoutDefaultReduce,
    Stage7ShiftsAndReducesLookaheadValue, StateID, SubstringGoto, Table, TerminalID,
};
...
fn escape_dot_text(s: &str) -> String {
    s.replace('\\', "\\\\")
        .replace('"', "\\\"")
        .replace('\n', "\\l")
        .replace('{', "\\{")
...
impl DynOrd for () {
    fn dyn_cmp(&self, _other: &dyn Any) -> std::cmp::Ordering {
...
impl GLRParser {
    pub fn new(
        ...
        let _converted_actions: BTreeMap<NonTerminalID, ActionFn> = actions
            .into_iter()
            .map(|(nt, func)| {
    ...
    pub fn init_parse_state_substring_with_all_states(&self) -> ParseState {
        let all_edges: Vec<ParseStateEdgeContent> = self
            .table
            .keys()
    ...
    pub fn format_state_details<W: std::fmt::Write>(
        ...
        if let Some(items) = self.item_set_map.get_by_right(&state_id) {
            ...
            if items.is_empty() {
                ...
                let mut grouped: BTreeMap<(&Production, usize), BTreeSet<Option<Terminal>>> = BTreeMap::new();
                for item in items {
                    grouped.entry((&item.production, item.dot_position)).or_default().insert(item.lookahead.clone());
                }
                for ((production, dot_pos), lookaheads) in grouped {
                    ...
                    if lookaheads.len() == 1 {
                        ...
                        write!(f, "{{")?;
                        let mut lookahead_strs: Vec<String> =
                            lookaheads.iter().map(|l| l.as_ref().map_or_else(|| "ε".to_string(), |t| t.to_string())).collect();
                        lookahead_strs.sort();
                        const MAX_SHOW: usize = 5;
                        if lookahead_strs.len() > MAX_SHOW {
                            let truncated: Vec<_> = lookahead_strs.iter().take(MAX_SHOW).cloned().collect();
        ...
        if let Some(row) = self.table.get(&state_id) {
            ...
            if row.gotos.is_empty() {
                ...
                for (non_terminal_id, goto) in sorted {
        ...
        if !self.substring_gotos.is_empty() {
            ...
            for (nt_id, gotos) in sorted_substring_gotos {
                ...
                for (goto_id, source_ids) in sorted_gotos_by_dest {
                    ...
                    writeln!(f, "    - goto: {:<4} from sources: [{}]", goto_id.0, sources.join(", "))?;
                }
            }
        }
...
impl<'a> GLRParserState<'a> {
            ...
            if let Some(action) = action_selector(row) {
                for peek in GSSNode::peek_iter(&state.stack) {
                    match action {
                        ...
                        Action::Normal(Stage7ShiftsAndReducesLookaheadValue::Reduce { nonterminal_id, len, .. }) => {
                            if per_state_fuel == Some(0) {
                                continue;
                            }
                            let new_per_state_fuel = per_state_fuel.map(|f| f - 1);
                            crate::debug!(5, "Action: Reduce by NT '{}' (len {})", self.parser.non_terminal_map.get_by_right(nonterminal_id).unwrap(), len);
                            let (s_new_arc, accepted_s_new_arc) =
                                self.reduce_and_goto(&peek, *nonterminal_id, *len, &action_selector, config);
        ...
        'a: 'b,
    {
        match config.below_bottom_mode {
            BelowBottomReductionMode::ContinueFromAll => self.parser.substring_gotos.get(&nt).unwrap_or(storage),
            BelowBottomReductionMode::ContinueFromEverything => {
                ...
                if let Some(goto) = self.parser.table.get(&everything).and_then(|row| row.gotos.get(&nt)) {
                    ...
                    if let Some(state_id) = goto.state_id {
                        storage.gotos.insert(state_id, BTreeSet::from([everything]));
                    }
                }
                storage
            }
    ...
    fn build_below_bottom_accs(&self, popper: &GSSPopper) -> BTreeMap<usize, Acc> {
        ...
        for (k, accs_by_edge) in popper.below_bottom() {
            for (last_edge_content, acc_arc) in accs_by_edge {
                ...
                // Build tokens on destinations via edge (0, Some(last_state_id))
                ...
                for existing in &acc.trie2_nodes {
                    ...
                    let fallback = Arc::new(RwLock::new(PrecomputeNode2::new(PrecomputedNodeContents::internal())));
                    inserter = inserter.try_destinations_iter_with(eligible_iter).try_destination_auto(fallback);

                    let final_dest_arc = inserter.clone_into_option().expect("below-bottom Acc construction failed");
    ...
    fn handle_below_bottom_accepts(
        ...
        for (k, acc) in below {
            ...
            for source_state_id in &gotos.accepting_sources {
                ...
                let new_trie2_node =
                    Arc::new(RwLock::new(PrecomputeNode2::new(PrecomputedNodeContents::internal())));
        ...
        for (k, acc) in below {
            ...
            for (goto_state_id, source_state_ids) in &gotos.gotos {
                for source_state_id in source_state_ids {
                    // Coarse cache key retained for compatibility
                    ...
                    for existing in &trie2_nodes {
                        ...
                        // Cached weak destinations
                        if let Some(cached_entries) = self.below_bottom_cache.get(&cache_key) {
                            let eligible_cached_iter = cached_entries.iter().filter_map(|(wrapper, cached_tokens)| {
                        ...
                        let eligible_iter = || {
                            ...
                            if let Some(dest_map) = g.children().get(&edge_key) {
                                for (node_ptr, _ev) in dest_map.iter() {
                                    ...
                                    if let Some(dest_arc) = node_ptr.upgrade() {
                                        let guard = dest_arc.read().expect("poison");
                                        if (&guard.value.live_tokens & &tokens_to_push).is_empty() && !guard.value.end {
                                            v.push(dest_arc.clone());
                                        }
                                    }
                                }
                        ...
                        let final_dest_arc = inserter.clone_into_option().expect("below-bottom goto: EdgeInserter failed");
                        let final_wr = ArcPtrWrapper::new(final_dest_arc.clone());
                        dest_agg
                            .entry(final_wr.clone())
                            .and_modify(|bv| *bv |= &tokens_to_push)
        ...
        if !popper.below_bottom().is_empty() {
            match config.below_bottom_mode {
                ...
                _ => {
                    ...
                    if let Some(accepted_merged) = self.handle_below_bottom_accepts(nt, &below_accs, gotos_for_nt) {
    ...
    pub fn process_token_advanced(&mut self, token_id: TerminalID, config: &ProcessTokenAdvancedConfig) {
        // Reset acceptance and below-bottom cache for new token
        self.accepted = false;
    ...
    pub fn process_default_reductions_advanced(&mut self, config: &ProcessDefaultReductionsAdvancedConfig) {
        self.log_gss("Phase3-start", TerminalID(0), false, false); // dummy token
        if self.phase == ParserPhase::ReadyForToken {
                ...
                for peek in GSSNode::peek_iter(&self.active_state.stack) {
                    ...
                    let action_opt = match self.phase {
                        ParserPhase::ReadyForToken => {
                            row.shifts_and_reduces_without_default_reduce.get(&token_id).map(Action::Normal)
                        }
    ...
    pub fn merge_active_states(&mut self) {
        // explicit GSS merge is handled elsewhere; left for API compatibility
    }
    ...
    pub fn log_gss(&self, phase: &str, token: TerminalID, explain_states: bool, generate_dot: bool) {
        if !crate::profiler::GSS_LOGGING_ENABLED {
            return;
        }
        const MAX: usize = 100;
...
impl GLRParser {
    pub fn gss_forest_to_dot(
        ...
        let mut node_ids: HashMap<*const GSSNode, usize> = HashMap::new();
        let mut edge_node_ids: HashMap<(*const GSSNode, ParseStateEdgeContent), usize> = HashMap::new();
        let mut next_id_counter = 0;

        let mut queue: VecDeque<Arc<GSSNode>> = roots.iter().map(|(_, n)| Arc::new((*n).clone())).collect();

        // subgraphs with labels
        for (i, (label, root)) in roots.iter().enumerate() {
            ...
            writeln!(&mut dot, "    label=\"{}\";", escape_dot_text(label)).unwrap();
        ...
        while let Some(node_arc) = queue.pop_front() {
            ...
            if visited_nodes.insert(node_ptr) {
                let acc_str = format_acc(&node_arc, &self.terminal_map, original_internal_bimap, llm_token_map);
                let label = format!("Node {}\\lDepth: {}\\l{}", parent_id, node_arc.max_depth(), acc_str);
                writeln!(&mut dot, "  N{} [label=\"{}\"];", parent_id, escape_dot_text(&label)).unwrap();
            }
            ...
            for (edge_val, preds_by_depth) in node_arc.predecessors() {
                ...
                let edge_node_id = *edge_node_ids.entry(edge_key).or_insert_with(|| {
                    ...
                    let label = format!("State {}\\l{}", state_id.0, explanation);
                    writeln!(
                        &mut dot,
                        "  E{} [label=\"{}\", shape=plaintext, fontname=\"Courier New\"];",
                        id,
                        escape_dot_text(&label)
                    )
                    .unwrap();
                    id
                });
...
fn default_reduce_chain(parser: &GLRParser, start_state_id: StateID, initial_nt: NonTerminalID) -> BTreeSet<StateID> {
