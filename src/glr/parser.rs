...
impl<'a, T: MergeAndIntersect + Debug> GLRParserState<'a, T> {
    ...
    pub fn step(&mut self, token_id: TerminalID) {
        // --------------------------------------------------------------------
        // 0.  Book-keeping vectors
        // --------------------------------------------------------------------
        let mut worklist         = std::mem::take(&mut self.active_states);          // states still to visit this step
        let mut shift_results    = Vec::<Arc<GSSNode<ParseStateNodeContent<T>>>>::new(); // stacks produced by shifts
        let mut not_found_states = Vec::<ParseState<T>>::new();                      // kept for caller-side inspection

        // --------------------------------------------------------------------
        // 1.  Consume the work-list
        // --------------------------------------------------------------------
        while let Some(parse_state) = worklist.pop() {
            let stack          = &parse_state.stack;
            let top            = stack.peek();
            let cur_state_id   = top.state_id;
            let cur_t          = &top.t;

            // 1.a  Table row lookup ------------------------------------------------
            let row = match self.parser.stage_7_table.get(&cur_state_id) {
                Some(r) => r,
                None => { not_found_states.push(parse_state); continue; }
            };

            // 1.b  Action lookup for *this* input symbol ---------------------------
            match row.shifts_and_reduces.get(&token_id) {
                ...
                Some(Stage7ShiftsAndReduces::Shift(next_state_id)) => {
                    ...
                    shift_results.push(Arc::new(new_stack));
                }

                // ---- single REDUCE ----------------------------------------------
                Some(Stage7ShiftsAndReduces::Reduce { nonterminal_id, len, .. }) => {
                    let new_stacks = self.perform_reduction(stack, *len, *nonterminal_id, cur_t);
                    worklist.extend(new_stacks.into_iter().map(|s| ParseState { stack: s }));
                }

                // ---- conflict  (possibly both shift + multiple reduces) ----------
                Some(Stage7ShiftsAndReduces::Split { shift, reduces }) => {
                    // optional shift branch
                    ...
                    // one or more reduce branches
                    for (len, nt_map) in reduces {
                        for nt_id in nt_map.keys() {
                            let new_stacks = self.perform_reduction(stack, *len, *nt_id, cur_t);
                            worklist.extend(new_stacks.into_iter().map(|s| ParseState { stack: s }));
                        }
                    }
                }
                ...
                // ---- no action ---------------------------------------------------
                None => not_found_states.push(parse_state),
            }
        } // while worklist.pop()

        // --------------------------------------------------------------------
        // 2.  Finish the step  – merge identical stacks produced by shifting
        // --------------------------------------------------------------------
        shift_results.bulk_merge();
        self.active_states = shift_results
            .into_iter()
            .map(|stack| ParseState { stack })
            .collect();

        // --------------------------------------------------------------------
        // 3.  Export “no-action” information (optional; caller might clear it)
        // --------------------------------------------------------------------
        self.action_not_found_states = not_found_states;
    }
