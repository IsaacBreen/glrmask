        ...
        if any_below_bottom {
            ...
            for (k, accs_by_edge) in popper.below_bottom.iter() {
                ...
                if !gotos_for_nt.accepting_sources.is_empty() || !gotos_for_nt.gotos.is_empty() {
                    ...
                    let mut source_to_gotos: BTreeMap<StateID, Vec<StateID>> = BTreeMap::new();
                    for (goto_id, source_ids) in &gotos_for_nt.gotos {
                        for source_id in source_ids {
                            source_to_gotos.entry(*source_id).or_default().push(*goto_id);
                        }
                    }

                    let mut trie2_dst_nodes = HashMap::new();
                    timeit!(format!("GLRParserState::reduce_and_goto: Processing pop below"), {});
                    for (source_state_id, goto_state_ids) in source_to_gotos {
