...
impl GLRParser {
    ...
    pub fn init_parse_state_with_acc(&self, initial_acc: Acc) -> ParseState { // No longer generic
        ...
        let root = GSSNode::new_fresh(); // Create a fresh root node
        let stack = Arc::new(root.push(initial_content, initial_acc)); // pushed node has initial_acc
        ParseState { stack }
    }
...
impl<'a> GLRParserState<'a> { // No longer generic
    ...
    pub fn do_phase1_and_2(&mut self, token_id: TerminalID) {
        ...
        assert_eq!(self.phase, ParserPhase::ReadyForPhase1);
