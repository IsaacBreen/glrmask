...
impl<'a, A: PathAccumulator> GLRParserState<'a, A> {
    ...
    pub fn step(&mut self, token_id: TerminalID) {
        ...
        let mut todo: Vec<(ParseState<A>, HashSet<*const GSSNode<ParseStateNodeContent, A>>)> = Vec::new();
        ...
        while let Some((state, visited_on_this_path)) = todo.pop() { // Process states from the worklist
            ...
            let current_stack_ptr = Arc::as_ptr(&state.stack);

            // Check for cycle: if current_stack_ptr is already in visited_on_this_path for this reduction chain.
            if visited_on_this_path.contains(&current_stack_ptr) {
                crate::debug!(2, "Cycle detected: GSSNode at {:p} (state {:?}) encountered again in reduction path.", current_stack_ptr, state.stack.value);
            ...
            // Add current_stack_ptr to the history for paths stemming from this node.
            ...
            next_visited_on_this_path.insert(current_stack_ptr);
