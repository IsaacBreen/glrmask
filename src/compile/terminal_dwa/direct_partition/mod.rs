//! direct-partition terminal DWA: direct 2-state construction for terminals with max path
//! length ≤ 1.
...
#[derive(Debug)]
struct DirectPartitionIdentityVocabOrder {
    token_ids_sorted: Arc<[u32]>,
    token_entries_sorted: Arc<[(u32, Arc<[u8]>)]>,
    original_to_internal: Arc<[u32]>,
    token_buckets: DirectPartitionSortedTokenBuckets,
}

impl crate::vocab::VocabDerivedArtifact for DirectPartitionIdentityVocabOrder {}

fn direct_partition_identity_vocab_order(vocab: &Vocab) -> Arc<DirectPartitionIdentityVocabOrder> {
    if let Some(cached) = vocab.vocab_derived_cache_get::<DirectPartitionIdentityVocabOrder>() {
        return cached;
    }
    ...
    let token_buckets = build_direct_partition_sorted_token_buckets(&token_entries_sorted);
    let order = Arc::new(DirectPartitionIdentityVocabOrder {
...
pub(crate) fn prepare_direct_partition_identity_vocab_order(vocab: &Vocab) {
    let _ = direct_partition_identity_vocab_order(vocab);
}
...
fn skip_direct_partition_max_length_for_partition(partition_label: &str) -> bool {
    if matches!(partition_label, "p4" | "p6") {
        return true;
    }
    static SKIPPED_DIRECT_PARTITION_PARTITIONS: OnceLock<Vec<String>> = OnceLock::new();
    SKIPPED_DIRECT_PARTITION_PARTITIONS
        .get_or_init(|| {
            std::env::var("GLRMASK_SKIP_DIRECT_PARTITION_MAX_LENGTH_PARTITIONS")
                .ok()
                .map(|value| {
                    value
                        .split(',')
...
fn direct_partition_max_length_min_states() -> usize {
    static MIN_STATES: OnceLock<usize> = OnceLock::new();
    *MIN_STATES.get_or_init(|| {
        std::env::var("GLRMASK_DIRECT_PARTITION_MAX_LENGTH_MIN_STATES")
            .ok()
...
fn should_skip_max_length_for_partition(
    ...
    skip_max_length_for_partition(partition_label)
        || skip_direct_partition_max_length_for_partition(partition_label)
        || initial_state_count < direct_partition_max_length_min_states()
        || (projected_by_global && initial_state_count <= 8192)
}

fn fast_projected_direct_partition_id_map_enabled() -> bool {
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        std::env::var("GLRMASK_DIRECT_PARTITION_FAST_PROJECTED_ID_MAP")
            .map(|value| {
...
fn fast_projected_direct_partition_id_map_max_tsids() -> usize {
    static MAX_TSID: OnceLock<usize> = OnceLock::new();
    *MAX_TSID.get_or_init(|| {
        std::env::var("GLRMASK_DIRECT_PARTITION_FAST_PROJECTED_ID_MAP_MAX_TSIDS")
            .ok()
            .and_then(|value| value.trim().parse::<usize>().ok())
            .unwrap_or(16_384)
    })
...
#[inline]
fn should_use_fast_projected_direct_partition_id_map(
    initial_state_map: Option<&ManyToOneIdMap>,
    num_dfa_states: usize,
) -> bool {
    if !fast_projected_direct_partition_id_map_enabled() {
        return false;
    }
    ...
    map.original_to_internal.len() == num_dfa_states
        ...
        && projected_states <= fast_projected_direct_partition_id_map_max_tsids()
}
...
use super::pair_partition::equivalence_analysis::compat::{compute_byte_classes, TokenizerView};
use super::types::{compile_profile_enabled, TerminalColoring, TerminalDwaPhaseProfile};

fn compact_direct_partition_terminal_dwa_enabled() -> bool {
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        std::env::var("GLRMASK_COMPACT_DIRECT_PARTITION_TERMINAL_DWA")
            .map(|value| {
                let trimmed = value.trim();
                !trimmed.is_empty() && trimmed != "0" && !trimmed.eq_ignore_ascii_case("false")
            })
...
/// Maximum direct-partition equivalence class count before falling back to pair-partition.
///
/// When the tokenizer DFA has more than this many distinct equivalence classes
/// for the active direct-partition terminals, the direct-partition trie traversal becomes more expensive
/// than pair-partition's NWA-based approach.
pub(crate) const MAX_DIRECT_PARTITION_TSIDS: usize = 50;

/// Quickly count direct-partition equivalence classes for the given active terminals.
///
/// Used by the partition builder to decide whether direct partition should be attempted
/// *before* launching the parallel direct partition / pair partition build, avoiding a wasteful
/// pair-partition double-build when the direct partition would be skipped.
pub(crate) fn count_direct_partition_equivalence_classes(
    tokenizer: &Tokenizer,
    vocab: &Vocab,
    ...
    let equiv_mapping = super::pair_partition::equivalence_analysis::state::max_length::find_state_equivalence_classes_byte_restricted(
        &tokenizer_view,
        &token_bytes,
        &states,
        Some(&byte_to_class),
    ...
    let order = direct_partition_identity_vocab_order(vocab);
    let flat_trans = build_flat_transition_table(tokenizer);
    let exact_mapping = find_direct_partition_exact_state_equivalence_by_token_signatures(
        tokenizer,
        order.as_ref(),
...
/// Build a direct-partition id_map and terminal DWA for the given vocab and terminal set.
///
...
/// The caller should pre-check `count_direct_partition_equivalence_classes()` and merge
/// direct-partition terminals into pair-partition when the count exceeds `MAX_DIRECT_PARTITION_TSIDS`.
pub(crate) fn build_direct_partition_terminal_dwa(
    partition_label: &str,
    tokenizer: &Tokenizer,
    vocab: &Vocab,
    ...
    let (mut id_map, vocab_order, _state_to_rep, id_map_profile) = build_direct_partition_id_map(
        partition_label,
        tokenizer,
        vocab,
        active_terminals,
    ...
    let (compact_report, compact_ms) = if compact_direct_partition_terminal_dwa_enabled() {
...
fn build_direct_partition_id_map<'a>(
    partition_label: &str,
    tokenizer: &Tokenizer,
    vocab: &'a Vocab,
    active_terminals: &[bool],
    ...
    InternalIdMap,
    Arc<DirectPartitionIdentityVocabOrder>,
    Vec<u32>,
    ...
    if should_use_fast_projected_direct_partition_id_map(initial_state_map, num_dfa_states) {
        ...
        let (vocab_tokens, vocab_order, token_identity_map_ms) =
            build_direct_partition_identity_vocab_map(vocab);
        let tokenizer_states = initial_state_map
            .expect("checked by should_use_fast_projected_direct_partition_id_map")
            .clone();
    ...
    let equiv_mapping = if max_length_skipped {
        ...
        let byte_to_class = compute_byte_classes(tokenizer_view.dfa());
        super::pair_partition::equivalence_analysis::state::max_length::find_state_equivalence_classes_byte_restricted(
            &tokenizer_view,
            &token_bytes,
            &states,
            Some(&byte_to_class),
    ...
    // Keep this exact: the direct-partition terminal DWA indexes weights by TSID, so every
...
fn build_direct_partition_identity_vocab_map(vocab: &Vocab) -> (ManyToOneIdMap, Arc<DirectPartitionIdentityVocabOrder>, f64) {
...
fn find_direct_partition_exact_state_equivalence_by_token_signatures(
    tokenizer: &Tokenizer,
    ...
    // Exact direct-partition equivalence has a useful factorization.  For a non-empty token
    ...
    let state_to_terminal_signature =
        build_direct_partition_state_to_terminal_signature(tokenizer, active_terminals);
    ...
    let build_target_profile = |&(byte, target): &(u8, u32)| {
        ...
        let profile = direct_partition_bucket_suffix_signature_profile(
            target,
            sorted_entries,
...
fn direct_partition_bucket_suffix_signature_profile(
    first_target: u32,
    ...
    for (bucket_pos, &internal_token_id) in token_ids.iter().enumerate() {
        ...
        if sig_id != 0 {
            append_direct_partition_signature_profile_run(&mut profile, sig_id, internal_token_id as u32);
        }
    }
...
#[derive(Debug)]
struct DirectPartitionSortedTokenBuckets {
...
fn build_direct_partition_sorted_token_buckets(sorted_entries: &[(u32, Arc<[u8]>)]) -> DirectPartitionSortedTokenBuckets {
...
fn build_direct_partition_state_to_terminal_signature(
    tokenizer: &Tokenizer,
    active_terminals: &[bool],
) -> Vec<u32> {
...
fn direct_partition_token_signature_profile_for_state(
    start_state: u32,
    sorted_entries: &[(u32, Arc<[u8]>)],
    buckets: &DirectPartitionSortedTokenBuckets,
    state_to_terminal_signature: &[u32],
    flat_trans: &[u32],
) -> Vec<(u32, u32, u32)> {
    let dead = u32::MAX;
    ...
    for &internal_token_id in &buckets.empty_token_indices {
        append_direct_partition_signature_profile_run(&mut profile, start_sig, internal_token_id as u32);
    }
    ...
    for (first_byte, token_ids) in buckets.token_indices_by_first_byte.iter().enumerate() {
        ...
        if first_target == dead {
            for &internal_token_id in token_ids {
                append_direct_partition_signature_profile_run(&mut profile, 0, internal_token_id as u32);
            }
            continue;
        }
...
fn append_direct_partition_signature_profile_run(profile: &mut Vec<(u32, u32, u32)>, sig_id: u32, token_id: u32) {
...
fn build_direct_partition_terminal_dwa_core(
    ...
    active_terminals: &[bool],
    flat_trans: &[u32],
) -> Option<(DWA, DirectPartitionTerminalBuildProfile)> {
...
fn collect_direct_partition_root_ranges_by_first_byte_lcp(
    start_state: u32,
    sorted_entries: &[(u32, Arc<[u8]>)],
...
fn merge_deferred_equivalent_tsids(
    ...
    let num_tsids = id_map.num_tsids() as usize;
    if num_tsids <= 1 {
        return DirectPartitionTsidProfileMergeReport {
...
struct DirectPartitionIdMapProfile {
...
struct DirectPartitionTsidProfileMergeReport {
    tsids_after: usize,
...
struct DirectPartitionTerminalBuildProfile {
