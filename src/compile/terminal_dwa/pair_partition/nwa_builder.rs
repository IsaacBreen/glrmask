...
pub(crate) struct TerminalNwaBuilder<'tok, 'cm, 'nwa> {
    ...
    can_match: &'cm mut CanMatchComputer<'tok>,
    nwa: &'nwa mut NWA,
    num_tsids: u32,
    leaf_state: u32,
...
impl<'tok, 'cm, 'nwa> TerminalNwaBuilder<'tok, 'cm, 'nwa> {
    pub(crate) fn new(
        tokenizer: &'tok Tokenizer,
        terminal_coloring: TerminalColoring,
        can_match: &'cm mut CanMatchComputer<'tok>,
    ...
    fn continuation_weight_for_match(
        ...
        if let Some(end_state) = end_state {
            let can_match = self
                .can_match
                .can_match_for_node(child_node, end_state);
            if let Some(matches_for_terminal) = can_match.get(&terminal_id) {
                subtract_can_match(&mut remaining, matches_for_terminal);
            }
        }
    ...
    /// Fast NWA construction for direct-partition-only grammars (all terminals have path
    ...
    pub(crate) fn build_direct_partition_fast(
        &mut self,
        ...
        while let Some((offset, nodes_at_offset)) = pending_by_offset.pop_first() {
            ...
            for (tokenizer_state, source_nodes) in nodes_at_offset {
                ...
                for matched in &matches_buf {
                    ...
                    // direct-partition terminals never appear in multi-terminal paths, so
...
fn subtract_can_match(
    continuation_tokens: &mut RangeSetBlaze<usize>,
    can_match: &RangeSetBlaze<u32>,
) {
    for token_id in can_match.iter() {
...
pub(crate) fn build_nwa_via_trie_walk<'a>(
    ...
    can_match: &mut CanMatchComputer<'a>,
