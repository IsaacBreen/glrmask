//! Pair-partition terminal DWA construction.
//!
//! This module handles terminals whose lexer behavior depends on at least two
//! distinguished terminal colors.  Such terminals cannot be represented by the
//! direct partition alone, so the builder computes an internal token partition
//! from terminal-pair behavior, constructs a weighted NWA over the vocab-prefix
//! trie, applies follow constraints and structural pruning, then determinizes
//! and minimizes the resulting Terminal DWA.
//!
//! The mathematical contract is the same as the direct partition path: for each
//! terminal sequence read by the automaton, the weight denotes exactly the
//! lexer-state/token pairs that can scan that sequence.
...
fn project_initial_state_map_enabled() -> bool {
    ...
    *ENABLED.get_or_init(|| {
        std::env::var("GLRMASK_PAIR_PARTITION_PROJECT_INITIAL_STATE_MAP")
            .map(|value| {
...
#[derive(Clone, Copy)]
struct PairPartitionTokenLengthStats {
    max_len: usize,
    gt_4: usize,
    gt_8: usize,
    gt_16: usize,
...
fn pair_partition_token_length_stats(vocab: &Vocab) -> PairPartitionTokenLengthStats {
    let mut stats = PairPartitionTokenLengthStats {
        max_len: 0,
        gt_4: 0,
        gt_8: 0,
        gt_16: 0,
...
/// Build a pair-partition id map and Terminal DWA for the given vocab and terminal set.
///
/// Builds its own id_map via `InternalIdMap::build_with_group_filter` (full DFA-
/// based equivalence analysis restricted to pair-partition terminal groups). Then builds
/// the Terminal DWA using the vocab-trie NWA pipeline:
///
/// 1. Build internal vocab entries
/// 2. Build vocab prefix trie
/// 3. Compute can_match (for root node seeding)
...
pub(crate) fn build_pair_partition_terminal_dwa(
    partition_label: &str,
    tokenizer: &Tokenizer,
    vocab: &Vocab,
    ...
    // remove this note. pair-partition state/vocab equivalence must always run fully.
    ...
    let (
        ...
        if internal_vocab.is_empty() {
            ...
            // ---- Step 4: Can-match (lazy via computer) ----
            let mut can_match_computer = CanMatchComputer::new(tokenizer_for_build);
            let can_match_ms = 0.0;
            ...
            let det = determinize(&nwa).expect("pair-partition terminal NWA determinization failed");
    ...
    if compile_profile_enabled() {
        eprintln!(
            "[glrmask/profile][pair_partition] partition={} vocab_tokens={} active_terminals={} original_states={} tsids={} internal_vocab_entries={} initial_states_considered={} max_length_skipped={} max_token_len={} token_len_gt_4={} token_len_gt_8={} token_len_gt_16={} token_len_gt_32={} token_len_gt_64={} prepare_inputs_ms={:.3} byte_class_setup_ms={:.3} token_dedup_ms={:.3} max_length_state_equiv_ms={:.3} vocab_equiv_ms={:.3} exact_state_equiv_ms={:.3} id_map_finalize_ms={:.3} max_length_reps={} exact_reps={} exact_rep_confirmation_used={} fast_sound_id_map_used={} max_length_reduction_pct={:.2} exact_reduction_pct={:.2} simplify_ms={:.3} simplify_cache_hit={} simplified_states={} id_map_ms={:.3} tsid_fallback_ms={:.3} vocab_tree_ms={:.3} can_match_ms={:.3} seed_ms={:.3} terminal_nwa_build_ms={:.3} nwa_states={}->{}->{}->{}->{} always_allowed_ms={:.3} collapse_ms={:.3} disallowed_ms={:.3} prune_ms={:.3} canonicalize_ms={:.3} postprocess_ms={:.3} determinize_ms={:.3} minimize_ms={:.3} minimize_states={} dwa_states={} dwa_transitions={} dwa_transition_pairs={} dwa_interned_ranges_before_compact={} dwa_interned_ranges_after_compact={} total_ms={:.3}",
            partition_label,
            vocab.entries.len(),
