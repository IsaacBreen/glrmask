...
pub(crate) fn resolve_pair_partition_pipeline_config(
    default_include_max_length: bool,
) -> StateEquivalencePipelineConfig {
    let default_passes = if default_include_max_length {
        &[StateEquivalencePassKind::MaxLength][..]
    ...
    resolve_pipeline_config("GLRMASK_PAIR_PARTITION_STATE_EQUIV_PASSES", default_passes)
}
        ...
        match *kind {
            StateEquivalencePassKind::MaxLength => {
                let mode = match scope {
                    ...
                    StateEquivalenceScope::PairPartition => MaxLengthMode::KBoundedByteRestricted,
                };
