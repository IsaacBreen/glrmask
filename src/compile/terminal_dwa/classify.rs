...
pub(crate) enum PairPartitionCostFn {
    Size,
    SizeLog,
    LogLog,
    UnionSize,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum PairPartitionObjective {
    Max,
    Sum,
}

impl PairPartitionObjective {
    pub(crate) fn as_str(self) -> &'static str {
        match self {
            Self::Max => "max",
            Self::Sum => "sum",
...
pub(crate) struct PairPartitionCostPartitioning {
    pub(crate) partitions: Vec<Vec<u32>>,
    pub(crate) estimated_partition_costs: Vec<f64>,
    pub(crate) estimated_pair_partition_terminals: Vec<usize>,
    pub(crate) objective_score: f64,
}

#[derive(Clone)]
struct PairPartitionTokenGroup {
    pair_partition_terminals: BitSet,
    token_ids: Vec<u32>,
}

#[derive(Clone)]
struct PairPartitionBucket {
    pair_partition_intersection: Option<BitSet>,
    pair_partition_union: Option<BitSet>,
    token_ids: Vec<u32>,
}

impl PairPartitionBucket {
    fn new() -> Self {
        Self {
            pair_partition_intersection: None,
            pair_partition_union: None,
            token_ids: Vec::new(),
        }
    }
    ...
    fn pair_partition_count(&self) -> usize {
        self.pair_partition_intersection.as_ref().map_or(0, BitSet::count_ones)
    }
}
...
/// (`":[]{},`) stay in P0 for efficient pair-partition terminal processing, while
...
pub(crate) fn classify_terminal_path_lengths(
    ...
    for t in 0..nt {
        if reachable_bytes[t].is_disjoint(&vocab_bytes) {
            ...
            result[t] = TerminalPathLength::MultiStep;
        } else {
...
fn token_pair_partition_terminals(
    bytes: &[u8],
    ...
    let mut pair_partition_set = BitSet::new(num_terminals);
    for terminal_1 in last_set.iter() {
        ...
        for terminal_2 in first_set.iter() {
            if !disallowed.map_or(false, |blocked| blocked.contains(terminal_2)) {
                pair_partition_set.set(terminal_1);
                pair_partition_set.set(terminal_2);
            }
        }
    }
...
fn compute_partition_cost(
    cost_fn: PairPartitionCostFn,
    pair_partition_terminals: usize,
    partition_size: usize,
) -> f64 {
    if pair_partition_terminals == 0 || partition_size == 0 {
        return 0.0;
    }

    let num_pair_partition = pair_partition_terminals as f64;
    let size = partition_size as f64;
    match cost_fn {
        PairPartitionCostFn::Size => num_pair_partition * size,
        PairPartitionCostFn::SizeLog => num_pair_partition * size.ln(),
        PairPartitionCostFn::LogLog => num_pair_partition.ln() * size.ln(),
        PairPartitionCostFn::UnionSize => num_pair_partition * size,
    }
}
...
fn partition_metric_count(
    ...
    match cost_fn {
        PairPartitionCostFn::UnionSize => union_count,
        PairPartitionCostFn::Size
        | PairPartitionCostFn::SizeLog
        | PairPartitionCostFn::LogLog => intersection_count,
    }
}

fn objective_score(objective: PairPartitionObjective, costs: &[f64]) -> f64 {
    match objective {
        PairPartitionObjective::Max => costs.iter().copied().fold(0.0, f64::max),
        PairPartitionObjective::Sum => costs.iter().sum(),
    }
}

fn compute_token_pair_partition_map(
    vocab: &Vocab,
    bytesets: &SharedClassifyBytesets,
    disallowed_follows: &BTreeMap<u32, BitSet>,
    num_terminals: u32,
    ...
    let mut token_pair_partition_map = BTreeMap::<u32, BitSet>::new();
    for (&token_id, bytes) in vocab.entries.iter() {
        token_pair_partition_map.insert(
            token_id,
            token_pair_partition_terminals(
                bytes,
                &byte_to_last,
                &byte_to_first,
                disallowed_follows,
...
pub(crate) fn estimate_pair_partition_objective_for_token_partitions(
    token_partitions: &[Vec<u32>],
    token_pair_partition_map: &BTreeMap<u32, BitSet>,
    cost_fn: PairPartitionCostFn,
    objective: PairPartitionObjective,
) -> (Vec<f64>, Vec<usize>, f64) {
    let mut costs = Vec::with_capacity(token_partitions.len());
    let mut pair_partition_counts = Vec::with_capacity(token_partitions.len());

    for token_ids in token_partitions {
        if token_ids.is_empty() {
            costs.push(0.0);
            pair_partition_counts.push(0);
            continue;
        }
        ...
        for &token_id in token_ids {
            if let Some(token_pair_partition) = token_pair_partition_map.get(&token_id) {
                if let Some(current) = &mut intersection {
                    current.intersect_with(token_pair_partition);
                } else {
                    intersection = Some(token_pair_partition.clone());
                }
                if let Some(current) = &mut union {
                    current.union_with(token_pair_partition);
                } else {
                    union = Some(token_pair_partition.clone());
                }
            }
        }

        let pair_partition_count = intersection.as_ref().map_or(0, BitSet::count_ones);
        let union_count = union.as_ref().map_or(0, BitSet::count_ones);
        pair_partition_counts.push(pair_partition_count);
        costs.push(compute_partition_cost(
            cost_fn,
            partition_metric_count(cost_fn, pair_partition_count, union_count),
            token_ids.len(),
        ));
    }

    let score = objective_score(objective, &costs);
    (costs, pair_partition_counts, score)
}

fn partition_token_pair_partition_map_by_cost(
    token_pair_partition_map: &BTreeMap<u32, BitSet>,
    num_partitions: usize,
    cost_fn: PairPartitionCostFn,
    objective: PairPartitionObjective,
) -> PairPartitionCostPartitioning {
    let mut grouped_index = BTreeMap::<Vec<u64>, usize>::new();
    let mut groups: Vec<PairPartitionTokenGroup> = Vec::new();

    for (&token_id, pair_partition_terminals) in token_pair_partition_map {
        let key = pair_partition_terminals.words().to_vec();
        if let Some(&group_idx) = grouped_index.get(&key) {
            ...
            groups.push(PairPartitionTokenGroup {
                pair_partition_terminals: pair_partition_terminals.clone(),
    ...
    groups.sort_by(|left, right| {
        let left_weight = left.pair_partition_terminals.count_ones() * left.token_ids.len();
        let right_weight = right.pair_partition_terminals.count_ones() * right.token_ids.len();
        right_weight
            .cmp(&left_weight)
            .then_with(|| right.pair_partition_terminals.count_ones().cmp(&left.pair_partition_terminals.count_ones()))
            .then_with(|| right.token_ids.len().cmp(&left.token_ids.len()))
    });

    let mut buckets = vec![PairPartitionBucket::new(); num_partitions.max(1)];
    ...
    for group in groups {
        ...
        let mut best_pair_partition_count = usize::MAX;
        let mut best_size = usize::MAX;

        for (idx, bucket) in buckets.iter().enumerate() {
            let candidate_intersection = if let Some(current) = &bucket.pair_partition_intersection {
                current.intersection(&group.pair_partition_terminals)
            } else {
                group.pair_partition_terminals.clone()
            };
            let candidate_union = if let Some(current) = &bucket.pair_partition_union {
                current.union(&group.pair_partition_terminals)
            } else {
                group.pair_partition_terminals.clone()
            };
            let candidate_pair_partition_count = candidate_intersection.count_ones();
            ...
            let candidate_cost = compute_partition_cost(
                cost_fn,
                partition_metric_count(cost_fn, candidate_pair_partition_count, candidate_union_count),
                candidate_size,
            );
            ...
            let better = score < best_score
                || (score == best_score
                    && (candidate_cost < best_cost
                        || (candidate_cost == best_cost
                            && (candidate_pair_partition_count < best_pair_partition_count
                                || (candidate_pair_partition_count == best_pair_partition_count
            ...
            if better {
                ...
                best_pair_partition_count = candidate_pair_partition_count;
                best_size = candidate_size;
            }
        }

        let bucket = &mut buckets[best_idx];
        if let Some(current) = &mut bucket.pair_partition_intersection {
            current.intersect_with(&group.pair_partition_terminals);
        } else {
            bucket.pair_partition_intersection = Some(group.pair_partition_terminals.clone());
        }
        if let Some(current) = &mut bucket.pair_partition_union {
            current.union_with(&group.pair_partition_terminals);
        } else {
            bucket.pair_partition_union = Some(group.pair_partition_terminals.clone());
        }
    ...
    let estimated_partition_costs = buckets
        .iter()
        .map(|bucket| {
            let union_count = bucket.pair_partition_union.as_ref().map_or(0, BitSet::count_ones);
            compute_partition_cost(
                cost_fn,
                partition_metric_count(cost_fn, bucket.pair_partition_count(), union_count),
                bucket.size(),
            )
        })
        .collect::<Vec<_>>();
    let estimated_pair_partition_terminals = buckets
        .iter()
        .map(PairPartitionBucket::pair_partition_count)
        .collect::<Vec<_>>();
...
pub(crate) fn partition_vocab_by_pair_partition_cost_with_token_map(
    vocab: &Vocab,
    bytesets: &SharedClassifyBytesets,
    disallowed_follows: &BTreeMap<u32, BitSet>,
    num_terminals: u32,
    ...
    cost_fn: PairPartitionCostFn,
    objective: PairPartitionObjective,
) -> (PairPartitionCostPartitioning, BTreeMap<u32, BitSet>) {
    let token_pair_partition_map = compute_token_pair_partition_map(vocab, bytesets, disallowed_follows, num_terminals);
    let partitioning =
        partition_token_pair_partition_map_by_cost(&token_pair_partition_map, num_partitions, cost_fn, objective);
    (partitioning, token_pair_partition_map)
}

pub(crate) fn partition_vocab_by_pair_partition_cost(
    vocab: &Vocab,
    bytesets: &SharedClassifyBytesets,
    disallowed_follows: &BTreeMap<u32, BitSet>,
    num_terminals: u32,
    ...
    partition_vocab_by_pair_partition_cost_with_token_map(
        vocab,
        bytesets,
        disallowed_follows,
        num_terminals,
