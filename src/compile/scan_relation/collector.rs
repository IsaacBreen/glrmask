//! Constraint-specific can-match collector.
//!
//! This collector keeps can-match token sets as trie-order intervals rather
...
/// A can-match row that applies the same token ranges to a whole terminal
...
pub(crate) type IntervalCanMatchMap = Vec<TerminalRangeGroup>;

pub(crate) struct TrieClassBuildResult {
    pub(crate) state_classes: Vec<u32>,
    pub(crate) class_maps: Vec<Arc<IntervalCanMatchMap>>,
}

impl TrieClassBuildResult {
    #[allow(dead_code)]
    pub(crate) fn expand_to_states(&self, entries: &[u32]) -> BTreeMap<u32, IntervalCanMatchMap> {
...
struct NodeClasses { classes: Vec<u32>, class_maps: Vec<Arc<IntervalCanMatchMap>> }
...
#[inline]
fn append_range(map: &mut IntervalCanMatchMap, terminals: &[TerminalID], range: TokenRange) {
    if range.0 <= range.1 {
...
#[inline]
fn append_ranges(map: &mut IntervalCanMatchMap, terminals: &[TerminalID], ranges: &[TokenRange]) {
    if !ranges.is_empty() {
        if let Some(terminals) = canonical_terminal_box(terminals) {
...
fn merge_interval_maps(into: &mut IntervalCanMatchMap, other: &IntervalCanMatchMap) {
...
fn normalize_interval_map(map: &mut IntervalCanMatchMap) {
    ...
    let mut merged: IntervalCanMatchMap = Vec::with_capacity(map.len());
...
fn dense_segment_cache_min_entries() -> usize {
    std::env::var("GLRMASK_SCAN_RELATION_DENSE_SEGMENT_CACHE_MIN_ENTRIES")
        .ok()
...
fn build_node(
    ...
    for (&state, &state_pos) in representative_states.iter().zip(representative_state_positions.iter()) {
        let mut result = IntervalCanMatchMap::default();
        if node.has_token() {
...
pub(crate) fn collect_can_match_interval_trie_class_build_with_classes(
    tokenizer: &Tokenizer,
    root: &VocabPrefixTreeNode,
    entries: &[u32],
    canonical_state: Option<&[u32]>,
) -> (TrieClassBuildResult, CanMatchProfile) {
    ...
    let parallel_depth = std::env::var("GLRMASK_SCAN_RELATION_ROOT_PARALLEL_DEPTH").ok().and_then(|v| v.parse::<u8>().ok()).unwrap_or(5);
    let parallel_min_active = std::env::var("GLRMASK_SCAN_RELATION_PARALLEL_MIN_ACTIVE_STATES")
        .ok()
    ...
    let profile = CanMatchProfile { cache_entries: root_result.class_maps.len(), root_compute_ms, ..Default::default() };
