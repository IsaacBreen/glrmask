//! Can-match tables for tokenizer states and vocab-prefix subtrees.
...
fn merge_can_match_maps(into: &mut CanMatchMap, other: &CanMatchMap) {
...
pub(crate) struct CanMatchComputer<'a> {
    tokenizer: &'a Tokenizer,
    canonical_state: Option<&'a [u32]>,
    cache: FxHashMap<(usize, u32), Rc<CanMatchMap>>,
...
impl<'a> CanMatchComputer<'a> {
    ...
    pub(crate) fn can_match_for_node(
        &mut self,
        node: &VocabPrefixTreeNode,
        tokenizer_state: u32,
    ) -> Rc<CanMatchMap> {
        ...
        let mut result = CanMatchMap::default();
        ...
        for (segment_bytes, child) in node.iter_children() {
            ...
            if !segment_blocked && !self.tokenizer.is_end(current_state) {
                ...
                let child_matches = self.can_match_for_node(child, descend_state);
                merge_can_match_maps(&mut result, child_matches.as_ref());
            }
        }
