...
/// Holds only the states vector for an NWA. This can be shared by multiple "views"
/// that use different start states and metadata.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
...
impl NWAStates {
    ...
    /// Append a deep copy of `other`'s states into `self`, returning a mapping from
    ...
    /// Process an input stack (sequence of u16 symbols) through this NWA instance
    /// configured by `cfg`.
    ///
    ...
    pub fn process_stack_u16(&self, cfg: &NWAConfig, input: &[u16]) -> Vec<(StateID, StateID, Weight)> {
        ...
        current.insert(cfg.start_state, Weight::all());
...
/// Non-state metadata for an NWA: currently just the start state.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct NWAConfig {
    pub start_state: StateID,
}

/// Combined NWA that bundles states + config for convenience.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct NWA {
...
impl NWA {
    ...
    pub fn new() -> Self {
        ...
        let start_state = states.add_state(); // Add start state 0
        NWA {
            states,
            cfg: NWAConfig { start_state },
        }
    }
    ...
    /// Determinize using the combined (states + cfg).
    pub fn determinize(&self) -> DWA {
        self.states.determinize(&self.cfg)
    }
}
...
impl NWAStates {
    ...
    pub fn determinize(&self, cfg: &NWAConfig) -> DWA {
        ...
        let mut get_or_create_dwa_state =
            |comp_key: Vec<(StateID, Weight)>,
             ...
             -> Option<StateID> {
                ...
                let new_id = dwa.states.states.len();
                dwa.states.states.push(DWAState::default());
        ...
        let start_composition_raw = BTreeMap::from([(cfg.start_state, Weight::all())]);
        ...
        while let Some(current_composition) = worklist.pop_front() {
            ...
            dwa.states.states[current_dwa_id].weight = aggregate_weight;
            if is_final {
                dwa.states.states[current_dwa_id].final_weight = Some(aggregate_final_weight);
            }
            ...
            dwa.states.states[current_dwa_id].transitions.default = default_target_dwa_id;
            if default_target_dwa_id.is_some() {
                dwa.states.states[current_dwa_id].trans_weight_default = Some(default_weight_agg);
            }
            ...
            for char_code in critical_points {
                ...
                if exception_target_dwa_id != default_target_dwa_id {
                    if let Some(target_id) = exception_target_dwa_id {
                        dwa.states.states[current_dwa_id]
                            .transitions
                            .exceptions
...
impl DWA {
    ...
    pub fn simplify(&mut self) {
        if self.states.states.is_empty() {
            return;
        }
    ...
    pub fn normalize_edges_inplace(&mut self) -> bool {
        ...
        for st in &mut self.states.states {
    ...
    pub fn minimize_partition_refinement(&mut self) -> bool {
        ...
        for i in 0..n {
            let key = (self.states.states[i].weight.clone(), self.states.states[i].final_weight.clone());
        ...
        for (pid, members) in &groups {
            let rep = members[0];
            let rep_state = &self.states.states[rep];
            ...
            if st.transitions.default.is_some() {
                ...
                for &old in members {
                    if let Some(w) = self.states.states[old].trans_weight_default.as_ref() {
            ...
            for ch in ex_keys {
                ...
                for &old in members {
                    if let Some(w) = self.states.states[old].trans_weights_exceptions.get(&ch) {
        ...
        self.states.states = new_states_vec;
        // Normalize edges (drop redundant exceptions)
        self.normalize_edges_inplace();
        // Remap start state
        let start_pid = part[self.cfg.start_state];
        self.cfg.start_state = *pid_to_new.get(&start_pid).unwrap();
        true
    }
    ...
    pub fn prune_unreachable(&mut self) -> bool {
        ...
        while let Some(u) = q.pop_front() {
            if let Some(d) = self.states.states[u].transitions.default {
                if !visited[d] {
                    visited[d] = true;
                    q.push_back(d);
                }
            }
            for &v in self.states.states[u].transitions.exceptions.values() {
                if !visited[v] {
                    visited[v] = true;
                    q.push_back(v);
                }
        ...
        for old in 0..n {
            ...
            let mut st = self.states.states[old].clone();
        ...
        self.states.states = new_states;
        self.cfg.start_state = map[self.cfg.start_state];
        true
    }
    ...
    pub fn merge_equivalent_states(&mut self) -> bool {
        ...
        for i in 0..n {
            let default = self.states.states[i].transitions.default;
            // BTreeMap iteration is sorted, so this Vec is in a stable order.
            let exceptions: Vec<(u16, usize)> = self.states.states[i]
                .transitions
                .exceptions
                .iter()
                .map(|(k, v)| (*k, *v))
                .collect();
            let sig = (
                self.states.states[i].weight.clone(),
                self.states.states[i].final_weight.clone(),
                default,
                exceptions,
            );
        ...
        for r in unique.iter().cloned() {
            ...
            new_states.push(self.states.states[r].clone());
        }
        ...
        self.cfg.start_state = *repr_to_new.get(&repr[self.cfg.start_state]).unwrap();
        self.states.states = new_states;
        true
    }
}
...
impl Display for NWA {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "NWA (start: {})", self.cfg.start_state)?;
...
impl Display for DWA {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "DWA (start: {})", self.cfg.start_state)?;
        for (id, state) in self.states.states.iter().enumerate() {
            writeln!(f, "  State {}:", id)?;
...
impl JSONConvertible for DWA {
    fn to_json(&self) -> JSONNode {
        ...
        obj.insert("start_state".to_string(), self.cfg.start_state.to_json());
        JSONNode::Object(obj)
    }
