...
impl NWA {
    ...
    /// Depth-limited (L-truncated) determinization.
    ///
    /// Builds a deterministic, epsilon-free automaton (DWA) that is behaviorally
    /// equivalent to this NWA up to depth `level` (i.e., for all input stacks
    /// of length ≤ level). Processing such stacks requires at most `level` transitions.
    ///
    /// The construction uses weighted subset propagation with epsilon-closures,
    /// and interns states by their full depth-`level` behavioral signature:
    /// - Base: signature is (state.weight, final_weight), with `state.weight` the join
    ///   of path-weights in the macro-state composition, and `final_weight` the join
    ///   of (path_w ∧ final_w) across members of the epsilon-closed composition.
    /// - Step: extends the signature with (for default and per-exception symbols):
    ///   the pair (edge-weight, signature at depth-1 of the next macro-state).
    ///   Exceptions equal to default are dropped (U16Map-normalized).
    ///
    /// Returns:
    /// - The truncated DWA.
    /// - A map from new DWA state ID to the set of original NWA state IDs merged into it.
    pub fn simplify_to_level(&self, level: usize) -> (DWA, BTreeMap<StateID, BTreeSet<StateID>>) {
        let states = &self.states;
        let has_eps = states.0.iter().any(|s| !s.epsilon_transitions.is_empty());

        // Canonicalize a composition map (state -> weight) into a sorted Vec.
        let canon_vec = |m: &BTreeMap<StateID, Weight>| -> Vec<(StateID, Weight)> {
            m.iter().filter(|(_, w)| !w.is_empty()).map(|(k, v)| (*k, v.clone())).collect()
        };

        // Compute epsilon-closure of a raw composition.
        let eps_close = |raw: BTreeMap<StateID, Weight>| -> BTreeMap<StateID, Weight> {
            states.epsilon_closure_with_flag(raw, has_eps, 0)
        };

        // Aggregate: union of weights in a composition.
        let agg_weight = |comp: &[(StateID, Weight)]| -> Weight {
        ...
        // Aggregate final weight at position 0 for a composition (epsilon-closed):
        // ⋃_{(v, wv)} (wv ∧ final(v)).
        let agg_final = |comp: &[(StateID, Weight)]| -> Option<Weight> {
            let mut acc: Option<Weight> = None;
            for (v, wv) in comp {
                if let Some(fv) = &states[*v].final_weight {
                    let x = wv & fv;
                    if !x.is_empty() {
        ...
        // Collect the set of exception characters present in any member of a composition.
        let collect_exc_chars = |comp: &[(StateID, Weight)]| -> BTreeSet<u16> {
            let mut s = BTreeSet::new();
            for (u, _) in comp {
                for &ch in states[*u].transitions.exceptions.keys() {
                    s.insert(ch);
                }
            }
            s
        ...
        // Compute the successor (default) composition and the aggregate default edge-weight
        // from a composition comp. The returned composition is epsilon-closed.
        let next_default = |comp: &[(StateID, Weight)]| -> (Weight, Vec<(StateID, Weight)>) {
            let mut next_raw: BTreeMap<StateID, Weight> = BTreeMap::new();
            let mut edge_weight = Weight::zeros();
            for (u, wu) in comp {
                if let Some(defs) = states[*u].transitions.default.as_ref() {
                    for (t, we) in defs {
                        let w2 = wu & we;
                        if !w2.is_empty() {
                            next_raw.entry(*t).or_insert_with(Weight::zeros).bitor_assign(&w2);
                            edge_weight |= &w2; // aggregate before epsilon-closure
                        }
                    }
                }
            }
            let next = eps_close(next_raw);
            (edge_weight, canon_vec(&next))
        };

        // Compute the successor (exception for `ch`) composition and aggregate edge-weight.
        let next_exception = |comp: &[(StateID, Weight)], ch: u16| -> (Weight, Vec<(StateID, Weight)>) {
            let mut next_raw: BTreeMap<StateID, Weight> = BTreeMap::new();
            let mut edge_weight = Weight::zeros();
            for (u, wu) in comp {
                if let Some(exs) = states[*u].transitions.exceptions.get(&ch) {
                    for (t, we) in exs {
                        let w2 = wu & we;
                        if !w2.is_empty() {
                            next_raw.entry(*t).or_insert_with(Weight::zeros).bitor_assign(&w2);
        ...
        // Interning key for a depth-d state: captures full L-depth behavior (weight, finals, and resolved child IDs).
        #[derive(Clone, Hash, Eq, PartialEq)]
        struct InternKey {
            depth: usize,
            state_weight: Weight,
            final_weight: Option<Weight>,
            def: Option<(Weight, StateID)>,
            exc: Vec<(u16, (Weight, StateID))>,
        }

        // Storage built on the fly via recursion from the (depth=level, start composition).
        let mut intern: HashMap<InternKey, StateID> = HashMap::new();
        // Memo from (depth, composition) -> DWA state ID to avoid recomputing along the same (comp, depth).
        let mut memo: HashMap<(usize, Vec<(StateID, Weight)>), StateID> = HashMap::new();
        let mut dwa_states = DWAStates::default();
        let mut id_to_orig: BTreeMap<StateID, BTreeSet<StateID>> = BTreeMap::new();

        // The builder recursively materializes states to a maximum remaining depth `d`.
        let mut build_state = |comp_key: Vec<(StateID, Weight)>, mut d: usize,
                               intern: &mut HashMap<InternKey, StateID>,
                               memo: &mut HashMap<(usize, Vec<(StateID, Weight)>), StateID>,
                               dwa_states: &mut DWAStates,
                               id_to_orig: &mut BTreeMap<StateID, BTreeSet<StateID>>|
         -> StateID {
            // Inner recursive function to allow self-call.
            fn go(
                comp_key: Vec<(StateID, Weight)>,
                d: usize,
                states: &NWAStates,
                has_eps: bool,
                collect_exc_chars: &dyn Fn(&[(StateID, Weight)]) -> BTreeSet<u16>,
                next_default: &dyn Fn(&[(StateID, Weight)]) -> (Weight, Vec<(StateID, Weight)>),
                next_exception: &dyn Fn(&[(StateID, Weight)], u16) -> (Weight, Vec<(StateID, Weight)>),
                agg_weight: &dyn Fn(&[(StateID, Weight)]) -> Weight,
                agg_final: &dyn Fn(&[(StateID, Weight)]) -> Option<Weight>,
                intern: &mut HashMap<InternKey, StateID>,
                memo: &mut HashMap<(usize, Vec<(StateID, Weight)>), StateID>,
                dwa_states: &mut DWAStates,
                id_to_orig: &mut BTreeMap<StateID, BTreeSet<StateID>>,
            ) -> StateID {
                if let Some(&id) = memo.get(&(d, comp_key.clone())) {
                    return id;
                }
                let state_w = agg_weight(&comp_key);
                let final_w = agg_final(&comp_key);

                // If no transitions allowed (depth 0), key has no outgoing edges.
                if d == 0 {
                    let key = InternKey { depth: d, state_weight: state_w.clone(), final_weight: final_w.clone(), def: None, exc: Vec::new() };
                    if let Some(&id) = intern.get(&key) {
                        // Merge origin set
                        let entry = id_to_orig.entry(id).or_insert_with(BTreeSet::new);
                        for (u, _) in &comp_key { entry.insert(*u); }
                        memo.insert((d, comp_key), id);
                        return id;
                    }
                    let id = dwa_states.0.len();
                    let mut st = DWAState::default();
                    st.weight = state_w;
                    st.final_weight = final_w;
                    dwa_states.0.push(st);
                ...
                // Depth > 0: compute default and exception successors (depth-1).
                let (def_edge_w, def_next_comp) = next_default(&comp_key);
                let def_id_opt = if !def_next_comp.is_empty() {
                    Some(go(
                        def_next_comp.clone(),
                        d - 1,
                        states,
                ...
                // Build normalized exception targets; drop ones equal to default (both target and weight).
                let mut exc_vec: Vec<(u16, (Weight, StateID))> = Vec::new();
                let exc_chars = collect_exc_chars(&comp_key);
                for ch in exc_chars {
                    let (ex_edge_w, ex_next_comp) = next_exception(&comp_key, ch);
                    if ex_next_comp.is_empty() {
                        continue;
                    }
                    let ex_id = go(
                        ex_next_comp.clone(),
                    ...
                    // Normalize: skip exceptions identical to default edge (target + edge-weight).
                    let same_as_default = if let Some(did) = def_id_opt {
                        did == ex_id && def_edge_w == ex_edge_w
                    } else {
                        false
                    };
                    if !same_as_default {
                        exc_vec.push((ch, (ex_edge_w, ex_id)));
                    }
                }
                exc_vec.sort_by_key(|(ch, _)| *ch);

                let def_norm = def_id_opt.map(|did| (def_edge_w.clone(), did));
                let key = InternKey {
                    depth: d,
                    state_weight: state_w.clone(),
                    final_weight: final_w.clone(),
                    def: def_norm.clone(),
                    exc: exc_vec.clone(),
                };
                if let Some(&id) = intern.get(&key) {
                ...
                // Materialize new DWA state.
                let id = dwa_states.0.len();
                let mut st = DWAState::default();
                st.weight = state_w;
                st.final_weight = final_w;
                if let Some((w, did)) = def_norm.clone() {
                    st.transitions.default = Some(did);
                    st.trans_weight_default = Some(w);
                }
                for (ch, (w, tid)) in exc_vec.into_iter() {
                    st.transitions.exceptions.insert(ch, tid);
            ...
            go(comp_key, d, states, has_eps, &collect_exc_chars, &next_default, &next_exception, &agg_weight, &agg_final, intern, memo, dwa_states, id_to_orig)
        };

        // Start composition = epsilon-closure of the original start set with ALL weight.
        let mut start_raw: BTreeMap<StateID, Weight> = BTreeMap::new();
        for &s in &self.body.start_states {
            start_raw.insert(s, Weight::all());
        }
        let start_comp = {
            let closed = states.epsilon_closure_with_flag(start_raw, has_eps, 0);
            let mut v = Vec::new();
            for (k, w) in closed {
                if !w.is_empty() {
                    v.push((k, w));
        ...
        let start_id = build_state(start_comp, level, &mut intern, &mut memo, &mut dwa_states, &mut id_to_orig);
        let dwa = DWA { states: dwa_states, body: DWABody { start_state: start_id } };
        (dwa, id_to_orig)
    }
