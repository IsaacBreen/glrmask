...
pub trait Tokenizer: Sized {
    ...
    fn execute_all_from_state(
        ...
        let mut queue: VecDeque<(usize, Option<usize>, Arc<Mutex<TrieNode<GroupID, (BTreeMap<LLMTokenID, Option<StateID>>, BTreeMap<TokenID, BitVec>, Option<BitVec>)>>>)> = VecDeque::new();
        let root = state_map_root_arc.clone();

        // Initialize the queue with the starting state
        queue.push_back((0, Some(state), root));

        while let Some((position, maybe_state, node)) = queue.pop_front() {
            ...
            for token in &execute_result.matches {
                ...
                let mut node_guard = node.try_lock().unwrap();
                let child = node_guard.get(&token.id);
                let child = match child {
                    Some(child) => {
                        let mut child_guard = child.try_lock().unwrap();
                        if child_guard.num_parents > 1 {
                ...
                queue.push_back((new_position, new_state, child));
            }
        }
    }
}
