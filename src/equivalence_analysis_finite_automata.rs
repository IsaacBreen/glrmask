...
pub fn find_equivalence_classes(
    ...
    let p1 = normalize(&impl1);
    let p2 = normalize(&impl2);
    let p3 = normalize(&impl3);

    println!("Equivalence Analysis Results:");
    println!("Impl1: {} classes", p1.len());
    println!("Impl2: {} classes", p2.len());
    println!("Impl3: {} classes", p3.len());

    if p1 != p2 {
        panic!("Mismatch between Impl1 and Impl2!\nImpl1: {:?}\nImpl2: {:?}", p1, p2);
    }
    if p2 != p3 {
        panic!("Mismatch between Impl2 and Impl3!\nImpl2: {:?}\nImpl3: {:?}", p2, p3);
    }
