...
use crate::precompute4::utils::decode_symbol_i16;
    ...
    for (&from_sid, row) in &parser.table {
        ...
        for &to_sid in row
            ...
            .filter_map(|action| match action {
                crate::glr::table::Stage7ShiftsAndReducesLookaheadValue::Shift(sid) => {
                    Some(sid)
                }
                crate::glr::table::Stage7ShiftsAndReducesLookaheadValue::Split { shift, .. } => {
                    shift.as_ref()
                }
                _ => None,
            })
...
///
/// This is currently unused in the main `precompute4` pipeline but kept here as an optional
/// optimization pass.
pub(crate) fn propagate_and_prune_labels(parser: &GLRParser, nwa: &mut NWA) {
    ...
    let mut state_info: Vec<BTreeMap<ParserStateID, Weight>> =
        vec![BTreeMap::new(); nwa.states.len()];
        ...
        for (target_state, w) in targets {
            ...
            for (label, _) in &nwa.states[s_init].transitions {
                if let Ok((is_pos, p_id)) = decode_symbol_i16(*label) {
                    if is_pos {
                        let entry =
                            state_info[s_init].entry(p_id).or_insert_with(Weight::zeros);
                        *entry |= w;
                    }
                }
            }
    ...
    while let Some(u) = worklist.pop_front() {
        ...
        for (l, targets) in &nwa.states[u].transitions {
            for (v, w_uv) in targets {
                ...
                let process_propagation = |pw: &Weight,
                                           followers: &StateIDBV,
                                           state_info: &mut Vec<BTreeMap<ParserStateID, Weight>>|
                 -> bool {
                    let mut any_change = false;
                    for follower_id_val in followers.iter() {
                        let follower_id = ParserStateID(follower_id_val);
                        let entry =
                            state_info[v].entry(follower_id).or_insert_with(Weight::zeros);
                ...
                match decode_symbol_i16(*l) {
                    ...
                    Err(_) => {
                        panic!(
                            "Unexpected unknown non-default label during label propagation: {}",
                            l
                        );
                    }
                }
    ...
    crate::debug!(
        4,
        "Label propagation fixpoint took: {:?}",
        now.elapsed()
    );
    ...
    for u in 0..nwa.states.len() {
        ...
        for (l, targets) in state.transitions.iter_mut() {
            let valid_incoming_weight = match decode_symbol_i16(*l) {
                ...
                Err(_) => {
                    panic!(
                        "Unexpected unknown non-default label during pruning: {}",
                        l
                    );
                }
            };
    ...
    crate::debug!(
        4,
        "Pruning pass changed {} weights and took: {:?}",
        changed_count,
        now_prune.elapsed()
    );
}
