...
pub fn precompute4(parser: &GLRParser, precomputed1: &BTreeMap<TokenizerStateID, PrecomputeNode1Index>, trie1_god: &Trie1GodWrapper) -> Precomputed4 {
    ...
    let mut step_time = Duration::new(0, 0);
    let mut merge_time = Duration::new(0, 0);
    let mut copy_time = Duration::new(0, 0);
    let mut combine_time = Duration::new(0, 0);
    ...
    Trie::special_map_grouped(
        ...
        |current_aug_body, edge_terminal_opt, dest_map| {
            ...
            let mut last_step_copy_time = Duration::new(0, 0);
            let mut last_step_combine_time = Duration::new(0, 0);
            ...
            for (dest_idx, llm_token_bv) in dest_map.iter() {
                ...
                let elapsed = combine_start.elapsed();
                combine_time += elapsed;
                last_step_combine_time += elapsed;
            ...
            if step_counter % 10 == 0 {
                println!(
                    "Step {}: total_step={:?}, total_merge={:?}, total_copy={:?}, total_combine={:?}, last_step={{copy: {:?}, combine: {:?}}}",
                    step_counter,
                    step_time,
                    merge_time,
                    copy_time,
    ...
    println!("precompute4 total time: {:?}", total_start.elapsed());
    precomputed4
}
