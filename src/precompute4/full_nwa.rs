...
pub fn precompute4(parser: &GLRParser, precomputed1: &BTreeMap<TokenizerStateID, PrecomputeNode1Index>, trie1_god: &Trie1GodWrapper) -> Precomputed4 {
    ...
    let options = crate::datastructures::trie::PrettyPrintOptions::default().omit_depth();
    crate::debug!(5, "\n--- Reversed Trie1 ---\n{}", Trie::pretty_print_with_options(&reversed_trie1_god, &[reversed_trie_root], &options));

    // 3. Traverse the reversed trie using per-automaton composition (no shared global state pool).
    //
    // Build initial AugmentedNwa: a single final start state with an empty stack in end_map.
    let mut initial_nwa = WaNWA::new();
    let initial_state = initial_nwa.rest.start_state;
    initial_nwa.states.set_final_weight(initial_state, WaWeight::all());
    let initial_aug_nwa = AugmentedNwa {
        states: initial_nwa.states.clone(),
        rest: AugmentedNwaRest {
            nwa: initial_nwa.rest.clone(),
    ...
    Trie::special_map_grouped(
        ...
        initial_values,
        // step function: carry full AugmentedNwa; combine left edge NWA into right (current) NWA.
        |current_aug_nwa: &AugmentedNwa, edge_terminal_opt: &Option<TerminalID>, dest_map| {
            ...
            if edge_terminal_opt.is_some() && *edge_terminal_opt != parser.ignore_terminal_id {
                ...
                // Epsilon-like edge in grammar trie. Use the ignore terminal NWA on the left.
                aug_nwa = &ignore_nwa;
            }
            ...
            for (dest_idx, llm_token_bv) in dest_map.iter() {
                ...
                new_aug_nwa.combine_right_into(current_aug_nwa, &weight).expect("Combine failed");
                let dbg_combined = new_aug_nwa.clone();
        ...
        // merge function: union two AugmentedNwa values into the first.
        |aug_nwa1: &mut AugmentedNwa, aug_nwa2: AugmentedNwa| {
    ...
    for (sid, aug_nwa) in final_nwas {
        let nwa = WaNWA { states: aug_nwa.states.clone(), rest: aug_nwa.rest.nwa.clone() };
