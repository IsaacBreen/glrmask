...
#[inline]
fn _label_to_fst_label(label: Label) -> u32 { (label as isize - Label::MIN as isize + 1) as u32 }

#[inline]
fn _fst_label_to_label(label: u32) -> Label { (label as isize + Label::MIN as isize - 1) as Label }

#[inline]
fn fst_label_to_label(label: u32) -> Label {
    assert_ne!(label, 0);
    _fst_label_to_label(label)
}
...
fn intern_weight(weight: Weight) -> Arc<Weight> {
    ...
    if let Some(w) = interner.get(&weight) { return w.clone(); }
...
impl Semiring for BitsetWeight {
    ...
    fn plus_assign<P: Borrow<Self>>(&mut self, rhs: P) -> Result<()> {
        let new_weight = &*self.0 | &*rhs.borrow().0; self.0 = intern_weight(new_weight); Ok(())
    }
    ...
    fn properties() -> SemiringProperties {
        SemiringProperties::LEFT_SEMIRING | SemiringProperties::RIGHT_SEMIRING | SemiringProperties::COMMUTATIVE | SemiringProperties::IDEMPOTENT | SemiringProperties::PATH
    }
}
...
impl WeaklyDivisibleSemiring for BitsetWeight {
    fn divide_assign(&mut self, rhs: &Self, _divide_type: DivideType) -> Result<()> {
        let new_weight = &*self.0 | &!&*rhs.0; self.0 = intern_weight(new_weight); Ok(())
    }
}
...
impl SerializableSemiring for BitsetWeight {
    ...
    fn parse_binary(i: &[u8]) -> IResult<&[u8], Self, NomCustomError<&[u8]>> {
        ...
        Ok((i, BitsetWeight(intern_weight(Weight::from_rsb(RangeSetBlaze::from_iter(ranges))))))
    }
    ...
    fn parse_text(i: &str) -> IResult<&str, Self> {
        ...
        map_res(nom::combinator::rest, |s: &str| -> Result<BitsetWeight, _> {
            serde_json::from_str::<Weight>(s).map(|w| BitsetWeight(intern_weight(w))).map_err(|e| e.to_string())
        })(i)
    }
}
...
pub fn nwa_to_vector_fst(nwa: &NWA) -> VectorFst<BitsetWeight> {
    ...
    for i in 0..nwa.states.len() { state_map.insert(i, fst.add_state()); }

    let fst_start = fst.add_state();
    fst.set_start(fst_start).unwrap();
    for &start_id in &nwa.body.start_states {
        if let Some(&mapped) = state_map.get(&start_id) {
            fst.add_tr(fst_start, Tr::new(EPS_LABEL, EPS_LABEL, BitsetWeight::one(), mapped)).unwrap();
        }
    }
    ...
    for (i, nwa_state) in nwa.states.0.iter().enumerate() {
        ...
        if let Some(w) = &nwa_state.final_weight {
            if !w.is_empty() { fst.set_final(fst_state_id, BitsetWeight::new(w.clone())).unwrap(); }
        }
        for (label, targets) in &nwa_state.transitions {
            for (target, weight) in targets {
                if !weight.is_empty() {
                    fst.add_tr(fst_state_id, Tr::new(label_to_fst_label(*label), label_to_fst_label(*label), BitsetWeight::new(weight.clone()), state_map[target])).unwrap();
                }
            }
        }
        for (target, weight) in &nwa_state.epsilons {
            if !weight.is_empty() {
                fst.add_tr(fst_state_id, Tr::new(EPS_LABEL, EPS_LABEL, BitsetWeight::new(weight.clone()), state_map[target])).unwrap();
            }
        }
    }
    fst
...
pub fn vector_fst_to_dwa(fst: &VectorFst<BitsetWeight>) -> DWA {
    let fst_start = match fst.start() { Some(s) => s, None => return DWA::new(), };
    let mut dwa = DWA::new();
    dwa.states.0.clear();
    let mut state_map = HashMap::<StateId, StateID>::new();

    for i in 0..fst.num_states() { state_map.insert(i as StateId, dwa.add_state()); }
    dwa.body.start_state = state_map[&fst_start];

    for i in 0..fst.num_states() {
        let fst_state_id = i as StateId;
        if !state_map.contains_key(&fst_state_id) { continue; }
        let dwa_state_id = state_map[&fst_state_id];

        if let Some(w) = fst.final_weight(fst_state_id).unwrap() {
            if !w.0.is_empty() { dwa.set_final_weight(dwa_state_id, w.value().clone()).unwrap(); }
        }
        for tr in fst.get_trs(fst_state_id).unwrap().trs() {
            if !tr.weight.0.is_empty() && state_map.contains_key(&tr.nextstate) {
                let _ = dwa.add_transition(dwa_state_id, fst_label_to_label(tr.ilabel), state_map[&tr.nextstate], tr.weight.value().clone());
            }
        }
    }
    dwa
...
pub fn vector_fst_to_nwa(fst: &VectorFst<BitsetWeight>) -> NWA {
    ...
    for i in 0..fst.num_states() { state_map.insert(i as StateId, nwa.add_state()); }
    nwa.body.start_states = if let Some(s) = fst.start() { vec![state_map[&s]] } else { vec![] };

    for i in 0..fst.num_states() {
        let fst_state_id = i as StateId;
        let nwa_state_id = state_map[&fst_state_id];
        if let Some(w) = fst.final_weight(fst_state_id).unwrap() {
            if !w.0.is_empty() { nwa.states[nwa_state_id].final_weight = Some(w.value().clone()); }
        }
        for tr in fst.get_trs(fst_state_id).unwrap().trs() {
            if !tr.weight.0.is_empty() {
                let target = state_map[&tr.nextstate];
                let w = tr.weight.value().clone();
                if tr.ilabel == EPS_LABEL { nwa.add_epsilon(nwa_state_id, target, w); }
                else { nwa.add_transition(nwa_state_id, fst_label_to_label(tr.ilabel), target, w).unwrap(); }
            }
        }
    }
    nwa
