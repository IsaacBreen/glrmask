...
pub struct AugmentedNwaBody {
    pub start_states: BTreeSet<WaStateID>,
...
pub fn build_augmented_nwa_for_ignore_terminal() -> AugmentedNwa {
    ...
    AugmentedNwa {
        ...
        body: AugmentedNwaBody {
            start_states: BTreeSet::from([start_state]),
...
pub fn build_augmented_nwa_from_characterization(
    ...
    let mut body = AugmentedNwaBody {
        start_states: BTreeSet::from([start]),
        nt_nodes: BTreeMap::new(),
...
impl AugmentedNwaBody {
    pub fn remap_states(&mut self, mapping: &[WaStateID]) {
        self.start_states = self.start_states.iter().map(|s| mapping[*s]).collect();
    ...
    pub fn process_stack(
        ...
        Ok(states.process_stack_u16_from_starts(&self.start_states, &encoded))
    }
    ...
    pub fn combine_right_into_on_shared(
        ...
        for (left_end_state, stacks) in left_end_snapshot {
            for left_stack in stacks {
                ...
                let stops = states.process_stack_u16_from_starts(&right.start_states, &encoded);
    ...
    pub fn union_with_on_shared(
        ...
        left.start_states.extend(&right.start_states);
    }
}
...
impl AugmentedNwa {
    ...
    pub fn determinize(&self) -> DWA {
        WaNWA::determinize_components_from_starts(&self.states, &self.body.start_states)
    }
}
...
impl Display for AugmentedNwa {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(f, "  Underlying NWA (starts: {:?}):", self.body.start_states)?;
...
mod tests {
    ...
    fn test_combine_with_ignore_on_right() {
        ...
        rhs.states.set_final_weight(*rhs.body.start_states.iter().next().unwrap(), WaWeight::all());
        let weight = WaWeight::all();
    ...
    fn build_terminal1_nwa() -> AugmentedNwa {
        ...
        AugmentedNwa {
            states,
            body: AugmentedNwaBody { start_states: BTreeSet::from([start_state]), nt_nodes, end_map },
        }
    }
    ...
    fn build_nwa_from_prompt_right() -> AugmentedNwa {
        ...
        AugmentedNwa {
            states,
            body: AugmentedNwaBody { start_states: BTreeSet::from([start_state]), nt_nodes: BTreeMap::new(), end_map },
        }
    }
