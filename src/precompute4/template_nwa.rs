...
pub enum FullDWABuildError {
    /// A parser state id was out of the expected range.
    ParserStateIdOutOfRange { state_id: ParserStateID },

    /// Error reported by the underlying NWA builder.
...
pub(crate) fn build_template_nwa_from_characterization(
    ...
    for &(initial_state, len, nt) in &bb.initial_reduces {
        let pos_initial = utils::encode_symbol_i16(initial_state)?;
        let target_nt_state = *nt_nodes
            .get(&nt)
            .expect("nt_node must exist for initial_reduce");
        ...
        for i in 0..len {
            ...
            nwa
                .states
                .add_transition(from, DEFAULT_TRANSITION_SYMBOL, to, w_all.clone())?;
            from = to;
        }
    }
    ...
    for (nt, rc) in &bb.reduce_characterizations {
        let src_nt_state = *nt_nodes
            .get(nt)
            .expect("nt_node must exist for reduce_char");

        for &(revealed_state, len, reduce_nt) in &rc.reveal_and_rereduces {
            let pos_revealed = utils::encode_symbol_i16(revealed_state)?;
            let dst_nt_state = *nt_nodes
                .get(&reduce_nt)
                .expect("dst nt_node must exist");
...
pub(crate) fn build_template_dwas(
    parser: &GLRParser,
