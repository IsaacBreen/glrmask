            ...
            |precomputed_node_data, glr_s| {
                ...
                if precomputed_node_data.value.end {
                    ...
                    for gss_root in glr_s.active_state.stack.get_roots() {
                        ...
                        crate::debug!(4, "Trie2: Inserting end edge into Trie2 node with active LLM tokens: {:?} into Trie2 nodes: {:?}", active_llm_tokens_for_root, gss_root_acc.trie2_nodes);
...
mod tests {
    use super::*;
    use crate::datastructures::gss::{Acc, GSSNode, PrecomputedNodeContents, PrecomputeNode2};
    ...
    #[test]
    fn test_gss_merge_prefix_path_bug() {
        // This test reproduces a bug where merging two GSS structures,
        // where one is a structural prefix of the other, results in an
        // incorrect merge that loses one of the paths.

        // Setup dummy Trie2 nodes to be stored in the GSS Acc.
        let trie2_node_a = Arc::new(RwLock::new(Trie::new(PrecomputedNodeContents::no_end())));
        let trie2_node_b = Arc::new(RwLock::new(Trie::new(PrecomputedNodeContents::no_end())));

        // GSS A represents a stack: root_a -> node1_a
        // node1_a is the top of the stack.
        let root_a = GSSNode::new_fresh();
        let node1_a = root_a.push(ParseStateEdgeContent { state_id: StateID(1) });
        let mut acc_a = Acc::new_fresh();
        acc_a.trie2_nodes.insert(ArcPtrWrapper::new(trie2_node_a.clone()));
        // Associate trie2_node_a with the top of stack A.
        Arc::make_mut(&mut node1_a.acc).merge_with(&acc_a);

        // GSS B represents a stack: root_b -> node1_b -> node2_b
        // node2_b is the top of the stack.
        let root_b = GSSNode::new_fresh();
        let node1_b = root_b.push(ParseStateEdgeContent { state_id: StateID(1) });
        let node2_b = node1_b.push(ParseStateEdgeContent { state_id: StateID(0) });
        let mut acc_b = Acc::new_fresh();
        acc_b.trie2_nodes.insert(ArcPtrWrapper::new(trie2_node_b.clone()));
        // Associate trie2_node_b with the top of stack B.
        Arc::make_mut(&mut node2_b.acc).merge_with(&acc_b);

        // Note on structural identity:
        // `root_a` and `root_b` are structurally identical (leaf nodes).
        // `node1_a` and `node1_b` are structurally identical (one predecessor, a root, via edge 1).

        // Merge the tops of the two stacks. This simulates merging two GLR parser states.
        let merged = GSSNode::merge_many_with_depth(usize::MAX, vec![Arc::new(node1_a), Arc::new(node2_b)]);

        // --- Assertions for the CORRECT merge result ---
        // The merged node should represent a fork, with two distinct predecessor paths
        // corresponding to the two original stacks.

        // 1. The Acc of the merged node should contain data from both original nodes.
        let merged_acc = merged.resolved_acc();
        assert_eq!(merged_acc.trie2_nodes.len(), 2);
        assert!(merged_acc.trie2_nodes.contains(&ArcPtrWrapper::new(trie2_node_a)));
        assert!(merged_acc.trie2_nodes.contains(&ArcPtrWrapper::new(trie2_node_b)));

        // 2. The merged node should have two distinct predecessor edges.
        let merged_predecessors = merged.predecessors.read().unwrap();
        assert_eq!(merged_predecessors.len(), 2, "Merged node should have 2 distinct edge types for predecessors. If this fails, the prefix path was likely absorbed incorrectly.");

        // 3. Check Path 1 (from GSS A)
        let preds_edge1 = merged_predecessors.get(&ParseStateEdgeContent { state_id: StateID(1) }).expect("Should have predecessor path for edge 1");
        assert_eq!(preds_edge1.len(), 1, "Should be one predecessor for edge 1");
        let pred_root = preds_edge1.iter().next().unwrap();
        assert!(pred_root.predecessors.read().unwrap().is_empty(), "Predecessor for edge 1 should be a root node");

        // 4. Check Path 2 (from GSS B)
        let preds_edge0 = merged_predecessors.get(&ParseStateEdgeContent { state_id: StateID(0) }).expect("Should have predecessor path for edge 0");
        assert_eq!(preds_edge0.len(), 1, "Should be one predecessor for edge 0");
        let pred_node1 = preds_edge0.iter().next().unwrap();
        let pred_node1_predecessors = pred_node1.predecessors.read().unwrap();
        let pred2_root = pred_node1_predecessors.get(&ParseStateEdgeContent { state_id: StateID(1) }).unwrap().iter().next().unwrap();

        // 5. The two paths should converge on the same memoized root node instance.
        assert!(Arc::ptr_eq(pred_root, pred2_root), "Paths should converge on the same root object due to memoization");
    }
}
