...
use crate::datastructures::hybrid_bitset::{HybridBitset, HybridL2Bitset};
...
use crate::datastructures::trie::{Arena, God, GodWrapper};
...
impl GrammarConstraint {
    ...
    pub fn new_with_config(
        ...
        let mut gc = Self {
            ...
            trie2_god,
            trie3_god,
            post_commit_allow_check_mode: TerminalAllowanceCheckMode::StepProbe,
        };
    ...
    pub fn precompute2(
        ...
        // This is now a stub. The real work is in precompute3.
    ...
    pub fn precompute3(
        ...
        let base_gss = Arc::new(gss_leaf.push(ParseStateEdgeContent { state_id: parser.everything_state_id }));
        let base_glr_state = parser.init_glr_parser_from_stack(base_gss).with_god(trie3_god.clone());

        let mut initial_values_for_map = Vec::new();
        for (tokenizer_state_id, trie1_root) in precomputed {
            let (cloned_trie3_root, trie3_map) = constraint_precompute2_utils::clone_trie3_graph(&base_trie3_root, &trie3_god);
            let cloned_gss = crate::datastructures::gss::deep_clone_gss_with_stored_trie_map(&base_glr_state.active_state.stack, &trie3_map);
        ...
        Trie::special_map_grouped(
            ...
            |precomputed_node_data, glr_s| {
                crate::datastructures::gss::merge_stored_trie_nodes(&mut glr_s.active_state.stack, &mut HashMap::new(), glr_s.active_state.stored_trie_god.as_ref().unwrap());
                ...
                if precomputed_node_data.value.end {
                    for (last_edge, gss_root_accs) in get_roots([glr_s.active_state.stack.as_ref()]) {
                        for gss_root_acc in gss_root_accs {
                            let tokens_to_push = gss_root_acc.union_llm_tokens();
                            if tokens_to_push.is_empty() {
                                continue;
                            }
                            for src_wr in gss_root_acc.stored_trie_nodes().iter() {
                                let mut state_id_bv = StateIDBV::zeros();
                                state_id_bv.insert(last_edge.state_id.0);
                                EdgeInserter::new(
                                    glr_s.active_state.stored_trie_god.as_ref().unwrap(),
                                    src_wr.as_arc().clone(),
                                    (0, tokens_to_push.clone()),
                                    state_id_bv,
                                    |e, n| *e |= n,
                                    |_, _| {},
                                    |_, _| {},
                                )
                                .try_destination(trie3_end)
                                .expect("Failed to insert end edge into Trie node");
                            }
                        }
                    }
                }
        ...
        let roots3_final: Vec<_> = precomputed3.values().cloned().collect();
        Trie::gc(&trie3_god, &roots3_final);
        Trie::recompute_all_max_depths(&trie3_god, &roots3_final);
