...
impl GrammarConstraint {
    ...
    fn build_with_config(
        ...
        if std::env::var("TEST_EPSILON_EXPLOSION").is_ok() {
            ...
            crate::debug!(1, "EPSILON EXPERIMENT: Original start state has {} outgoing transitions", start_out_degree);
            
            let first_hop_states: std::collections::HashSet<_> = terminal_dwa.states[start_id]
                .transitions.values().cloned().collect();
            crate::debug!(1, "EPSILON EXPERIMENT: {} unique first-hop states from start", first_hop_states.len());
            
            let mut first_hop_out_trans = 0;
            for &s in &first_hop_states {
                first_hop_out_trans += terminal_dwa.states[s].transitions.len();
            }
            crate::debug!(1, "EPSILON EXPERIMENT: {} total outgoing transitions from first-hop states", first_hop_out_trans);
            ...
            crate::debug!(1, "EPSILON EXPERIMENT: Replaced with {} epsilon transitions", num_eps);
            
            crate::debug!(1, "EPSILON EXPERIMENT: Starting determinize...");
            ...
            crate::debug!(1, "EPSILON EXPERIMENT: After determinize: {} states, {} trans", det_states, det_trans);
            ...
            crate::debug!(1, "EPSILON EXPERIMENT: Modified start has {} outgoing transitions", mod_start_out);
            
            crate::debug!(1, "EPSILON EXPERIMENT (simplify): {} states, {} trans", mod_states, mod_trans);
            crate::debug!(1, "EPSILON EXPERIMENT: Original {} states/{} trans -> Modified {} states/{} trans",
                orig_states, orig_trans, mod_states, mod_trans);
            crate::debug!(1, "EPSILON EXPERIMENT: State factor {:.2}x, Trans factor {:.2}x",
                mod_states as f64 / orig_states as f64, mod_trans as f64 / orig_trans as f64);
            
            if mod_trans > orig_trans {
                crate::debug!(1, "EPSILON EXPERIMENT RESULT: EXPLOSION! ({:.2}x expansion)", 
                    mod_trans as f64 / orig_trans as f64);
            } else {
                crate::debug!(1, "EPSILON EXPERIMENT RESULT: No explosion");
            }
        }
