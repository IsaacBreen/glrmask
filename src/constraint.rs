...
fn update_live_tokens_trie2(roots: &BTreeMap<TokenizerStateID, Arc<RwLock<PrecomputeNode2>>>) {
    let mut all_nodes = Vec::new();
    ...
    while changed {
        ...
        for node_arc in all_nodes.iter().rev() {
            let mut new_live_tokens = if node_arc.read().unwrap().value.end {
            ...
            for dest_map in children_snapshot.values() {
                for (child_wrapper, edge_bv) in dest_map.iter() {
                    if let Some(child_arc) = child_wrapper.upgrade() {
                        ...
                        let child_live_tokens = &child_guard.value.live_tokens;
                        new_live_tokens |= &(edge_bv.clone() & child_live_tokens);
                    }
                }
            }

            let mut node_guard = node_arc.write().unwrap();
            if node_guard.value.live_tokens != new_live_tokens {
                node_guard.value.live_tokens = new_live_tokens;
                changed = true;
            }
        }
    }
...
fn prune_dead_paths_trie2(roots: &mut BTreeMap<TokenizerStateID, Arc<RwLock<PrecomputeNode2>>>) {
    ...
    update_live_tokens_trie2(roots);
...
impl<'r> Precomputer<'r> {
    fn update_live_tokens(&self) {
        ...
        while changed {
            changed = false;
            // Iterate in reverse topological order (from children to parents).
            // `all_nodes` is roughly in BFS order, so reversing it is a good approximation.
            for node_arc in all_nodes.iter().rev() {
                let mut new_live_tokens = if node_arc.read().unwrap().value.end {
