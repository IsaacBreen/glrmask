...
impl<'a> GrammarConstraintState<'a> {
    ...
    pub fn get_mask4(&self) -> LLMTokenBV {
        ...
        let sp = self.parent.precompute_special();

        // Group edges for efficient lookup during traversal.
        let mut edges_by_src_term: FxHashMap<(Option<NonTerminalID>, TerminalID), Vec<&SpecialPrecomputeNormalEdge>> = FxHashMap::default();
        for edge in &sp.normal_edges {
            edges_by_src_term.entry((edge.0, edge.2)).or_default().push(edge);
        }

        // Queue for the main traversal over the precompute1 trie.
        // State is (pci1_node_index, glr_parser_state).
        let mut pci1_queue: VecDeque<(PrecomputeNode1Index, GLRParserState<'a>)> = VecDeque::new();
        let mut pci1_visited: FxHashMap<(PrecomputeNode1Index, Arc<GSSNode>), ()> = FxHashMap::default();

        for (&tokenizer_state_id, glr_state) in &self.state {
            if glr_state.active_state.stack.is_empty() { continue; }
            if let Some(pci1_root) = self.parent.precomputed1.get(&tokenizer_state_id) {
        ...
        fn find_initial_stack_tops<'a>(gss_head: &Arc<GSSNode>, src_nt: Option<NonTerminalID>, parser: &'a GLRParser) -> Vec<(Arc<GSSNode>, StateID)> {
            let mut results = Vec::new();
            let mut q = vec![gss_head.clone()];
            let mut visited = HashSet::new();

            while let Some(node) = q.pop() {
                if node.is_root() || !visited.insert(node.clone()) { continue; }

                if let Some(nt_id) = src_nt {
                    if let Some(preds) = node.get_predecessors() {
                        for pred in preds {
                            if pred.is_root() { continue; }
        ...
        while let Some((pci1_idx, glr_state)) = pci1_queue.pop_front() {
            let pci1_node_guard = pci1_idx.read(&self.parent.trie1_god).unwrap();

            if pci1_node_guard.value.end {
                *final_mask_internal.borrow_mut() |= &glr_state.active_state.stack.allowed_llm_tokens();
            }
            ...
            for (grammar_token_opt, dest_map) in pci1_node_guard.children() {
                for (next_pci1_idx, edge_llm_bv) in dest_map {
                    ...
                    allow_only_llm_tokens_and_prune_arc(&mut base_glr_state.active_state.stack, edge_llm_bv, &mut HashMap::new());
                    if !base_glr_state.is_ok() { continue; }

                    let new_gss_nodes = if let Some(terminal_id) = grammar_token_opt {
                        let mut final_gss_for_terminal: Vec<Arc<GSSNode>> = Vec::new();
                        let mut sp_queue: VecDeque<(Option<NonTerminalID>, Arc<GSSNode>)> = VecDeque::new();
                        let mut sp_visited: FxHashMap<(Option<NonTerminalID>, Arc<GSSNode>), ()> = FxHashMap::default();
                        ...
                        while let Some((src_nt, gss_head)) = sp_queue.pop_front() {
                            let mut tops_by_start_state: FxHashMap<StateID, Vec<Arc<GSSNode>>> = FxHashMap::default();
                            for (top_node, start_state_id) in find_initial_stack_tops(&gss_head, src_nt, self.parent.parser) {
                                tops_by_start_state.entry(start_state_id).or_default().push(top_node);
                            }
                                            ...
                                            match dest {
                                                ...
                                                SpecialPrecomputeDest::Escape { push_states } => {
                                                    ...
                                                    for state_id in push_states {
                                                        new_gss_head = GSSNode::with_single_pred(new_gss_head, ParseStateEdgeContent { state_id: *state_id, ..Default::default() });
                                                    }
        ...
        self.parent.internal_bv_to_original_precompute1(&final_mask_internal.into_inner())
    }
