    ...
    pub fn get_mask3(&self) -> LLMTokenBV {
        ...
        for (&tokenizer_state_id, glr_state) in &self.state {
            ...
            if let Some(precomputed_trie_root_arc) = self.parent.precomputed3.get(&tokenizer_state_id) {
                ...
                crate::debug!(1, "  SEED: sid={}, root_idx={}, gss_ptr={:p}, mask={}", tokenizer_state_id.0, precomputed_trie_root_arc, glr_state.active_state.stack, allowed_bv.to_json().to_json_string());
        ...
        Trie::special_map_grouped(
            ...
            |state, (pop, llm_token_bv_from_edge), dest_map| {
                ...
                crate::debug!(1, "  - STEP: gss_ptr={:p}, allowed_bv={}", glr_s.active_state.stack, allowed_bv.to_json().to_json_string());
                crate::debug!(1, "    - Edge: pop={}, llm_bv={}", pop, llm_token_bv_from_edge.to_json().to_json_string());
                ...
                for (dest_idx, state_id_bv) in dest_map.iter() {
                    ...
                    crate::debug!(1, "      - Dest: idx={}, state_bv={}", dest_idx, state_id_bv.to_json().to_json_string());
                    ...
                    crate::debug!(1, "        - Child mask: {}", new_allowed_bv.to_json().to_json_string());

                    if new_glr_s.is_ok() && !new_allowed_bv.is_empty() {
                        crate::debug!(1, "        - Enqueue {}: CREATING gss_ptr={:p}, mask={}", dest_idx, new_glr_s.active_state.stack, new_allowed_bv.to_json().to_json_string());
            ...
            |state1, state2| {
                ...
                crate::debug!(1, "    - MERGE: gss1_ptr={:p}, mask1={} WITH gss2_ptr={:p}, mask2={}", glr_s1.active_state.stack, allowed_bv1.to_json().to_json_string(), glr_s2.active_state.stack, allowed_bv2.to_json().to_json_string());
                glr_s1.merge_with(glr_s2);
                *allowed_bv1 |= allowed_bv2;
                crate::debug!(1, "      - Merged result: gss_ptr={:p}, mask={}", glr_s1.active_state.stack, allowed_bv1.to_json().to_json_string());
            },
            // process_fn: (precomputed_node_data, final_state_for_this_path)
            |precomputed_node_data, state| {
                let (glr_s, allowed_bv) = state;
                crate::debug!(1, "  - PROCESS: node_ptr={:p}, gss_ptr={:p}, mask={}", precomputed_node_data as *const _, glr_s.active_state.stack, allowed_bv.to_json().to_json_string());
                ...
                if precomputed_node_data.value.end {
                    ...
                    crate::debug!(1, "      - final_mask before: {}", final_mask_internal.borrow().to_json().to_json_string());
                    crate::debug!(1, "      - glr_active_tokens to union: {}", final_allowed_tokens.to_json().to_json_string());
                    *final_mask_internal.borrow_mut() |= final_allowed_tokens;
                    crate::debug!(1, "      - final_mask after:  {}", final_mask_internal.borrow().to_json().to_json_string());
                }
                keep_going
            },
        );
        ...
        crate::debug!(1, "\n--- get_mask3 END ---");
        crate::debug!(1, "Final mask internal: {}", final_mask_internal.borrow().to_json().to_json_string());
        crate::debug!(4, "Final mask internal: {}", final_mask_internal.borrow().to_json().to_json_string());
        let final_mask_mapped = self.parent.internal_bv_to_original(&final_mask_internal.into_inner());
        crate::debug!(1, "Final mask mapped: {}", final_mask_mapped.to_json().to_json_string());
        crate::debug!(4, "Final mask mapped: {}", final_mask_mapped.to_json().to_json_string());
