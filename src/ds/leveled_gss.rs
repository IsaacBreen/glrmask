...
impl<T: Clone + Eq + Hash> Lower<T> {
    ...
    fn max_depth(&self) -> u32 {
        match self {
            Lower::General { max_depth, .. } | Lower::Segment { max_depth, .. } => *max_depth,
        }
    }
    ...
    fn children_contains_key(&self, key: &T) -> bool {
        match self {
            ...
            Lower::Segment { values, .. } => values.last().unwrap() == key,
        }
    }
    ...
    fn segment_top_value(&self) -> &T {
        match self {
            Lower::Segment { values, .. } => values.last().unwrap(),
    ...
    /// If len==1, returns next directly. Otherwise creates a new shorter Segment.
    ...
    fn children_keys(&self) -> SmallVec<[T; 8]> {
        match self {
            ...
            Upper::Interface(interface) => match &*interface.inner {
                Lower::Segment { values, .. } => smallvec![values.last().unwrap().clone()],
                Lower::General { children, .. } => children.keys().cloned().collect(),
            },
        }
    }
            ...
            Upper::Interface(interface) => {
                if interface.inner.children_len() == 1 {
                    match &*interface.inner {
                        Lower::Segment { values, .. } => Some(values.last().unwrap().clone()),
...
fn merge_lower<T: Clone + Eq + Hash>(l1: &Arc<Lower<T>>, l2: &Arc<Lower<T>>) -> Arc<Lower<T>> {
    ...
    let merged_children = match (&**l1, &**l2) {
        ...
        (Lower::Segment { .. }, Lower::General { children, .. }) |
        (Lower::General { children, .. }, Lower::Segment { .. }) => {
...
/// Works when the top of the GSS is a Segment chain, regardless of what's below.
///
/// The stack has a "floor" — the Lower node at the bottom of the extracted chain.
/// Operations above the floor are O(1) array operations. When a pop goes past
/// the floor (the chain portion is exhausted), `pop()` returns `PopResult::HitFloor`
/// and the caller must fall back to the general GSS path.
///
...
pub struct VirtualStack<T: Clone + Eq + Hash, A: Merge + Clone + Eq + Hash> {
    ...
    /// another Segment, or an empty terminal.
    floor: Arc<Lower<T>>,
}

/// Result of a pop operation on VirtualStack.
pub enum PopResult {
    /// Pop succeeded — all values removed from the virtual portion.
    Ok,
    /// Pop would go below the floor. The remaining `remaining` values
    /// need to be popped from the floor's GSS. The VirtualStack is now empty
    /// and should be discarded.
    HitFloor { remaining: usize },
}
...
impl<T: Clone + Eq + Hash, A: Merge + Clone + Eq + Hash> VirtualStack<T, A> {
    ...
    /// Returns `PopResult::Ok` if all values were removed from the virtual stack.
    /// Returns `PopResult::HitFloor { remaining }` if the pop goes past the floor.
    #[inline]
    pub fn pop(&mut self, n: usize) -> PopResult {
        if n <= self.states.len() {
            self.states.truncate(self.states.len() - n);
            PopResult::Ok
        } else {
            let remaining = n - self.states.len();
            self.states.clear();
            PopResult::HitFloor { remaining }
        }
    }
    ...
    /// Get a reference to the floor (the Lower node below the chain).
    /// Module-private since Lower is not pub.
    #[inline]
    fn floor(&self) -> &Arc<Lower<T>> {
        &self.floor
    }
    ...
    /// If the stack is empty, returns a GSS wrapping just the floor.
    /// Otherwise builds Segments on top of the floor.
    pub fn into_gss(self) -> LeveledGSS<T, A> {
        if self.states.is_empty() {
            // Nothing on the virtual stack — reconstruct from floor
        ...
        // Build from bottom to top, one Segment per value, on top of floor.
        ...
        LeveledGSS {
            inner: new_interface(current, self.acc),
        }
    }

    /// Materialize the floor portion into a GSS (discarding the virtual stack states).
    /// Useful when the virtual stack is exhausted and the caller needs to continue
...
impl<T: Clone + Eq + Hash, A: Merge + Clone + Eq + Hash> LeveledGSS<T, A> {
    ...
    pub fn inner_ptrs_eq(&self, other: &Self) -> bool {
        match (&*self.inner, &*other.inner) {
            ...
            (Upper::Interface(i1), Upper::Interface(i2)) => {
                if i1.acc != i2.acc || i1.inner.children_len() != i2.inner.children_len() || i1.inner.max_depth() != i2.inner.max_depth() {
                    return false;
                }
                let c1 = i1.inner.children();
                let c2 = i2.inner.children();
        ...
        I: IntoIterator<Item = (T, T)>,
    {
        ...
        match &*self.inner {
            Upper::Interface(i) => {
                ...
                match &*i.inner {
                    Lower::Segment { values, .. } => {
                        let top_val = values.last().unwrap();
                        let rest = i.inner.segment_rest_arc();
                        inner_children = None;
                    ...
                    if remaining == 0 {
                        ...
                        if current.children_len() == 1 {
                            let goto_from = match &**current {
                                Lower::Segment { values, .. } => values.last().unwrap().clone(),
                                Lower::General { children, .. } => {
        ...
        fn popn_lower<T: Clone + Eq + Hash, A: Merge + Clone + Eq + Hash>(
            ...
            let merged: Option<Arc<Lower<T>>> = if node.is_segment() {
                ...
                if k >= seg_len {
                    ...
                    let popped = popn_lower::<T, A>(node.segment_next(), k - seg_len, memo_lower);
                    Some(popped)
                } else {
    ...
    fn popn_single_interface_path(&self, n: isize) -> Option<Self> {
        ...
        let mut remaining = n;
        while remaining > 0 {
            let next_child = if current.is_segment() {
    ...
    ///
    /// Returns `None` if the GSS is not an Interface or the top is not a Segment.
    pub fn try_virtual_stack(&self) -> Option<VirtualStack<T, A>> {
        ...
        // Traverses both Segment nodes and single-child General nodes.
        let mut current: &Lower<T> = &interface.inner;
        loop {
            match current {
                Lower::Segment { values, next, .. } => {
    ...
    pub fn for_each_top_value<F: FnMut(T)>(&self, mut f: F) {
        match &*self.inner {
            ...
            Upper::Interface(interface) => {
                match &*interface.inner {
                    Lower::Segment { values, .. } => f(values.last().unwrap().clone()),
                    Lower::General { children, .. } => {
