...
impl<T: Clone + Eq + Hash, A: Merge + Clone + Eq + Hash> LeveledGSS<T, A> {
    ...
    fn popn_single_interface_path(&self, n: isize) -> Option<Self> {
        ...
        let mut remaining = n;
        while remaining > 0 {
            let next_child = if current.is_chain() {
                Some(current.chain_next().clone())
            } else {
    ...
    pub fn decompose_and_pop(&self) -> Vec<(T, Self)> {
        match &*self.inner {
            ...
            Upper::Interface(i) => {
                // Chain fast path: single child, no iteration needed
                if i.inner.is_chain() {
    ...
    pub fn isolate(&self, value: Option<T>) -> Self {
        ...
        let new_inner = if let Some(val) = value {
            match &*self.inner {
                ...
                Upper::Interface(i) => {
                    // Fast path for Chain: if isolating the chain's value, return self
