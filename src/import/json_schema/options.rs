/// JSON Schema import-time configuration.
///
/// These options affect grammar shape, not JSON Schema meaning.  They are kept
/// deliberately close to the importer because they are policy choices for the
/// value-to-grammar lowering, not global compile options.
#[derive(Debug, Clone)]
...
/// simplification pass.  JSON-specific normalization happens before lowering.
pub(crate) fn simplify_grammar_enabled() -> bool {
    false
}

/// Exact terminal subtraction is enabled by default because open-object lowering
/// uses `JSON_STRING - {fixed literal keys}` for additional-property keys.
///
...
pub(crate) fn lower_exact_subtractions_enabled() -> bool {
    match std::env::var("GLRMASK_JSON_SCHEMA_LOWER_EXACT_SUBTRACTIONS") {
        Ok(value) => {
...
/// JSON importer leaves choices as written and lets the grammar compiler decide
/// whether to factor them.
