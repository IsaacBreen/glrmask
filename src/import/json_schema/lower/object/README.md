# JSON Schema object lowering

This directory owns `object`-specific grammar emission.  The current chunk moves
the implementation into a phase-local directory so future work can split helper
strategies without changing the public module path `lower::object`.
