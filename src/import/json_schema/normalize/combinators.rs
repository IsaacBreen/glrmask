//! Semantic combinator normalization and factoring.
//!
//! These routines are the algebraic middle layer between loaded schema syntax
//! and grammar emission.  They decide when allOf/anyOf/oneOf object and array
//! patterns can be merged, factored, distributed, or deliberately broadened.
...
use super::super::lower::string::string_value_satisfies_schema;
...
pub(crate) fn open_object_any_of_covers_json_object(branches: &[Schema]) -> bool {
    if branches.len() < 2 {
        return false;
    }
...
fn broad_string_assertions(assertions: &SchemaAssertions) -> Option<&super::super::schema::StringSchema> {
    if assertions.const_value.is_some()
        || assertions.enum_values.is_some()
...
fn option_arrays_shape_equivalent(
    left: Option<&super::super::schema::ArraySchema>,
    right: Option<&super::super::schema::ArraySchema>,
) -> bool {
    match (left, right) {
        (None, None) => true,
...
fn option_strings_shape_equivalent(
    left: Option<&super::super::schema::StringSchema>,
    right: Option<&super::super::schema::StringSchema>,
) -> bool {
    match (left, right) {
        (None, None) => true,
        (Some(left), Some(right)) => {
...
fn option_numbers_shape_equivalent(
    left: Option<&super::super::schema::NumberSchema>,
    right: Option<&super::super::schema::NumberSchema>,
) -> bool {
    match (left, right) {
        (None, None) => true,
        (Some(left), Some(right)) => {
...
fn is_vacuous_json_value_schema(schema: &Schema) -> bool {
    ...
    if !option_objects_shape_equivalent(assertions.object.as_ref(), Some(&ObjectSchema::default()))
        || !option_arrays_shape_equivalent(assertions.array.as_ref(), Some(&super::super::schema::ArraySchema::default()))
        || !option_strings_shape_equivalent(assertions.string.as_ref(), Some(&super::super::schema::StringSchema::default()))
        || !option_numbers_shape_equivalent(assertions.number.as_ref(), Some(&super::super::schema::NumberSchema::default()))
    {
        return false;
    }
    let Some(types) = &assertions.types else {
