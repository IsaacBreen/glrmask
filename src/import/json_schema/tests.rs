...
fn assert_glrm_has_split_literal_key(glrm: &str, key: &str) {
    let split_across_edges = glrm.contains("-- JSON_QUOTE -->")
        && glrm.contains(&format!("-- \"{key}\" -->"))
        && glrm.contains("-- JSON_KEY_SUFFIX -->");
    let split_on_one_edge = glrm.contains(&format!(
        "-- JSON_QUOTE \"{key}\" JSON_KEY_SUFFIX -->"
    ));
    assert!(split_across_edges || split_on_one_edge, "{glrm}");
    assert!(
        !glrm.contains(&format!("\\\"{key}\\\": ")),
