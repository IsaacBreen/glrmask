...
fn test_constraint_from_serialized_compiled_grammar_and_gpt2_vocab() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (seq_idx, seq_terminal_names) in test_sequences_str.iter().enumerate() {
        ...
        let mut glr_state = compiled_grammar.glr_parser.init_glr_parser_with_acc(LLMTokenInfo::default());
    ...
    for llm_token_for_comp in llm_tokens_for_comp {
        ...
        assert!(mask_before_commit.contains(llm_token_id_for_comp.0), "Token mismatch during comparison setup for token {:?} (ID {})", String::from_utf8_lossy(llm_token_for_comp), llm_token_id_for_comp.0);
        constraint_state_for_comp.commit(*llm_token_id_for_comp);
    }
