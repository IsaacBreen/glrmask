...
pub fn optimize_intermediate_trie3_template(
    ...
    for _ in 0..2 {
        ...
        let normalized_paths1: BTreeSet<_> = IntermediatePrecomputeNode3::get_all_paths_with_cycles(&god, roots, |idx, n| idx == *end_node, |_, _, _| true, 1000000)
            .into_iter()
        ...
        let normalized_paths2: BTreeSet<_> = IntermediatePrecomputeNode3::get_all_paths_with_cycles(&god, roots, |idx, n| idx == *end_node, |_, _, _| true, 1000000)
            .into_iter()
            .map(|(_r, p)| normalize_path(p.into_iter().map(|(ek, _, _)| ek).collect()))
            .collect();
