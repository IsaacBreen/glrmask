...
impl Constraint {
    ...
    pub fn load(bytes: &[u8]) -> crate::Result<Self> {
        ...
        let mut constraint = if version == CONSTRAINT_VERSION {
            ...
            constraint
        } else if version == PREVIOUS_COMPOSITION_TEMPLATE_CONSTRAINT_VERSION {
            let artifact: ConstraintArtifactV14 = bincode::deserialize(serialized)
                .map_err(|err| crate::GlrMaskError::Serialization(err.to_string()))?;
            let mut constraint = artifact.constraint;
            constraint.ignore_expr = artifact.ignore_expr;
            constraint.parser_state_domain_labels = artifact.parser_state_domain_labels;
            ...
            constraint
                ...
                .map_err(crate::GlrMaskError::Serialization)?;
            constraint
        } else if version == PREVIOUS_COMPOSITION_RESET_CONSTRAINT_VERSION {
...
mod tests {
    ...
    #[test]
    fn constraint_envelope_loads_previous_v14_without_characterization_cache() {
        let constraint = tiny_constraint();
        assert!(constraint
            .composition_parser_characterizations_by_terminal
            .iter()
            .any(Option::is_some));
        let raw = bincode::serialize(&ConstraintArtifactV14Ref {
            constraint: &constraint,
            ignore_expr: &constraint.ignore_expr,
        ...
        .expect("v14 artifact should remain loadable without characterization cache");
        assert_eq!(
