...
const COMPOSITION_METADATA_SPLIT_MAGIC: [u8; 4] = *b"CMS3";
...
fn decode_composition_metadata(input: &[u8]) -> Result<ConstraintCompositionMetadata, String> {
    ...
    if input.starts_with(&COMPOSITION_METADATA_SPLIT_MAGIC) {
        ...
        let link: ConstraintCompositionLinkMetadata =
...
struct ConstraintArtifactCurrentRuntimeRef<'a> {
    terminal_live_states: &'a [Vec<u32>],
    segmented_runtime: Option<SegmentedRuntimeArtifactV22Ref<'a>>,
...
fn segmented_runtime_artifact_ref(
    constraint: &Constraint,
) -> Option<SegmentedRuntimeArtifactV22Ref<'_>> {
    ...
    let components = overlay
        ...
        .map(|component| SegmentedParserComponentV22Ref {
            // A is persisted as the exact retained component tree.  Do not
            // synthesize/modify a parser DWA merely for serialization: each
            // component's parser runtime remains authoritative and unchanged.
        ...
        .collect();
    Some(SegmentedRuntimeArtifactV22Ref {
        // Kept in the v22 wire shape for backwards compatibility. New
        // artifacts no longer flatten static A into a synthesized parser DWA.
        ...
        // All components are retained, so these indices remain exact.
        segmented_component_union_root_dispatch: &overlay.segmented_component_union_root_dispatch,
        boundary_parser: overlay
            .segmented_boundary_parser
            .as_deref()
            .map(boundary_parser_artifact_ref),
        boundary_terminal_trie: overlay
            .segmented_boundary_terminal_trie
            .as_deref()
            .map(boundary_terminal_trie_v22_ref),
    })
}
...
mod tests {
    ...
    #[test]
    fn load_with_vocab_shares_exact_vocab_for_late_binding() {
        ...
        .unwrap();
        let loaded = Constraint::load_with_vocab(constraint.save(), &vocab).unwrap();
        ...
        assert!(std::sync::Arc::ptr_eq(
            &loaded
                .late_bind_vocab
                .get()
                .expect("load_with_vocab should seed late-bind vocab")
                .entries_arc(),
            &vocab.entries_arc(),
        ));

        let incompatible = crate::Vocab::new(vec![(0, b"x".to_vec())]);
        assert!(Constraint::load_with_vocab(constraint.save(), &incompatible).is_err());
    }
