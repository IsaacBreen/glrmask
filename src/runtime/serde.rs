...
/// V2 adds the depth-one parser acceptance overlay.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
/// V3 adds grammar-level exact-token terminals.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
impl Constraint {
    /// Serialize the execution payload without the depth-one acceptance overlay
    /// or exact-token terminals.
    ...
    /// Serialize the execution payload including the depth-one acceptance
    /// overlay, but without exact-token terminals.
...
mod tests {
    ...
    fn runtime_payload_v3_preserves_special_token_terminals_and_v2_rejects_them() {
        ...
        assert_eq!(
            loaded.special_token_terminals,
            constraint.special_token_terminals
        );
