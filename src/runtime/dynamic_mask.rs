...
>(
    ...
    let direct_residual_slice = if direct_residual_ascii_word_slice_experiment_enabled()
    {
...
#[inline]
fn direct_residual_ascii_word_slice_experiment_enabled() -> bool {
    std::env::var_os("GLRMASK_EXPERIMENT_DIRECT_RESIDUAL_ASCII_WORD_SLICE").is_some()
}

#[inline]
fn dynamic_virtual_dense_cache_key_experiment_enabled() -> bool {
    std::env::var_os("GLRMASK_EXPERIMENT_DYNAMIC_VIRTUAL_DENSE_CACHE_KEY").is_some()
}
...
fn dynamic_mask_lookup_query(
    ...
    let virtual_dense_cache_enabled = dynamic_virtual_dense_cache_key_experiment_enabled();
