...
mod tests {
    ...
    #[test]
    fn dynamic_giant_fixed_sequence_suffix_intersection_is_lazy_and_exact() {
        use crate::automata::regex::Expr;
        use crate::ds::u8set::U8Set;
        use crate::grammar::flat::{GrammarDef, Rule, Symbol, Terminal};

        let component = |max, first: U8Set, second: U8Set| {
            Expr::Seq(vec![
                Expr::Repeat {
                    expr: Box::new(Expr::U8Seq(b"a".to_vec())),
                    min: 0,
            ...
            terminals: vec![Terminal::Expr {
                ...
                expr: Expr::Intersect {
                    expr: Box::new(component(
                        max,
                        U8Set::from_bytes(b"bcd"),
                        U8Set::from_bytes(b"xy"),
                    )),
                    intersect: Box::new(component(
                        max,
                        U8Set::from_bytes(b"cde"),
                        U8Set::from_bytes(b"yz"),
                    )),
                },
            }],
            ..GrammarDef::default()
        ...
        assert!(
            (0..dynamic.inner.tokenizer.num_states() as u32)
                .any(|state| dynamic.inner.tokenizer.has_compressed_transition_state(state)),
            "the factored class suffix must use the compressed zero-minimum giant runtime",
        );
        assert_eq!(dynamic.start().mask(), oracle.start().mask());
        for sequence in [
            vec![5u32],
        ...
        assert!(
            (0..loaded.inner.tokenizer.num_states() as u32)
                .any(|state| loaded.inner.tokenizer.has_compressed_transition_state(state)),
            "save/load must preserve the compressed class-suffix runtime",
        );
        let mut loaded_state = loaded.start();
        loaded_state.commit_token(6).unwrap();
        assert!(loaded_state.is_accepting());
    }
