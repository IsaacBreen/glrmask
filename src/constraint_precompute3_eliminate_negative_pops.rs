...
fn consume_edge<EK, EV, T, FGet, FReplace, FIntersect>(
    state: &mut PathState<EK, EV, T>,
    ek: EK,
    ev: EV,
    child_value: &T,
    get_pop: &mut FGet,
    ...
    FIntersect: FnMut(&EK, &EK) -> bool,
{
    ...
    let p = get_pop(&ek);
    if p == 0 {
        // Skip for output, but continue traversal
        state.last_value_hint = Some(child_value.clone());
        return;
    }
    if p < 0 {
        if state.in_pos {
            // Resolve the pair first.
            let pair = process_pair_ev(
                state.neg_buf.clone(),
                state.pos_buf.clone(),
                get_pop,
                replace_pop,
                ...
                Some((leftover_neg, leftover_pos)) => {
                    if !leftover_pos.is_empty() {
                        let val = state
                            .last_pos_value
                            .as_ref()
                            .unwrap_or(child_value);
                        let new_tail =
                            emit_sequence(stateful_arena(god_placeholder()), state.new_idx, &[], val);
                        // placeholder, replaced below (we cannot call unknown god here)
                        // This stub is immediately replaced in actual context; see below.
                    }
                    // We cannot emit here because we don't have god in this signature.
                    // consume_edge is rewritten in actual call-site to handle emitting.
                }
            }
        }
        // Buffer this negative
        state.in_pos = false;
        state.pos_buf.clear();
        state.neg_buf.push((ek, ev));
        state.last_value_hint = Some(child_value.clone());
        return;
    } else {
        // p > 0
        if !state.in_pos && state.neg_buf.is_empty() {
            // Positive prefix: emit immediately
            // Emission is done by caller (where god is available).
        } else {
            // Part of a positive run following negatives
            state.in_pos = true;
            state.pos_buf.push((ek, ev));
            state.last_pos_value = Some(child_value.clone());
            state.last_value_hint = Some(child_value.clone());
        }
    }
}
