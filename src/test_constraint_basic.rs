...
/// Generated by report_to_rust_test.py
/// Report ID:        test_arch_refactor_001
/// Parent report ID: 5c15d27fb439f6a6
/// Problem:          builtin/schema/jsb_clinical_concept_display_group
/// Framework:        sep1
/// Error type:       false_positive
/// Mismatch token:   ' Draft'  (hex=204472616674)
/// Prefix:           ''  (hex=)
/// Grammar source:   grammar-ir-ebnf
#[test]
fn test_jsb_clinical_concept_display_group_false_positive_draft() {
    let _guard = crate::GLOBAL_DIMS_MUTEX.lock().unwrap_or_else(|e| e.into_inner());

    let ebnf = indoc! {r#"
        start ::= json_object ;
        STRING_CHAR ::= [ -!#-\[\]-\x7F] | [\xC2-\xDF] [\x80-\xBF] | [\xE0-\xEF] [\x80-\xBF] [\x80-\xBF] | [\xF0-\xF4] [\x80-\xBF] [\x80-\xBF] [\x80-\xBF] ;
        HEX ::= [0-9A-Fa-f] ;
        ESCAPE_SHORT_CHAR ::= ["/\\bfnrt] ;
        ESCAPE_SEQ ::= "\\" ESCAPE_SHORT_CHAR | "\\" "u" HEX HEX HEX HEX ;
        STRING_CONTENT ::= (STRING_CHAR | ESCAPE_SEQ)* ;
        JSON_STRING ::= "\"" STRING_CONTENT "\"" ;
        DIGIT ::= [0-9] ;
        ...
        json_value ::= json_object | json_array | JSON_STRING | JSON_NUMBER | JSON_INTEGER | JSON_BOOL | JSON_NULL ;
    "#};
    let grammar_definition = GrammarDefinition::from_ebnf(ebnf).expect("from_ebnf");

    let mut llm_token_map = LLMTokenMap::new();
    llm_token_map.insert(b" Draft".to_vec(), LLMTokenID(0));  // orig_id=13650
    llm_token_map.insert(b"<|endoftext|>".to_vec(), LLMTokenID(1));  // orig_id=50256
    let max_id = 1usize;
    ...
    assert!(!mask.contains(0), "Expected token ' Draft' (local_id=0) to be REJECTED at prefix='', but sep1 accepted it (false_positive).");
}

/// Regression test: sep1 false negative on `","` token (close-string + comma + open-key)
/// when a grammar property rule uses `(JSON_STRING | JSON_NULL)` alternation (nullable type).
///
/// Root cause from the 500-sample sweep (8 sep1 false negatives, all Snowplow sp_66):
/// The Sep1Adapter generates Lark grammar via `grammar_ir_to_sep1_lark`, which uses
/// binary-tree ordered property rules with `(JSON_STRING | JSON_NULL)` for nullable types.
///
/// Lark grammar excerpt (simplified from Snowplow sp_66):
/// ```
/// obj_ord_0_1_nc: "\"couponCode\"" ":" (JSON_STRING | JSON_NULL) obj_ord_0_2_c
///               | obj_ord_0_2_nc
/// obj_ord_0_2_c: "," "\"id\"" ":" JSON_STRING obj_ord_0_3_c
/// ```
///
/// The `","` token (bytes 22 2c 22 = close-string + comma + open-quote) must be allowed at:
///   prefix = `{"affiliation":"Example Store","couponCode":"SUMMER`  (inside couponCode's value)
///   because it closes couponCode's JSON_STRING in the `(JSON_STRING|JSON_NULL)` alternation,
///   adds the separator comma, and opens the next key's starting quote.
///
/// Expected: token allowed (DWA must correctly handle terminal completion across alternations)
/// Actual (before fix): token incorrectly blocked
#[test]
fn test_nullable_string_property_allows_quote_comma_quote_boundary_token() {
    let _guard = crate::GLOBAL_DIMS_MUTEX.lock().unwrap_or_else(|e| e.into_inner());

    // Minimal Lark grammar that reproduces the bug (stripped from Snowplow sp_66 Lark grammar).
    // Uses the same structure as grammar_ir_to_sep1_lark with (JSON_STRING | JSON_NULL).
    let lark_grammar = r###"
PATTERN_0: /[\x20-\x21\x23-\x5B\x5D-\x7F]/
PATTERN_6: /[\x22\x2F\x5C\x62\x66\x6E\x72\x74]/
...
"###;
    ...
    let grammar_definition = GrammarDefinition::from_lark(lark_grammar).expect("parse Lark grammar");

    // Minimal vocab: just the disputed token
    let tok_disputed = LLMTokenID(0); // b'","'  = close-string + comma + open-key
    let mut llm_token_map = LLMTokenMap::new();
    llm_token_map.insert(b"\",\"".to_vec(), tok_disputed);
    let max_id = 0usize;
    ...
    // Prefix: inside couponCode's JSON_STRING value (after "affiliation":"Example Store","couponCode":"SUMMER")
    // After token '","': closes couponCode's string, `,` separator, `"` opens next key.
    let prefix = b"{\"affiliation\":\"Example Store\",\"couponCode\":\"SUMMER";

    let mut sep1_state = constraint.init();
    sep1_state.commit_bytes(prefix);
    let mask = sep1_state.get_mask();
    let sep1_vote = mask.contains(tok_disputed.0);

    assert!(
        sep1_vote,
        "LOUD_FAIL nullable_string_property sep1-vs-ground-truth false negative: \
         token '\",' (bytes=22 2c 22) should be allowed after prefix={prefix:?} \
         but sep1 disallowed it. sep1={sep1_vote} mask={mask:?}. \
         Bug: DWA false negative when (JSON_STRING | JSON_NULL) alternation is in grammar rule. \
         Fix: ensure DWA correctly handles multi-byte token completion across alternation boundaries."
    );
}


#[test]
fn test_nullable_string_print_ebnf() {
    let _guard = crate::GLOBAL_DIMS_MUTEX.lock().unwrap_or_else(|e| e.into_inner());
    ...
    let ebnf = json_schema_to_ebnf(schema_json).expect("convert schema to EBNF");
    println!("=== FAILING EBNF ===\n{}\n===", ebnf);

    // Also show the non-nullable version
    let schema_ok = r#"{
        "type": "object",
        "properties": {
            "b": { "type": "string" },
    ...
    let ebnf_ok = json_schema_to_ebnf(schema_ok).expect("convert ok schema to EBNF");
    println!("=== WORKING EBNF ===\n{}\n===", ebnf_ok);
}

#[test]
fn test_snowplow_sp66_print_ebnf() {
    let _guard = crate::GLOBAL_DIMS_MUTEX.lock().unwrap_or_else(|e| e.into_inner());

    let schema_json = r#"{
        "additionalProperties": false,
        "properties": {
            "affiliation": { "maxLength": 500, "type": ["string", "null"] },
            "couponCode": { "type": ["string", "null"] },
            "currencyCode": { "maxLength": 10, "type": ["string", "null"] },
            "id": { "maxLength": 500, "type": "string" },
            "revenue": { "multipleOf": 0.01, "type": ["number", "null"] },
            "shipping": { "multipleOf": 0.01, "type": ["number", "null"] },
            "tax": { "multipleOf": 0.01, "type": ["number", "null"] }
        },
        "required": ["id"],
        "type": "object"
    }"#;

    let ebnf = json_schema_to_ebnf(schema_json).expect("convert Snowplow schema to EBNF");
    println!("=== Snowplow sp_66 EBNF ===\n{}\n===", ebnf);
}

#[test]
fn test_snowplow_sp66_property_ordering_investigation() {
    let _guard = crate::GLOBAL_DIMS_MUTEX.lock().unwrap_or_else(|e| e.into_inner());

    // Snowplow sp_66 schema
    let schema_json = r#"{
        "additionalProperties": false,
        "properties": {
            "affiliation": { "maxLength": 500, "type": ["string", "null"] },
    ...
    let ebnf = json_schema_to_ebnf(schema_json).expect("convert Snowplow schema to EBNF");
    let grammar_definition = GrammarDefinition::from_ebnf(&ebnf).expect("parse EBNF");

    // Minimal vocab with the disputed token  
    let tok_disputed = LLMTokenID(0); // b'","'
    let mut llm_token_map = LLMTokenMap::new();
    llm_token_map.insert(b"\",\"".to_vec(), tok_disputed);
    let max_id = 0usize;
    ...
    // Case from sweep: prefix = '{"affiliation":"Example Store","couponCode":"SUMMER'
    // token = '","'  (bytes 22 2c 22)
    // sep1=False (sweep says this is a false negative)
    let prefix = b"{\"affiliation\":\"Example Store\",\"couponCode\":\"SUMMER";
    ...
    // The sep1 EBNF grammar starts with: root ::= '{' '"id"' ':' JSON_STRING (...)* '}'
    // It requires "id" to come FIRST. The prefix starts with "affiliation" not "id".
    // So no tokens should be valid — the parser is in an error/dead state.
    println!("After prefix '{{\"affiliation\":\"Example Store\",\"couponCode\":\"SUMMER':");
    println!("  sep1 vote for token '\",' = {}", sep1_vote);
    println!("  mask = {:?}", mask);
    println!("  (sep1 grammar requires 'id' first; prefix starts with 'affiliation' -> dead state)");

    // Also check valid prefix: '{"id":"...",...' 
    let valid_prefix = b"{\"id\":\"txn-123\",\"affiliation\":\"Example Store\",\"couponCode\":\"SUMMER";
    let mut sep1_state2 = constraint.init();
    sep1_state2.commit_bytes(valid_prefix);
    let mask2 = sep1_state2.get_mask();
    let sep1_vote2 = mask2.contains(tok_disputed.0);
    println!("After prefix '{{\"id\":\"txn-123\",\"affiliation\":\"Example Store\",\"couponCode\":\"SUMMER':");
    println!("  sep1 vote for token '\",' = {}", sep1_vote2);
    println!("  (This should be True - valid prefix for sep1 grammar)");
}

#[test]
fn test_nullable_string_dwa_debug_individual_bytes() {
    let _guard = crate::GLOBAL_DIMS_MUTEX.lock().unwrap_or_else(|e| e.into_inner());
...
"###;
    ...
    // Build vocab with individual byte tokens + the 3-byte joint token
    let tok_close_quote = LLMTokenID(0); // b'"'
    let tok_comma       = LLMTokenID(1); // b','
    let tok_open_quote  = LLMTokenID(2); // b'"'  (same bytes, different id)
    let tok_joint       = LLMTokenID(3); // b'","'
    let tok_id_char     = LLMTokenID(4); // b'i'
    let mut llm_token_map = LLMTokenMap::new();
    llm_token_map.insert(b"\"".to_vec(),   tok_close_quote);
    llm_token_map.insert(b",".to_vec(),    tok_comma);
    llm_token_map.insert(b"\",\"".to_vec(), tok_joint);
    llm_token_map.insert(b"i".to_vec(),    tok_id_char);
    let max_id = 4usize;
    ...
    let allows_close_quote = mask.contains(tok_close_quote.0);
    let allows_comma       = mask.contains(tok_comma.0);
    let allows_joint       = mask.contains(tok_joint.0);
    let allows_id_char     = mask.contains(tok_id_char.0);

    println!(
        "At prefix after 'couponCode:SUMMER:  allows '\"' = {}, ',' = {}, 'i' = {}, '\",' = {}",
        allows_close_quote, allows_comma, allows_id_char, allows_joint
    );

    // After the closing quote of couponCode's string:
    assert!(
        allows_close_quote,
        "Should allow closing quote '\"' (closes JSON_STRING within (JSON_STRING|JSON_NULL))"
    );

    // After completing 'SUMMER"' (close of JSON_STRING), comma is the separator:
    let mut state2 = constraint.init();
    state2.commit_bytes(prefix);
    state2.commit(tok_close_quote).expect("commit '\"'");
    let mask2 = state2.get_mask();
    let allows_comma_after_close = mask2.contains(tok_comma.0);
    println!("After '\"': allows ',' = {}", allows_comma_after_close);
    assert!(
        allows_comma_after_close,
        "Should allow ',' after closing JSON_STRING in (JSON_STRING|JSON_NULL)"
    );

    // The 3-byte joint token should also be allowed:
    assert!(
        allows_joint,
        "LOUD_FAIL DWA false negative: joint token '\",' blocked but individual tokens '\"' then ',' then '\"' are allowed. \
         This is a DWA false negative on multi-byte token crossing terminal boundary with nullable alternation."
    );
}

#[test]
fn test_nullable_string_helper_rule_variant() {
    // Test: does the bug also appear when (JSON_STRING | JSON_NULL) is a named helper rule?
    let _guard = crate::GLOBAL_DIMS_MUTEX.lock().unwrap_or_else(|e| e.into_inner());
...
"###;
    ...
    let tok_joint = LLMTokenID(0); // b'","'
    let tok_quote = LLMTokenID(1); // b'"'
    let tok_comma = LLMTokenID(2); // b','
    let mut llm_token_map = LLMTokenMap::new();
    llm_token_map.insert(b"\",\"".to_vec(), tok_joint);
    llm_token_map.insert(b"\"".to_vec(), tok_quote);
    llm_token_map.insert(b",".to_vec(), tok_comma);
    let max_id = 2usize;
    ...
    let allows_joint = mask.contains(tok_joint.0);
    let allows_quote = mask.contains(tok_quote.0);
    let allows_comma = mask.contains(tok_comma.0);

    println!("HELPER RULE variant: allows '\"' = {}, ',' = {}, '\",' = {}", allows_quote, allows_comma, allows_joint);

    assert!(
        allows_joint,
        "LOUD_FAIL helper_rule_variant: joint '\",' also blocked with explicit helper rule 'str_or_null: JSON_STRING | JSON_NULL'. \
         Sep1=false mask={mask:?}"
    );
}

#[test]  
fn test_nullable_string_no_alternation_works() {
    // Sanity check: without alternation, the joint token IS allowed.
    let _guard = crate::GLOBAL_DIMS_MUTEX.lock().unwrap_or_else(|e| e.into_inner());
...
"###;
    ...
    let tok_joint = LLMTokenID(0);
    let tok_quote = LLMTokenID(1);
    let tok_comma = LLMTokenID(2);
    let mut llm_token_map = LLMTokenMap::new();
    llm_token_map.insert(b"\",\"".to_vec(), tok_joint);
    llm_token_map.insert(b"\"".to_vec(), tok_quote);
    llm_token_map.insert(b",".to_vec(), tok_comma);
    ...
    let allows_joint = mask.contains(tok_joint.0);
    println!("NO-ALT variant: allows '\",' = {}", allows_joint);
    assert!(allows_joint, "Expected '\",' to be allowed with plain JSON_STRING (no alternation)");
}

#[test]
fn test_nullable_string_print_comma_characterization() {
...
"###;
    ...
    let definition = GrammarDefinition::from_lark(lark_grammar).expect("parse Lark grammar");
    let parser = CompiledGrammar::glr_parser_from_definition(&definition);

    // Print all terminal ID to name mappings to find the comma terminal
    println!("=== Terminal Map ===");
    for (terminal, terminal_id) in &parser.terminal_map {
        println!("  Terminal {:?} => id={}", terminal, terminal_id.0);
    }

    println!("\n=== Non-Terminal Map ===");
    for (nt, nt_id) in &parser.non_terminal_map {
        println!("  NT {:?} => id={}", nt.0, nt_id.0);
    }

    // Find the comma terminal (literal b",")
    let comma_bytes = b",".to_vec();
    let comma_terminal = Terminal::Literal(comma_bytes);
    let comma_id = parser.terminal_map.get_by_left(&comma_terminal)
        .expect("comma terminal must exist in parser");
    println!("\n=== Comma terminal id={} ===", comma_id.0);

    let tc = compute_terminal_characterization(&parser, *comma_id);
    println!("{}", tc);

    // Also print production rules for clarity
    println!("=== Productions ===");
    for (i, prod) in parser.productions.iter().enumerate() {
        let rhs: Vec<String> = prod.rhs.iter().map(|sym| format!("{:?}", sym)).collect();
        println!("  [{}] {} ::= {}", i, prod.lhs.0, rhs.join(" "));
    }
}

#[test]
fn test_nullable_string_print_comma_char_no_alternation() {
    use crate::precompute4::characterize::compute_terminal_characterization;

    // No-alternation variant: direct JSON_STRING (should work)
    let lark_grammar = r###"
PATTERN_0: /[\x20-\x21\x23-\x5B\x5D-\x7F]/
PATTERN_6: /[\x22\x2F\x5C\x62\x66\x6E\x72\x74]/
STRING_CHAR: PATTERN_0
...
"###;
    ...
    println!("=== Non-Terminal Map (no-alternation) ===");
    for (nt, nt_id) in &parser.non_terminal_map {
        println!("  NT {:?} => id={}", nt.0, nt_id.0);
    }

    let comma_bytes = b",".to_vec();
    let comma_terminal = crate::glr::grammar::Terminal::Literal(comma_bytes);
    let comma_id = parser.terminal_map.get_by_left(&comma_terminal)
        .expect("comma terminal must exist in parser");
    println!("\n=== Comma terminal id={} (no-alternation variant) ===", comma_id.0);

    let tc = compute_terminal_characterization(&parser, *comma_id);
    println!("{}", tc);

    println!("=== Productions (no-alternation) ===");
    for (i, prod) in parser.productions.iter().enumerate() {
        let rhs: Vec<String> = prod.rhs.iter().map(|sym| format!("{:?}", sym)).collect();
        println!("  [{}] {} ::= {}", i, prod.lhs.0, rhs.join(" "));
    }
...
#[test]
fn test_print_parser_dwa_structure() {
    let _guard = crate::GLOBAL_DIMS_MUTEX.lock().unwrap_or_else(|e| e.into_inner());

    // Minimal grammar: just capture what the parser_dwa looks like
    let lark_grammar = r###"
JSON_STRING: "\"" "content" "\""
start: JSON_STRING
"###;
    ...
    // Use a minimal vocab: the token '"content"'
    let tok_a = LLMTokenID(0);
    let tok_b = LLMTokenID(1);
    let mut llm_token_map = LLMTokenMap::new();
    llm_token_map.insert(b"\"content\"".to_vec(), tok_a);
    llm_token_map.insert(b"\"other\"".to_vec(), tok_b);
    let max_id = 1usize;
    ...
    // Print the parser DWA structure
    println!("=== Parser DWA Stats: {} ===", constraint.parser_dwa.stats());
    println!("Start state: {}", constraint.parser_dwa.body.start_state);
    for (i, state) in constraint.parser_dwa.states.0.iter().enumerate() {
        let final_info = if let Some(fw) = &state.final_weight {
            format!("final={}", fw.len())
        } else {
            "".to_string()
        };
        let trans: Vec<String> = state.transitions.iter()
            .map(|(l, t)| format!("{}→{}", l, t))
            .collect();
        println!("  State {}: {} transitions=[{}]", i, final_info, trans.join(", "));
    }
    
    // Try getting mask from initial state
    let mut state = constraint.init();
    let mask = state.get_mask();
    println!("Initial mask (before any commits): allows tok_a={}, tok_b={}", 
             mask.contains(tok_a.0), mask.contains(tok_b.0));
}

#[test]
fn test_nullable_string_print_terminals_and_token_matches() {
    let _guard = crate::GLOBAL_DIMS_MUTEX.lock().unwrap_or_else(|e| e.into_inner());
...
"###;
    ...
    let parser = CompiledGrammar::glr_parser_from_definition(&grammar_definition);
    let compiled = CompiledGrammar::from_definition(Arc::new(grammar_definition));
    let tokenizer = compiled.tokenizer();

    println!("=== Grammar Terminals ===");
    for (terminal, tid) in &parser.terminal_map {
        println!("  Terminal {:?} => id={}", terminal, tid.0);
    }

    println!("\n=== Tokenizer execute on b\"\\\"\" from initial ===");
    let result1 = tokenizer.execute_from_state(b"\"", crate::dfa_u8::tokenizer_ops::TokenizerStateID(0));
    println!("  matches: {:?}", result1.matches);
    println!("  end_state: {:?}", result1.end_state);

    println!("\n=== Tokenizer execute on b\",\" from initial ===");
    let result2 = tokenizer.execute_from_state(b",", crate::dfa_u8::tokenizer_ops::TokenizerStateID(0));
    println!("  matches: {:?}", result2.matches);
    println!("  end_state: {:?}", result2.end_state);

    println!("\n=== Tokenizer execute on b\"\\\",\\\"\" from initial ===");
    let result3 = tokenizer.execute_from_state(b"\",\"", crate::dfa_u8::tokenizer_ops::TokenizerStateID(0));
    println!("  matches: {:?}", result3.matches);
    println!("  end_state: {:?}", result3.end_state);

    // After "SUMMER — what state is tokenizer in? state 2 = started JSON_STRING with opening "
    // Actually, let's trace: after opening " and string chars, we're in some state
    // Simulate the mid-string state by executing "SUMMER from initial
    let mid_state_result = tokenizer.execute_from_state(b"\"SUMMER", crate::dfa_u8::tokenizer_ops::TokenizerStateID(0));
    println!("\n=== After '\"SUMMER' from initial ===");
    println!("  matches: {:?}", mid_state_result.matches);
    println!("  end_state: {:?}", mid_state_result.end_state);

    if let Some(mid_state) = mid_state_result.end_state {
        println!("\n=== Tokenizer execute on b\"\\\",\\\"\" from mid-JSON_STRING state ({}) ===", mid_state);
        let result4 = tokenizer.execute_from_state(b"\",\"", crate::dfa_u8::tokenizer_ops::TokenizerStateID(mid_state));
        println!("  matches: {:?}", result4.matches);
        println!("  end_state: {:?}", result4.end_state);

        println!("\n=== Tokenizer execute on b\"\\\"\" from mid-JSON_STRING state ({}) ===", mid_state);
        let result5 = tokenizer.execute_from_state(b"\"", crate::dfa_u8::tokenizer_ops::TokenizerStateID(mid_state));
        println!("  matches: {:?}", result5.matches);
        println!("  end_state: {:?}", result5.end_state);

        // Now check what happens from initial state with remaining bytes ",\"
        println!("\n=== Tokenizer execute on b\",\\\"\" from initial (simulates continuation after JSON_STRING matches) ===");
        let result6 = tokenizer.execute_from_state(b",\"", crate::dfa_u8::tokenizer_ops::TokenizerStateID(0));
        println!("  matches: {:?}", result6.matches);
        println!("  end_state: {:?}", result6.end_state);

    // Test approx DFA state transitions
    use crate::glr::approximate_dfa::build_approximate_parser_dfa;
    let mut approx_dfa = build_approximate_parser_dfa(&parser);
    let start_state = approx_dfa.start_state;
    let json_string_id = crate::glr::table::TerminalID(1); // JSON_STRING is terminal 1
    let comma_id = crate::glr::table::TerminalID(6); // comma is terminal 6

    println!("\n=== ApproxDFA transitions ===");
    println!("  start_state = {}", start_state);
    if let Some(after_json_string) = approx_dfa.step(start_state, json_string_id) {
        let state_set_js = approx_dfa.state_set(after_json_string).map(|b| b.iter().collect::<Vec<_>>());
        println!("  step(start, JSON_STRING) = state={} set={:?}", after_json_string, state_set_js);
        if let Some(after_comma) = approx_dfa.step(after_json_string, comma_id) {
            let state_set_c = approx_dfa.state_set(after_comma).map(|b| b.iter().collect::<Vec<_>>());
            println!("  step(after_JS, comma) = state={} set={:?}", after_comma, state_set_c);
        } else {
            println!("  step(after_JS, comma) = None (fallback needed)");
        }
    } else {
        println!("  step(start, JSON_STRING) = None");
    }

    // Try step(start, comma) for the fallback path
    if let Some(start_comma) = approx_dfa.step(start_state, comma_id) {
        let state_set_sc = approx_dfa.state_set(start_comma).map(|b| b.iter().collect::<Vec<_>>());
        println!("  step(start, comma) = state={} set={:?}", start_comma, state_set_sc);
    } else {
        println!("  step(start, comma) = None");
    }
    }
}
...
/// Generated by report_to_rust_test.py
/// Report ID:        482e05c236fe729d
/// Parent report ID: 511407ffbefed949
/// Problem:          jsb/data/Github_easy---o63377
/// Framework:        sep1
/// Error type:       false_positive
/// Mismatch token:   'a'  (hex=61)
/// Prefix:           '"""'  (hex=222222)
/// Grammar source:   grammar-ir-lark
#[test]
...
/// Temporary test: measure constraint build time for a simple JSON schema.
///
/// Schema: {"type":"object","properties":{"x":{"type":"string"}},"required":["x"],"additionalProperties":false}
///
/// The Lark grammar below was generated by: schema -> schema_to_grammar_ir -> grammar_ir_to_sep1_lark
/// Uses actual GPT-2 vocabulary.
/// Run with: MACRO_DEBUG_LEVEL=5 cargo test test_simple_schema_build_time -- --nocapture
#[test]
fn test_simple_schema_build_time() {
    let _guard = crate::GLOBAL_DIMS_MUTEX.lock().unwrap_or_else(|e| e.into_inner());

    // Lark grammar for: {"type":"object","properties":{"x":{"type":"string"}},"required":["x"],"additionalProperties":false}
    let lark_grammar = indoc! {r#"
        PATTERN_2: /[\x80-\xBF]/
        ...
        json_value: json_object
            ...
            | JSON_NULL
        obj_ord_0_1_nc: ""
        obj_ord_0_0_nc: "\"x\":" JSON_STRING obj_ord_0_1_nc
        start: "{" obj_ord_0_0_nc "}"
    "#};

    // --- Load GPT-2 vocabulary (not timed) ---
    let (llm_token_map, max_id) = match load_gpt2_vocab_exact_decode() {
        Some(v) => v,
        None => {
            println!("Skipping test_simple_schema_build_time: GPT-2 vocab not found.");
            println!("Expected at: python/.cache/py_benchmark_vocabs/gpt2_vocab.json");
            return;
        }
    };
    println!("Loaded GPT-2 vocab: {} tokens, max_id={}", llm_token_map.len(), max_id);

    // --- Phase 1: Parse Lark grammar into GrammarDefinition ---
    let t_parse_start = Instant::now();
    let grammar_definition = GrammarDefinition::from_lark(lark_grammar)
        .expect("Failed to parse Lark grammar");
    let t_parse = t_parse_start.elapsed();
    println!("[TIMING] GrammarDefinition::from_lark: {:?}", t_parse);

    // Debug: print terminals and groups
    println!("[DEBUG] regex_name_to_group_id: {:?}", grammar_definition.regex_name_to_group_id);
    println!("[DEBUG] literal_to_group_id: {:?}", grammar_definition.literal_to_group_id);
    println!("[DEBUG] num productions: {}", grammar_definition.productions.len());
    for (i, prod) in grammar_definition.productions.iter().enumerate() {
        println!("[DEBUG] prod {}: {:?}", i, prod);
    }

    // --- Phase 2: Build GrammarConstraint (this is the main thing we're timing) ---
    let grammar_definition = Arc::new(grammar_definition);
    ...
    println!("[TIMING] GrammarConstraint build: {:?}", t_build);

    // --- Phase 3: Quick sanity check ---
    let t_init_start = Instant::now();
    let mut state = constraint.init();
    let t_init = t_init_start.elapsed();
    println!("[TIMING] constraint.init(): {:?}", t_init);

    let t_mask_start = Instant::now();
    let mask = state.get_mask();
    let t_mask = t_mask_start.elapsed();
    println!("[TIMING] get_mask(): {:?}", t_mask);

    println!("[TIMING] Total (parse + build): {:?}", t_parse + t_build);
    println!("Initial mask allows {} / {} tokens", mask.len(), llm_token_map.len() + 1);

    // The mask should allow at least '{' 
    // (since the grammar starts with '{')
    let has_open_brace = llm_token_map.iter()
        .any(|(bytes, id)| bytes == &vec![b'{'] && mask.contains(id.0));
