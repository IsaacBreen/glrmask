...
/// Regression test for sep1 over-accepting tokens on JSON schema constraints.
///
/// The CFA sweep (llguidance vs sep1) found that sep1 returns mask_size = 50256
/// (the full GPT-2 vocabulary) at every token position for JSON schema problems.
/// Ground truth and llguidance both confirm: at position 0 (empty prefix) for any
/// JSON object schema, the only valid first byte is `{`. Token 'EF' (ID 25425,
/// bytes 0x45 0x46 = ASCII "EF") must therefore be rejected.
///
/// Schema: builtin/schema/jsb_schedule_meeting_6a4d8038
/// (Glaiveai2K/schedule_meeting_6a4d8038.json from JsonSchemaBench)
#[test]
fn test_schedule_meeting_rejects_ef_token_at_position_0() {
    let _guard = crate::GLOBAL_DIMS_MUTEX.lock().unwrap_or_else(|e| e.into_inner());

    // Inline schema from Glaiveai2K/schedule_meeting_6a4d8038.json
    let schema_json = r#"{
        "properties": {
            "date": {
        ...
        "required": ["title", "participants", "date", "duration"],
        "type": "object"
    }"#;

    let ebnf = json_schema_to_ebnf(schema_json)
        .expect("json_schema_to_ebnf should succeed for jsb_schedule_meeting schema");
    println!("Generated EBNF:\n{}", ebnf);

    let grammar_definition = GrammarDefinition::from_ebnf(&ebnf)
        .expect("GrammarDefinition::from_ebnf should succeed");

    let (llm_token_map, max_id) = match load_gpt2_vocab() {
        Some(v) => v,
        None => {
            println!(
                "Skipping test_schedule_meeting_rejects_ef_token_at_position_0: gpt2 vocab not found."
            );
            println!("Provide vocab.json in one of the searched paths (e.g. benchmarking/gpt2_vocab.json).");
            return;
        }
    };
    ...
    println!("Initial mask size: {}", mask_size);

    // Sanity check: the mask must be selective. A JSON object can only start with '{',
    // so the vast majority of tokens should be rejected at position 0.
    assert!(
        mask_size < 50000,
        "Initial mask is not selective (mask_size={}); sep1 is over-accepting (full-vocab bug)",
        mask_size,
    );

    // Core assertion: token 'EF' (GPT-2 ID 25425, bytes b"EF" = 0x45 0x46) is invalid
    // at position 0 because a JSON object must start with '{', not 'E'.
    // llguidance and ground truth both reject this token; sep1 wrongly accepts it.
    let token_id_ef: usize = 25425;
    assert!(
        !mask.contains(token_id_ef),
        "Token 'EF' (ID {}) must be rejected at position 0 for the schedule_meeting JSON schema \
         (prefix is empty; only '{{' is a valid first byte). \
         sep1 is incorrectly accepting this token (full-vocabulary false-positive regression).",
        token_id_ef,
    );
}
...
fn test_jsb_clinical_concept_display_group_false_positive_draft() {
  ...
  "title": "Clinical Concept Display Group"
    ...
    let ebnf = json_schema_to_ebnf(schema_json).expect("json_schema_to_ebnf");
    let grammar_definition = GrammarDefinition::from_ebnf(&ebnf).expect("from_ebnf");

    let mut llm_token_map = LLMTokenMap::new();
    llm_token_map.insert(vec![0x20, 0x44, 0x72, 0x61, 0x66, 0x74], LLMTokenID(0));  // orig_id=13650  ' Draft'
    llm_token_map.insert(vec![0x3c, 0x7c, 0x65, 0x6e, 0x64, 0x6f, 0x66, 0x74, 0x65, 0x78, 0x74, 0x7c, 0x3e], LLMTokenID(1));  // orig_id=50256  '<|endoftext|>'
    let max_id = 1usize;
