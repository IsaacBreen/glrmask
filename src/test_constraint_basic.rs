...
#[test]
fn test_ignore_with_partial_input() {
    // Grammar: program ::= 'a' 'a'; #![ignore(WS)]; WS ::= ' ';
    // LLM Vocab: "a", " "
    // Input: "a "
    ...
    // Initial state: parser expects 'a', but can skip ' '.
    // So, 'a' and ' ' are allowed.
    let mask1 = state.get_mask();
    assert_eq!(mask1, HybridBitset::from_iter(vec![llm_a.0, llm_ws.0]));

    // Commit 'a'
    state.commit(llm_a);

    // After 'a', parser expects another 'a', but can skip ' '.
    // So, 'a' and ' ' are allowed.
    let mask2 = state.get_mask();
    assert_eq!(mask2, HybridBitset::from_iter(vec![llm_a.0, llm_ws.0]));

    // Commit ' '
    state.commit(llm_ws);

    // After 'a ', parser has skipped the space and still expects the second 'a'.
    // It can also skip more spaces. So, 'a' and ' ' are allowed.
    let mask3 = state.get_mask();
    assert_eq!(mask3, HybridBitset::from_iter(vec![llm_a.0, llm_ws.0]));
}
