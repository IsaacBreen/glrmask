...
/// Generated by report_to_rust_test.py
/// Report ID:        a901ff70634adc8e
/// Parent report ID: 511407ffbefed949
/// Problem:          jsb/data/Github_easy---o63377
/// Framework:        sep1
/// Error type:       false_positive
/// Mismatch token:   'kward'  (hex=6b77617264)
/// Prefix:           '{"last_update":"'  (hex=7b226c6173745f757064617465223a22)
/// Grammar source:   grammar-ir-lark
#[test]
fn test_github_easy_o63377_false_positive_kward() {
    let _guard = crate::GLOBAL_DIMS_MUTEX.lock().unwrap_or_else(|e| e.into_inner());

    let lark = indoc! {r#"
        PATTERN_0: /[^\x2F]+/
        STR_TERM_0: PATTERN_0
        obj_ord_0_0_nc: STR_TERM_0
        start: "{" obj_ord_0_0_nc
    "#};
    ...
    let mut llm_token_map = LLMTokenMap::new();
    llm_token_map.insert(b"_".to_vec(), LLMTokenID(0));  // orig_id=62
    llm_token_map.insert(vec![0x22, 0x3a, 0x22], LLMTokenID(1));  // orig_id=2404
    llm_token_map.insert(vec![0x7b, 0x22], LLMTokenID(2));  // orig_id=4895
    llm_token_map.insert(b"kward".to_vec(), LLMTokenID(3));  // orig_id=12378
    llm_token_map.insert(b"last".to_vec(), LLMTokenID(4));  // orig_id=12957
    llm_token_map.insert(b"update".to_vec(), LLMTokenID(5));  // orig_id=19119
    llm_token_map.insert(b"<|endoftext|>".to_vec(), LLMTokenID(6));  // orig_id=50256
    let max_id = 6usize;
    ...
    let mut state = constraint.init();
    state.commit_bytes(&vec![0x7b, 0x22, 0x6c, 0x61, 0x73, 0x74, 0x5f, 0x75, 0x70, 0x64, 0x61, 0x74, 0x65, 0x22, 0x3a, 0x22]);
