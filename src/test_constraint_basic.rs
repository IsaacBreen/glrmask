...
fn test_precompute_a_plus_tokenizer() {
    ...
    let child_node = child_arc_wrapper.upgrade().unwrap().read().unwrap();
...
fn test_precompute_x_eq() {
    ...
    assert_eq!(*x_edge_bv, HybridBitset::from_iter(vec![x_llm_id]), "Edge for 'X' has wrong LLM token bitset");let x_dest_node = x_dest_wrapper.upgrade().unwrap().read().unwrap();
    ...
    assert_eq!(*space_edge_bv, HybridBitset::from_iter(vec![space_equals_llm_id]), "Edge for 'SPACE' has wrong LLM token bitset");let node_after_space = space_dest_wrapper.upgrade().unwrap().read().unwrap();
    ...
    assert_eq!(*equals_edge_bv, HybridBitset::from_iter(vec![space_equals_llm_id]), "Edge for 'EQUALS' has wrong LLM token bitset");let equals_dest_node = equals_dest_wrapper.upgrade().unwrap().read().unwrap();
