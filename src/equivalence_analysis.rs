...
fn compute_hash(
    ...
    while let Some(pos) = ctx.discovery_stack.pop() {
        ...
        let state = if pos == 0 { start_state_id } else { regex.dfa.start_state };
    ...
    for &pos in &nodes {
        ...
        for m in &res.matches {
            ...
            // If target is valid, read its hash from memo (computed in previous iterations due to sort)
            let target_hash = if target_pos <= text.len() {
                unsafe { *ctx.memo.get_unchecked(target_pos) }
            } else {
                0
            };
            ctx.edge_buffer.push((m.group_id, target_hash));
        }
...
pub fn find_equivalence_classes(regex: &Regex, strings: &[Vec<u8>], starts: &[usize]) -> EquivalenceResult {
    ...
    let mut signatures: Vec<(u64, usize)> = strings.par_iter()
        .enumerate()
        .map_init(
