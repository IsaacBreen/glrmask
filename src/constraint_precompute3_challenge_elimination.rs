...
mod tests {
    ...
    #[test]
    fn test_eliminate_complex_trie_min_repro() {
        // This is a minimized version of the complex test that still reproduces
        // the path-mismatch bug. It preserves:
        // - The five initial Pop choices that create different normalized pop sets
        // - The Push(0) -> Push(9) -> CheckLLM(1) segment
        // - The pop(0,9) -> pop(1,ALL) -> pop(0,[0,3,7]) loop with the [0,3,7]/7 cycle
        // - The trailing NoOp -> Push(5) -> Push(10) -> CheckLLM(1) into a leaf
        // - A tiny split trigger at the Push(0) node (a NoOp branch + extra Push branch)
        //
        // The structure mirrors the path reported missing in the large test:
        // [NoOp, Pop(0, 5|6|8|9|10)..., Pop(1 or 2, ALL), Pop(0, 3),
        //  Push(0), Push(9), CheckLLM(1),
        //  Pop(0, 9), Pop(1, [ALL]), Pop(0, [0, 3, 7]), NoOp, Push(5), Push(10), CheckLLM(1)]
        let god = IntermediateTrie3GodWrapper::new();
        ...
        let mut node_map = HashMap::new();
        // Node IDs (arbitrary but compact). 122 is the single leaf (END).
        let node_ids = vec![
            100, // R (root)
            101, // A
            102, // B5
            ...
            111, // E: Push(0) target
            112, // F: Push(9) target
            113, // G: after CheckLLM(1)
            114, // H1: after Pop(0,9)
            115, // I: after Pop(1,ALL)
            116, // J: after Pop(0,[0,3,7])
            117, // Kc1: cycle node after Pop(0,7)
            118, // Lc2: cycle node after Pop(1,ALL)
            119, // Kpush: NoOp from J
            120, // L2: Push(5) target
            121, // M: Push(10) target
            122, // END leaf
            123, // E_noop_target (for split trigger): E --NoOp--> 123 --Pop(1,ALL)--> 115(I)
            124, // E_push2 (for split trigger): E --Push(2)--> 124 --CheckLLM(1)--> 101(A)
        ];
        for id in node_ids {
            let contents = if id == 122 {
        ...
        // Root -> NoOp -> A
        god.insert_edge_simple(n(100), n(101), IntermediateTrie3EdgeKey::NoOp, ());

        // Initial Pop choices converging to C. These replicate the prelude from the large test
        // and are exactly the ones that formed the reported "missing" normalized paths.
        // A -> Pop(0,5) -> B5 -> Pop(1,ALL) -> C
        god.insert_edge_simple(n(101), n(102), IntermediateTrie3EdgeKey::Pop(0, StateIDBV::from_item(5)), ());
        god.insert_edge_simple(n(102), n(103), IntermediateTrie3EdgeKey::Pop(1, StateIDBV::max_ones()), ());
        // A -> Pop(0,6) -> B6 -> Pop(2,ALL) -> C
        god.insert_edge_simple(n(101), n(104), IntermediateTrie3EdgeKey::Pop(0, StateIDBV::from_item(6)), ());
        god.insert_edge_simple(n(104), n(103), IntermediateTrie3EdgeKey::Pop(2, StateIDBV::max_ones()), ());
        // A -> Pop(0,8) -> B8 -> Pop(2,ALL) -> C
        god.insert_edge_simple(n(101), n(105), IntermediateTrie3EdgeKey::Pop(0, StateIDBV::from_item(8)), ());
        god.insert_edge_simple(n(105), n(103), IntermediateTrie3EdgeKey::Pop(2, StateIDBV::max_ones()), ());
        // A -> Pop(0,9) -> B9 -> Pop(1,ALL) -> C
        god.insert_edge_simple(n(101), n(106), IntermediateTrie3EdgeKey::Pop(0, StateIDBV::from_item(9)), ());
        god.insert_edge_simple(n(106), n(103), IntermediateTrie3EdgeKey::Pop(1, StateIDBV::max_ones()), ());
        // A -> Pop(0,10) -> B10 -> Pop(1,ALL) -> B10a -> Pop(0,5) -> B10b -> Pop(1,ALL) -> C
        god.insert_edge_simple(n(101), n(107), IntermediateTrie3EdgeKey::Pop(0, StateIDBV::from_item(10)), ());
        god.insert_edge_simple(n(107), n(108), IntermediateTrie3EdgeKey::Pop(1, StateIDBV::max_ones()), ());
        god.insert_edge_simple(n(108), n(109), IntermediateTrie3EdgeKey::Pop(0, StateIDBV::from_item(5)), ());
        god.insert_edge_simple(n(109), n(103), IntermediateTrie3EdgeKey::Pop(1, StateIDBV::max_ones()), ());

        // C -> Pop(0,3) -> D
        god.insert_edge_simple(n(103), n(110), IntermediateTrie3EdgeKey::Pop(0, StateIDBV::from_item(3)), ());

        // D -> Push(0) -> E
        god.insert_edge_simple(n(110), n(111), IntermediateTrie3EdgeKey::Push(StateIDBV::from_item(0)), ());

        // Split-trigger edges at E (incoming push, with both push and non-push out):
        // E -> NoOp -> 123 -> Pop(1,ALL) -> I (115)
        god.insert_edge_simple(n(111), n(123), IntermediateTrie3EdgeKey::NoOp, ());
        god.insert_edge_simple(n(123), n(115), IntermediateTrie3EdgeKey::Pop(1, StateIDBV::max_ones()), ());
        // E -> Push(2) -> 124 -> CheckLLM(1) -> A (cycle)
        god.insert_edge_simple(n(111), n(124), IntermediateTrie3EdgeKey::Push(StateIDBV::from_item(2)), ());
        god.insert_edge_simple(n(124), n(101), IntermediateTrie3EdgeKey::CheckLLM(LLMTokenBV::from_item(1)), ());

        // E -> Push(9) -> F
        god.insert_edge_simple(n(111), n(112), IntermediateTrie3EdgeKey::Push(StateIDBV::from_item(9)), ());
        // F -> CheckLLM(1) -> G
        god.insert_edge_simple(n(112), n(113), IntermediateTrie3EdgeKey::CheckLLM(LLMTokenBV::from_item(1)), ());

        // G -> Pop(0,9) -> H1 -> Pop(1,ALL) -> I
        god.insert_edge_simple(n(113), n(114), IntermediateTrie3EdgeKey::Pop(0, StateIDBV::from_item(9)), ());
        god.insert_edge_simple(n(114), n(115), IntermediateTrie3EdgeKey::Pop(1, StateIDBV::max_ones()), ());
        ...
        // Cycle: J -> Pop(0,7) -> Kc1 -> Pop(1,ALL) -> Lc2 -> Pop(0,[0,3,7]) -> J
        god.insert_edge_simple(n(116), n(117), IntermediateTrie3EdgeKey::Pop(0, StateIDBV::from_item(7)), ());
        god.insert_edge_simple(n(117), n(118), IntermediateTrie3EdgeKey::Pop(1, StateIDBV::max_ones()), ());
        god.insert_edge_simple(
            n(118),
            n(116),
            IntermediateTrie3EdgeKey::Pop(0, StateIDBV::from_iter([0, 3, 7])),
        ...
        // J -> NoOp -> Kpush -> Push(5) -> L2 -> Push(10) -> M -> CheckLLM(1) -> END
        god.insert_edge_simple(n(116), n(119), IntermediateTrie3EdgeKey::NoOp, ());
        god.insert_edge_simple(n(119), n(120), IntermediateTrie3EdgeKey::Push(StateIDBV::from_item(5)), ());
        god.insert_edge_simple(n(120), n(121), IntermediateTrie3EdgeKey::Push(StateIDBV::from_item(10)), ());
        god.insert_edge_simple(n(121), n(122), IntermediateTrie3EdgeKey::CheckLLM(LLMTokenBV::from_item(1)), ());

        let mut roots = BTreeMap::new();
        roots.insert(TokenizerStateID(0), n(100));

        // Verify normalized paths are preserved by elimination.
        let paths_before = get_normalized_paths(&roots, &god);
        eliminate_pushes_and_pops(&mut roots, &god);
        let paths_after = get_normalized_paths(&roots, &god);
        assert_paths_eq(&paths_before, &paths_after);

        // And the "no pops reachable from pushes" invariant holds.
        assert_no_pops_reachable_from_pushes(&roots, &god);
    }
