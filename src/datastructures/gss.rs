...
#[test]
    ...
    #[test]
    fn test_merge_keeps_root_and_nonroot_predecessors_separate() {
        // Build a chain A --(0)--> B --(1)--> C (A is the top node)
        // and merge with A --(0)--> B (where B is a bottom/root node).
        //
        // After merging at A, we must keep the distinction between:
        // - B that has its own predecessor (non-root), and
        // - B that is a root/bottom node.
        // Specifically, A's predecessors under edge 0 should include both forms.

        // C: bottom/root
        let c = Arc::new(GSSNode::new(empty_acc()));
        assert!(c.is_root());

        // B via C (non-root, has a predecessor)
        let b_via_c = Arc::new(GSSNode::new_with_single_predecessor(
            c.clone(),
            mock_edge(1),
            empty_acc(),
        ));
        assert!(!b_via_c.is_root());

        // A1 has predecessor B via C on edge 0
        let mut a1 = GSSNode::new_with_single_predecessor(
            b_via_c.clone(),
            mock_edge(0),
            empty_acc(),
        );
        ...
        // B root (bottom)
        let b_root = Arc::new(GSSNode::new(empty_acc()));
        assert!(b_root.is_root());

        // A2 has predecessor B root on edge 0
        let a2 = GSSNode::new_with_single_predecessor(b_root.clone(), mock_edge(0), empty_acc());

        // Merge: A1 ⊔ A2
        a1.merge_with_depth(1, &a2);

        // After merging, A should have two distinct predecessors under edge 0:
        // one is_root() and one not is_root().
        assert_eq!(a1.num_predecessors(), 2, "Merged A should have exactly two predecessors");

        let preds_by_edge0 = a1.predecessors().get(&mock_edge(0)).expect("Edge 0 missing after merge");
        // Flatten all predecessor arcs under edge 0 across depths
        let mut preds: Vec<Arc<GSSNode>> = Vec::new();
        for (_depth, vecs) in preds_by_edge0 {
            preds.extend(vecs.iter().cloned());
        }
        assert_eq!(preds.len(), 2, "Expected exactly two predecessor nodes on edge 0");

        let num_roots = preds.iter().filter(|n| n.is_root()).count();
        let num_nonroots = preds.iter().filter(|n| !n.is_root()).count();
        assert_eq!(num_roots, 1, "Exactly one predecessor should be a root (bottom) node");
        assert_eq!(num_nonroots, 1, "Exactly one predecessor should be a non-root (has its own predecessor)");

        // Additionally, verify the two different depths (dest keys) are present:
        // - 0 for the root B,
        // - 1 for B that has C as a predecessor.
        assert!(preds_by_edge0.contains_key(&0), "Should contain dest key 0 (root B)");
        assert!(preds_by_edge0.contains_key(&1), "Should contain dest key 1 (B with predecessor)");
    }
}
