...
#[test]
    ...
    #[test]
    fn test_merge_complex_from_logs() {
        // This test captures the essence of a complex merge operation observed in logs.
        // GSS Main:
        //   - Path 1: ... -> (edge 41) -> (edge 32) -> Leaf with Acc_A
        //   - Path 2: ... -> (edge 41) -> Leaf with Acc_B
        // GSS Ref:
        //   - Path 3: ... -> (edge 41) -> Leaf with Acc_C (where Acc_C is same as Acc_A)
        //
        // The merge should combine Path 2 and Path 3, while leaving Path 1 intact.

        // --- 1. Define Accs ---
        // Acc_A and Acc_C: Disallow token 100 and terminal (11, 1)
        let mut acc_a_terminals = HybridL2Bitset::all();
        let mut disallowed_t11 = HybridBitset::zeros();
        disallowed_t11.insert(1);
        acc_a_terminals.insert_l2_bitset(11, HybridBitset::max_ones() - disallowed_t11);

        let mut acc_a_llm = HybridBitset::max_ones();
        acc_a_llm.remove(100);
        let acc_a = Acc::new_with_local_constraints(acc_a_llm, acc_a_terminals);
        let acc_c = acc_a.clone();

        // Acc_B: Disallow token 200, all terminals allowed.
        let mut acc_b_llm = HybridBitset::max_ones();
        acc_b_llm.remove(200);
        let acc_b = Acc::new_with_local_constraints(acc_b_llm, HybridL2Bitset::all());

        // --- 2. Construct GSSs ---

        // GSS Main
        let leaf_a = Arc::new(GSSNode::new(acc_a.clone()));
        let node_32 = Arc::new(leaf_a.push(mock_edge(32))); // path ... -> 32 -> leaf_a

        let leaf_b = Arc::new(GSSNode::new(acc_b.clone()));

        let mut main_preds = NodeMap::new();
        // Path 1 predecessor for edge 41
        main_preds.entry(mock_edge(41)).or_default().insert(node_32.dest_key(), vec![node_32.clone()]);
        // Path 2 predecessor for edge 41
        main_preds.entry(mock_edge(41)).or_default().insert(leaf_b.dest_key(), vec![leaf_b.clone()]);
        let mut gss_main = GSSNode::new_with_map(Arc::new(empty_acc()), main_preds);

        // GSS Ref
        let leaf_c = Arc::new(GSSNode::new(acc_c.clone()));
        let gss_ref = GSSNode::new_with_single_predecessor(leaf_c.clone(), mock_edge(41), empty_acc());

        // --- 3. Merge ---
        gss_main.merge_with_depth(usize::MAX, &gss_ref);

        // --- 4. Assertions ---
        // The merged GSS should still have two predecessor paths for edge 41, at different depths.
        let final_preds_map = gss_main.predecessors();
        assert_eq!(final_preds_map.len(), 1, "Should only have predecessors for edge 41");
        let preds_for_41 = final_preds_map.get(&mock_edge(41)).unwrap();
        assert_eq!(preds_for_41.len(), 2, "Should have predecessors at two different depths for edge 41");

        // Path 1 check (depth 2)
        let pred_depth2_vec = preds_for_41.get(&node_32.dest_key()).unwrap();
        assert_eq!(pred_depth2_vec.len(), 1);
        let pred_depth2 = &pred_depth2_vec[0];
        assert_eq!(**pred_depth2, *node_32, "The longer path should be unchanged");
        assert_eq!(*pred_depth2.acc(), acc_a, "The accumulator for the longer path should be Acc_A");

        // Path 2 check (depth 1)
        let pred_depth1_vec = preds_for_41.get(&leaf_b.dest_key()).unwrap();
        assert_eq!(pred_depth1_vec.len(), 1);
        let pred_depth1 = &pred_depth1_vec[0];
        assert!(pred_depth1.is_root(), "The merged shorter path should be a root node");

        // The leaf of the merged shorter path should have the merged Acc of B and C.
        let expected_merged_acc = Acc::merge(&acc_b, &acc_c);
        assert_eq!(*pred_depth1.acc(), expected_merged_acc, "The accumulator for the shorter path should be the merge of Acc_B and Acc_C");

        // Check the merged Acc contents
        let merged_acc = pred_depth1.acc();
        // LLM tokens: (ALL - {200}) | (ALL - {100}) = ALL
        assert_eq!(merged_acc.llm_tokens_union, HybridBitset::max_ones(), "Merged LLM tokens should be the union");
        // Terminals: (ALL) | (ALL - {(11,1)}) = ALL
        assert_eq!(merged_acc.terminals_union, HybridL2Bitset::all(), "Merged terminals should be the union");
    }
}
