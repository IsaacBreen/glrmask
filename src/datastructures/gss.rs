...
use crate::datastructures::trie::{EdgeInserter, NodeId, NodeRef};
...
pub(crate) type PrecomputeNode2 = NodeRef<(usize, Option<StateID>), LLMTokenBV, PrecomputedNodeContents>;
...
pub(crate) struct Acc {
    ...
    pub(crate) trie2_nodes: BTreeSet<PrecomputeNode2>,
}
...
pub(crate) fn merge_trie2_nodes_if_needed(
    ...
    let mut root_closure = |root: &GSSRoot| -> Option<Arc<Acc>> {
        ...
        if !root.acc.trie2_nodes.iter().any(
            |n| god_guard.get(*n).value.live_tokens != root.acc.llm_tokens_union
        ) {
        ...
        let new_destination = new_destinations.entry((new_acc.trie2_nodes.clone(), root.acc.llm_tokens_union.clone()))
            .or_insert_with(|| god.0.write().unwrap().alloc_node(PrecomputedNodeContents::internal()));
        let edge_key = (0, None);
        let tokens_for_edge = new_acc.llm_tokens_union.clone();

        for source_ref in &new_acc.trie2_nodes {
            ...
            inserter.try_destination(*new_destination).expect("Cycle detected when merging trie2 nodes; this should be impossible.");
        }

        // Update the live tokens on the new destination node.
        god.0.write().unwrap().get_mut(*new_destination).value.live_tokens |= &tokens_for_edge;
...
pub(crate) fn deep_clone_gss_with_trie2_map(
    root: &Arc<GSSNode>,
    trie2_map: &HashMap<NodeId, PrecomputeNode2>,
) -> Arc<GSSNode> {
    fn clone_one(
        node: &Arc<GSSNode>,
        trie2_map: &HashMap<NodeId, PrecomputeNode2>,
        ...
        let out = match node.as_ref() {
            GSSNode::Root(root_node) => {
                ...
                if !new_acc.trie2_nodes.is_empty() {
                    ...
                    for old_ref in &new_acc.trie2_nodes {
                        let old_id = old_ref.id();
                        if let Some(new_ref) = trie2_map.get(&old_id) {
                            new_set.insert(*new_ref);
                        } else {
                            new_set.insert(*old_ref);
                        }
                    }
    ...
    let trie2_nodes_str = {
        ...
        if n == 0 {
            ...
            let ids: Vec<String> = acc
                .trie2_nodes
                .iter()
                .map(|node_ref| node_ref.id().to_string())
                .collect();
            Some(format!("Trie2(n={}, ids:[{}])", n, ids.join(", ")))
        } else {
...
#[test]
    ...
    fn test_merge_preserves_trie2_nodes() {
        ...
        let god = Trie2GodWrapper::new(Trie2God::new());
        let mut god_guard = god.0.write().unwrap();

        // --- GSS 1 Setup ---
        let trie2_node1 = god_guard.alloc_node(PrecomputedNodeContents::internal());
        let trie2_node2 = god_guard.alloc_node(PrecomputedNodeContents::internal());
        let trie2_node3 = god_guard.alloc_node(PrecomputedNodeContents::internal());
        drop(god_guard);
    ...
    fn test_merge_with_different_depth_predecessors() {
        ...
        // --- GSS A setup ---
        let trie2_node_a = god_guard.alloc_node(PrecomputedNodeContents::internal());
        ...
        // --- GSS B setup ---
        let trie2_node_b = god_guard.alloc_node(PrecomputedNodeContents::internal());
    ...
    fn test_merge_unions_trie2_nodes_across_identical_towers() {
        ...
        let t1 = god_guard.alloc_node(PrecomputedNodeContents::internal());
        let t2 = god_guard.alloc_node(PrecomputedNodeContents::internal());
        drop(god_guard);
