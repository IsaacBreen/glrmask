...
/// A key for the predecessor map, combining edge value and depth.
pub type PredecessorKey = (ParseStateEdgeContent, MaxDepth);
/// Maps a (edge, depth) key to a predecessor node.
type NodeMap = BTreeMap<PredecessorKey, Arc<GSSNode>>;
...
fn process_predecessors(incoming: &NodeSet) -> NodeMap {
    // Group by (edge_val, depth)
    let mut grouped: BTreeMap<PredecessorKey, Vec<Arc<GSSNode>>> = BTreeMap::new();
    for (pred_arc, edge_val) in incoming {
        let key = (edge_val.clone(), pred_arc.max_depth);
        grouped.entry(key).or_default().push(pred_arc.clone());
    }
...
impl GSSNode {
    ...
    fn new_with_single_predecessor(predecessor_arc: Arc<GSSNode>, edge_value: ParseStateEdgeContent, local_acc: Acc) -> Self {
        ...
        let key = (edge_value, predecessor_arc.max_depth);
        predecessors_map.insert(key, predecessor_arc.clone());
...
impl GSSNode {
    ...
    pub fn pop(&self) -> GSSPop {
        ...
        for ((edge_val, depth), pred_arc) in &self.predecessors {
            ...
            let key = (edge_val.clone(), *depth);
            new_node_map.insert(key, new_pred_arc);
        }
    ...
    pub fn peek_iter(&self) -> impl Iterator<Item = GSSPeek<'_>> {
        self.predecessors.iter().map(|((edge_val, _depth), pred_arc)| {
            GSSPeek {
                parent_node: self,
                edge_value: edge_val,
        ...
        Some((new_local_acc, continue_recursion)) => {
            let new_node_predecessors_map = if continue_recursion {
                let new_predecessors_set = node_arc.predecessors.iter()
                    .filter_map(|((edge_val, _depth), pred_arc)| {
...
pub fn fuse_predecessors_recursive(
    ...
    for ((edge_val, _depth), pred_arc) in &node_arc.predecessors {
...
fn get_structural_id(
    ...
    structural_cache: &mut BTreeMap<BTreeMap<PredecessorKey, usize>, usize>,
) -> usize {
    let node_ptr = node as *const GSSNode;
    ...
    for (key, pred_arc) in &node.predecessors {
        let pred_id = get_structural_id(pred_arc.as_ref(), memo, structural_cache);
        pred_structural_ids.insert(key.clone(), pred_id);
    }
...
pub fn find_longest_path(root_node: &Arc<GSSNode>) -> Option<Vec<(ParseStateEdgeContent, Arc<GSSNode>)>> {
    ...
    fn find_longest_recursive(
        ...
        for ((edge_val, _depth), pred_arc) in node_arc.predecessors.iter() {
...
pub fn print_gss_forest(
    ...
    fn print_predecessors_recursive(
        ...
        let predecessors: Vec<_> = node_arc.predecessors()
            .iter()
            .map(|((edge_val, _depth), pred_arc)| (edge_val, pred_arc))
            .collect();
