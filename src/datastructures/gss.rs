...
pub fn simplify(mut roots: Vec<Arc<GSSNode>>) -> Vec<Arc<GSSNode>> {
    ...
    // Pass 2: Hash-cons the forest (canonicalization).
    // Bottom-up rebuild with structural interning to maximize sharing:
    // - Rebuild each node from canonicalized children
    // - Deduplicate identical children vectors by pointer identity
    // - Intern the rebuilt node so identical structures share the same Arc
    fn canonicalize(
        node: &Arc<GSSNode>,
        rewire_memo: &mut HashMap<*const GSSNode, Arc<GSSNode>>,
        intern: &mut HashMap<GSSNode, Arc<GSSNode>>,
    ) -> Arc<GSSNode> {
        let ptr = Arc::as_ptr(node);
        if let Some(cached) = rewire_memo.get(&ptr) {
            return cached.clone();
        }

        let out_arc = match node.as_ref() {
            GSSNode::Root(r) => {
                // Candidate root with identical Acc; intern by structural equality.
                let candidate = GSSNode::new((*r.acc).clone());
                if let Some(shared) = intern.get(&candidate) {
                    shared.clone()
                } else {
                    let arc = Arc::new(candidate);
                    // Key is a value clone to leverage GSSNode::Eq/Hash on structure.
                    intern.insert(arc.as_ref().clone(), arc.clone());
                    arc
                }
            }
            GSSNode::Internal(i) => {
                // Rebuild successors from canonicalized children, regroup by edge and dest_key.
                let mut rebuilt: NodeMap = BTreeMap::new();
                for (edge_val, preds_by_depth) in &i.predecessors {
                    for (_dest_key, pred_vec) in preds_by_depth {
                        for pred_arc in pred_vec {
                            let c = canonicalize(pred_arc, rewire_memo, intern);
                            rebuilt
                ...
                // Deduplicate per-depth vectors by pointer identity to avoid duplicates.
                for by_depth in rebuilt.values_mut() {
                    for vec in by_depth.values_mut() {
                        vec.sort_by_key(|a| Arc::as_ptr(a) as usize);
                        vec.dedup_by_key(|a| Arc::as_ptr(a));
                    }
                }

                // Build candidate internal node and intern it.
                let candidate = GSSNode::new_with_map(Arc::new(Acc::new_fresh()), rebuilt);
                if let Some(shared) = intern.get(&candidate) {
                    shared.clone()
                } else {
                    let arc = Arc::new(candidate);
        ...
        rewire_memo.insert(ptr, out_arc.clone());
        out_arc
    }

    let mut rewire_memo: HashMap<*const GSSNode, Arc<GSSNode>> = HashMap::new();
