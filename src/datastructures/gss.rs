            ...
            if !continue_recursion || node_arc.is_root() {
                match node_arc.as_ref() {
                    GSSNode::Root(root) => {
                        ...
                        if acc_changed {
                            new_local_acc.needs_push_down = false;
...
#[test]
    ...
    #[test]
    fn test_allow_only_llm_tokens_and_prune_arc() {
        // --- Setup ---
        // Create a GSS with three branches ending in leaves with different token constraints.
        // leaf1 disallows token 1.
        // leaf2 disallows token 2.
        // leaf3 disallows token 3.
        //
        // Structure:
        //      root
        //      /   \
        ...
        let leaf1 = Arc::new(GSSNode::new(mock_acc(1)));
        let leaf2 = Arc::new(GSSNode::new(mock_acc(2)));
        let leaf3 = Arc::new(GSSNode::new(mock_acc(3)));

        let b = Arc::new(leaf1.push(mock_edge(10)));

        let mut c_preds = NodeSet::new();
        c_preds.insert((leaf2.clone(), mock_edge(20)));
        c_preds.insert((leaf3.clone(), mock_edge(30)));
        let c_preds_map = process_predecessors(&c_preds);
        let c = Arc::new(GSSNode::new_with_map(Arc::new(empty_acc()), c_preds_map));

        let mut root_preds = NodeSet::new();
        root_preds.insert((b.clone(), mock_edge(100)));
        root_preds.insert((c.clone(), mock_edge(200)));
        let root_preds_map = process_predecessors(&root_preds);
        let root_node = GSSNode::new_with_map(Arc::new(empty_acc()), root_preds_map);

        // --- Initial State Assertions ---
        let initial_allowed = root_node.allowed_llm_tokens();
        assert_eq!(initial_allowed, HybridBitset::max_ones());

        // --- Pruning Scenario 1: Allow only token 1 ---
        // This should prune the branch going through leaf1.
        let mut root_arc_1 = Arc::new(root_node.clone());
        let mut allowed1 = LLMTokenBV::zeros();
        allowed1.insert(1);
        let mut memo1 = HashMap::new();
        allow_only_llm_tokens_and_prune_arc(&mut root_arc_1, &allowed1, &mut memo1);

        assert_eq!(root_arc_1.allowed_llm_tokens(), allowed1);
        assert_eq!(root_arc_1.num_predecessors(), 1);
        let pred_c_arc = GSSNode::peek_iter(&root_arc_1).next().unwrap().predecessor_node().clone();
        assert_eq!(pred_c_arc.num_predecessors(), 2);
        assert_eq!(pred_c_arc.allowed_llm_tokens(), allowed1);

        // --- Pruning Scenario 2: Allow only token 2 ---
        let mut root_arc_2 = Arc::new(root_node.clone());
        let mut allowed2 = LLMTokenBV::zeros();
        allowed2.insert(2);
        let mut memo2 = HashMap::new();
        allow_only_llm_tokens_and_prune_arc(&mut root_arc_2, &allowed2, &mut memo2);
        assert_eq!(root_arc_2.allowed_llm_tokens(), allowed2);
        assert_eq!(root_arc_2.num_predecessors(), 2);
        let new_c_arc = GSSNode::peek_iter(&root_arc_2).find(|p| *p.edge_value() == mock_edge(200)).unwrap().predecessor_node().clone();
        assert_eq!(new_c_arc.num_predecessors(), 1);
        assert_eq!(*GSSNode::peek_iter(&new_c_arc).next().unwrap().edge_value(), mock_edge(30));

        // --- Pruning Scenario 3: Allow nothing ---
        let mut root_arc_3 = Arc::new(root_node.clone());
        let allowed_none = LLMTokenBV::zeros();
        let mut memo3 = HashMap::new();
        allow_only_llm_tokens_and_prune_arc(&mut root_arc_3, &allowed_none, &mut memo3);
        assert_eq!(*root_arc_3, GSSNode::new_fresh(), "A fully pruned GSS should be equivalent to a fresh GSS");
    }
