...
impl GSSPopper {
    pub fn new_from_node(node: Arc<GSSNode>, acc: Arc<Acc>) -> Self {
        ...
        if node.is_root() {
            // A root node has no incoming edge, so we cannot populate below_bottom.
            // The popper will be empty. Popping from it will do nothing.
        } else {
                    ...
                    for pred_vec in preds_by_depth.values() {
                        for child in pred_vec {
                            if child.is_root() {
                                ...
                                edge_map.entry(edge_value.clone()).and_modify(|e| *e = Arc::new(Acc::merge(e, &combined))).or_insert(combined);
                            } else {
...
mod tests {
    ...
    fn test_gss_pop() {
        ...
        let combined_acc_map = pop_result.below_bottom.get(&1).unwrap();
        let combined_acc = combined_acc_map.values().next().unwrap();
    ...
    fn test_popper_new_from_root_and_shift() {
        ...
        // Pop once to get to the root.
        let mut popper = pushed.popn(1);
        assert!(popper.paths.is_empty());
        assert_eq!(popper.below_bottom.len(), 1);
        let edge_map1 = popper.below_bottom.get(&1).unwrap();
        assert_eq!(edge_map1.len(), 1);
        let acc1 = edge_map1.values().next().unwrap().clone();

        // Pop again; entry should shift from 1 -> 2.
        popper.popn(1);
        assert!(popper.below_bottom.get(&0).is_none());
        assert!(popper.below_bottom.get(&1).is_none());
        let edge_map2 = popper.below_bottom.get(&2).unwrap();
        let acc2 = edge_map2.values().next().unwrap();
        assert_eq!(*acc1, **acc2);
    }
    ...
    fn test_popper_below_bottom_shifts_from_non_root() {
        ...
        let acc0 = popper.below_bottom.get(&1).unwrap().values().next().unwrap().clone();
    ...
    fn test_popper_merges_below_bottom_accs() {
        ...
        let acc_below = Arc::new(Acc::merge(acc1, acc2));
...
#[test]
    fn test_get_roots() {
        ...
        // Test from multiple sources. Accs should be merged.
        ...
        let merged_acc1 = Arc::new(Acc::merge(&expected.get(&mock_edge(1)).unwrap(), &path_acc_b_l1));
        expected_multi.insert(mock_edge(1), merged_acc1);
