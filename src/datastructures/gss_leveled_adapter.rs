...
impl GSSStats {
    pub fn unique_nodes(&self) -> usize { self.total_unique_nodes }
    pub fn total_edges(&self) -> usize { self.total_edges }
}
...
pub fn gather_gss_stats(roots: &[&GSSNode]) -> LeveledGSSStats<ParseStateEdgeContent, Acc> {
    ...
    for r in &active_roots { merged_gss = merged_gss.merge(&r.inner); }
...
impl PartialEq for GSSNode {
    fn eq(&self, other: &Self) -> bool {
        ...
        if (count & (count - 1)) == 0 {
            eprintln!(
                "PERFORMANCE WARNING: GSSNode::eq performs expensive full stack comparison. Total calls: {}",
                count
            );
        }
...
impl Hash for GSSNode {
    fn hash<H: Hasher>(&self, _state: &mut H) {
        // Intentionally left cheap; full stack hashing would be expensive.
    }
}
...
impl GSSNode {
    ...
    pub fn stored_trie_nodes(&self) -> BTreeSet<StoredPrecomputeNodeIndex> {
        // Stored trie nodes are no longer tracked.
        BTreeSet::new()
    }
...
pub(crate) fn allow_only_llm_tokens_and_prune_arc(
    ...
    transform_all(root_arc, |a| {
        ...
        if na.llm_tokens_union.is_empty() { None } else { Some(na) }
    });
}
...
pub fn fuse_predecessors_recursive(
    ...
    levels: usize,
    _memo: &mut HashMap<*const GSSNode, Arc<GSSNode>>,
) -> Arc<GSSNode> {
...
pub(crate) fn deep_add_precompute_trie_edges(
    ...
    // No-op: stored trie nodes are no longer tracked in Acc.
}
...
pub(crate) fn map_trie3_node_ids(
    _root_arc: &mut Arc<GSSNode>,
    _id_map: &HashMap<Trie2Index, Trie2Index>,
) {
    // No-op: stored trie nodes are no longer tracked in Acc.
}
