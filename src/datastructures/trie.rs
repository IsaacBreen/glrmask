        ...
        while let Some(node_arc) = queue.pop_front() {
            ...
            for children_map in node_guard.children.values() { // Use node_guard
                for node_ptr in children_map.keys() {
                    // A weak pointer may be temporarily dangling during graph transformations
                    // before a cleanup pass like `purge_dangling_weak_edges` runs.
                    // It is correct to simply not traverse down a dangling weak pointer.
    ...
    EK: Ord + Clone,
{
    ...
    pub fn promote_weak_edges_to_strong(
        ...
        // Visit all nodes reachable through either strong and weak edges.
        ...
        while let Some(src_arc) = queue.pop_front() {
            ...
            for (ek, dst_arc) in weak_edges {
                ...
                //  2) Raise dst.max_depth if needed and propagate to its descendants.
    ...
    /// Sweep pass: remove any dangling weak edges whose upgrade() returns None.
    /// Only nodes reachable via strong edges from the provided roots are visited.
    /// Returns the number of removed edges.
    pub fn purge_dangling_weak_edges(root_nodes: &[Arc<RwLock<Trie<EK, EV, T>>>]) -> usize {
        ...
        while let Some(node_arc) = queue.pop_front() {
            // Clean dangling weak edges on this node.
            let strong_children_after_cleanup: Vec<Arc<RwLock<Trie<EK, EV, T>>>> = {
                let mut guard = node_arc.write().expect("RwLock poisoned during purge_dangling_weak_edges");
                // Retain only edges where strong OR weak can still be upgraded.
                ...
                for (ek, dest_map) in guard.children_mut().iter_mut() {
                    ...
                    removed += before_len.saturating_sub(after_len);
                }
                // Remove empty edge-key maps
                for ek in empty_keys {
                    guard.children_mut().remove(&ek);
                }

                // Collect strong children to continue BFS
                guard.children().values()
                    .flat_map(|m| m.keys())
                    .filter_map(|np| {
                        if let NodePtr::Strong(apw) = np {
                            Some(apw.as_arc().clone())
                        } else {
                            None
                        }
                    })
            ...
            // Enqueue strong children
            for child_arc in strong_children_after_cleanup {
