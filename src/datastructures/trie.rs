...
#[derive(Debug, Clone)]
pub struct TrieWrapper<EK, EV, T>
where
    EK: Ord + Clone,
    EV: Clone,
    T: Clone,
{
    node: Arc<RwLock<Trie<EK, EV, T>>>,
}

impl<EK, EV, T> TrieWrapper<EK, EV, T>
where
    EK: Ord + Clone,
    EV: Clone,
    T: Clone,
{
    pub fn new(node: Arc<RwLock<Trie<EK, EV, T>>>) -> Self {
        TrieWrapper { node }
    }

    pub fn as_arc(&self) -> Arc<RwLock<Trie<EK, EV, T>>> {
        self.node.clone()
    }

    pub fn into_arc(self) -> Arc<RwLock<Trie<EK, EV, T>>> {
        self.node
    }

    pub fn read(&self) -> LockResult<RwLockReadGuard<'_, Trie<EK, EV, T>>> {
        self.node.read()
    }

    pub fn write(&self) -> LockResult<RwLockWriteGuard<'_, Trie<EK, EV, T>>> {
        self.node.write()
    }
}