...
use crate::datastructures::trie2::GodWrapper;
...
/// Represents a node in a Trie1–like structure (allowing shared subtrees and DAGs).
...
#[derive(Debug, Clone)]
pub struct Trie1<EK: Ord, EV, T> {
    pub value: T,
    /// Stores a map from EdgeKey to a map of destination nodes and edge values.
    children: BTreeMap<EK, OrderedHashMap<ArcPtrWrapper<RwLock<Trie1<EK, EV, T>>>, EV>>,
...
impl<EK, EV, T> JSONConvertible for Trie1<EK, EV, T>
where
    ...
    T: Clone + JSONConvertible,
{
    fn to_json(&self) -> JSONNode {
        ...
        // Maps the raw pointer of an Arc<RwLock<Trie1>> to its index in nodes_json_list
        let mut arc_ptr_to_idx_map: HashMap<*const RwLock<Trie1<EK, EV, T>>, usize> = HashMap::new();
        // Queue for BFS traversal, storing Arcs to keep them alive and allow locking
        let mut bfs_q: VecDeque<Arc<RwLock<Trie1<EK, EV, T>>>> = VecDeque::new();
        ...
        while let Some(current_arc) = bfs_q.pop_front() {
            ...
            let node_guard = current_arc.read().expect("RwLock poisoned during Trie1 serialization (BFS part)");
    ...
    fn from_json(node: JSONNode) -> Result<Self, String> {
        match node {
            JSONNode::Object(mut obj) => {
                let nodes_json = obj.remove("nodes").ok_or_else(|| "Missing 'nodes' field for Trie1 deserialization".to_string())?;
                let root_idx_json = obj.remove("root_idx").ok_or_else(|| "Missing 'root_idx' field for Trie1 deserialization".to_string())?;
                ...
                let mut deserialized_arcs: HashMap<usize, Arc<RwLock<Trie1<EK, EV, T>>>> = HashMap::new();
                ...
                for (i, node_data_json) in nodes_array.iter().enumerate() {
                    match node_data_json {
                        JSONNode::Object(n_obj) => {
                            ...
                            let new_node_arc = Arc::new(RwLock::new(Trie1 {
                                value,
            ...
            _ => Err("Expected JSONNode::Object for Trie1 graph structure".to_string()),
        }
    }
}
...
// Implementation block for core Trie1 functionality
// Added Clone bound for EK needed in insertion and others
impl<EK: Ord + Clone, EV, T> Trie1<EK, EV, T> {
    ...
    pub fn force_insert_to_new_node(&mut self, edge_key: EK, edge_value: EV, value: T) -> Arc<RwLock<Trie1<EK, EV, T>>> {
        let new_node = Arc::new(RwLock::new(Trie1::new(value)));
    ...
    pub fn force_insert_to_node(&mut self, edge_key: EK, edge_value: EV, dst: &Arc<RwLock<Trie1<EK, EV, T>>>) {
        let dst_comparable = ArcPtrWrapper::new(dst.clone());
        self.children.entry(edge_key).or_default().insert(dst_comparable, edge_value);
    }

    pub fn already_has_dst(&self, edge_key: EK, dst: &Arc<RwLock<Trie1<EK, EV, T>>>) -> bool {
    ...
    pub fn already_has_dst_for_any_key(&self, dst: &Arc<RwLock<Trie1<EK, EV, T>>>) -> bool {
        let lookup_key = ArcPtrWrapper::new(dst.clone());
        self.children.values().any(|dest_map| dest_map.contains_key(&lookup_key))
    }

    pub fn get_edge_value(&self, edge_key: EK, dst: &Arc<RwLock<Trie1<EK, EV, T>>>) -> Option<&EV> {
        let lookup_key = ArcPtrWrapper::new(dst.clone());
        self.children.get(&edge_key).and_then(|dest_map| dest_map.get(&lookup_key))
    }

    pub fn get_edge_value_mut(&mut self, edge_key: EK, dst: &Arc<RwLock<Trie1<EK, EV, T>>>) -> Option<&mut EV> {
        let lookup_key = ArcPtrWrapper::new(dst.clone());
    ...
    pub fn try_insert(
        ...
        child: Arc<RwLock<Trie1<EK, EV, T>>>,
    ) {
    ...
    pub fn get(
        &self,
        edge_key: &EK,
    ) -> Option<&OrderedHashMap<ArcPtrWrapper<RwLock<Trie1<EK, EV, T>>>, EV>>
    {
    ...
    pub fn get_mut(
        &mut self,
        edge_key: &EK,
    ) -> Option<&mut OrderedHashMap<ArcPtrWrapper<RwLock<Trie1<EK, EV, T>>>, EV>>
    {
        self.children.get_mut(edge_key)
    }

    pub fn children(&self) -> &BTreeMap<EK, OrderedHashMap<ArcPtrWrapper<RwLock<Trie1<EK, EV, T>>>, EV>> {
        &self.children
    }

    pub fn children_mut(&mut self) -> &mut BTreeMap<EK, OrderedHashMap<ArcPtrWrapper<RwLock<Trie1<EK, EV, T>>>, EV>> {
        &mut self.children
    }
    ...
    pub fn all_nodes(roots: &[Arc<RwLock<Trie1<EK, EV, T>>>]) -> Vec<Arc<RwLock<Trie1<EK, EV, T>>>> {
        let mut visited_arcs: HashSet<*const RwLock<Trie1<EK, EV, T>>> = HashSet::new();
...
// Helper to get the raw pointer to the Trie1 data from an Arc<RwLock<Trie1>>.
...
#[allow(dead_code)]
pub(crate) fn try_get_node_data_ptr<EK: Ord, EV, T>(node_arc: &Arc<RwLock<Trie1<EK, EV, T>>>) -> Option<*const Trie1<EK, EV, T>> {
    match node_arc.try_read() {
        Ok(guard) => {
            let ptr = &*guard as *const Trie1<EK, EV, T>;
            Some(ptr)
        }
...
#[allow(dead_code)]
pub(crate) fn node_ptr<EK: Ord, EV, T>(node_arc: &Arc<RwLock<Trie1<EK, EV, T>>>) -> *const Trie1<EK, EV, T> {
...
impl<EK, EV, T> Trie1<EK, EV, T>
where
    EK: Ord,
    EV: PartialEq + Clone,
    T: PartialEq,
{
    /// Recursively compares two Trie1 nodes wrapped in Arcs for equality.
    ///
    /// - `self_arc`, `other_arc`: The Arcs pointing to the Trie1 nodes to compare.
    ...
    fn compare_arcs_recursive(
        self_arc: &Arc<RwLock<Trie1<EK, EV, T>>>,
        other_arc: &Arc<RwLock<Trie1<EK, EV, T>>>,
        ...
        for (self_ek, self_dest_map) in &self_node.children {
            match other_node.children.get(self_ek) {
                ...
                Some(other_dest_map) => {
                    ...
                    let self_child_pairs: Vec<(Arc<RwLock<Trie1<EK, EV, T>>>, EV)> = self_dest_map.iter()
                        .map(|(apw, ev)| (apw.as_arc().clone(), ev.clone()))
                        .collect();

                    let mut other_child_pairs: Vec<(Arc<RwLock<Trie1<EK, EV, T>>>, EV)> = other_dest_map.iter()
                        .map(|(apw, ev)| (apw.as_arc().clone(), ev.clone()))
                        .collect();
                    ...
                    'self_pair_loop: for (s_arc, s_ev) in &self_child_pairs {
                        ...
                        for i in 0..other_child_pairs.len() {
                            if s_ev == &other_child_pairs[i].1 {
                                ...
                                if Trie1::compare_arcs_recursive(s_arc, &o_arc_for_recursion, comparison_cache) {
...
impl<T: Clone, EK: Ord + Clone, EV: Clone> Trie1<EK, EV, T> {
    fn count_all_edges(root_nodes: &[Arc<RwLock<Trie1<EK, EV, T>>>]) -> usize {
        let mut visited_arcs: HashSet<*const RwLock<Trie1<EK, EV, T>>> = HashSet::new();
        let mut queue: VecDeque<Arc<RwLock<Trie1<EK, EV, T>>>> = VecDeque::new();
    ...
    pub fn special_map<V: Clone>(
        initial_nodes_and_values: Vec<(Arc<RwLock<Trie1<EK, EV, T>>>, V)>,
        mut step: impl FnMut(&V, &EK, &EV, &Trie1<EK, EV, T>) -> Option<V>,
        mut merge: impl FnMut(&mut V, V),
        mut process: impl FnMut(&Trie1<EK, EV, T>, &mut V) -> bool,
    ) {
        // ------------------------------------------------------------------
    ...
    where
        V: Clone,
        S: FnMut(
            &V, &EK, &OrderedHashMap<ArcPtrWrapper<RwLock<Trie1<EK, EV, T>>>, EV>
        ) -> I,
        I: IntoIterator<Item = (ArcPtrWrapper<RwLock<Trie1<EK, EV, T>>>, V)>,
    {
        // ------------------------------------------------------------------
...
// Implement PartialEq for Trie1
impl<EK, EV, T> PartialEq for Trie1<EK, EV, T>
where
    EK: Ord,
    EV: PartialEq + Clone,
    T: PartialEq,
{
    fn eq(&self, other: &Self) -> bool {
        ...
        type NodeRwLockPtr<EKK, EVV, TT> = *const RwLock<Trie1<EKK, EVV, TT>>;
        ...
        for (self_ek, self_dest_map) in &self.children {
            match other.children.get(self_ek) {
                ...
                Some(other_dest_map) => {
                    ...
                    let self_child_pairs: Vec<(Arc<RwLock<Trie1<EK, EV, T>>>, &EV)> = self_dest_map
                        .iter()
                        .map(|(np, ev)| (np.as_arc().clone(), ev))
                        .collect();

                    let mut other_child_pairs: Vec<(Arc<RwLock<Trie1<EK, EV, T>>>, EV)> = other_dest_map
                        .iter()
                    ...
                    'self_pair_loop: for (s_arc, s_ev) in self_child_pairs {
                        for i in 0..other_child_pairs.len() {
                            if s_ev == &other_child_pairs[i].1 {
                                let o_arc_for_recursion = other_child_pairs[i].0.clone();
                                if Trie1::compare_arcs_recursive(&s_arc, &o_arc_for_recursion, &mut comparison_cache) {
                                    other_child_pairs.remove(i);
...
// Implement Eq for Trie1
impl<EK, EV, T> Eq for Trie1<EK, EV, T>
where
    EK: Ord,
    EV: Eq + Clone,
    T: Eq,
...
// Implement Hash for Trie1
impl<EK, EV, T> Hash for Trie1<EK, EV, T>
where
    EK: Ord + Hash,
    ...
    T: PartialEq + Hash,
{
    fn hash<H: Hasher>(&self, state: &mut H) {
        ...
        // Maps the raw data pointer of a Trie1 node to a marker (depth) to break revisits.
        let mut recursion_marker: HashMap<*const Trie1<EK, EV, T>, usize> = HashMap::new();
...
where
    ...
    T: PartialEq + Hash,
{
    /// Helper function to hash a &Trie1 instance.
    fn hash_trie_recursive<S: Hasher>(
        node: &Trie1<EK, EV, T>,
        state: &mut S,
        recursion_marker: &mut HashMap<*const Trie1<EK, EV, T>, usize>,
        current_depth: usize,
    ) {
...
/// A helper struct to facilitate inserting an edge into a Trie1,
...
{
    source_arc: Arc<RwLock<Trie1<EK, EV, T>>>, // The source node for the edge
    ...
    result: Option<Arc<RwLock<Trie1<EK, EV, T>>>>, // Stores the successful destination node
}
    ...
    FMergeEV_T: FnMut(&mut EV, &T),
{
    ...
    pub fn new(
        ...
        source_arc: Arc<RwLock<Trie1<EK, EV, T>>>,
        edge_key: EK,
        edge_value: EV,
    ...
    #[time_it]
    pub fn try_destination(mut self, destination: Arc<RwLock<Trie1<EK, EV, T>>>) -> Self {
        if self.result.is_some() {
            return self; // Already found a destination
        }

        let mut update_info: Option<(Arc<RwLock<Trie1<EK, EV, T>>>, EV)> = None;
    ...
    /// Returns `self` to allow chaining.
    #[time_it]
    pub fn try_destinations(mut self, destinations: &[Arc<RwLock<Trie1<EK, EV, T>>>]) -> Self {
    ...
    #[time_it]
    pub fn try_destinations_iter(mut self, destinations: impl Iterator<Item = Arc<RwLock<Trie1<EK, EV, T>>>>) -> Self {
        for destination in destinations {
            if self.result.is_some() {
                break; // Stop trying once a destination is found
            }
    ...
    where
        F: Fn() -> R,
        R: Iterator<Item = Arc<RwLock<Trie1<EK, EV, T>>>>,
    {
        for destination in destinations() {
            if self.result.is_some() {
                break;
    ...
    pub fn try_children(mut self) -> Self {
        ...
        let children_for_this_key: Vec<Arc<RwLock<Trie1<EK, EV, T>>>> = {
    ...
    pub fn else_create_destination_with_value(mut self, value: T) -> Self {
        ...
        let new_node_arc = Arc::new(RwLock::new(Trie1::new(value)));
        let edge_val_clone = self.edge_value.as_ref().unwrap().clone();
    ...
    pub fn into_option(self) -> Option<Arc<RwLock<Trie1<EK, EV, T>>>> {
        self.result
    }
    ...
    pub fn clone_into_option(&self) -> Option<Arc<RwLock<Trie1<EK, EV, T>>>> {
        self.result.clone()
    }

    /// Returns the resulting destination node, panicking if none was found or created.
    pub fn unwrap(self) -> Arc<RwLock<Trie1<EK, EV, T>>> {
    ...
    pub fn expect(self, msg: &str) -> Arc<RwLock<Trie1<EK, EV, T>>> {
        self.result.expect(msg)
    }
}

// Optional: Add a convenience method to Trie1 to create an EdgeInserter easily.
impl<EK: Ord + Clone + Debug, EV: Clone + Debug, T: Clone> Trie1<EK, EV, T> {
    ...
    /// }
    ///
    /// let root_node: Arc<RwLock<Trie1<String, i32, NodeValue>>> = Arc::new(RwLock::new(Trie1::new(NodeValue::default())));
    ///
    /// let potential_destinations: Vec<Arc<RwLock<Trie1<String, i32, NodeValue>>>> = vec![/* ... */];
    ///
...
/// Returns `Some(Arc<RwLock<Trie1<...>>>)` if merge or insert succeeded.
pub fn try_destination<EK, EV, T, FMergeEV, FUpdateT, FMergeEV_T>(
    god: &GodWrapper<EK, EV, T>,
    source: Arc<RwLock<Trie1<EK, EV, T>>>,
    edge_key: EK,
    edge_value: EV,
    destination: Arc<RwLock<Trie1<EK, EV, T>>>,
    merge_edge_value: FMergeEV,
    update_node_value: FUpdateT,
    merge_edge_value_and_source_node_value: FMergeEV_T,
) -> Option<Arc<RwLock<Trie1<EK, EV, T>>>>
where
    EK: Ord + Clone + Debug,
    EV: Clone + Debug,
    T: Clone,
...
pub fn try_destination_with<EK, EV, T, FMergeEV, FUpdateT, FMergeEV_T>(
    ...
    edge_key: EK,
    edge_value: EV,
    destinations: &[Arc<RwLock<Trie1<EK, EV, T>>>],
    merge_edge_value: FMergeEV,
    update_node_value: FUpdateT,
    merge_edge_value_and_source_node_value: FMergeEV_T,
) -> Option<Arc<RwLock<Trie1<EK, EV, T>>>>
