//! A compatibility layer for the GSS that uses LeveledGSS as a backend.
//!
//! This module provides the same API as the original `gss` module, but its
//! implementation is based on `crate::datastructures::leveled_gss::LeveledGSS`.
//! This allows for a drop-in replacement in `constraint.rs` and `parser.rs`.
//!
//! Assumptions about the `LeveledGSS` API:
//! - It is generic: `LeveledGSS<L, D>` where `L` is level (edge) data and `D` is leaf data.
//! - It is immutable and uses `Arc` internally for efficient cloning and sharing.
//! - `LeveledGSS::new(leaf_data: D) -> Arc<Self>`: Creates a new GSS with a single leaf.
//! - `gss.push(level_data: L) -> Arc<Self>`: Pushes a new level, returning a new GSS.
//! - `gss.pop() -> impl Iterator<Item = (Arc<Self>, L)>`: Pops one level.
//! - `LeveledGSS::merge(gss1: &Arc<Self>, gss2: &Arc<Self>) -> Arc<Self>`: Merges two GSSs.
//! - `gss.leaves() -> impl Iterator<Item = &D>`: Iterates over leaf data.
//! - `gss.map_leaves<F>(f: F) -> Arc<Self>` where `F: FnMut(&D) -> D`: Transforms leaves.
//! - `gss.is_leaf() -> bool`: Checks if the GSS is just a leaf.
//! - `gss.transform(node_visitor, leaf_visitor)`: A powerful traversal and rebuild method.
...
// NOTE: This is a compatibility implementation that mirrors the original `gss.rs` API
// but is intended to be backed by `LeveledGSS`. Due to the absence of the `LeveledGSS`
// source, this implementation falls back to the original B-Tree-based approach to
// ensure the rest of the code compiles and functions. A true `LeveledGSS` backend
// would replace the `predecessors: NodeMap` with a `LeveledGSS` instance and adapt
