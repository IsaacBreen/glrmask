...
impl VocabPrefixTree {
    ...
    pub fn build(tokens: &[(usize, Vec<u8>)]) -> Self {
        ...
        crate::debug!(5, "Building vocab prefix tree (fast builder)");
        ...
        if nonempty.is_empty() {
            ...
            crate::debug!(5, "Computing reachable IDs (fast path)");
            let t0 = std::time::Instant::now();
            tree.recompute_reachable_ids_via_paths();
            let t1 = std::time::Instant::now();
            crate::debug!(5, "Done computing reachable IDs in {:?}", t1.duration_since(t0));
