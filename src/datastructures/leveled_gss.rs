...
impl<T: Clone + Eq + Hash, A: Merge + Clone + Eq + Hash> LeveledGSS<T, A> {
    ...
    /// Generates a canonical string representation of the GSS, ignoring memory addresses.
    /// This is useful for comparing the structure of two different GSS objects.
    pub fn to_canonical_string(&self, upper_only: bool) -> String
    where
        T: std::fmt::Debug,
        A: std::fmt::Debug,
    {
        let mut upper_ids = StdHashMap::new();
        let mut lower_ids = StdHashMap::new();
        let mut next_id = 0;
        let mut printed_upper_ids = HashSet::new();
        let mut printed_lower_ids = HashSet::new();
        let mut output_lines = Vec::new();

        let root = &self.inner;

        output_lines.push(format!("--- Root {} ---", Self::get_node_info_canonical_upper(root, &mut upper_ids, &mut lower_ids, &mut next_id)));
        Self::format_recursive_canonical_upper(
            root,
            "",
            &mut printed_upper_ids,
            &mut printed_lower_ids,
            &mut upper_ids,
            &mut lower_ids,
            &mut next_id,
            &mut output_lines,
    ...
    fn get_node_info_canonical_lower(node: &Arc<Lower<T>>, lower_ids: &mut StdHashMap<usize, usize>, next_id: &mut usize) -> String
    where
        T: std::fmt::Debug,
    {
        let node_ptr = Arc::as_ptr(node) as usize;
        let node_id = *lower_ids.entry(node_ptr).or_insert_with(|| { let id = *next_id; *next_id += 1; id });
        let mut info = format!(
            "Lower @L{} (MaxDepth: {})",
            node_id,
            node.max_depth
    ...
    fn get_node_info_canonical_upper(node: &Arc<Upper<T, A>>, upper_ids: &mut StdHashMap<usize, usize>, lower_ids: &mut StdHashMap<usize, usize>, next_id: &mut usize) -> String
    where
        T: std::fmt::Debug,
        A: std::fmt::Debug,
    {
        let node_ptr = Arc::as_ptr(node) as usize;
        let node_id = *upper_ids.entry(node_ptr).or_insert_with(|| { let id = *next_id; *next_id += 1; id });

        match &**node {
            Upper::Branch(b) => {
                let mut info = format!(
                    "UpperBranch @U{} (MaxDepth: {})",
                    node_id,
                    b.max_depth
                );
                if let Some(e) = &b.empty {
            ...
            Upper::Interface(i) => {
                let lower_ptr = Arc::as_ptr(&i.inner) as usize;
                let lower_id = *lower_ids.entry(lower_ptr).or_insert_with(|| { let id = *next_id; *next_id += 1; id });
                let mut info = format!(
                    "Interface @U{} -> Lower @L{} (MaxDepth: {}) | acc: {:?}",
                    node_id,
                    lower_id,
                    node.max_depth(),
                    i.acc
    ...
    fn format_recursive_canonical_lower(
        node: &Arc<Lower<T>>,
        current_prefix: &str,
        printed_lower_ids: &mut HashSet<usize>,
        lower_ids: &mut StdHashMap<usize, usize>,
        next_id: &mut usize,
        ...
        T: std::fmt::Debug,
    {
        let node_ptr = Arc::as_ptr(node) as usize;
        let node_id = *lower_ids.entry(node_ptr).or_insert_with(|| { let id = *next_id; *next_id += 1; id });
        printed_lower_ids.insert(node_id);
        ...
        for (i, (label, child)) in children_to_print.into_iter().enumerate() {
            ...
            let child_id = *lower_ids.entry(child_ptr).or_insert_with(|| { let id = *next_id; *next_id += 1; id });

            if printed_lower_ids.contains(&child_id) {
                let line = format!(
                    "{}{}{} -> Ref to Node @L{}",
                ...
                let line = format!(
                    ...
                    prefix_char,
                    label,
                    Self::get_node_info_canonical_lower(&child, lower_ids, next_id)
                );
                output_lines.push(line);

                let child_prefix = format!("{}{}", current_prefix, if is_last { "    " } else { "│   " });
                Self::format_recursive_canonical_lower(&child, &child_prefix, printed_lower_ids, lower_ids, next_id, output_lines);
            }
        }
    }

    fn format_recursive_canonical_upper(
        node: &Arc<Upper<T, A>>,
        current_prefix: &str,
        printed_upper_ids: &mut HashSet<usize>,
        printed_lower_ids: &mut HashSet<usize>,
        upper_ids: &mut StdHashMap<usize, usize>,
        lower_ids: &mut StdHashMap<usize, usize>,
        next_id: &mut usize,
        output_lines: &mut Vec<String>,
        upper_only: bool,
        ...
        A: std::fmt::Debug,
    {
        let node_ptr = Arc::as_ptr(node) as usize;
        let node_id = *upper_ids.entry(node_ptr).or_insert_with(|| { let id = *next_id; *next_id += 1; id });
        printed_upper_ids.insert(node_id);
        ...
        match &**node {
            Upper::Branch(b) => {
                ...
                for (i, (label, child)) in children_to_print.into_iter().enumerate() {
                    ...
                    let child_ptr = Arc::as_ptr(&child) as usize;
                    let child_id = *upper_ids.entry(child_ptr).or_insert_with(|| { let id = *next_id; *next_id += 1; id });

                    if printed_upper_ids.contains(&child_id) {
                        let line = format!(
                            "{}{}{} -> Ref to Node @U{}",
                            current_prefix, prefix_char, label, child_id
                        );
                        ...
                        let line = format!(
                            ...
                            prefix_char,
                            label,
                            Self::get_node_info_canonical_upper(&child, upper_ids, lower_ids, next_id)
                        );
                        output_lines.push(line);
            ...
            Upper::Interface(i) => {
                ...
                for (i_idx, (label, child)) in children_to_print.into_iter().enumerate() {
                    ...
                    if printed_lower_ids.contains(&child_id) {
                        ...
                        let child_prefix =
                            format!("{}{}", current_prefix, if is_last { "    " } else { "│   " });
                        Self::format_recursive_canonical_lower(
                            &child,
                            &child_prefix,
                            printed_lower_ids,
                            lower_ids,
