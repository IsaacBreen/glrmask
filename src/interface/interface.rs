...
impl GrammarDefinition {
    /// Optimizes the grammar by removing unreachable productions, inlining intermediate
    /// non-terminals, and deduplicating terminals.
    pub fn optimize(&mut self) {
        ...
        for _ in 0..10 {
            ...
            changed = false;
            changed |= self.remove_unreachable_productions(&start_symbol);
            changed |= self.inline_non_terminals(&start_symbol);
        }

        // Final cleanup pass
        self.remove_unreachable_productions(&start_symbol);
        self.deduplicate_terminals();

        // Restore start_production_id as indices might have shifted
        if let Some(idx) = self
            .productions
            .iter()
            .position(|p| p.lhs == start_symbol)
        {
            self.start_production_id = idx;
        }
    }

    fn remove_unreachable_productions(&mut self, start_symbol: &NonTerminal) -> bool {
        let initial_len = self.productions.len();
        let mut reachable = HashSet::new();
        let mut queue = vec![start_symbol.clone()];
        reachable.insert(start_symbol.clone());

        // Build adjacency map: LHS -> Set<RHS NonTerminals>
        let mut lhs_to_rhs_nts: HashMap<NonTerminal, HashSet<NonTerminal>> = HashMap::new();
        for prod in &self.productions {
            let entry = lhs_to_rhs_nts.entry(prod.lhs.clone()).or_default();
            for sym in &prod.rhs {
        ...
        while idx < queue.len() {
            ...
            if let Some(children) = lhs_to_rhs_nts.get(current) {
        ...
        self.productions.retain(|p| reachable.contains(&p.lhs));
        self.productions.len() != initial_len
    }

    fn inline_non_terminals(&mut self, start_symbol: &NonTerminal) -> bool {
        // Count usages of each non-terminal
        let mut usage_counts: HashMap<NonTerminal, usize> = HashMap::new();
        for prod in &self.productions {
            for sym in &prod.rhs {
                if let Symbol::NonTerminal(nt) = sym {
                    *usage_counts.entry(nt.clone()).or_default() += 1;
                }
            }
        }

        // Group productions by LHS
        let mut productions_by_lhs: HashMap<NonTerminal, Vec<Production>> = HashMap::new();
        for prod in &self.productions {
            productions_by_lhs
        ...
        let mut to_inline = HashSet::new();
        for (nt, count) in &usage_counts {
            if nt == start_symbol {
                continue;
            }

            // Criteria for inlining:
            // 1. Used exactly once (safe to inline regardless of complexity)
            // 2. Defined by a single production (simple substitution), unless it's self-recursive
            let prods = productions_by_lhs.get(nt);
            if let Some(prods) = prods {
                let is_recursive = prods.iter().any(|p| p.rhs.contains(&Symbol::NonTerminal(nt.clone())));
                if is_recursive {
                    continue;
                }

                if *count == 1 || prods.len() == 1 {
                    to_inline.insert(nt.clone());
                }
            }
        }
        ...
        for prod in &self.productions {
            if to_inline.contains(&prod.lhs) {
                // Skip productions of inlined non-terminals (they are removed)
                // Unless the non-terminal is still used (e.g. we only inlined some usages? No, we inline all or nothing here)
                // Wait, if we inline based on usage_count==1, we remove it.
                // If usage_count > 1 but single production, we remove it (substitute everywhere).
                continue;
            }

            // Check if this production needs expansion
            let needs_expansion = prod.rhs.iter().any(|sym| match sym {
                Symbol::NonTerminal(nt) => to_inline.contains(nt),
                _ => false,
            });
            ...
            changed = true;
            // Perform expansion (Cartesian product if multiple inlined symbols or multiple rules for an inlined symbol)
            let mut current_rhss = vec![Vec::new()];

            for sym in &prod.rhs {
                match sym {
                    Symbol::NonTerminal(nt) if to_inline.contains(nt) => {
                        let inline_prods = &productions_by_lhs[nt];
                        let mut next_rhss = Vec::new();
                        for existing_rhs in current_rhss {
                            for inline_prod in inline_prods {
                                let mut new_rhs = existing_rhs.clone();
                                new_rhs.extend(inline_prod.rhs.clone());
                                next_rhss.push(new_rhs);
                            }
                        }
                        current_rhss = next_rhss;
                    }
                    _ => {
                        for rhs in &mut current_rhss {
                            rhs.push(sym.clone());
                        }
                    }
                }
    ...
    fn deduplicate_terminals(&mut self) -> bool {
        let mut expr_to_group_id: BTreeMap<Expr, usize> = BTreeMap::new();
        let mut remapping: HashMap<usize, usize> = HashMap::new();
        let mut new_group_id_to_expr = BTreeMap::new();

        for (group_id, expr) in &self.group_id_to_expr {
            if let Some(&existing_id) = expr_to_group_id.get(expr) {
                remapping.insert(*group_id, existing_id);
            } else {
                expr_to_group_id.insert(expr.clone(), *group_id);
                new_group_id_to_expr.insert(*group_id, expr.clone());
            }
        }
        ...
        self.group_id_to_expr = new_group_id_to_expr;

        // Remap regex names
        for group_id in self.regex_name_to_group_id.right_values_mut() {
            if let Some(&new_id) = remapping.get(group_id) {
                *group_id = new_id;
            }
        }

        // Remap literals
        for group_id in self.literal_to_group_id.right_values_mut() {
            if let Some(&new_id) = remapping.get(group_id) {
                *group_id = new_id;
            }
        }
        ...
        // Remap externals
        for group_id in self.external_name_to_group_id.right_values_mut() {
            if let Some(&new_id) = remapping.get(group_id) {
                *group_id = new_id;
            }
        }
