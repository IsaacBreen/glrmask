    ...
    fn to_json(&self) -> JSONNode {
        ...
        for (name, group_id) in &self.regex_name_to_group_id {
            ...
            let expr = self.group_id_to_expr.get(group_id)
                .unwrap_or_else(|| panic!("Internal consistency error: group_id {} for name '{}' not found in group_id_to_expr.", group_id, name))
                .clone();
...
impl GrammarDefinition {
    ...
    pub fn add_external_terminal(&mut self, name: &str) -> usize {
        ...
        let all_gids: BTreeSet<usize> = self.group_id_to_expr.keys().copied()
...
impl GrammarDefinition {
    ...
    fn try_convert_to_regex(
        expr: &GrammarExpr,
        terminals: &BTreeMap<String, Expr>,
    ) -> Option<Expr> {
        match expr {
            GrammarExpr::Ref(name) => terminals.get(name).cloned(),
            GrammarExpr::Literal(bytes) => Some(Expr::U8Seq(bytes.clone())),
            GrammarExpr::AnyChar => Some(Expr::U8Class(U8Set::all())),
            GrammarExpr::Sequence(exprs) => {
                let mut converted = Vec::new();
                for e in exprs {
                    converted.push(Self::try_convert_to_regex(e, terminals)?);
                }
                Some(Expr::Seq(converted))
            }
            GrammarExpr::Choice(exprs) => {
                let mut converted = Vec::new();
                for e in exprs {
                ...
                Some(Expr::Choice(converted))
            }
            GrammarExpr::Optional(e) => {
                let converted = Self::try_convert_to_regex(e, terminals)?;
                Some(Expr::Quantifier(Box::new(converted), QuantifierType::ZeroOrOne))
            }
            GrammarExpr::Repeat(e) => {
                let converted = Self::try_convert_to_regex(e, terminals)?;
                Some(Expr::Quantifier(Box::new(converted), QuantifierType::ZeroOrMore))
            }
            GrammarExpr::CharClass(s) => {
                ...
                let mut parse_char = |it: &mut std::iter::Peekable<std::str::Chars>| -> Option<char> {
                    if let Some(c) = it.next() {
                        if c == '\\' {
                            if let Some(escaped_char) = it.next() {
                                Some(match escaped_char {
                                    'n' => '\n',
                                    't' => '\t',
                                    'r' => '\r',
                                    'b' => '\u{0008}',
                ...
                Some(Expr::U8Class(if negated { u8set.complement() } else { u8set }))
            }
        }
    }

    fn optimize_grammar_rules(
        mut grammar_exprs: Vec<(String, GrammarExpr)>,
        mut terminals: BTreeMap<String, Expr>,
    ) -> (Vec<(String, GrammarExpr)>, BTreeMap<String, Expr>) {
        let start_symbol = if !grammar_exprs.is_empty() {
            grammar_exprs[0].0.clone()
        } else {
            return (grammar_exprs, terminals);
        };

        let mut changed = true;
        while changed {
            changed = false;
            let mut next_grammar_exprs = Vec::new();
            let mut newly_converted = Vec::new();

            for (name, expr) in grammar_exprs {
                if let Some(regex_expr) = Self::try_convert_to_regex(&expr, &terminals) {
                    newly_converted.push((name, regex_expr));
                    changed = true;
                } else {
                    next_grammar_exprs.push((name, expr));
                }
            }

            for (name, expr) in newly_converted {
                terminals.insert(name, expr);
            }
            grammar_exprs = next_grammar_exprs;
        }

        let start_symbol_still_exists = grammar_exprs.iter().any(|(n, _)| n == &start_symbol);
        if !start_symbol_still_exists && terminals.contains_key(&start_symbol) {
            let root_name = format!("{}_root", start_symbol);
            grammar_exprs.insert(0, (root_name, GrammarExpr::Ref(start_symbol)));
        }

        (grammar_exprs, terminals)
    }
    ...
    pub fn from_exprs(
        ...
        let mut terminals_map: BTreeMap<String, Expr> = regex_exprs.into_iter().collect();
        let (grammar_exprs, terminals_map) = Self::optimize_grammar_rules(grammar_exprs, terminals_map);
        let regex_exprs: Vec<(String, Expr)> = terminals_map.into_iter().collect();
