...
mod tests {
    ...
    #[test]
    fn test_terminal_merge_simple_pair() {
        let terminals = vec![
            ("T1".to_string(), eat_u8(b'A')),
            ("T2".to_string(), eat_u8(b'B')),
        ];
        let rules = vec![(
            "S".to_string(),
            sequence(vec![
                crate::interface::r#ref("T1"),
                crate::interface::r#ref("T2"),
            ]),
        )];

        let grammar_def = GrammarDefinition::from_exprs(rules, terminals).unwrap();

        // Find the merged terminal whose regex is "AB".
        let mut merged_gid: Option<usize> = None;
        for (gid, expr) in &grammar_def.group_id_to_expr {
            if let RegexExpr::U8Seq(bytes) = expr {
                if bytes == b"AB" {
                    merged_gid = Some(*gid);
                    break;
                }
        ...
        assert!(
            merged_gid.is_some(),
            "Expected a merged 'AB' terminal to be created"
        );
        let merged_gid = merged_gid.unwrap();

        // There should be exactly one production for S, with a single symbol on the RHS,
        // referring to the merged terminal.
        let s_prods: Vec<&Prod> = grammar_def
            .productions
            .iter()
            .filter(|p| p.lhs.0 == "S")
            .collect();
        assert_eq!(s_prods.len(), 1, "Expected exactly one production for S");
        let s_prod = s_prods[0];
        assert_eq!(
            s_prod.rhs.len(),
            1,
            "S production should have a single symbol after merging"
        );

        match &s_prod.rhs[0] {
            Sym::Terminal(Terminal::RegexName(name)) => {
                let gid = grammar_def
                    .regex_name_to_group_id
                    .get_by_left(name)
                    .expect("Merged terminal name missing from regex_name_to_group_id");
                assert_eq!(
                    *gid, merged_gid,
                    "S should reference the merged terminal's group id"
                );
            }
            other => panic!(
                "Unexpected RHS symbol for S after merge: {:?}",
                other
            ),
        }

        // Ensure original terminals T1 and T2 no longer appear in any production RHS.
        for prod in &grammar_def.productions {
            for sym in &prod.rhs {
                if let Sym::Terminal(Terminal::RegexName(name)) = sym {
                    assert_ne!(name, "T1", "T1 should not appear in productions after merge");
                    assert_ne!(name, "T2", "T2 should not appear in productions after merge");
                }
            }
        }
    }
    ...
    #[test]
    fn test_terminal_merge_requires_single_use() {
        let terminals = vec![
            ("T1".to_string(), eat_u8(b'A')),
            ("T2".to_string(), eat_u8(b'B')),
        ];
        let rules = vec![
            ...
            // Second use of T1: this should prevent merging T1 and T2.
            (
                "R".to_string(),
                crate::interface::r#ref("T1"),
            ),
        ...
        // Because T1 is used twice, S should still have two symbols: T1 followed by T2.
        assert_eq!(
            s_prod.rhs.len(),
            2,
            "S should still have two symbols when T1 is used more than once"
        );

        match (&s_prod.rhs[0], &s_prod.rhs[1]) {
            (
                Sym::Terminal(Terminal::RegexName(name1)),
                Sym::Terminal(Terminal::RegexName(name2)),
            ) => {
                assert_eq!(name1, "T1");
                assert_eq!(name2, "T2");
            }
            other => panic!(
                "Unexpected RHS for S without merge: {:?}",
                other
            ),
        }
    }
