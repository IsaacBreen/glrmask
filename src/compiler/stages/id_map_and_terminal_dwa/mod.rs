...
pub(crate) fn install_secondary_virtual_state_space(tokenizer: &mut Tokenizer, vocab: &Vocab) {
    ...
    // The virtual product must be closed under every byte that compiler paths
    // may query. Mask lookahead is vocab-token driven, but shared flat transition
    // tables and arbitrary byte commits can still ask about any byte. Use all
    // bytes here so missing product pairs are real invariant failures rather
    // than artifacts of an under-enumerated closure.
