...
pub(crate) fn build_partition_id_map_and_terminal_dwa(
    ...
    let choose_smaller_state_map = |structural: Option<ManyToOneIdMap>| {
        structural
            .filter(|map| {
    ...
    let l1_structural_state_map = has_l1
        ...
        .flatten();
    let l1_structural_state_map = choose_smaller_state_map(l1_structural_state_map);
    ...
    let split_l1_structural_state_map = has_split_l1
        ...
        .flatten();
    let split_l1_structural_state_map = choose_smaller_state_map(split_l1_structural_state_map);
    let split_l1_initial_state_map = split_l1_structural_state_map.as_ref().or(initial_state_map);
    if compile_profile_enabled() {
        eprintln!(
            "[glrmask/profile][inactive_component_state_map] partition={} raw_states={} inherited_reps={} l1_reps={} split_l1_reps={}",
            partition_label,
