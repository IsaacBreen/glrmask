...
fn analyze_equivalences_impl(
    ...
    // Partition C is an exact global scanner-state substitution map for this
    // local vocabulary partition. Feed it into the ordinary state-equivalence
    // pipeline as the initial map. Restricted observation can therefore refine
    // only C representatives; if that pass is omitted, the later max-length
    // and exact-token phases begin directly from C. The older global seed is
    // retained only on partitions where C is unavailable.
    let state_equivalence_seed = token_position_partition
        .map(GlobalTokenPositionStatePartition::as_many_to_one)
    ...
    if tokenizer.has_epsilon_transitions() {
        ...
        let prepared = prepare_equivalence_inputs(tokenizer, vocab, state_equivalence_seed);
        let token_len_stats = token_length_stats(&prepared.token_bytes);
    ...
    let effective_disallowed = token_path_disallowed_follows
        .as_ref()
        .unwrap_or(disallowed_follows);
    // The specialized raw restricted-observation route has no representative
    // seed input. When C exists, keep the ordinary pipeline so restricted
    // observation actually runs over C representatives instead of rescanning
    // every raw state. Partitions without C retain the established raw route.
    if token_position_partition.is_none() {
    ...
    let internal_id_map = build_internal_id_map_from_combined_result(
        tokenizer,
        state_equivalence_seed,
        &prepared,
        &result,
