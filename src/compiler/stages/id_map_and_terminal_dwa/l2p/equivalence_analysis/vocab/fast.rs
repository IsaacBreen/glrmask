...
fn try_first_transition_factor_plan<S: AsRef<[u8]> + Sync>(
    ...
    if profiling {
        let mut depth_two_pairs = 0usize;
        let mut depth_two_subgroups = 0usize;
        for bucket in &buckets {
            let mut one_byte_tokens = 0usize;
            let mut tokens_by_second_class =
                HashMap::<usize, Vec<usize>>::with_capacity(num_classes);
            for &token_idx in &bucket.token_indices {
                let token = strings[token_idx].as_ref();
                if token.len() == 1 {
                    one_byte_tokens += 1;
                } else {
                    let second_class = dfa.byte_to_class[token[1] as usize] as usize;
                    tokens_by_second_class
                        .entry(second_class)
                        .or_default()
                        .push(token_idx);
                }
            }
            depth_two_pairs = depth_two_pairs.saturating_add(
                one_byte_tokens.saturating_mul(bucket.initial_outcomes.len()),
            );
            for token_indices in tokens_by_second_class.into_values() {
                depth_two_subgroups += 1;
                let second_byte = strings[token_indices[0]].as_ref()[1];
                let mut prefix_keys = HashMap::<(usize, Vec<u8>), ()>::new();
                for &first_outcome in &bucket.initial_outcomes {
                    let mut latest_positions = vec![0u8; dfa.num_groups];
                    let suffix_state = if first_outcome == STATE_NONE {
                        STATE_NONE
                    } else {
                        for &gid in &dfa.finalizers[first_outcome] {
                            if gid < dfa.num_groups {
                                latest_positions[gid] = 1;
                            }
                        }
                        if dfa.is_dead_end[first_outcome] {
                            STATE_NONE
                        } else {
                            let target = dfa.transition(first_outcome, second_byte);
                            if target == NONE {
                                STATE_NONE
                            } else {
                                let target = target as usize;
                                for &gid in &dfa.finalizers[target] {
                                    if gid < dfa.num_groups {
                                        latest_positions[gid] = 2;
                                    }
                                }
                                if dfa.is_dead_end[target] {
                                    STATE_NONE
                                } else {
                                    target
                                }
                    ...
                    prefix_keys.insert((suffix_state, latest_positions), ());
                }
                depth_two_pairs = depth_two_pairs.saturating_add(
                    token_indices.len().saturating_mul(prefix_keys.len()),
                );
            }
        }
        eprintln!(
            "[glrmask/profile][vocab_first_transition_factor_depth_two_potential] depth_one_pairs={} depth_two_pairs={} depth_two_subgroups={} reduction_pct={:.2}",
            preliminary_state_token_pairs,
            depth_two_pairs,
            depth_two_subgroups,
            100.0
                * (1.0
                    - depth_two_pairs as f64
                        / preliminary_state_token_pairs.max(1) as f64),
        );
    }
