    ...
    fn process_child_segment(
        ...
        while let Some((offset, nodes_at_offset)) = pending_by_offset.pop_first() {
            ...
            for (tokenizer_state, source_nodes) in nodes_at_offset {
                ...
                let match_filter_started_at = self.profile_timing.then(std::time::Instant::now);
                matches_buf.clear();
                ...
                if let Some(started_at) = match_filter_started_at {
                    self.profile.trie_match_filter_ms +=
                        started_at.elapsed().as_secs_f64() * 1000.0;
