...
pub(crate) fn debug_repeat_bounds(expr: &Expr) -> Vec<(String, usize, Option<usize>, usize, u128)> {
    fn visit(
        expr: &Expr,
        path: &mut Vec<usize>,
        out: &mut Vec<(String, usize, Option<usize>, usize, u128)>,
    ) {
        match expr {
            ...
            Expr::Shared(inner) => visit(inner, path, out),
            Expr::U8Seq(_) | Expr::U8Class(_) | Expr::Dfa(_) | Expr::Epsilon => {}
        }
    }

    let mut out = Vec::new();
    visit(expr, &mut Vec::new(), &mut out);
    out
}

pub(crate) fn debug_repeat_vocab_horizons(
    expr: &Expr,
    vocab: &Vocab,
) -> Vec<(String, usize, Option<usize>, usize, Option<usize>)> {
    fn visit(
        expr: &Expr,
        vocab: &Vocab,
        horizons: &VocabularyRepeatHorizonCache,
        path: &mut Vec<usize>,
        out: &mut Vec<(String, usize, Option<usize>, usize, Option<usize>)>,
    ) {
        match expr {
            Expr::Repeat { expr: body, min, max } => {
                out.push((
                    ...
                    *max,
                    minimum_consumed_bytes(body),
                    horizons.horizon_for_expr(body, vocab),
                ));
                path.push(0);
                visit(body, vocab, horizons, path, out);
                path.pop();
            }
            Expr::Intersect { expr, intersect } | Expr::Exclude { expr, exclude: intersect } => {
                path.push(0);
                visit(expr, vocab, horizons, path, out);
                path.pop();
                path.push(1);
                visit(intersect, vocab, horizons, path, out);
                path.pop();
            }
            Expr::Seq(parts) | Expr::Choice(parts) => {
                for (index, part) in parts.iter().enumerate() {
                    path.push(index);
                    visit(part, vocab, horizons, path, out);
                    path.pop();
                }
            }
            Expr::Shared(inner) => visit(inner, vocab, horizons, path, out),
            Expr::U8Seq(_) | Expr::U8Class(_) | Expr::Dfa(_) | Expr::Epsilon => {}
        }
    }
    ...
    let mut out = Vec::new();
    visit(expr, vocab, &horizons, &mut Vec::new(), &mut out);
    out
}
