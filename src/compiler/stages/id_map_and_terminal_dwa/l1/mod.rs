            ...
            && rayon::current_num_threads() > 1
        {
            ...
            let auto_prefers_packed = token_ids.len() > 500
                ...
                && token_buckets.packed_suffix_tries_by_first_byte[byte_idx]
                    ...
                    .len()
                    > 4_000;
            if flat_supported && (prefer_flat_empty_suffix || !auto_prefers_packed) {
                build_flat()
            } else {
                build_packed()
            }
...
fn l1_bucket_suffix_signature_profiles_packed(
    ...
    // the root terminal observation; computing that larger fingerprint did not
    // repay its cost on either the dominant or medium vocabulary buckets.
    if !has_empty_suffix && !first_suffix_bytes.is_empty() {
        let canonical_state = horizon_maps
            .map(|maps| maps[suffix_horizon.saturating_sub(1)].as_ref());
        let mut fp_groups = FxHashMap::<(u32, Vec<u32>), Vec<u32>>::default();
        for &target in &walk_targets {
            let root_signature = if has_empty_suffix {
                state_to_terminal_signature[target as usize]
            } else {
                0
            };
            let fp = first_suffix_bytes
                ...
                .collect::<Vec<_>>();
            fp_groups
                .entry((root_signature, fp))
                .or_default()
                .push(target);
        }
        let mut deduped_targets = Vec::<u32>::new();
        let mut others = Vec::<Vec<u32>>::new();
        for ((_, fp), group) in fp_groups {
            if !has_empty_suffix && fp.iter().all(|&state| state == dead) {
                for target in group {
                    results.push(((first_byte, target), Arc::from([])));
                }
                continue;
