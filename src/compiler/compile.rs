...
fn inline_autogenerated_helper_nonterminals(
    ...
    loop {
        ...
        let mut consumer_lhses = std::collections::BTreeMap::<NonterminalID, std::collections::BTreeSet<NonterminalID>>::new();

        for (index, rule) in rules.iter().enumerate() {
            productions_by_lhs.entry(rule.lhs).or_default().push(index);
            for symbol in &rule.rhs {
                if let Symbol::Nonterminal(nonterminal) = symbol {
                    consumer_lhses.entry(*nonterminal).or_default().insert(rule.lhs);
                }
            }
        }
        ...
        let candidate = productions_by_lhs
            ...
            .filter_map(|(&nonterminal, production_indexes)| {
                ...
                let consumer_count = consumer_lhses.get(&nonterminal).map_or(0, |lhses| lhses.len());
...
mod tests {
    ...
    fn test_inline_autogenerated_helper_nonterminals_compacts_repetition_tail_chain() {
        ...
        assert!(!rules.iter().any(|rule| matches!(rule.lhs, 5 | 6 | 7 | 9)));
        ...
        assert!(rules.contains(&Rule {
            lhs: 4,
            rhs: vec![Symbol::Nonterminal(4), Symbol::Terminal(2), Symbol::Nonterminal(1)],
        }));
        assert!(rules.contains(&Rule {
            lhs: 8,
        ...
        assert!(rules.contains(&Rule {
            lhs: 8,
            rhs: vec![Symbol::Nonterminal(8), Symbol::Terminal(2), Symbol::Nonterminal(2)],
        }));
    }
}
