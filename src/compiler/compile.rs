...
mod tests {
    ...
    #[test]
    fn test_terminal_dwa_path_o62060_probe() {
        // Probe: dump all accepting paths and lengths for various schema+token combos
        let schemas_and_tokens: &[(&str, &[u8])] = &[
            // original o62060 schema
            ("{\"type\":\"object\",\"properties\":{\"a\":{}},\"required\":[\"a\"],\"additionalProperties\":false}", b" {},"),
            ("{\"type\":\"object\",\"properties\":{\"a\":{}},\"required\":[\"a\"],\"additionalProperties\":false}", b"{}"),
            ("{\"type\":\"object\",\"properties\":{\"a\":{}},\"required\":[\"a\"],\"additionalProperties\":false}", b","),
            ("{\"type\":\"object\",\"properties\":{\"a\":{}},\"required\":[\"a\"],\"additionalProperties\":false}", b" ,"),
            ("{\"type\":\"number\"}", b"1"),
        ...
        for (schema, token) in schemas_and_tokens {
            let grammar = match json_schema_to_grammar(schema) {
                Ok(g) => g,
                Err(_) => continue,
            };
            let vocab = Vocab::new(vec![(0u32, token.to_vec())], None);
            let constraint = compile(&grammar, &vocab);
            ...
            let id_map = InternalIdMap {
                tokenizer_states: ManyToOneIdMap {
                    ...
                    representative_original_ids: constraint.internal_tsid_to_states.iter().map(|v| v[0]).collect(),
                ...
                vocab_tokens: ManyToOneIdMap {
                    ...
                    representative_original_ids: constraint.internal_token_to_tokens.iter().map(|v| v[0]).collect(),
            ...
            let (terminal_dwa, _) =
                crate::compiler::stages::terminal_dwa_compat::build_terminal_dwa_for_existing_id_map_with_possible_matches_and_coloring(
                    ...
                    &vocab,
                    &id_map,
                    &crate::compiler::stages::id_map_and_terminal_dwa::types::TerminalColoring::identity(analyzed_grammar.num_terminals as usize),
                    false,
                    constraint.ignore_terminal,
                    None,
                );
            let max_path_len = terminal_dwa.max_accepting_path_length_with_nonempty_weight();
            let token_str = std::str::from_utf8(token).unwrap_or("(non-utf8)");
            eprintln!(
                "schema={:?} token={:?} token_bytes={} max_path_len={:?} gt={} ignore_terminal={:?}",
                &schema[..schema.len().min(60)],
                token_str,
                token.len(),
                max_path_len,
                max_path_len.map_or(false, |l| l > token.len()),
                constraint.ignore_terminal,
            );
        }
    }
