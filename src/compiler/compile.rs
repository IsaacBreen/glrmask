...
mod tests {
    ...
    fn test_terminal_dwa_path_o62060() {
        ...
        let path = [6, 7, 9, 8].map(|t| t as i32);
        let weight = terminal_dwa.eval_word(&path);
        assert!(
            !weight.is_empty(),
            "path 6->7->9->8 should exist in the terminal DWA"
        );
        assert!(
            weight.token_union().contains(0),
            "path 6->7->9->8 should map to token 0"
        );
    }
