    ...
    loop {
        ...
        let candidate = productions_by_lhs
            ...
            .filter_map(|(&nonterminal, production_indexes)| {
                if nonterminal == start || nonterminal_names.contains_key(&nonterminal) || production_indexes.len() != 1 {
                    return None;
                }
...
pub(crate) fn prepare_grammar_for_compile(grammar: &GrammarDef) -> (GrammarDef, Tokenizer) {
    ...
    inline_autogenerated_helper_nonterminals(&mut normalized.rules, normalized.start, &normalized.nonterminal_names);
