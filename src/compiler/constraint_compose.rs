    ...
    for global_state in 0..global_state_count {
        ...
        let selected = match candidates.len() {
            ...
            _ => {
                ...
                for (component_index, mut weight) in candidates {
                    let map = &component_maps[component_index as usize];
                    let mut refs = vec![&mut weight];
                    remap_weights_with_maps(
                        &mut refs,
                        &map.local_to_global_tsids,
                        &map.local_to_global_tokens,
                        global_tsid_count,
                    );
                    if !weight.is_empty() {
    ...
    fn materialize_positive_dwa(
        ...
        match self {
            ...
            Self::Deferred {
                ...
                let mut parser_nwa = build_parser_nwa_from_terminal_dwa_with_precomputed_templates(
                    &terminal_automaton,
                    &analyzed,
                    &templates,
                    table,
                )
                .ok_or_else(|| "boundary terminal automaton induced no parser NWA".to_string())?;
...
fn ensure_positive_runtime_parser_dwa(dwa: &DWA) -> Result<(), String> {
...
/// filesystem capture.
fn load_boundary_terminal_capture(path: &str) -> Result<MappedArtifact<DWA>, String> {
...
fn finish_eager_concrete_boundary_delta_plan(
...
struct EagerChangedParentTemplates {
    ...
    delta_templates: BTreeMap<u32, (UnweightedDfa, UnweightedDfa)>,
    unsafe_parent: BTreeSet<u32>,
    build_ms: f64,
}

struct EagerConcreteDeltaPrecompute {
    delta_templates: BTreeMap<u32, (UnweightedDfa, UnweightedDfa)>,
    unsafe_terminals: BTreeSet<u32>,
}
...
fn try_build_changed_parent_templates_for_terminal_count(
    ...
    if compose_profile_enabled() {
        eprintln!(
            "[glrmask/profile][constraint_eager_changed_parent_templates] candidates={} selected_parent={} changed_parent={} states={} candidate_ms={candidate_ms:.3} template_compile_ms={template_compile_ms:.3} delta_ms={delta_ms:.3} build_ms={build_ms:.3}",
...
fn try_prepare_cached_composition_template_plan_for_terminal_count(
    composed_table: &ComposedTable,
    components: &[&Constraint],
    num_terminals: u32,
...
fn try_build_cached_composition_templates_for_terminal_count(
    composed_table: &ComposedTable,
    components: &[&Constraint],
    num_terminals: u32,
    selected: &[bool],
...
pub(crate) fn compose_constraints_owned_parent(
    ...
    let mut segmented_source_constraints =
        if segmented_runtime_requested && !segmented_skip_requested && !two_dwa_runtime_requested {
            ...
            if compose_profile_enabled() {
                eprintln!(
                    "[glrmask/profile][constraint_segment_clone] components={} parser_states={} overlapped=false total_ms={:.3}",
                    sources.len(),
                    sources
                        .iter()
    ...
    if segmented_skip_requested || two_dwa_runtime_requested {
        ...
        source_constraints.insert(0, parent);
        ...
        let token_cache_prebuild_ms = 0.0;
    ...
    let component_union_work = {
        ...
        let union_parser_components = union_component_constraints
            ...
            .collect::<Vec<_>>();
        let work = if let Some(view) = DeferredComponentParserUnionView::new(
        ...
        // `DeferredComponentParserUnionView` stores the component references,
        // not this temporary Vec, so dropping the Vec does not invalidate it.
    ...
    let parser_default_domain_labels = parser_default_domains
        ...
        .collect::<Vec<_>>();
    let mut boundary_work = boundary_work;
            ...
            .is_some_and(|overlay| {
                !overlay.segmented_parser_components.is_empty()
                    && (overlay.segmented_boundary_parser.is_some()
                        || std::env::var_os("GLRMASK_EXPERIMENT_BOUNDARY_TERMINAL_CAPTURE").is_none())
            });
