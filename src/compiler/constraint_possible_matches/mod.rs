    ...
    for class_id in used_state_class_ids(state_classes) {
        ...
        for (&terminal_id, ranges) in class_map.iter() {
            ...
            for &(lo, mut hi) in ranges {
                ...
                hi = hi.min(max_token);
                if lo > hi { continue; }
                push_sweep_event(&mut events, &mut event_positions, lo, SweepEvent { add: true, label });
                let after = hi.saturating_add(1);
                if after <= num_ordered_tokens as u32 {
                    push_sweep_event(&mut events, &mut event_positions, after, SweepEvent { add: false, label });
                }
            }
        }
    }
...
fn apply_sweep_events(active: &mut BTreeSet<StateTerminalLabel>, events: &[SweepEvent]) {
    for event in events.iter().filter(|event| !event.add) { active.remove(&event.label); }
    for event in events.iter().filter(|event| event.add) { active.insert(event.label); }
}
...
fn build_possible_match_vocab_and_weights_from_interval_maps(
    ...
    let possible_matches = ids_by_label.into_iter().map(|(terminal_id, by_state)| {
        let entries = by_state.into_iter().filter_map(|(state, mut ids)| {
            ...
            if token_set.is_empty() { None } else { Some((state, shared_rangeset(token_set))) }
        });
