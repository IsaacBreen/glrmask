...
fn assert_release_benchmark() {
    if cfg!(debug_assertions) {
        panic!("github_trivial_o70256_build_multithreaded must be run in release/bench mode, e.g. `cargo bench --bench github_trivial_o70256_build_multithreaded`");
    }
}
...
fn bench_github_trivial_o70256_build(c: &mut Criterion) {
    ...
    eprintln!("[bench][github_trivial_o70256_build_multithreaded] vocab_tokens={} threading_env=external diagnostic=0", vocab.len());
    run_one_warmup_build(&vocab);
    c.bench_function("github_trivial_o70256_glrmask_build_llama3_multithreaded", |b| {
        b.iter(|| {
