...
When the observed split is between a schema-fixed object key path and a generic `JSON_STRING`/additional-property key path, preserve that class explicitly. First identify the live parser states in the actual replay and map them to fixed-key versus generic-key table actions. Also verify the generated additional-property key terminal subtracts all co-occurring fixed keys; a witness that lets a completed fixed key such as `"FAQs": ` or `"a10": ` be accepted by the same-object additional-property branch is not faithful JSON Schema semantics. A valid minimized witness may still allow the fixed-key and additional-property branches to share an incomplete prefix such as the opening quote before the key is known. A generic regex ambiguity or unrelated separator-follow/table shape is not equivalent just because the final `parser_path_count` is the same. Assert survival at the point that matters, such as the next-key quote after a value comma, not merely at the first transient split.
