...
For parser-shape and path-count oracles, do not stop when the witness no longer resembles the source language. Keep trying to delete fixed prefixes, separators, field/key literals, wrappers, optional branches, and helper nonterminals entirely. A JSON/object witness may reduce all the way to a single regex terminal and one token; that is a better MRE when it preserves the exact oracle.
